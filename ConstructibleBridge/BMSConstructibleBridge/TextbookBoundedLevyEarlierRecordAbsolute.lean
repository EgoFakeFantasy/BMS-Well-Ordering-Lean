...
`TextbookBoundedLevyRecordAbsolute` 中证明只需传递性。本模块把二者组合，得到后续
...
theorem textbookBoundedLevyEarlierRecordFormula_absolute_l
    {M : ZFSet.{u}} (hM : M.IsTransitive) (s : Tuple ZFSet.{u} 8)
    (hs : ∀ index, s index ∈ M) :
    Model.SatisfiesIn (M : Set ZFSet.{u})
        textbookBoundedLevyEarlierRecordFormula_l s ↔
      FOFormula.Satisfies Delta0Formula.ZFMem
        textbookBoundedLevyEarlierRecordFormula_l s := by
  simp only [textbookBoundedLevyEarlierRecordFormula_l,
  ...
  rw [Model.satisfiesIn_delta0_iff hM textbookBoundedLevyEarlierRecordDelta_l
      (fun index => s (![1, 2, 3] index)) hPrefixParameters,
    Delta0Formula.satisfies_toFO]
  exact and_congr Iff.rfl
    (textbookBoundedLevyRecordComponentsFormula_absolute_l hM
      (fun index => s (![0, 3, 4, 5, 6, 7] index))
      hRecordParameters)

/-- 在 `omega < theta` 的可构造层中可直接使用的专门版本。 -/
theorem textbookBoundedLevyEarlierRecordFormula_stage_absolute_l
    ...
    Model.SatisfiesIn (LStageZF θ : Set ZFSet.{u})
      ...
      FOFormula.Satisfies Delta0Formula.ZFMem
        textbookBoundedLevyEarlierRecordFormula_l s :=
  textbookBoundedLevyEarlierRecordFormula_absolute_l
...
-/
theorem satisfiesIn_textbookBoundedLevyEarlierRecordFormula_local_iff_l
    ...
    (hGraph : textbookBoundedLevyTraceGraphZF_l trace ∈ M)
  ...
  exact (textbookBoundedLevyEarlierRecordFormula_absolute_l
