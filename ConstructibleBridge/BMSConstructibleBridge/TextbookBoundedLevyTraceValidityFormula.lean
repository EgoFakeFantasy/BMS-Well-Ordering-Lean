...
-/
def textbookBoundedLevyTraceRowMatrix_l : FOFormula 10 :=
  .conj
...
def textbookBoundedLevyTraceRowWitnessFormula_l : FOFormula 5 :=
  externalExistentialClosure_l 5 textbookBoundedLevyTraceRowMatrix_l

/-- 只要求落在记录长度内的索引通过逐行检查。 -/
def textbookBoundedLevyTraceRowFormula_l : FOFormula 5 :=
...
-/
def textbookBoundedLevyTraceGraphEntryDelta_l : Delta0Formula 5 :=
...
def textbookBoundedLevyTraceGraphExactDelta_l : Delta0Formula 4 :=
  .boundedAll (3 : Fin 4) textbookBoundedLevyTraceGraphEntryDelta_l

/-- 图元素解码体确实读取一个索引落在长度内的有序对。 -/
@[simp]
theorem satisfies_textbookBoundedLevyTraceGraphEntryDelta_iff_l
    (omega sequence length graph q : ZFSet.{u}) :
    Delta0Formula.Satisfies Delta0Formula.ZFMem
      textbookBoundedLevyTraceGraphEntryDelta_l
      ![omega, sequence, length, graph, q] ↔
      ∃ value index : ZFSet.{u},
        q = ZFSet.pair value index ∧ index ∈ length := by
  simp only [textbookBoundedLevyTraceGraphEntryDelta_l,
...
都能解码为规范记录且满足五分支局部规则。
-/
def textbookBoundedLevyTraceValidityFormula_l : FOFormula 2 :=
  .ex (.ex
    (.conj
      (Delta0Formula.kuratowskiPairEqAt
        (1 : Fin 4) (2 : Fin 4) (3 : Fin 4)).toFO <|
    .conj (.mem (2 : Fin 4) (0 : Fin 4)) <|
    .conj textbookBoundedLevyTraceGraphExactDelta_l.toFO <|
    .conj (.all IndexedSequenceZF.totalFunctionalBody)
      (.all textbookBoundedLevyTraceRowFormula_l)))

/-- 五个逐行见证追加到外部五字段后的显式布局。 -/
theorem textbookBoundedLevyTraceRowAssignment_l {Carrier : Type u}
...
theorem satisfies_textbookBoundedLevyTraceRowWitnessFormula_iff_l
  ...
  change (∃ w : Tuple ZFSet.{u} 5,
    FOFormula.Satisfies Delta0Formula.ZFMem textbookBoundedLevyTraceRowMatrix_l
      (Fin.append base w)) ↔ _
  ...
  · rintro ⟨w, hGraph, hComponents, hRule⟩
    ...
    subst entry
    apply (satisfies_textbookBoundedLevyLocalRuleFormula_iff_l trace index).mp
  ...
  · intro hRule
    ...
    refine ⟨w, ?_⟩
    simp only [textbookBoundedLevyTraceRowMatrix_l, FOFormula.Satisfies,
      IndexedSequenceZF.satisfies_functionGraphValueAt,
      FOFormula.satisfies_rename]
    refine ⟨?_, ?_, ?_⟩
    · exact (textbookBoundedLevyTraceGraph_value_iff_l trace index _).mpr rfl
    · convert
        (satisfies_textbookBoundedLevyRecordComponentsFormula_iff_l
          (textbookBoundedLevyRecordZF_l entry)
          (natCode (textbookBoundedLevyPolarityCode_l entry.isSigma))
    ...
    · convert
        (satisfies_textbookBoundedLevyLocalRuleFormula_iff_l trace index).mpr hRule
          using 1 <;> ext i <;> fin_cases i <;> rfl

/-- 任意候选图上一行的解码结果；局部公式暂时保留该候选图作为参数。 -/
structure TextbookBoundedLevyDecodedRow_l
...
theorem satisfies_textbookBoundedLevyTraceRowWitnessFormula_to_decoded_l
    ...
    (hRow : FOFormula.Satisfies Delta0Formula.ZFMem
      ...
      ![(Ordinal.omega0.toZFSet : ZFSet.{u}), sequence, length, graph,
        index]) :
    Nonempty (TextbookBoundedLevyDecodedRow_l graph index) := by
  rw [textbookBoundedLevyTraceRowWitnessFormula_l,
...
/-- 完整痕迹公式的原始集合语义。 -/
theorem satisfies_textbookBoundedLevyTraceValidityFormula_l
    (omega sequence : ZFSet.{u}) :
    FOFormula.Satisfies Delta0Formula.ZFMem
      textbookBoundedLevyTraceValidityFormula_l ![omega, sequence] ↔
  ...
  simp [textbookBoundedLevyTraceValidityFormula_l, textbookBoundedLevyTraceRowFormula_l,
    textbookBoundedLevyTraceGraphExactDelta_l,
    Model.snoc_eq_finSnoc]

/-- 规范编码满足完整痕迹公式，当且仅当它的每一行满足有限索引规则。 -/
theorem satisfies_textbookBoundedLevyTraceValidityFormula_iff_l
    ...
    FOFormula.Satisfies Delta0Formula.ZFMem
      ...
      ∀ index : Fin trace.length,
        textbookBoundedLevyIndexedRule_l trace.get index := by
  rw [satisfies_textbookBoundedLevyTraceValidityFormula_l]
  constructor
  · rintro ⟨length, graph, hSequence, _, _, _, hRows⟩
    ...
    subst graph
    intro index
    apply (satisfies_textbookBoundedLevyTraceRowWitnessFormula_iff_l trace index).mp
    exact hRows (natCode index.1)
      ((IndexedSequenceZF.mem_natCode_iff_exists_lt _ trace.length).mpr
        ⟨index.1, index.2, rfl⟩)
  · intro hRules
    refine ⟨natCode trace.length, textbookBoundedLevyTraceGraphZF_l trace,
      ?_, ?_, ?_, ?_, ?_⟩
    · simp [textbookBoundedLevyTraceZF_l, IndexedSequenceZF.sequenceCode,
        textbookBoundedLevyTraceGraphZF_l]
    ...
    · intro q hq
      ...
      refine ⟨textbookBoundedLevyRecordZF_l (trace.get index),
        natCode index.1, ?_, ?_⟩
      · congr 1
        simp [index]
    ...
    · intro index hIndex
      ...
      refine ⟨textbookBoundedLevyRecordZF_l (trace.get i), ?_, ?_⟩
      · exact (textbookBoundedLevyTraceGraph_value_iff_l trace i _).mpr rfl
      · intro other hOther
        exact (textbookBoundedLevyTraceGraph_value_iff_l trace i other).mp hOther
    · intro index hIndex
      obtain ⟨position, hPosition, rfl⟩ :=
        (IndexedSequenceZF.mem_natCode_iff_exists_lt index trace.length).mp hIndex
      let i : Fin trace.length := ⟨position, hPosition⟩
      exact (satisfies_textbookBoundedLevyTraceRowWitnessFormula_iff_l trace i).mpr
        (hRules i)
...
-/
theorem satisfies_textbookBoundedLevyTraceValidityFormula_to_canonical_l
    (sequence : ZFSet.{u})
  ...
  have hDecoded : ∀ index : Fin n,
      Nonempty (TextbookBoundedLevyDecodedRow_l graph (natCode index.1)) := by
    intro index
    apply satisfies_textbookBoundedLevyTraceRowWitnessFormula_to_decoded_l
      sequence (natCode n) graph (natCode index.1)
    exact hRows (natCode index.1)
      ((IndexedSequenceZF.mem_natCode_iff_exists_lt _ n).mpr
        ⟨index.1, index.2, rfl⟩)
  let decodedRow : (index : Fin n) →
      TextbookBoundedLevyDecodedRow_l graph (natCode index.1) :=
  ...
  let index : Fin n :=
    ⟨traceIndex.1, by simpa [hTraceLength] using traceIndex.2⟩
  apply (satisfies_textbookBoundedLevyLocalRuleFormula_iff_l trace traceIndex).mp
