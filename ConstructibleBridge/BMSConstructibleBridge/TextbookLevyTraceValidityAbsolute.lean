...
private theorem satisfiesIn_textbookLevyTraceRowFormula_iff_l
    (M : Set ZFSet.{u})
    (omega sequence length graph index : ZFSet.{u}) :
    Model.SatisfiesIn M textbookLevyTraceRowFormula_l
        ![omega, sequence, length, graph, index] ↔
      (index ∈ length →
        Model.SatisfiesIn M textbookLevyTraceRowWitnessFormula_l
          ![omega, sequence, length, graph, index]) := by
  rw [textbookLevyTraceRowFormula_l]
  exact satisfiesIn_imp_trace_iff M
    (.mem (4 : Fin 5) (2 : Fin 5))
      textbookLevyTraceRowWitnessFormula_l _

/-- 规范痕迹图的一行在层内成立，当且仅当该行满足有限索引局部规则。 -/
theorem satisfiesIn_textbookLevyTraceRowWitnessFormula_iff_l
  ...
  change (∃ w : Tuple ZFSet.{u} 5,
    ...
    Model.SatisfiesIn (LStageZF θ : Set ZFSet.{u})
      textbookLevyTraceRowMatrix_l (Fin.append base w)) ↔ _
  ...
  · rintro ⟨w, hWitnesses, hGraph, hComponents, hRule⟩
    ...
    have hAssignment : ∀ position, assignment position ∈ LStageZF θ := by
      ...
      fin_cases position <;>
        first
          ...
          | exact LStageZF_mono (le_of_lt hω) (textbookLevyTraceZF_mem_LStageOmega_l trace)
          | exact natCode_mem_stage_l hω _
          | exact LStageZF_mono (le_of_lt hω) (textbookLevyTraceGraphZF_mem_LStageOmega_l trace)
          | exact hWitnesses _
    have hValue : w 0 = textbookLevyRecordZF_l (trace.get index) :=
      (textbookLevyTraceGraph_value_iff_l trace index (w 0)).mp (by
        ...
        simpa [textbookLevyTraceRowMatrix_l, assignment] using hGraph)
    ...
    subst entry
    apply (satisfiesIn_textbookLevyLocalRuleFormula_iff_l
  ...
  · intro hRule
    ...
    · apply (satisfiesIn_functionGraphValueAt_iff_l
      ...
      simpa [assignment, base, w, entry] using
        (textbookLevyTraceGraph_value_iff_l trace index _).mpr rfl
    · simp only [textbookLevyTraceRowMatrix_l, Model.SatisfiesIn,
...
theorem satisfiesIn_textbookLevyTraceValidityFormula_iff_l
    {θ : Ordinal.{u}} (hθ : Order.IsSuccLimit θ)
    (hω : Ordinal.omega0 < θ)
    ...
    Model.SatisfiesIn (LStageZF θ : Set ZFSet.{u})
      ...
      ∀ index : Fin trace.length,
        textbookLevyIndexedRule_l trace.get index := by
  let sequence : ZFSet.{u} := textbookLevyTraceZF_l trace
  ...
  change (∃ length : ZFSet.{u}, length ∈ stage ∧
    ∃ graph : ZFSet.{u}, graph ∈ stage ∧
      ...
      Model.SatisfiesIn stage textbookLevyTraceGraphExactDelta_l.toFO
        ![Ordinal.omega0.toZFSet, sequence, length, graph] ∧
      Model.SatisfiesIn stage (.all IndexedSequenceZF.totalFunctionalBody)
        ![Ordinal.omega0.toZFSet, sequence, length, graph] ∧
      Model.SatisfiesIn stage (.all textbookLevyTraceRowFormula_l)
  ...
  · rintro ⟨length, hLengthStage, graph, hGraphStage, hPairLocal,
    ...
    have hParts := ZFSet.pair_inj.mp (by
      simpa [sequence, textbookLevyTraceZF_l,
        IndexedSequenceZF.sequenceCode, textbookLevyTraceGraphZF_l]
        using hSequence)
    ...
    subst graph
    intro index
    apply (satisfiesIn_textbookLevyTraceRowWitnessFormula_iff_l
      hθ hω trace index).mp
    apply (satisfiesIn_textbookLevyTraceRowFormula_iff_l
      stage Ordinal.omega0.toZFSet sequence (natCode trace.length)
        (textbookLevyTraceGraphZF_l trace) (natCode index.1)).mp
      ((satisfiesIn_all_trace_iff stage textbookLevyTraceRowFormula_l
        ![Ordinal.omega0.toZFSet, sequence, natCode trace.length,
          textbookLevyTraceGraphZF_l trace]).mp hRows
        (natCode index.1) (natCode_mem_stage_l hω _))
    exact (IndexedSequenceZF.mem_natCode_iff_exists_lt _ trace.length).mpr
      ⟨index.1, index.2, rfl⟩
  · intro hRules
    have hAmbient :=
      (satisfies_textbookLevyTraceValidityFormula_iff_l trace).mpr hRules
    obtain ⟨length, graph, hSequence, hLengthOmega, hExact,
      ...
      (satisfies_textbookLevyTraceValidityFormula_l
        ...
        (textbookLevyTraceZF_l trace)).mp hAmbient
    have hParts := ZFSet.pair_inj.mp (by
    ...
    have hGraphStage : textbookLevyTraceGraphZF_l trace ∈ LStageZF θ :=
      LStageZF_mono (le_of_lt hω)
        (textbookLevyTraceGraphZF_mem_LStageOmega_l trace)
    have hBase : ∀ position : Fin 4,
        ![Ordinal.omega0.toZFSet, sequence, natCode trace.length,
          textbookLevyTraceGraphZF_l trace] position ∈ LStageZF θ := by
      intro position
      fin_cases position
      · exact omega_toZFSet_mem_stage_l hω
    ...
    refine ⟨natCode trace.length, hLengthStage,
      textbookLevyTraceGraphZF_l trace, hGraphStage, ?_, hLengthOmega,
      ?_, ?_, ?_⟩
    · apply (Model.satisfiesIn_delta0_iff (LStageZF_isTransitive θ)
      ...
      simpa [Delta0Formula.satisfies_toFO,
        ...
        textbookLevyTraceZF_l, IndexedSequenceZF.sequenceCode] using hSequence
    · apply (Model.satisfiesIn_delta0_iff (LStageZF_isTransitive θ)
        textbookLevyTraceGraphExactDelta_l _ hBase).mpr
      simpa [textbookLevyTraceGraphExactDelta_l,
    ...
    · apply (satisfiesIn_all_trace_iff
      ...
      apply (satisfiesIn_totalFunctionalBody_iff_l
        ...
        (natCode trace.length) (textbookLevyTraceGraphZF_l trace) index
    ...
    · apply (satisfiesIn_all_trace_iff
        (LStageZF θ : Set ZFSet.{u}) textbookLevyTraceRowFormula_l _).mpr
      intro index hIndexStage
      apply (satisfiesIn_textbookLevyTraceRowFormula_iff_l
        (LStageZF θ : Set ZFSet.{u}) Ordinal.omega0.toZFSet sequence
        (natCode trace.length) (textbookLevyTraceGraphZF_l trace) index).mpr
      intro hIndexLength
      ...
      exact (satisfiesIn_textbookLevyTraceRowWitnessFormula_iff_l
        hθ hω trace traceIndex).mpr (hRules traceIndex)

/-- 完整痕迹公式在规范痕迹码上对后继极限层绝对。 -/
theorem textbookLevyTraceValidityFormula_stage_absolute_l
    {θ : Ordinal.{u}} (hθ : Order.IsSuccLimit θ)
    (hω : Ordinal.omega0 < θ)
    (trace : List TextbookLevyJudgment) :
    Model.SatisfiesIn (LStageZF θ : Set ZFSet.{u})
  ...
  (satisfiesIn_textbookLevyTraceValidityFormula_iff_l
    hθ hω trace).trans
      (satisfies_textbookLevyTraceValidityFormula_iff_l trace).symm

/-- 任意候选图上一行的层内解码结果。 -/
structure TextbookLevyDecodedRowIn_l
...
theorem satisfiesIn_textbookLevyTraceRowWitnessFormula_to_decoded_l
    {θ : Ordinal.{u}}
    ...
    Nonempty (TextbookLevyDecodedRowIn_l θ graph index) := by
  rw [textbookLevyTraceRowWitnessFormula_l,
  ...
  change ∃ w : Tuple ZFSet.{u} 5,
    (∀ position, w position ∈ LStageZF θ) ∧
    Model.SatisfiesIn (LStageZF θ : Set ZFSet.{u})
      textbookLevyTraceRowMatrix_l (Fin.append base w) at hRow
  ...
  have hGraphMem : ZFSet.pair (w 0) index ∈ graph := by
    apply (satisfiesIn_functionGraphValueAt_iff_l
      (LStageZF_isTransitive θ) (3 : Fin 10) (5 : Fin 10)
      (4 : Fin 10) assignment hAssignment).mp
    simpa [textbookLevyTraceRowMatrix_l, assignment, base] using hGraphFormula
  have hComponentsAmbient :
      FOFormula.Satisfies Delta0Formula.ZFMem
        textbookLevyRecordComponentsFormula_l
        ![Ordinal.omega0.toZFSet, w 0, w 1, w 2, w 3, w 4] := by
...
-/
theorem satisfiesIn_textbookLevyTraceValidityFormula_to_canonical_l
    {θ : Ordinal.{u}} (hθ : Order.IsSuccLimit θ)
  ...
  have hDecoded : ∀ index : Fin n,
      Nonempty (TextbookLevyDecodedRowIn_l θ graph (natCode index.1)) := by
    intro index
    apply satisfiesIn_textbookLevyTraceRowWitnessFormula_to_decoded_l
  ...
  let decodedRow : (index : Fin n) →
      TextbookLevyDecodedRowIn_l θ graph (natCode index.1) :=
    ...
    · intro hq
      ...
      have hValueStage : value ∈ LStageZF θ := by
        ...
        exact (pair_components_mem_of_transitive_l
          (LStageZF_isTransitive θ) hPairStage).1
      have hDecodedStage :
          textbookLevyRecordZF_l (decodedRow index).entry ∈ LStageZF θ := by
