...
theorem satisfies_isCartesianRow_iff
    ...
    {depth : Nat} (env : Env ℳ depth) (row left right : Term depth) :
...
theorem denote_cartesianRow_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (env : Env ℳ 1) (left row : ℳ.Domain) :
...
theorem exists_cartesianRow
    ...
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (left right : ℳ.Domain) :
  ...
  rcases exists_functionalImageOn hZF (Definitional.Project.BinarySchema.orderedPairWithLeft 𝒞) env right (by
        intro rightValue _
        rcases 𝕀.total left rightValue with ⟨pair, hCode⟩
        exact ⟨pair, (Definitional.Project.Formula.denote_orderedPairWithLeft_iff
            𝕀 env rightValue pair).mpr hCode⟩) (by
  ...
  · rintro ⟨rightValue, hRightValue, hPair⟩
    exact ⟨rightValue, hRightValue, (Definitional.Project.Formula.denote_orderedPairWithLeft_iff
...
theorem exists_cartesianProduct
  ...
  have hTotal :
    ...
    rcases exists_cartesianRow hZF 𝕀 leftValue right with
      ⟨row, hRow⟩
    exact ⟨row, (Definitional.Project.Formula.denote_cartesianRow_iff
        𝕀 env leftValue row).mpr hRow⟩
  have hUnique :
      ∀ leftValue, ℳ.mem leftValue left → ∀ first second, (Definitional.Project.BinarySchema.cartesianRow 𝒞).denote
          env leftValue first → (Definitional.Project.BinarySchema.cartesianRow 𝒞).denote
  ...
  rcases exists_functionalImageOn hZF (Definitional.Project.BinarySchema.cartesianRow 𝒞)
  ...
  · rintro ⟨row, hRowMem, hPairRow⟩
    ...
    rcases ((Definitional.Project.Formula.denote_cartesianRow_iff
...
theorem union_right
    ...
    {product leftProduct rightProduct
      left rightUnion rightLeft rightRight : ℳ.Domain} (hProduct :
      ℳ.IsCartesianProduct 𝕀 product left rightUnion) (hLeftProduct :
      ℳ.IsCartesianProduct 𝕀 leftProduct left rightLeft) (hRightProduct :
      ℳ.IsCartesianProduct 𝕀 rightProduct left rightRight) (hRightUnion :
  ...
  · intro hPair
    ...
    · rcases (hLeftProduct pair).mp hLeftPair with
      ...
      exact (hProduct pair).mpr
        ⟨leftValue, hLeftValue,
          rightValue, (hRightUnion rightValue).mpr <| Or.inl hRightValue,
          hCode⟩
    · rcases (hRightProduct pair).mp hRightPair with
      ...
      exact (hProduct pair).mpr
        ⟨leftValue, hLeftValue,
          rightValue, (hRightUnion rightValue).mpr <| Or.inr hRightValue,
          hCode⟩
...
theorem disjoint_right
    ...
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ}
    {leftProduct rightProduct left rightLeft rightRight : ℳ.Domain} (hLeftProduct :
      ℳ.IsCartesianProduct 𝕀 leftProduct left rightLeft) (hRightProduct :
      ℳ.IsCartesianProduct 𝕀 rightProduct left rightRight) (hRightDisjoint :
