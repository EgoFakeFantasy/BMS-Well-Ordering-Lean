...
def extensionality : Definitional.Project.Sentence :=
  sentence! ⟪∀ left right, (∀ element, element ∈ left ↔ element ∈ right) → left = right⟫
/-- 空集公理。 -/
...
def infinity : Definitional.Project.Sentence :=
  sentence! ⟪∃ omegaSet, (∃ empty, (∀ element, element ∉ empty) ∧ empty ∈ omegaSet) ∧ (∀ set ∈ omegaSet, ∃ successor, ((∀ element,
        element ∈ successor ↔ (element ∈ set ∨ element = set)) ∧
        successor ∈ omegaSet))⟫
/-- 正则/基础公理。 -/
def foundation : Definitional.Project.Sentence :=
  sentence! ⟪∀ set, (∃ element, element ∈ set) →
      ∃ minimal ∈ set, ∀ element ∈ set, element ∉ minimal⟫
/-- 选择公理的选择集形式。 -/
def choice : Definitional.Project.Sentence :=
  sentence! ⟪∀ family, ((∀ member ∈ family, ∃ element, element ∈ member) ∧ (∀ first ∈ family, ∀ second ∈ family,
        first ≠ second →
          ¬ (∃ element, element ∈ first ∧ element ∈ second))) → ∃ choice,
      ∀ member ∈ family, ∃ selected, ((selected ∈ choice ∧ selected ∈ member) ∧ (∀ other, (other ∈ choice ∧ other ∈ member) → other = selected))⟫
namespace Schema
/-- 分离模式在参数上下文中的核心公式。 -/
def separationCore {parameterCount : Nat} (schema : Definitional.Project.UnarySchema parameterCount) :
    Definitional.Project.Formula 1 parameterCount :=
  .forallE <| .existsE <| .forallE <|
    .iff (.mem (.bound 0) (.bound 1)) <|
      .conj (.mem (.bound 0) (.bound 2)) (schema.body.rename BoundEmbedding.unaryUnderTwo)
/-- 分离模式实例。 -/
def separation {parameterCount : Nat} (schema : Definitional.Project.UnarySchema parameterCount) :
  ...
  Definitional.Project.Sentence.forallClosure (separationCore schema) <| by
    simp [Definitional.Formula.FreeClosed, separationCore,
      schema.freeClosed]
/-- 收集模式在参数上下文中的核心公式。 -/
def collectionCore {parameterCount : Nat} (schema : Definitional.Project.BinarySchema parameterCount) :
    Definitional.Project.Formula 1 parameterCount :=
  .forallE <|
    .imp (Definitional.Project.Formula.forallMem (.bound 0) <| .existsE <|
        schema.body.rename BoundEmbedding.binaryUnderOne) (.existsE <|
...
/-- 收集模式实例。 -/
def collection {parameterCount : Nat} (schema : Definitional.Project.BinarySchema parameterCount) :
    Definitional.Project.Sentence :=
...
def replacementCore {parameterCount : Nat} (schema : Definitional.Project.BinarySchema parameterCount) :
    Definitional.Project.Formula 1 parameterCount :=
  .imp (.forallE <| .forallE <| .forallE <|
      .imp (.conj (schema.body.rename ReplacementEmbedding.firstOutput) (schema.body.rename ReplacementEmbedding.secondOutput))
        (Definitional.Project.Formula.extensionalEq (.bound 1) (.bound 0))) (.forallE <| .existsE <| .forallE <|
      .iff (.mem (.bound 0) (.bound 1)) <|
        Definitional.Project.Formula.existsMem (.bound 2) <|
          schema.body.rename ReplacementEmbedding.imageBody)
/-- Jech 风格替换模式实例。 -/
def replacement {parameterCount : Nat} (schema : Definitional.Project.BinarySchema parameterCount) :
    Definitional.Project.Sentence :=
