...
def imageMembership {parameterCount : Nat} (schema : BinarySchema parameterCount) :
...
private theorem reindex_binaryImageBody {ℳ : Structure.{u}}
    {parameterCount : Nat} (env : Env ℳ parameterCount) (source output input : ℳ.Domain) : (((env.push source).push output).push input).reindex
        (BinaryImageEmbedding.body (parameterCount := parameterCount)) = (env.push input).push output := by
  rw [Env.mk.injEq]
  constructor
  · funext entry
...
-/
theorem exists_functionalImageOn {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    ...
    ∃ image, ∀ output,
      ℳ.mem output image ↔
        ∃ input, ℳ.mem input source ∧
          schema.denote env input output := by
  rcases SetTheory.ZF.collection_exists_d hZF schema env source (by simpa [Definitional.Project.BinarySchema.denote] using hTotal) with
    ⟨collection, hCollection⟩
  rcases SetTheory.ZF.separation_exists_d hZF schema.imageMembership (env.push source) collection with
    ⟨image, hImage⟩
...
theorem exists_functionalImage {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {parameterCount : Nat} (schema : Definitional.Project.BinarySchema parameterCount) (env : Env ℳ parameterCount) (source : ℳ.Domain)
    (hTotal : ∀ input, ℳ.mem input source →
      ∃ output, schema.denote env input output) (hUnique : ∀ input first second,
      schema.denote env input first →
      schema.denote env input second →
      first = second) :
    ∃ image, ∀ output,
