...
private def cantorDiagonalSchema (𝒞 : Definitional.Project.OrderedPairConvention) :
...
private theorem satisfies_cantorDiagonalSchema_iff
    {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (env : Env ℳ 1) (value : ℳ.Domain) :
    Definitional.Project.Formula.satisfies (env.push value) (cantorDiagonalSchema 𝒞).body ↔
      ¬ ∃ output,
...
private theorem denote_singletonGraphSchema_iff
    {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 0) (input pair : ℳ.Domain) :
...
theorem not_surjectiveOnto_powerSet
    ...
    {function source power : ℳ.Domain} (hFunction :
      ℳ.IsSetFunctionFromTo 𝕀 function source power) (hPower : ℳ.IsPowerSetOf power source) :
    ¬ ℳ.IsSetSurjectiveOnto 𝕀 function source power := by
  let env : Env ℳ 1 := {
    bound := fun _ => function
    free := fun _ => Classical.choice ℳ.nonempty
  }
  rcases separation_exists_d hZF (cantorDiagonalSchema 𝒞) env source with
  ...
  have hDiagonalPower : ℳ.mem diagonal power := (hPower diagonal).mpr fun value hValue =>
  ...
  · exact (hDiagonal input).mp hMember |>.2
...
theorem exists_singletonInjectionToPowerSet
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {source power : ℳ.Domain} (hPower : ℳ.IsPowerSetOf power source) :
    ∃ function,
  ...
  have hTotal : ∀ input, ℳ.mem input source →
      ∃ pair, (singletonGraphSchema 𝒞).denote
          env input pair := by
    intro input _
    ...
    rcases 𝕀.total input singleton with
      ⟨pair, hPair⟩
    refine ⟨pair, ?_⟩
    apply (denote_singletonGraphSchema_iff
        𝕀 hZF.1 env input pair).mpr
    exact ⟨singleton, hSingleton, hPair⟩
  have hUnique : ∀ input, ℳ.mem input source →
      ∀ first second, (singletonGraphSchema 𝒞).denote
            env input first → (singletonGraphSchema 𝒞).denote
            env input second →
          first = second := by
    intro input _ first second hFirst hSecond
    rcases (denote_singletonGraphSchema_iff
  ...
  rcases exists_functionalImageOn hZF (singletonGraphSchema 𝒞)
  ...
  have hSetFunction : ℳ.IsSetFunction 𝕀 function := by
    ...
    exact
      Structure.IsSingletonOf.eq hZF.1 ((hPairMember input first).mp hFirst).2 ((hPairMember input second).mp hSecond).2
  ...
  have hIntoPower :
    ...
    exact
      ⟨singleton, hSingletonPower, (hPairMember input singleton).mpr
          ⟨hInput, hSingleton⟩⟩
  have hInjective : ℳ.IsSetInjective 𝕀 function := by
    intro first second output hFirst hSecond
    have hFirstSingleton := ((hPairMember first output).mp hFirst).2
    have hSecondSingleton := ((hPairMember second output).mp hSecond).2
    exact (hSecondSingleton first).mp <| (hFirstSingleton first).mpr rfl
  exact
    ⟨function,
