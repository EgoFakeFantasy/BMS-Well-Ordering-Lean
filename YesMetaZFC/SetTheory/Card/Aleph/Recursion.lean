...
theorem eq {ℳ : Structure.{u}}
    ...
    {first second predecessor : ℳ.Domain} (hFirst : ℳ.IsCardinalSuccessor 𝕀 first predecessor) (hSecond : ℳ.IsCardinalSuccessor 𝕀 second predecessor) :
    first = second := by
  ...
  · rcases hSecond.2.2 first hFirst.1 hFirst.2.1 with
    ...
    · have hSelf : ℳ.mem first first :=
        hFirst.1.1.transitive second hSecondMem first hFirstMem
      exact False.elim <|
...
private theorem satisfies_cardinalAboveMembership_iff
    ...
    Definitional.Project.Formula.satisfies (env.push candidate) (cardinalAboveMembership 𝒞).body ↔
  ...
  change (ℳ.IsCardinal 𝕀 candidate ∧
      ℳ.mem (env.bound 0) candidate) ↔ (ℳ.IsCardinal 𝕀 candidate ∧
      ℳ.mem (env.bound 0) candidate)
  rfl
/-- “当前指标处的每个 Aleph 值都是基数”的一参数归纳模式。 -/
private def alephValueCardinalAt (𝒞 : Definitional.Project.OrderedPairConvention) :
    Definitional.Project.UnarySchema 1 where
...
private theorem satisfies_alephValueCardinalAt_iff
    {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 1) (index : ℳ.Domain) :
    Definitional.Project.Formula.satisfies (env.push index) (alephValueCardinalAt 𝒞).body ↔
      ∀ value,
...
theorem cardinalSuccessor_existsUnique
    ...
    {predecessor : ℳ.Domain} (hPredecessor : ℳ.IsOrdinal predecessor) :
    ∃ successor,
  ...
  rcases separation_exists_d hZF (cardinalAboveMembership 𝒞) env bound with
  ...
  have hCandidatesSemantic (candidate : ℳ.Domain) :
    ...
    · rintro ⟨hCandidateBound, hCandidate⟩
      exact ⟨hCandidateBound, (satisfies_cardinalAboveMembership_iff
  ...
  have hSuccessorData := (hCandidatesSemantic successor).mp hSuccessorCandidates
  ...
  have hMinimal :
    ...
    rcases Structure.IsOrdinal.trichotomy hZF.1
        hCandidateCardinal.1 hBoundCardinal.1 (KP.difference_exists_d (ZF.modelsKP hZF)) (KP.intersection_exists_d (ZF.modelsKP hZF)
    ...
    · rcases hLeast candidate <| (hCandidatesSemantic candidate).mpr
...
theorem alephSuccessorOperation_existsUnique
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (previous : ℳ.Domain) :
    ∃ output,
...
theorem alephStep_isClassFunctionOnTransfiniteSequences
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (ω : ℳ.Domain) :
    ℳ.IsClassFunctionOnTransfiniteSequences 𝕀 (ℳ.IsAlephStep 𝕀 ω) := by
  exact zeroSuccessorLimitStep_isClassFunctionOnTransfiniteSequences
    hZF 𝕀 (fun initial => initial = ω) (ℳ.IsAlephSuccessorOperation 𝕀)
    ⟨ω, rfl, fun other hOther => hOther⟩ (alephSuccessorOperation_existsUnique hZF 𝕀)
...
theorem alephOperator_isClassFunctionOnTransfiniteSequences
    ...
    ℳ.IsClassFunctionOnTransfiniteSequences 𝕀 ((Definitional.Project.BinarySchema.alephOperator 𝒞).denote env) := by
  have hStep :=
  ...
  rcases hStep sequence hSequence with
    ⟨output, hOutput, hUnique⟩
  refine ⟨output, ?_, ?_⟩
  · exact (Definitional.Project.BinarySchema.denote_alephOperator_iff
...
theorem aleph_isClassFunctionOnOrdinals
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (ω : ℳ.Domain) :
    ℳ.IsClassFunctionOnOrdinals (fun index value => ℳ.IsAlephNumber 𝕀 ω index value) := by
  let env : Env ℳ 1 := {
    bound := fun _ => ω
    free := fun _ => Classical.choice ℳ.nonempty
  }
  have hClass :=
    transfiniteRecursion_isClassFunctionOnOrdinals
      hZF 𝕀 env (Definitional.Project.BinarySchema.alephOperator 𝒞) (alephOperator_isClassFunctionOnTransfiniteSequences
        hZF 𝕀 env)
  intro index hIndex
  rcases hClass index hIndex with
    ⟨value, hValue, hUnique⟩
  refine ⟨value, ?_, ?_⟩
  · have hSemantic := (Definitional.Project.BinarySchema.denote_aleph_iff
        𝕀 hZF.1 env index value).mp hValue
    simpa [env] using hSemantic
  · intro other hOther
    apply hUnique other
    apply (Definitional.Project.BinarySchema.denote_aleph_iff
        𝕀 hZF.1 env index other).mpr
    simpa [env] using hOther
/-- Aleph 数对每个序数指标存在唯一。 -/
theorem aleph_existsUnique
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (ω : ℳ.Domain) {index : ℳ.Domain} (hIndex : ℳ.IsOrdinal index) :
    ∃ value,
...
theorem aleph_zero_iff
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω zero value : ℳ.Domain} (hZero : ∀ member, ¬ ℳ.mem member zero) :
    ℳ.IsAlephNumber 𝕀 ω zero value ↔
      value = ω := by
  have value_eq {selected : ℳ.Domain} (hSelected :
  ...
  · intro hValue
    subst value
    rcases aleph_existsUnique hZF 𝕀 ω (Structure.IsOrdinal.of_no_members hZero) with
...
theorem aleph_successor_iff
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω predecessor successor value : ℳ.Domain} (hPredecessor : ℳ.IsOrdinal predecessor) (hSuccessor : ℳ.SuccessorOf successor predecessor) :
  ...
  have characterize {selected : ℳ.Domain} (hSelected :
...
theorem aleph_limit_iff
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω limit value : ℳ.Domain} (hLimit : ℳ.IsLimitOrdinal limit) :
  ...
  have characterize {selected : ℳ.Domain} (hSelected :
    ...
    · intro index hIndex first second hFirst hSecond
      rcases aleph_existsUnique hZF 𝕀 ω (hLimit.1.mem hIndex) with
        ⟨_, _, hUnique⟩
...
theorem aleph_isCardinal
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω index value : ℳ.Domain} (hωCardinal : ℳ.IsCardinal 𝕀 ω) (hIndex : ℳ.IsOrdinal index) (hValue : ℳ.IsAlephNumber 𝕀 ω index value) :
    ℳ.IsCardinal 𝕀 value := by
  let env : Env ℳ 1 := {
    bound := fun _ => ω
    free := fun _ => Classical.choice ℳ.nonempty
  ...
  have hProperty : property index := by
    ...
    · rcases separation_exists_d hZF (alephValueCardinalAt 𝒞).neg env index with
    ...
    · intro current hCurrent hPrevious selected hSelected
      ...
      · have hSelectedEq := (aleph_zero_iff hZF 𝕀 hZero).mp hSelected
...
theorem aleph_successor_cardinal_iff
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω predecessor successor value : ℳ.Domain} (hωCardinal : ℳ.IsCardinal 𝕀 ω) (hPredecessor : ℳ.IsOrdinal predecessor)
...
theorem aleph_isIncreasingOnOrdinals
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω : ℳ.Domain} (hωCardinal : ℳ.IsCardinal 𝕀 ω) :
    ℳ.IsIncreasingOnOrdinals (fun index value =>
        ℳ.IsAlephNumber 𝕀 ω index value) := by
  let env : Env ℳ 1 := {
    bound := fun _ => ω
    free := fun _ => Classical.choice ℳ.nonempty
  }
  have hRelation : (Definitional.Project.BinarySchema.aleph 𝒞).denote env =
        fun index value =>
          ℳ.IsAlephNumber 𝕀 ω index value := by
    funext index value
    apply propext
  ...
  have hIncreasing :
      ℳ.IsIncreasingOnOrdinals ((Definitional.Project.BinarySchema.aleph 𝒞).denote env) := by
    apply increasingOnOrdinals_of_progressive hZF env (Definitional.Project.BinarySchema.aleph 𝒞)
    ...
    · rcases hSuccessor with
      ...
      · have hEarlierValue :
        ...
        exact (aleph_isCardinal hZF 𝕀 hωCardinal hα hαValue).1.transitive
      ...
      · have hPredecessorEq :=
        ...
        rcases aleph_existsUnique hZF 𝕀 ω hImmediateOrdinal with
          ⟨_, _, hUnique⟩
        have hValueEq : predecessorValue = immediateValue := (hUnique predecessorValue hPredecessorValue).trans (hUnique immediateValue hImmediateValue).symm
    ...
    · rcases (aleph_limit_iff hZF 𝕀 hLimit).mp hαValue with
      ...
      rcases aleph_existsUnique hZF 𝕀 ω (hLimit.1.mem hLarger) with
...
theorem aleph_isContinuousOnOrdinals
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (ω : ℳ.Domain) :
    ℳ.IsContinuousOnOrdinals (fun index value =>
        ℳ.IsAlephNumber 𝕀 ω index value) := by
...
theorem aleph_isNormalOrdinalFunction
    ...
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω : ℳ.Domain} (hωCardinal : ℳ.IsCardinal 𝕀 ω) :
    ℳ.IsNormalOrdinalFunction (fun index value =>
        ℳ.IsAlephNumber 𝕀 ω index value) :=
...
theorem aleph_isInfiniteCardinal
  ...
  have hZeroValue : ℳ.IsAlephNumber 𝕀 ω zero ω := (aleph_zero_iff hZF 𝕀 hZero).mpr rfl
  rcases Structure.IsOrdinal.trichotomy hZF.1
      hZeroOrdinal hIndex (KP.difference_exists_d (ZF.modelsKP hZF)) (KP.intersection_exists_d (ZF.modelsKP hZF)
        zero index) with
  ...
  · have hIndexEq :=
    ...
    rcases aleph_existsUnique hZF 𝕀 ω hZeroOrdinal with
      ⟨_, _, hUnique⟩
    have hValueEq : value = ω := (hUnique value hValue).trans (hUnique ω hZeroValue).symm
    subst value
  ...
  · have hωValue : ℳ.mem ω value :=
    ...
    exact exists_inclusionInjection hZF 𝕀 <| (aleph_isCardinal hZF 𝕀 hωCardinal
