...
def IsCountablyInfinite {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (ω set : ℳ.Domain) : Prop :=
...
def IsCardinalSuccessor {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (successor predecessor : ℳ.Domain) : Prop :=
...
def IsAlephSuccessorOperation {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (previous output : ℳ.Domain) : Prop := (ℳ.IsOrdinal previous ∧
      ℳ.IsCardinalSuccessor 𝕀 output previous) ∨ (¬ ℳ.IsOrdinal previous ∧
      ∀ value, ¬ ℳ.mem value output)
/-- 以 `ω` 为首项、后继处取基数后继、极限处取并的 Aleph 递归步。 -/
def IsAlephStep {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (ω sequence output : ℳ.Domain) : Prop :=
  ℳ.IsZeroSuccessorLimitStep 𝕀 (fun initial => initial = ω) (ℳ.IsAlephSuccessorOperation 𝕀)
    sequence output
/-- `aleph` 是以 `ω` 为首项的 Aleph 枚举在 `index` 处的递归值。 -/
def IsAlephNumber {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (ω index aleph : ℳ.Domain) : Prop :=
  ℳ.IsRecursionValue 𝕀 (ℳ.IsAlephStep 𝕀 ω) index aleph
end Structure
...
theorem satisfies_isInfiniteCardinal_iff
    ...
    satisfies env (isInfiniteCardinal 𝒞 ω κ) ↔
      ℳ.IsInfiniteCardinal 𝕀 (ω.eval env) (κ.eval env) := by
...
theorem satisfies_isCountablyInfinite_iff
    ...
    satisfies env (isCountablyInfinite 𝒞 ω set) ↔
      ℳ.IsCountablyInfinite 𝕀 (ω.eval env) (set.eval env) := by
...
theorem satisfies_isAtMostCountable_iff
    ...
    satisfies env (isAtMostCountable 𝒞 ω set) ↔
      ℳ.IsAtMostCountable 𝕀 (ω.eval env) (set.eval env) := by
...
theorem satisfies_isCardinalSuccessor_iff
    ...
    satisfies env (isCardinalSuccessor 𝒞 successor predecessor) ↔
      ℳ.IsCardinalSuccessor 𝕀 (successor.eval env) (predecessor.eval env) := by
...
theorem denote_alephOperator_iff
    ...
    (alephOperator 𝒞).denote env sequence output ↔
      ℳ.IsAlephStep 𝕀 (env.bound 0) sequence output := by
...
theorem denote_aleph_iff
    ...
    (aleph 𝒞).denote env index value ↔
      ℳ.IsAlephNumber 𝕀 (env.bound 0) index value := by
  have hOperator : (alephOperator 𝒞).denote env =
...
theorem satisfies_isAlephNumber_iff
    ...
    satisfies env (isAlephNumber 𝒞 ω index aleph) ↔
      ℳ.IsAlephNumber 𝕀 (ω.eval env) (index.eval env) (aleph.eval env) := by
  rw [isAlephNumber, satisfies_related_iff]
  exact BinarySchema.denote_aleph_iff
    𝕀 hExt ((TermVector.singleton ω).evalEnv env) (index.eval env) (aleph.eval env)
end Formula
end Project
end Definitional
end SetTheory
