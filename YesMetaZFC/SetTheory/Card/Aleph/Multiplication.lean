...
private def infiniteCardinalSelfMultiplicationAt (𝒞 : Definitional.Project.OrderedPairConvention) :
...
private theorem satisfies_infiniteCardinalSelfMultiplicationAt_iff
    {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 1) (cardinal : ℳ.Domain) :
    Definitional.Project.Formula.satisfies (env.push cardinal) (infiniteCardinalSelfMultiplicationAt 𝒞).body ↔
  ...
  change (ℳ.IsInfiniteCardinal 𝕀 (env.bound 0) cardinal →
...
theorem infiniteCardinal_isLimitOrdinal
    ...
    {ω κ : ℳ.Domain} (hω : ℳ.IsOmega ω) (hωCardinal : ℳ.IsCardinal 𝕀 ω) (hκ : ℳ.IsInfiniteCardinal 𝕀 ω κ) :
    ℳ.IsLimitOrdinal κ := by
  have hωOrdinal := hω.isOrdinal hZF
  have hκOrdinal := hκ.1.1
  rcases Structure.IsOrdinal.trichotomy hZF.1
      hωOrdinal hκOrdinal (KP.difference_exists_d (ZF.modelsKP hZF)) (KP.intersection_exists_d (ZF.modelsKP hZF) ω κ) with
    hSame | hωκ | hκω
  ...
  · refine ⟨hκOrdinal, ⟨ω, hωκ⟩, ?_⟩
    ...
    rcases Structure.IsOrdinal.trichotomy hZF.1
        hSuccessorOrdinal hκOrdinal (KP.difference_exists_d (ZF.modelsKP hZF)) (KP.intersection_exists_d (ZF.modelsKP hZF)
          successor κ) with
...
theorem infiniteCardinal_selfMultiplication_of_orderType_not_above
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω κ ordinal : ℳ.Domain} (hω : ℳ.IsOmega ω) (hκ : ℳ.IsInfiniteCardinal 𝕀 ω κ) (hType : ℳ.IsCanonicalOrdinalPairOrderType 𝕀 ordinal κ)
  ...
  have hOrdinalEq : ordinal = κ := by
    rcases Structure.IsOrdinal.trichotomy hZF.1
        hOrdinal hκ.1.1 (KP.difference_exists_d (ZF.modelsKP hZF)) (KP.intersection_exists_d (ZF.modelsKP hZF)
          ordinal κ) with
...
theorem infiniteCardinal_selfMultiplication_of_successorSquareBound
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω κ ordinal : ℳ.Domain} (hω : ℳ.IsOmega ω) (hκ : ℳ.IsInfiniteCardinal 𝕀 ω κ) (hType : ℳ.IsCanonicalOrdinalPairOrderType 𝕀 ordinal κ) (hBound :
...
theorem cartesianSquare_cardinalLessOrEqual_of_selfMultiplication
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {set square κ : ℳ.Domain} (hκSet : ℳ.IsCardinalOf 𝕀 κ set) (hSquare : ℳ.IsCartesianProduct 𝕀 square set set) (hκMultiplication :
...
theorem cartesianSquare_bounded_in_omega
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω number square : ℳ.Domain} (hω : ℳ.IsOmega ω) (hNumber : ℳ.mem number ω) (hSquare :
...
theorem omega_selfMultiplication
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω : ℳ.Domain} (hω : ℳ.IsOmega ω) (hωCardinal : ℳ.IsCardinal 𝕀 ω) :
...
theorem infiniteCardinal_selfMultiplication_of_successorCardinalClosure
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω κ ordinal : ℳ.Domain} (hω : ℳ.IsOmega ω) (hκ : ℳ.IsInfiniteCardinal 𝕀 ω κ) (hType : ℳ.IsCanonicalOrdinalPairOrderType 𝕀 ordinal κ) (hClosure :
      ∀ maximum, ℳ.mem maximum κ →
        ∀ successor, ℳ.SuccessorOf successor maximum →
          ∃ μ,
...
theorem infiniteCardinal_selfMultiplication
  ...
  have hProperty : property κ := by
    ...
    · rcases separation_exists_d hZF (infiniteCardinalSelfMultiplicationAt 𝒞).neg env κ with
    ...
    · intro current hCurrent hPrevious hCurrentInfinite
      have hωOrdinal := hω.isOrdinal hZF
      rcases Structure.IsOrdinal.trichotomy hZF.1
          hωOrdinal hCurrent (KP.difference_exists_d (ZF.modelsKP hZF)) (KP.intersection_exists_d (ZF.modelsKP hZF)
            ω current) with
      ...
      · rcases canonicalOrdinalPairOrderType_existsUnique
        ...
        have hμCurrent : ℳ.mem μ current := by
          rcases Structure.IsOrdinal.trichotomy hZF.1
              hμSuccessor.1.1 hSuccessorOrdinal (KP.difference_exists_d (ZF.modelsKP hZF)) (KP.intersection_exists_d (ZF.modelsKP hZF)
                μ successor) with
        ...
        rcases Structure.IsOrdinal.trichotomy hZF.1
            hμSuccessor.1.1 hωOrdinal (KP.difference_exists_d (ZF.modelsKP hZF)) (KP.intersection_exists_d (ZF.modelsKP hZF)
              μ ω) with
          hμSame | hμω | hωμ
        · have hμEq :=
          ...
          have hμInfinite :
            ...
            simpa [hμEq] using (show ℳ.IsInfiniteCardinal 𝕀 ω ω from
                ⟨hωCardinal,
...
theorem aleph_selfMultiplication
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω index value : ℳ.Domain} (hω : ℳ.IsOmega ω) (hωCardinal : ℳ.IsCardinal 𝕀 ω) (hIndex : ℳ.IsOrdinal index) (hValue : ℳ.IsAlephNumber 𝕀 ω index value) :
