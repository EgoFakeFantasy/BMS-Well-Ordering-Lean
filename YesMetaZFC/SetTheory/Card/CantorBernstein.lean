...
def IsCantorBernsteinClosed {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (forward reverse source target closed : ℳ.Domain) : Prop :=
  (∀ input, ℳ.mem input source → (¬ ∃ output, ℳ.mem output target ∧
...
def IsCantorBernsteinValue {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (forward reverse closed input output : ℳ.Domain) : Prop :=
  (ℳ.mem input closed ∧
    ℳ.PairMember 𝕀 input output forward) ∨ (¬ ℳ.mem input closed ∧
    ℳ.PairMember 𝕀 output input reverse)
end Structure
...
def isCantorBernsteinClosed (𝒞 : OrderedPairConvention)
    {depth : Nat} (forward reverse source target closed : Term depth) :
    Formula 1 depth :=
  .conj (Formula.forallMem source <|
      .imp (.neg <| Formula.existsMem target.weaken <|
          orderedPairMem 𝒞 Term.newest (.bound 1)
            reverse.weaken.weaken) (.mem Term.newest closed.weaken)) <|
...
theorem satisfies_isCantorBernsteinClosed_iff
    ...
    satisfies env (isCantorBernsteinClosed 𝒞
          forward reverse source target closed) ↔
      ℳ.IsCantorBernsteinClosed 𝕀 (forward.eval env) (reverse.eval env) (source.eval env) (target.eval env) (closed.eval env) := by
...
theorem satisfies_cantorBernsteinClosure_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (env : Env ℳ 4) (value : ℳ.Domain) :
    satisfies (env.push value) (UnarySchema.cantorBernsteinClosure 𝒞).body ↔
      ∀ closed,
        ℳ.IsCantorBernsteinClosed 𝕀 (env.bound 0) (env.bound 1) (env.bound 2) (env.bound 3) closed →
...
theorem denote_cantorBernsteinGraph_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (env : Env ℳ 3) (input pair : ℳ.Domain) :
    (BinarySchema.cantorBernsteinGraph 𝒞).denote
        env input pair ↔
      ∃ output,
        ℳ.IsCantorBernsteinValue 𝕀 (env.bound 0) (env.bound 1) (env.bound 2)
          input output ∧
...
private theorem input_mem_of_pairMember
    ...
    ℳ.mem input source := (hFunction.2.1 input).mpr ⟨output, hPair⟩
...
theorem equinumerous_of_injections
    ...
    {source target forward reverse : ℳ.Domain} (hForward :
      ℳ.IsSetInjectionFromTo 𝕀 forward source target) (hReverse :
  ...
  rcases separation_exists_d hZF (Definitional.Project.UnarySchema.cantorBernsteinClosure 𝒞)
  ...
  have hClosedIs :
    ...
    · intro input middle output hInput hForwardPair hReversePair
      ...
      intro candidate hCandidate
      exact
        hCandidate.2 input middle output (((hClosedSemantic input).mp hInput).2
  ...
  have hForwardPreimage_of_reverse_mem (output : ℳ.Domain) (hOutput : ℳ.mem output target) (input : ℳ.Domain)
      (hReversePair : ℳ.PairMember 𝕀 output input reverse) (hInputClosed : ℳ.mem input closed) :
      ∃ predecessor,
    ...
    · exfalso
      ...
      rcases KP.difference_exists_d (modelsKP hZF) singleton closed with
        ⟨reduced, hReduced⟩
      have hReducedClosed :
          ℳ.IsCantorBernsteinClosed 𝕀
        ...
        · intro current hCurrent hNoReverse
          ...
          have hCurrentEq := (hSingleton current).mp hCurrentSingleton
          subst current
        ...
        · intro current middle next hCurrent hForwardPair hNext
          ...
          have hMiddleEq :=
            hReverseInjective middle output input (by simpa [hNextEq] using hNext)
              hReversePair
          subst middle
          exact hExists
      ...
      exact ((hReduced input).mp hInputReduced).2 <| (hSingleton input).mpr rfl
  ...
  have hGraphTotal :
      ∀ input, ℳ.mem input source →
        ∃ pair, (Definitional.Project.BinarySchema.cantorBernsteinGraph 𝒞).denote
    ...
    rcases 𝕀.total input output with
      ⟨pair, hCode⟩
    refine ⟨pair, ?_⟩
    apply (Definitional.Project.Formula.denote_cantorBernsteinGraph_iff
        𝕀 graphEnv input pair).mpr
    exact ⟨output, hValue, hCode⟩
  have hGraphUnique :
      ∀ input, ℳ.mem input source →
        ∀ first second, (Definitional.Project.BinarySchema.cantorBernsteinGraph 𝒞).denote
              graphEnv input first → (Definitional.Project.BinarySchema.cantorBernsteinGraph 𝒞).denote
              graphEnv input second →
            first = second := by
    intro input _ first second hFirst hSecond
    rcases (Definitional.Project.Formula.denote_cantorBernsteinGraph_iff
  ...
  rcases exists_functionalImageOn hZF (Definitional.Project.BinarySchema.cantorBernsteinGraph 𝒞)
  ...
  have hSetFunction : ℳ.IsSetFunction 𝕀 bijection := by
    ...
    intro input first second hFirst hSecond
    exact
      hValueUnique input first second ((hPairMember input first).mp hFirst).2 ((hPairMember input second).mp hSecond).2
  ...
  have hIntoTarget :
    ...
    exact
      ⟨output, hOutput, (hPairMember input output).mpr
          ⟨hInput, hValue⟩⟩
  ...
  have hSurjective :
    ...
    · rcases hForwardImage with
      ...
      exact
        ⟨input, hInput, (hPairMember input output).mpr
            ⟨hInput, Or.inl
...
theorem equinumerous_of_cardinalLessOrEqual
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {left right : ℳ.Domain} (hLeft : ℳ.CardinalLessOrEqual 𝕀 left right) (hRight : ℳ.CardinalLessOrEqual 𝕀 right left) :
