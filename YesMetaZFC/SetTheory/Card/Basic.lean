...
def Equinumerous {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (left right : ℳ.Domain) : Prop :=
  ∃ function,
...
def IsCardinalOf {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (κ set : ℳ.Domain) : Prop :=
  ℳ.IsCardinal 𝕀 κ ∧
    ℳ.Equinumerous 𝕀 κ set
namespace Equinumerous
/-- 任意集合与自身等势。 -/
theorem refl {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
...
/-- 等势关系具有对称性。 -/
theorem symm {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {left right : ℳ.Domain} (hEquinumerous : ℳ.Equinumerous 𝕀 left right) :
...
/-- 等势关系具有传递性。 -/
theorem trans {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {left middle right : ℳ.Domain} (hLeft : ℳ.Equinumerous 𝕀 left middle) (hRight : ℳ.Equinumerous 𝕀 middle right) :
...
/-- 两个等势的初始序数相等。 -/
theorem eq_of_equinumerous {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {κ μ : ℳ.Domain} (hκ : ℳ.IsCardinal 𝕀 κ) (hμ : ℳ.IsCardinal 𝕀 μ) (hEquinumerous : ℳ.Equinumerous 𝕀 κ μ) :
    κ = μ := by
...
theorem eq {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {κ μ set : ℳ.Domain} (hκ : ℳ.IsCardinalOf 𝕀 κ set) (hμ : ℳ.IsCardinalOf 𝕀 μ set) :
    κ = μ := by
  apply hκ.1.eq_of_equinumerous hZF 𝕀 hμ.1
  exact hκ.2.trans hZF 𝕀 <| hμ.2.symm hZF 𝕀
/-- 等势集合具有相同的基数。 -/
theorem of_equinumerous {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {κ left right : ℳ.Domain} (hκ : ℳ.IsCardinalOf 𝕀 κ left) (hEquinumerous : ℳ.Equinumerous 𝕀 left right) :
...
theorem satisfies_isInjectionFromTo_iff
    ...
    satisfies env (isInjectionFromTo 𝒞 function source target) ↔
      ℳ.IsSetInjectionFromTo 𝕀 (function.eval env) (source.eval env) (target.eval env) := by
...
theorem satisfies_isBijectionFromTo_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ)
    {depth : Nat} (env : Env ℳ depth) (function source target : Term depth) :
    satisfies env (isBijectionFromTo 𝒞 function source target) ↔
      ℳ.IsSetBijectionFromTo 𝕀 (function.eval env) (source.eval env) (target.eval env) := by
...
theorem satisfies_cardinalLessOrEqual_iff
    ...
    satisfies env (cardinalLessOrEqual 𝒞 left right) ↔
      ℳ.CardinalLessOrEqual 𝕀 (left.eval env) (right.eval env) := by
