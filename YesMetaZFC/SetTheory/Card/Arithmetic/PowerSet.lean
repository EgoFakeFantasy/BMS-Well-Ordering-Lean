...
def IsCharacteristicFunctionOf {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (source two zero one subset function : ℳ.Domain) : Prop :=
  ℳ.IsSetFunctionFromTo 𝕀 function source two ∧
    ∀ input output,
      ℳ.PairMember 𝕀 input output function ↔
        ℳ.mem input source ∧ ((ℳ.mem input subset ∧ output = one) ∨ (¬ ℳ.mem input subset ∧ output = zero))
end Structure
...
theorem denote_characteristicValue_iff
    ...
    BinarySchema.characteristicValue.denote env input output ↔ (ℳ.mem input (env.bound 0) ∧ output = env.bound 2) ∨
...
theorem denote_characteristicFunction_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 4) (subset function : ℳ.Domain) :
    (BinarySchema.characteristicFunction 𝒞).denote
        env subset function ↔
      ℳ.IsCharacteristicFunctionOf 𝕀 (env.bound 0) (env.bound 1) (env.bound 2) (env.bound 3) subset function := by
...
theorem equinumerous_powerSet_functionSpace
    ...
    {source power two zero one space : ℳ.Domain} (hPower : ℳ.IsPowerSetOf power source) (hTwo : ∀ value, ℳ.mem value two ↔
      value = zero ∨ value = one) (hDistinct : zero ≠ one) (hSpace : ℳ.IsFunctionSpace 𝕀 space source two) :
  ...
  · intro subset hSubsetPower
    ...
    rcases exists_setFunctionFromTo_of_denote
        hZF 𝕀 Definitional.Project.BinarySchema.characteristicValue valueEnv (source := source) (target := two) (by
          intro input _
          classical
          by_cases hInput : ℳ.mem input subset
          · exact ⟨one, (Definitional.Project.Formula.denote_characteristicValue_iff
                hZF.1 valueEnv input one).mpr <|
                  Or.inl ⟨hInput, rfl⟩⟩
          · exact ⟨zero, (Definitional.Project.Formula.denote_characteristicValue_iff
                hZF.1 valueEnv input zero).mpr <|
                  Or.inr ⟨hInput, rfl⟩⟩) (by
          ...
          · exact hFirst.2.trans hSecond.2.symm) (by
    ...
    refine ⟨function, (Definitional.Project.Formula.denote_characteristicFunction_iff
  ...
  · intro first second function hFirstPower hSecondPower hFirst hSecond
    ...
    constructor
    · intro hInputFirst
      have hInputSource := (hPower first).mp hFirstPower input hInputFirst
      have hPair :
          ℳ.PairMember 𝕀 input one function := (hFirst.2 input one).mpr
    ...
    · intro hInputSecond
      have hInputSource := (hPower second).mp hSecondPower input hInputSecond
      have hPair :
          ℳ.PairMember 𝕀 input one function := (hSecond.2 input one).mpr
  ...
  · intro function hFunctionSpace
    ...
    rcases separation_exists_d hZF (Definitional.Project.UnarySchema.oneFiber 𝒞) fiberEnv source with
      ⟨subset, hSubset⟩
    have hSubsetPower : ℳ.mem subset power := by
      apply (hPower subset).mpr
      intro input hInput
      exact ((hSubset input).mp hInput).1
    refine ⟨subset, hSubsetPower, (Definitional.Project.Formula.denote_characteristicFunction_iff
        𝕀 hZF.1 env subset function).mpr ?_⟩
    refine ⟨hFunction, fun input output => ?_⟩
    constructor
    · intro hPair
      ...
      · refine ⟨hInputSource, Or.inr ⟨?_, hOutputZero⟩⟩
        ...
        have hOnePair := (Definitional.Project.Formula.satisfies_oneFiber_iff
            𝕀 fiberEnv input).mp <| ((hSubset input).mp hInputSubset).2
...
theorem twoExponentiation_isCardinalOf_powerSet
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {source power κ two twoSet zero one result : ℳ.Domain} (hPower : ℳ.IsPowerSetOf power source) (hκ : ℳ.IsCardinalOf 𝕀 κ source)
    (hTwoCardinal : ℳ.IsCardinalOf 𝕀 two twoSet) (hTwo : ∀ value, ℳ.mem value twoSet ↔
      value = zero ∨ value = one) (hDistinct : zero ≠ one) (hExponentiation :
