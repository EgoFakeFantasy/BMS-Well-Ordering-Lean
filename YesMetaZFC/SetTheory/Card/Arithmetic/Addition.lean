...
theorem eq {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    ...
    {first second left right : ℳ.Domain} (hFirst : ℳ.IsCardinalAddition 𝕀 first left right) (hSecond : ℳ.IsCardinalAddition 𝕀 second left right) :
    first = second := by
  rcases hFirst with
...
theorem commutative {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {first second left right : ℳ.Domain} (hFirst : ℳ.IsCardinalAddition 𝕀 first left right) (hSecond : ℳ.IsCardinalAddition 𝕀 second right left) :
    first = second := by
  rcases hFirst with
...
theorem associative {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    ...
    {first second third firstSecond secondThird
      ...
      ℳ.IsCardinalAddition 𝕀 firstSecond first second) (hLeftAssociated :
      ℳ.IsCardinalAddition 𝕀
        leftAssociated firstSecond third) (hSecondThird :
      ℳ.IsCardinalAddition 𝕀 secondThird second third) (hRightAssociated :
      ℳ.IsCardinalAddition 𝕀
  ...
  rcases ZF.tagged_union_exists_d hZF 𝕀 hTags firstSet secondSet with
    ⟨leftMiddle, hLeftMiddle⟩
  rcases ZF.tagged_union_exists_d hZF 𝕀 hTags leftMiddle thirdSet with
  ...
  rcases ZF.tagged_union_exists_d hZF 𝕀 hTags firstSet middleRight with
    ⟨normalizedRight, hNormalizedRight⟩
  have hFirstUnionToLeftMiddle :=
    ZF.equinumerous_unionToTaggedUnion hZF 𝕀
      hFirstUnion hFirstDisjoint hLeftMiddle (Structure.Equinumerous.refl hZF 𝕀 firstSet) (Structure.Equinumerous.refl hZF 𝕀 secondSet)
  ...
  have hLeftNormalized :=
    ZF.equinumerous_unionToTaggedUnion hZF 𝕀
      ...
      hFirstSecondToLeftMiddle (Structure.Equinumerous.refl hZF 𝕀 thirdSet)
  have hAssociate :=
  ...
  exact hLeftCardinal.2.trans hZF 𝕀 <|
    hLeftNormalized.trans hZF 𝕀 <|
      hAssociate.trans hZF 𝕀 <| (hRightToNormalized.symm hZF 𝕀).trans hZF 𝕀 <|
