...
def isCardinalAddition (𝒞 : OrderedPairConvention)
  ...
  .existsE <| .existsE <| .existsE <|
    ...
    .conj (isUnionOfTwo (.bound 0) (.bound 2) (.bound 1)) (isCardinalOf 𝒞 sum.weaken.weaken.weaken (.bound 0))
...
def isCardinalExponentiation (𝒞 : OrderedPairConvention)
  ...
  .existsE <| .existsE <| .existsE <|
    .conj (isCardinalOf 𝒞 base.weaken.weaken.weaken (.bound 2)) <|
    .conj (isCardinalOf 𝒞 exponent.weaken.weaken.weaken (.bound 1)) <|
    .conj (isFunctionSpace 𝒞 (.bound 0) (.bound 1) (.bound 2)) (isCardinalOf 𝒞 power.weaken.weaken.weaken (.bound 0))
end Formula
end Project
end Definitional
end SetTheory
