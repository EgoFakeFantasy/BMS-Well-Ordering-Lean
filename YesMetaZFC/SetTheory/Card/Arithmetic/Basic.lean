...
theorem satisfies_isCardinalAddition_iff
    ...
    satisfies env (isCardinalAddition 𝒞 sum left right) ↔
      ℳ.IsCardinalAddition 𝕀 (sum.eval env) (left.eval env) (right.eval env) := by
...
theorem satisfies_isCardinalMultiplication_iff
    ...
    satisfies env (isCardinalMultiplication 𝒞 product left right) ↔
      ℳ.IsCardinalMultiplication 𝕀 (product.eval env) (left.eval env) (right.eval env) := by
...
theorem satisfies_isCardinalExponentiation_iff
    ...
    satisfies env (isCardinalExponentiation 𝒞 power base exponent) ↔
      ℳ.IsCardinalExponentiation 𝕀 (power.eval env) (base.eval env) (exponent.eval env) := by
