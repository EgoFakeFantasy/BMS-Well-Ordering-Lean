...
theorem eq {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    ...
    {first second left right : ℳ.Domain} (hFirst :
      ℳ.IsCardinalMultiplication 𝕀 first left right) (hSecond :
...
/-- 基数乘法满足交换律。 -/
theorem commutative {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {first second left right : ℳ.Domain} (hFirst :
      ℳ.IsCardinalMultiplication 𝕀 first left right) (hSecond :
...
theorem associative {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    ...
    {first second third firstSecond secondThird
      leftAssociated rightAssociated : ℳ.Domain} (hFirstSecond :
      ℳ.IsCardinalMultiplication 𝕀 firstSecond first second) (hLeftAssociated :
      ℳ.IsCardinalMultiplication 𝕀
        leftAssociated firstSecond third) (hSecondThird :
      ℳ.IsCardinalMultiplication 𝕀 secondThird second third) (hRightAssociated :
      ℳ.IsCardinalMultiplication 𝕀
...
/-- 基数乘法对基数加法满足左分配律。 -/
theorem leftDistributive {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {first second third secondThird firstSecond firstThird
      leftDistributed rightDistributed : ℳ.Domain} (hSecondThird :
      ℳ.IsCardinalAddition 𝕀 secondThird second third) (hLeftDistributed :
      ℳ.IsCardinalMultiplication 𝕀
        leftDistributed first secondThird) (hFirstSecond :
      ℳ.IsCardinalMultiplication 𝕀 firstSecond first second) (hFirstThird :
      ℳ.IsCardinalMultiplication 𝕀 firstThird first third) (hRightDistributed :
  ...
  have hTags : leftTag ≠ rightTag := by
    intro hEq
    have hMember : ℳ.mem leftTag rightTag := (hRightTag leftTag).mpr rfl
    rw [← hEq] at hMember
    exact hLeftTag leftTag hMember
  rcases ZF.tagged_union_exists_d hZF 𝕀 hTags secondSet thirdSet with
  ...
  rcases ZF.tagged_union_exists_d hZF 𝕀 hTags
      normalizedFirstProduct normalizedThirdProduct with
    ⟨normalizedRight, hNormalizedRight⟩
  have hSecondUnionToNormalized :=
    ZF.equinumerous_unionToTaggedUnion hZF 𝕀
      hSecondUnion hSecondDisjoint hNormalizedSum (Structure.Equinumerous.refl hZF 𝕀 secondSet) (Structure.Equinumerous.refl hZF 𝕀 thirdSet)
  ...
  have hNormalizedFirstToRight :
    ...
    hNormalizedFirstToProduct.trans hZF 𝕀 <| (hFirstSecondCardinal.2.symm hZF 𝕀).trans hZF 𝕀
        hFirstSecondSet.2
  have hLeftToThird :
      ℳ.Equinumerous 𝕀 leftSet firstSet' :=
    hLeftSet.2.symm hZF 𝕀 |>.trans hZF 𝕀 hFirstSet'.2
  ...
  have hNormalizedThirdToRight :
    ...
    hNormalizedThirdToProduct.trans hZF 𝕀 <| (hFirstThirdCardinal.2.symm hZF 𝕀).trans hZF 𝕀
        hFirstThirdSet.2
  have hRightToNormalized :=
    ZF.equinumerous_unionToTaggedUnion hZF 𝕀
      hRightUnion hRightDisjoint hNormalizedRight (hNormalizedFirstToRight.symm hZF 𝕀) (hNormalizedThirdToRight.symm hZF 𝕀)
  apply hLeftCardinal.1.eq_of_equinumerous
      hZF 𝕀 hRightCardinal.1
  exact hLeftCardinal.2.trans hZF 𝕀 <|
    hLeftNormalized.trans hZF 𝕀 <|
      hDistribute.trans hZF 𝕀 <| (hRightToNormalized.symm hZF 𝕀).trans hZF 𝕀 <|
          hRightCardinal.2.symm hZF 𝕀
