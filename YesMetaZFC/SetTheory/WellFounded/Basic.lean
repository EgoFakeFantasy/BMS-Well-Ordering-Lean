...
def IsMinimal {P : Type u} (E : P → P → Prop) (part : P → Prop) (candidate : P) : Prop :=
  part candidate ∧
...
/-- `sequence` 沿关系 `E` 严格下降。 -/
def IsDescendingSequence {P : Type u} (E : P → P → Prop) (sequence : Nat → P) : Prop :=
  ∀ index, E (sequence index.succ) (sequence index)
/-- Lean 良基关系满足纸面的极小元定义。 -/
theorem min_exists_of_wf_l {P : Type u}
...
theorem wf_of_min_exists_l {P : Type u}
...
theorem wf_iff_min_exists_l {P : Type u}
    {E : P → P → Prop} :
    _root_.WellFounded E ↔ HasMinimalElements E :=
  ⟨min_exists_of_wf_l,
    wf_of_min_exists_l⟩
/-- 由纸面良基定义得到的良基归纳原理。 -/
theorem induction {P : Type u} {E : P → P → Prop} (hE : HasMinimalElements E) (property : P → Prop) (hProgressive : ∀ current,
      (∀ predecessor, E predecessor current → property predecessor) →
        property current) :
    ∀ current, property current :=
  fun current =>
    (wf_of_min_exists_l hE).induction
      current hProgressive
/-- Lean 良基关系上没有给定的无限下降序列。 -/
theorem no_desc_of_wf_l
    {P : Type u} {E : P → P → Prop} (hE : _root_.WellFounded E) (sequence : Nat → P) :
...
theorem no_inf_desc_of_wf_l
    {P : Type u} {E : P → P → Prop} (hE : _root_.WellFounded E) :
...
theorem no_inf_desc_l
    {P : Type u} {E : P → P → Prop} (hE : HasMinimalElements E) :
    ¬ ∃ sequence, IsDescendingSequence E sequence :=
  no_inf_desc_of_wf_l (wf_of_min_exists_l hE)
end WellFounded
end SetTheory
end YesMetaZFC
