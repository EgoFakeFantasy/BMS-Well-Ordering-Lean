...
def Predecessor {P : Type u} (E : P → P → Prop) (current : P) : Type u :=
  {predecessor // E predecessor current}
/-- 将一个全域函数限制到 `current` 的所有前驱。 -/
def restrict {P : Type u} {E : P → P → Prop}
    {C : P → Sort v} (solution : ∀ current, C current) (current : P) :
...
def SatisfiesEquation {P : Type u} {E : P → P → Prop}
    {C : P → Sort v} (step : ∀ current, (∀ predecessor, E predecessor current → C predecessor) →
        C current) (solution : ∀ current, C current) : Prop :=
  ∀ current,
    solution current =
      step current (fun predecessor _ => solution predecessor)
/-- 良基递归方程的规范解。 -/
noncomputable def solve {P : Type u} {E : P → P → Prop}
    {C : P → Sort v} (hE : _root_.WellFounded E) (step : ∀ current, (∀ predecessor, E predecessor current → C predecessor) →
        C current) :
    ∀ current, C current :=
  hE.fix step
/-- 规范解满足对应的递归方程。 -/
theorem solve_satisfiesEquation {P : Type u}
    {E : P → P → Prop} {C : P → Sort v} (hE : _root_.WellFounded E) (step : ∀ current, (∀ predecessor, E predecessor current → C predecessor) →
        C current) :
...
theorem eq_of_satisfiesEquation {P : Type u}
    {E : P → P → Prop} {C : P → Sort v} (hE : _root_.WellFounded E) (step : ∀ current, (∀ predecessor, E predecessor current → C predecessor) →
        C current)
    {left right : ∀ current, C current} (hLeft : SatisfiesEquation step left) (hRight : SatisfiesEquation step right) :
    left = right := by
  funext current
...
theorem existsUnique {P : Type u} {E : P → P → Prop}
  ...
  intro other hOther
  exact eq_of_satisfiesEquation hE step hOther (solve_satisfiesEquation hE step)
/-- 纸面极小元定义下的良基递归方程同样存在唯一解。 -/
theorem existsUnique_of_hasMinimalElements
    {P : Type u} {E : P → P → Prop} {C : P → Sort v} (hE : HasMinimalElements E) (step : ∀ current, (∀ predecessor, E predecessor current → C predecessor) →
        C current) :
    ∃ solution,
      SatisfiesEquation step solution ∧
        ∀ other, SatisfiesEquation step other →
          other = solution :=
  existsUnique (wf_of_min_exists_l hE) step
/-- 把消费前驱限制函数的算子转成 Lean 核心良基递归所需的一步算子。 -/
def restrictionStep {P : Type u} {E : P → P → Prop}
    {C : P → Sort v} (G : ∀ current, (∀ predecessor : Predecessor E current, C predecessor.1) →
        C current) (current : P) (previous : ∀ predecessor,
...
def SatisfiesRestrictionEquation
    {P : Type u} {E : P → P → Prop} {C : P → Sort v} (G : ∀ current, (∀ predecessor : Predecessor E current, C predecessor.1) →
        C current) (solution : ∀ current, C current) : Prop :=
  ∀ current,
    solution current =
...
theorem satisfiesRestrictionEquation_iff
    ...
    {G : ∀ current, (∀ predecessor : Predecessor E current, C predecessor.1) →
        C current}
...
noncomputable def solveRestriction
    {P : Type u} {E : P → P → Prop} {C : P → Sort v} (hE : _root_.WellFounded E) (G : ∀ current, (∀ predecessor : Predecessor E current, C predecessor.1) →
        C current) :
    ∀ current, C current :=
...
theorem eq_solveRestriction
    {P : Type u} {E : P → P → Prop} {C : P → Sort v} (hE : _root_.WellFounded E) (G : ∀ current, (∀ predecessor : Predecessor E current, C predecessor.1) →
        C current)
    {solution : ∀ current, C current} (hSolution : SatisfiesRestrictionEquation G solution) :
    solution = solveRestriction hE G :=
  eq_of_satisfiesEquation hE (restrictionStep G) (satisfiesRestrictionEquation_iff.mp hSolution) (satisfiesRestrictionEquation_iff.mp <|
...
theorem restriction_existsUnique_of_hasMinimalElements
    {P : Type u} {E : P → P → Prop} {C : P → Sort v} (hE : HasMinimalElements E) (G : ∀ current, (∀ predecessor : Predecessor E current, C predecessor.1) →
        C current) :
    ∃ solution,
      SatisfiesRestrictionEquation G solution ∧
        ∀ other, SatisfiesRestrictionEquation G other →
          other = solution :=
  restriction_existsUnique (wf_of_min_exists_l hE) G
end Recursion
end WellFounded
end SetTheory
end YesMetaZFC
