...
/-- 当前秩是所有前驱秩的后继之上确界。 -/
def step {P : Type u} {OrdinalValue : Type v} (E : P → P → Prop) (operations : SuccessorSupremum.{u, v} OrdinalValue) (current : P) (previous : ∀ predecessor,
      E predecessor current → OrdinalValue) :
    OrdinalValue :=
  operations.supremum fun predecessor :
      Recursion.Predecessor E current =>
    operations.successor (previous predecessor.1 predecessor.2)
/-- `ρ` 满足良基关系的秩递归方程。 -/
def IsRankFunction {P : Type u} {OrdinalValue : Type v} (E : P → P → Prop) (operations : SuccessorSupremum.{u, v} OrdinalValue) (ρ : P → OrdinalValue) : Prop :=
...
noncomputable def functionOfWellFounded
    {P : Type u} {OrdinalValue : Type v}
    {E : P → P → Prop} (hE : _root_.WellFounded E) (operations : SuccessorSupremum.{u, v} OrdinalValue) :
    P → OrdinalValue :=
...
theorem functionOfWellFounded_isRankFunction
    {P : Type u} {OrdinalValue : Type v}
    {E : P → P → Prop} (hE : _root_.WellFounded E) (operations : SuccessorSupremum.{u, v} OrdinalValue) :
    IsRankFunction E operations (functionOfWellFounded hE operations) :=
  Recursion.solve_satisfiesEquation hE (step E operations)
/-- 规范秩函数在每一点等于所有前驱秩后继的上确界。 -/
theorem functionOfWellFounded_apply
    {P : Type u} {OrdinalValue : Type v}
    {E : P → P → Prop} (hE : _root_.WellFounded E) (operations : SuccessorSupremum.{u, v} OrdinalValue) (current : P) :
    functionOfWellFounded hE operations current =
      operations.supremum (fun predecessor :
            Recursion.Predecessor E current =>
          operations.successor (functionOfWellFounded hE operations predecessor.1)) :=
  isRankFunction_iff.mp (functionOfWellFounded_isRankFunction hE operations)
    current
/-- 任意满足方程的秩函数都等于规范秩函数。 -/
theorem eq_functionOfWellFounded
    {P : Type u} {OrdinalValue : Type v}
    {E : P → P → Prop} (hE : _root_.WellFounded E) (operations : SuccessorSupremum.{u, v} OrdinalValue)
    {ρ : P → OrdinalValue} (hρ : IsRankFunction E operations ρ) :
    ρ = functionOfWellFounded hE operations :=
  Recursion.eq_of_satisfiesEquation hE (step E operations) hρ (functionOfWellFounded_isRankFunction hE operations)
/-- 由纸面极小元定义确定的规范秩函数。 -/
noncomputable def function
    {P : Type u} {OrdinalValue : Type v}
    {E : P → P → Prop} (hE : HasMinimalElements E) (operations : SuccessorSupremum.{u, v} OrdinalValue) :
    P → OrdinalValue :=
  functionOfWellFounded (wf_of_min_exists_l hE) operations
/-- 纸面定义下的规范秩函数满足秩递归方程。 -/
theorem function_isRankFunction
    {P : Type u} {OrdinalValue : Type v}
    {E : P → P → Prop} (hE : HasMinimalElements E) (operations : SuccessorSupremum.{u, v} OrdinalValue) :
    IsRankFunction E operations (function hE operations) :=
  functionOfWellFounded_isRankFunction (wf_of_min_exists_l hE) operations
/-- 纸面定义下的规范秩函数逐点满足上确界方程。 -/
theorem function_apply
    {P : Type u} {OrdinalValue : Type v}
    {E : P → P → Prop} (hE : HasMinimalElements E) (operations : SuccessorSupremum.{u, v} OrdinalValue) (current : P) :
    function hE operations current =
      operations.supremum (fun predecessor :
            Recursion.Predecessor E current =>
          operations.successor (function hE operations predecessor.1)) :=
  functionOfWellFounded_apply (wf_of_min_exists_l hE) operations current
/-- 纸面定义下的任意秩函数都等于规范秩函数。 -/
theorem eq_function
    {P : Type u} {OrdinalValue : Type v}
    {E : P → P → Prop} (hE : HasMinimalElements E) (operations : SuccessorSupremum.{u, v} OrdinalValue)
  ...
  eq_functionOfWellFounded (wf_of_min_exists_l hE) operations hρ
/-- Lean 良基关系具有唯一的秩函数。 -/
