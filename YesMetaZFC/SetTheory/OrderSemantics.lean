...
namespace TransitiveSet
/-- 传递集的任意成员，其所有成员仍属于原集合。 -/
theorem memberSubset {ℳ : Structure.{u}} {set member : ℳ.Domain}
    (hSet : ℳ.TransitiveSet set) (hMember : ℳ.mem member set) :
    ℳ.MemberSubset member set :=
  hSet member hMember
end TransitiveSet
/-- 原始隶属关系线序 `carrier`。 -/
structure MembershipLinearOrder (ℳ : Structure.{u}) (carrier : ℳ.Domain) : Prop where
  irrefl :
...
/-- 原始隶属关系良序 `carrier`。 -/
structure MembershipWellOrder (ℳ : Structure.{u}) (carrier : ℳ.Domain) : Prop where
  linear : MembershipLinearOrder ℳ carrier
  least :
    ∀ subset, MemberSubset ℳ subset carrier → (∃ value, ℳ.mem value subset) →
...
/-- 对象公式中的真子集定义与纸面语义一致。 -/
theorem proper_subset_sat_iff_d {ℳ : Structure.{u}} {depth : Nat} (env : Env ℳ depth) (left right : Term depth) :
...
/-- 对象公式中的隶属良序定义与纸面语义一致。 -/
theorem mem_well_order_sat_iff_d
    {ℳ : Structure.{u}} {depth : Nat} (env : Env ℳ depth) (carrier : Term depth) :
    satisfies env (isWellOrderOn RelationSchema.membership TermVector.empty carrier) ↔
  ...
  simp [isWellOrderOn, isLinearOrderOn, isStrictPartialOrderOn,
    ...
    satisfies_imp_iff,  satisfies_forall_iff,
