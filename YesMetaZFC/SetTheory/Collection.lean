...
theorem collection_sat_iff_d {ℳ : Structure.{u}}
    {parameterCount : Nat} (env : Env ℳ parameterCount) (schema : Definitional.Project.BinarySchema parameterCount) :
    Definitional.Project.Formula.satisfies env (collectionCore schema) ↔
      ∀ source, (∀ input, ℳ.mem input source →
          ∃ output,
  ...
  · intro h source hTotal
    have hAntecedent :
        ∀ input, ℳ.mem input source →
          ∃ output,
            Definitional.Project.Formula.satisfies (((env.push source).push input).push output) (schema.body.rename
  ...
  · intro h source hAntecedent
    have hTotal :
        ∀ input, ℳ.mem input source →
          ∃ output,
            Definitional.Project.Formula.satisfies ((env.push input).push output) schema.body := by
      intro input hInput
...
theorem collection_exists_d {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {parameterCount : Nat} (schema : Definitional.Project.BinarySchema parameterCount) (env : Env ℳ parameterCount) (source : ℳ.Domain)
    ...
    ∃ collection, ∀ input, ℳ.mem input source →
      ∃ output, ℳ.mem output collection ∧
        Definitional.Project.Formula.satisfies ((env.push input).push output)
          schema.body := by
  have hSentence :=
    hZF.2 (Axioms.Schema.collection schema) (Axiom.collection schema) env.free
  have hCore :
      Definitional.Project.Formula.satisfies env (Axioms.Schema.collectionCore schema) := by
    have hAll := (Definitional.Project.Formula.satisfies_forallClosure_iff env.free (Axioms.Schema.collectionCore schema)).mp hSentence
    simpa only using hAll env.bound
  exact (Axioms.Schema.collection_sat_iff_d env schema).mp
      hCore source hTotal
end ZF
namespace KP
/-- KP 收集带 `Δ₀` 证据的二元公式模式。 -/
theorem collection_exists_d {ℳ : Structure.{u}} (hKP : ℳ.Models SetTheory.KP)
    {parameterCount : Nat} (schema : Definitional.Project.Delta0BinarySchema parameterCount) (env : Env ℳ parameterCount) (source : ℳ.Domain)
    (hTotal : ∀ input, ℳ.mem input source →
      ∃ output,
        Definitional.Project.Formula.satisfies ((env.push input).push output)
          schema.body) :
    ∃ collection, ∀ input, ℳ.mem input source →
      ∃ output, ℳ.mem output collection ∧
        Definitional.Project.Formula.satisfies ((env.push input).push output)
          schema.body := by
  have hSentence :=
    hKP.2 (Axioms.Schema.collection schema.toBinarySchema) (Axiom.collection schema) env.free
  have hCore :
      Definitional.Project.Formula.satisfies env (Axioms.Schema.collectionCore
          schema.toBinarySchema) := by
    have hAll := (Definitional.Project.Formula.satisfies_forallClosure_iff env.free (Axioms.Schema.collectionCore
          schema.toBinarySchema)).mp hSentence
    simpa only using hAll env.bound
  exact (Axioms.Schema.collection_sat_iff_d
