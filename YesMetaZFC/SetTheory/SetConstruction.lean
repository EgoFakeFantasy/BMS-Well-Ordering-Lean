...
def IsFunctionSpace {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (space source target : ℳ.Domain) : Prop :=
  ∀ function,
...
theorem eq {ℳ : Structure.{u}} (hExt : Extensional ℳ)
    {left right element : ℳ.Domain} (hLeft : ℳ.IsSingletonOf left element) (hRight : ℳ.IsSingletonOf right element) :
    left = right := by
...
/-- 同一集合族的两个并集相等。 -/
theorem eq {ℳ : Structure.{u}} (hExt : Extensional ℳ)
    {left right family : ℳ.Domain} (hLeft : ℳ.IsUnionOf left family) (hRight : ℳ.IsUnionOf right family) :
    left = right := by
  apply hExt.eq_of_same_members
  intro member
  exact (hLeft member).trans (hRight member).symm
...
/-- 同一集合的两个幂集相等。 -/
theorem eq {ℳ : Structure.{u}} (hExt : Extensional ℳ)
    {left right set : ℳ.Domain} (hLeft : ℳ.IsPowerSetOf left set) (hRight : ℳ.IsPowerSetOf right set) :
    left = right := by
  apply hExt.eq_of_same_members
  intro subset
...
theorem eq {ℳ : Structure.{u}}
    ...
    {left right source target : ℳ.Domain} (hLeft : ℳ.IsFunctionSpace 𝕀 left source target) (hRight : ℳ.IsFunctionSpace 𝕀 right source target) :
    left = right := by
  apply hExt.eq_of_same_members
  intro function
...
theorem symm {ℳ : Structure.{u}}
    {left right : ℳ.Domain} (hDisjoint : ℳ.IsDisjoint left right) :
...
theorem swap {ℳ : Structure.{u}}
    {union left right : ℳ.Domain} (hUnion : ℳ.IsUnionOfTwo union left right) :
...
/-- `left` 与 `right` 不相交。 -/
def isDisjoint {depth : Nat} (left right : Term depth) : Formula 1 depth :=
  .forallE <|
    .neg <| .conj (.mem Term.newest left.weaken) (.mem Term.newest right.weaken)
/-- `union` 的成员恰好来自 `left` 或 `right`。 -/
def isUnionOfTwo {depth : Nat} (union left right : Term depth) : Formula 1 depth :=
  .forallE <|
    .iff (.mem Term.newest union.weaken) <|
      .disj (.mem Term.newest left.weaken) (.mem Term.newest right.weaken)
...
theorem satisfies_isUnionOfTwo_iff
    ...
    satisfies env (isUnionOfTwo union left right) ↔
      ℳ.IsUnionOfTwo (union.eval env) (left.eval env) (right.eval env) := by
...
theorem satisfies_isCartesianProduct_iff
    ...
    satisfies env (isCartesianProduct 𝒞 product left right) ↔
      ℳ.IsCartesianProduct 𝕀 (product.eval env) (left.eval env) (right.eval env) := by
...
theorem satisfies_isFunctionSpace_iff
    ...
    {depth : Nat} (env : Env ℳ depth) (space source target : Term depth) :
    satisfies env (isFunctionSpace 𝒞 space source target) ↔
      ℳ.IsFunctionSpace 𝕀 (space.eval env) (source.eval env) (target.eval env) := by
...
namespace Axioms
/-- 空集公理的项目核语义。 -/
theorem satisfies_emptySet_iff {ℳ : Structure.{u}} (free : FreeVarId → ℳ.Domain) :
  ...
  simp only [
    ...
    Definitional.Term.eval_newest, Definitional.Term.eval_weaken]
/-- 配对公理的项目核语义。 -/
theorem satisfies_pairing_iff {ℳ : Structure.{u}} (free : FreeVarId → ℳ.Domain) :
    Definitional.Project.Formula.satisfies ({ bound := Fin.elim0, free := free } : Env ℳ 0)
        pairing.formula ↔
      ∀ left right, ∃ pair, ∀ value,
        ℳ.mem value pair ↔ (∀ element, ℳ.mem element value ↔ ℳ.mem element left) ∨ (∀ element, ℳ.mem element value ↔ ℳ.mem element right) := by
  unfold pairing
  dsimp only [Definitional.Project.Sentence.ofFormula]
  simp only [
    Definitional.Project.Formula.satisfies_forall_iff,
    ...
    Definitional.Term.eval_newest, Definitional.Term.eval_weaken]
/-- 并集公理的项目核语义。 -/
theorem satisfies_union_iff {ℳ : Structure.{u}} (free : FreeVarId → ℳ.Domain) :
    Definitional.Project.Formula.satisfies ({ bound := Fin.elim0, free := free } : Env ℳ 0)
        union.formula ↔
      ∀ family, ∃ union, ∀ value,
        ℳ.mem value union ↔
  ...
  simp only [
    ...
    Definitional.Term.eval_newest, Definitional.Term.eval_weaken]
/-- 幂集公理的项目核语义。 -/
theorem satisfies_powerSet_iff {ℳ : Structure.{u}} (free : FreeVarId → ℳ.Domain) :
    Definitional.Project.Formula.satisfies ({ bound := Fin.elim0, free := free } : Env ℳ 0)
        powerSet.formula ↔
      ∀ set, ∃ power, ∀ subset,
        ℳ.mem subset power ↔
...
theorem exists_pair {ℳ : Structure.{u}} (hKP : ℳ.Models SetTheory.KP) (left right : ℳ.Domain) :
  ...
  change ℳ.mem value pair ↔ (∀ element, ℳ.mem element value ↔ ℳ.mem element left) ∨ (∀ element, ℳ.mem element value ↔ ℳ.mem element right) at hPairValue
  rw [hPairValue]
  constructor
  · intro h
...
/-- KP 模型中任意对象都有单元素集。 -/
theorem exists_singleton {ℳ : Structure.{u}} (hKP : ℳ.Models SetTheory.KP) (value : ℳ.Domain) :
    ∃ singleton, ∀ member,
...
/-- KP 模型中任意集合族都有并集。 -/
theorem exists_union {ℳ : Structure.{u}} (hKP : ℳ.Models SetTheory.KP) (family : ℳ.Domain) :
    ∃ union, ∀ value,
      ℳ.mem value union ↔
        ∃ member, ℳ.mem member family ∧ ℳ.mem value member := by
  let free : FreeVarId → ℳ.Domain := fun _ =>
...
theorem exists_insert {ℳ : Structure.{u}} (hKP : ℳ.Models SetTheory.KP) (set value : ℳ.Domain) :
  ...
  · intro h
    ...
    · subst member
      exact ⟨singleton, (hFamily singleton).mpr (Or.inr rfl), (hSingleton value).mpr rfl⟩
/-- KP 中任意两个集合都有二元并。 -/
theorem exists_unionOfTwo {ℳ : Structure.{u}} (hKP : ℳ.Models SetTheory.KP) (left right : ℳ.Domain) :
