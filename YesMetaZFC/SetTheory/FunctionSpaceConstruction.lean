...
def IsFunctionTransport {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (domainMap baseMap targetDomain targetBase input output : ℳ.Domain) :
    Prop :=
...
def IsFunctionExtension {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (domainMap baseMap defaultValue targetDomain targetBase
      input output : ℳ.Domain) : Prop :=
  ℳ.IsSetFunctionFromTo 𝕀 output targetDomain targetBase ∧
    ∀ targetInput targetOutput,
      ℳ.PairMember 𝕀 targetInput targetOutput output ↔
        ℳ.mem targetInput targetDomain ∧ ((∃ sourceInput sourceOutput,
              ℳ.PairMember 𝕀 sourceInput targetInput domainMap ∧
                ℳ.PairMember 𝕀 sourceInput sourceOutput input ∧
                  ℳ.PairMember 𝕀 sourceOutput targetOutput baseMap) ∨ ((¬ ∃ sourceInput,
                ℳ.PairMember 𝕀 sourceInput targetInput domainMap) ∧
...
def transportedFunctionValue (𝒞 : OrderedPairConvention) : BinarySchema 3 where
...
def functionTransportValue (𝒞 : OrderedPairConvention) : BinarySchema 4 where
  body := .conj (Formula.isFunctionFromTo 𝒞 (.bound 0) (.bound 4) (.bound 5)) <|
    .forallE <| .forallE <|
      .iff (Formula.orderedPairMem 𝒞 (.bound 1) (.bound 0) (.bound 2)) <|
...
theorem denote_transportedFunctionValue_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (env : Env ℳ 3) (input output : ℳ.Domain) :
...
theorem denote_functionTransportValue_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 4) (input output : ℳ.Domain) :
    (BinarySchema.functionTransportValue 𝒞).denote env input output ↔
      ℳ.IsFunctionTransport 𝕀 (env.bound 0) (env.bound 1) (env.bound 2) (env.bound 3) input output := by
...
theorem denote_extendedFunctionValue_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 4) (input output : ℳ.Domain) :
    (BinarySchema.extendedFunctionValue 𝒞).denote env input output ↔ (∃ sourceInput sourceOutput,
          ℳ.PairMember 𝕀 sourceInput input (env.bound 1) ∧
            ℳ.PairMember 𝕀 sourceInput sourceOutput (env.bound 0) ∧
              ℳ.PairMember 𝕀 sourceOutput output (env.bound 2)) ∨ ((¬ ∃ sourceInput,
            ℳ.PairMember 𝕀 sourceInput input (env.bound 1)) ∧
...
theorem denote_functionExtensionValue_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 5) (input output : ℳ.Domain) :
    (BinarySchema.functionExtensionValue 𝒞).denote env input output ↔
      ℳ.IsFunctionExtension 𝕀 (env.bound 0) (env.bound 1) (env.bound 2) (env.bound 3) (env.bound 4) input output := by
...
theorem satisfies_functionFromTo_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 2) (function : ℳ.Domain) :
    satisfies (env.push function) (UnarySchema.functionFromTo 𝒞).body ↔
...
theorem exists_functionSpaceTransportInjection
    ...
    {sourceSpace targetSpace sourceDomain sourceBase
      targetDomain targetBase domainMap baseMap : ℳ.Domain} (hSourceSpace :
      ℳ.IsFunctionSpace 𝕀 sourceSpace sourceDomain sourceBase) (hTargetSpace :
      ℳ.IsFunctionSpace 𝕀 targetSpace targetDomain targetBase) (hDomainMap :
      ℳ.IsSetBijectionFromTo 𝕀
        domainMap sourceDomain targetDomain) (hBaseMap :
  ...
  · intro input hInput
    ...
    rcases exists_setFunctionFromTo_of_denote
        ...
        valueEnv (source := targetDomain) (target := targetBase) (by
          ...
          exact ⟨targetOutput, (Definitional.Project.Formula.denote_transportedFunctionValue_iff
              𝕀 valueEnv targetInput targetOutput).mpr
                ⟨sourceInput, sourceOutput,
                  hDomainPair, hInputPair, hBasePair⟩⟩) (by
    ...
    refine ⟨output, (Definitional.Project.Formula.denote_functionTransportValue_iff
  ...
  · intro first second output hFirstMem hSecondMem hFirst hSecond
    ...
    · intro hFirstPair
      ...
      rcases hBaseMap.1.2.2 sourceOutput hSourceOutput with
        ⟨targetOutput, _, hBasePair⟩
      have hOutputPair :
          ℳ.PairMember 𝕀 targetInput targetOutput output := (hFirst.2 targetInput targetOutput).mpr
    ...
    · intro hSecondPair
      ...
      rcases hBaseMap.1.2.2 sourceOutput hSourceOutput with
        ⟨targetOutput, _, hBasePair⟩
      have hOutputPair :
          ℳ.PairMember 𝕀 targetInput targetOutput output := (hSecond.2 targetInput targetOutput).mpr
          ⟨hTargetInput, sourceInput, sourceOutput,
...
theorem exists_functionSpaceExtensionInjection
    ...
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {sourceSpace targetSpace sourceDomain sourceBase
      targetDomain targetBase domainMap baseMap defaultValue : ℳ.Domain} (hSourceSpace :
      ℳ.IsFunctionSpace 𝕀 sourceSpace sourceDomain sourceBase) (hTargetSpace :
      ℳ.IsFunctionSpace 𝕀 targetSpace targetDomain targetBase) (hDomainMap :
      ℳ.IsSetInjectionFromTo 𝕀
        domainMap sourceDomain targetDomain) (hBaseMap :
      ℳ.IsSetInjectionFromTo 𝕀
        baseMap sourceBase targetBase) (hDefault : ℳ.mem defaultValue targetBase) :
    ∃ extension,
    ...
    rcases exists_setFunctionFromTo_of_denote
        ...
        valueEnv (source := targetDomain) (target := targetBase) (by
          ...
          · rcases hImage with ⟨sourceInput, hDomainPair⟩
            ...
            rcases hBaseMap.1.2.2 sourceOutput hSourceOutput with
              ⟨targetOutput, _, hBasePair⟩
            exact ⟨targetOutput, (Definitional.Project.Formula.denote_extendedFunctionValue_iff
                𝕀 hZF.1 valueEnv targetInput targetOutput).mpr <|
                  Or.inl ⟨sourceInput, sourceOutput,
                    hDomainPair, hInputPair, hBasePair⟩⟩
          · exact ⟨defaultValue, (Definitional.Project.Formula.denote_extendedFunctionValue_iff
                𝕀 hZF.1 valueEnv targetInput defaultValue).mpr <|
                  Or.inr ⟨hImage, rfl⟩⟩) (by
          intro targetInput _ first second hFirst hSecond
          ...
          · rcases hSecond with hSecondImage | hSecondDefault
            ...
            · exact hFirstDefault.2.trans hSecondDefault.2.symm) (by
          intro targetInput targetOutput _ hValue
          rw [Definitional.Project.Formula.denote_extendedFunctionValue_iff 𝕀 hZF.1]
            at hValue
...
theorem exists_functionSpace
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (source target : ℳ.Domain) :
  ...
  rcases separation_exists_d hZF (Definitional.Project.UnarySchema.functionFromTo 𝒞) env power with
    ⟨space, hSpace⟩
