...
inductive Formula (κ : Kappa.{i}) : Type i → Type (i + 1) where
  ...
  | atom {Γ : Type i} (symbol : Definitional.Project.CoreAtom) : (Fin (Definitional.Project.coreSignature.arity symbol) → Term Γ) →
        Formula κ Γ
...
def rename {κ : Kappa.{i}} {Γ Δ : Type i} (indexMap : Γ → Δ) : Formula κ Γ → Formula κ Δ
  | .falsum => .falsum
  | .truth => .truth
...
/-- 无穷层中的原生外延等同原子。 -/
def extensionalEq {κ : Kappa.{i}} {Γ : Type i} (left right : Term Γ) : Formula κ Γ :=
  .atom .extensionalEq (pairArguments left right)
/-- 无穷层中的原生子集原子。 -/
def subset {κ : Kappa.{i}} {Γ : Type i} (left right : Term Γ) : Formula κ Γ :=
...
namespace Env
def push {ℳ : SetTheory.Structure.{u}} {Γ I : Type i} (env : Env ℳ Γ) (values : I → ℳ.Domain) : Env ℳ (Sum Γ I) where
  bound
    | .inl entry => env.bound entry
    | .inr index => values index
  free := env.free
def pushOne {ℳ : SetTheory.Structure.{u}} {Γ : Type i} (env : Env ℳ Γ) (value : ℳ.Domain) : Env ℳ (Sum Γ Unit) where
  bound
    | .inl entry => env.bound entry
    | .inr _ => value
  free := env.free
...
namespace Term
def eval {ℳ : SetTheory.Structure.{u}} {Γ : Type i} (env : Env ℳ Γ) : Term Γ → ℳ.Domain
...
def satisfies {κ : Kappa.{i}} {ℳ : SetTheory.Structure.{u}}
  ...
  | .atom symbol arguments =>
      Definitional.Project.Semantics.interpretation.atom symbol (fun entry => (arguments entry).eval env) env.free
