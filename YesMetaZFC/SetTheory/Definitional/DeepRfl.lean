...
private def isForbiddenFormulaReduction (formula : Expr) : Bool := (formula.find? fun expression =>
...
private def replaceLastArgument (expression : Expr) (arguments : Array Expr) (replacement : Expr) : Expr :=
  mkAppN expression.getAppFn (arguments.set! (arguments.size - 1) replacement)
private def replaceLastTwoArguments (expression : Expr) (arguments : Array Expr) (left right : Expr) : Expr :=
  mkAppN expression.getAppFn <| (arguments.set! (arguments.size - 2) left).set! (arguments.size - 1) right
