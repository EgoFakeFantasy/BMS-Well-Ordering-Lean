...
@[simp] theorem expand_embed {depth : Nat} (term : Pure.Term depth) :
  ...
  cases term <;> rfl
@[simp] theorem freeSupport_embed {depth : Nat} (term : Pure.Term depth) : (embed term).freeSupport = term.freeSupport := by
  cases term <;> rfl
@[simp] theorem freeSupport_expand {depth : Nat} (term : Definitional.Term depth) : (expand term).freeSupport = term.freeSupport := by
  cases term <;> rfl
@[simp] theorem eval_expand {ℳ : SetTheory.Structure.{v}} {depth : Nat} (env : SetTheory.Env ℳ depth) (term : Definitional.Term depth) :
...
theorem freeClosed_embed {σ : AtomSignature.{u}}
    ...
    Definitional.Formula.FreeClosed (embed (σ := σ) (availableStage := availableStage) formula) := by
  induction formula with
  | falsum =>
...
theorem expand_freeClosed {σ : AtomSignature.{u}} (definitions : Definitions σ) :
    {availableStage depth : Nat} → (formula : Definitional.Formula σ availableStage depth) →
  ...
  | _, _, .atom symbol _ arguments, hClosed => by
      ...
      rw [expand]
      change ((expand definitions (definitions.body symbol)).bind
  ...
  | _, _, .iff left right, hClosed => by
      ...
      simpa [expand, Pure.Formula.FreeClosed,
        Pure.Formula.freeSupport] using
        And.intro (expand_freeClosed definitions left hLeft) (expand_freeClosed definitions right hRight)
...
/-- 把新核句子完全展开到旧纯隶属核。 -/
def expand {σ : AtomSignature.{u}} (definitions : Definitions σ) (sentence : Definitional.Sentence σ) : Pure.Sentence where
  formula := Formula.expand definitions sentence.formula
  freeClosed :=
    Formula.expand_freeClosed definitions sentence.formula
      sentence.freeClosed
/-- 旧核句子嵌入后再展开，严格回到原句子。 -/
@[simp] theorem expand_embed {σ : AtomSignature.{u}} (definitions : Definitions σ) (sentence : Pure.Sentence) :
...
namespace Theory
/-- 新核理论完全展开后在旧核中的像。 -/
def expand {σ : AtomSignature.{u}} (definitions : Definitions σ) (theory : Definitional.Theory σ) : Pure.Theory :=
  fun candidate =>
    ∃ sentence, theory sentence ∧ Sentence.expand definitions sentence = candidate
/-- 旧核理论沿完全展开逆像提升到新核。 -/
def lift {σ : AtomSignature.{u}} (definitions : Definitions σ) (theory : Pure.Theory) : Definitional.Theory σ :=
  fun sentence => theory (Sentence.expand definitions sentence)
/-- 提升旧核理论后再展开，严格回到原理论。 -/
theorem expand_lift {σ : AtomSignature.{u}} (definitions : Definitions σ) (theory : Pure.Theory) :
...
private theorem substitute_arguments {σ : AtomSignature.{u}}
    {ℳ : SetTheory.Structure.{v}} {depth : Nat} (env : SetTheory.Env ℳ depth) {symbol : σ.Symbol} (arguments : TermVector (σ.arity symbol) depth) :
...
theorem satisfies_expand {σ : AtomSignature.{u}} (kernel : Kernel.{u, v} σ)
    ...
    {availableStage depth : Nat} → (env : SetTheory.Env ℳ depth) → (formula : Definitional.Formula σ availableStage depth) →
      Definitional.Semantics.satisfies kernel.interpretation env formula ↔
        Pure.Formula.satisfies env (Formula.expand kernel.definitions formula)
  | _, _, env, .falsum => by
  ...
  | _, _, env, .atom symbol _ arguments => by
      ...
      exact hAtom.trans (satisfies_expand kernel hExtensional (Definitional.Semantics.atomEnv env arguments) (kernel.definitions.body symbol))
  ...
  | _, _, env, .conj left right => by
      simpa [Definitional.Semantics.satisfies, Formula.expand,
        Pure.Formula.satisfies] using
        and_congr (satisfies_expand kernel hExtensional env left) (satisfies_expand kernel hExtensional env right)
  | _, _, env, .disj left right => by
      simpa [Definitional.Semantics.satisfies, Formula.expand,
        Pure.Formula.satisfies] using
        or_congr (satisfies_expand kernel hExtensional env left) (satisfies_expand kernel hExtensional env right)
  | _, _, env, .imp left right => by
      simpa [Definitional.Semantics.satisfies, Formula.expand,
        Pure.Formula.satisfies] using
        imp_congr (satisfies_expand kernel hExtensional env left) (satisfies_expand kernel hExtensional env right)
  | _, _, env, .iff left right => by
      simpa [Definitional.Semantics.satisfies, Formula.expand,
        Pure.Formula.satisfies] using
        iff_congr (satisfies_expand kernel hExtensional env left) (satisfies_expand kernel hExtensional env right)
...
theorem satisfies_embed {σ : AtomSignature.{u}} (kernel : Kernel.{u, v} σ) {ℳ : SetTheory.Structure.{v}} (hExtensional : Extensional ℳ)
    {availableStage depth : Nat} (env : SetTheory.Env ℳ depth) (formula : Pure.Formula depth) :
    Definitional.Semantics.satisfies kernel.interpretation env (Formula.embed (σ := σ) (availableStage := availableStage) formula) ↔
      Pure.Formula.satisfies env formula := by
  rw [satisfies_expand kernel hExtensional, Formula.expand_embed]
end Semantics
namespace Structure
/-- 新核句子与其旧核完全展开具有相同满足关系。 -/
theorem satisfiesSentence_expand {σ : AtomSignature.{u}} (kernel : Kernel.{u, v} σ) (ℳ : SetTheory.Structure.{v}) (hExtensional : Extensional ℳ)
    (sentence : Definitional.Sentence σ) :
    Definitional.Structure.SatisfiesSentence kernel ℳ sentence ↔
      Pure.Structure.SatisfiesSentence ℳ (Sentence.expand kernel.definitions sentence) := by
  apply forall_congr'
  intro free
  exact Semantics.satisfies_expand kernel hExtensional _ sentence.formula
/-- 新核理论模型恰好是其旧核完全展开理论的模型。 -/
theorem models_expand {σ : AtomSignature.{u}} (kernel : Kernel.{u, v} σ) (ℳ : SetTheory.Structure.{v}) (theory : Definitional.Theory σ) :
    Definitional.Structure.Models kernel ℳ theory ↔
      Pure.Structure.Models ℳ (Theory.expand kernel.definitions theory) := by
  constructor
  · rintro ⟨hExtensional, hModels⟩
    refine ⟨hExtensional, ?_⟩
    intro candidate hCandidate
    rcases hCandidate with ⟨sentence, hSentence, rfl⟩
    exact (satisfiesSentence_expand kernel ℳ hExtensional sentence).mp (hModels sentence hSentence)
  · rintro ⟨hExtensional, hModels⟩
    refine ⟨hExtensional, ?_⟩
...
/-- 新核蕴涵与完全展开后的旧核蕴涵等价。 -/
theorem semanticallyEntails_expand {σ : AtomSignature.{u}} (kernel : Kernel.{u, v} σ) (theory : Definitional.Theory σ) (target : Definitional.Sentence σ) :
    Definitional.SemanticallyEntails kernel theory target ↔
      Pure.SemanticallyEntails.{v} (Theory.expand kernel.definitions theory) (Sentence.expand kernel.definitions target) := by
  constructor
  · intro hEntails ℳ hModels
    apply (Structure.satisfiesSentence_expand kernel ℳ hModels.1 target).mp
    exact hEntails ℳ ((Structure.models_expand kernel ℳ theory).mpr hModels)
  · intro hEntails ℳ hModels
    apply (Structure.satisfiesSentence_expand kernel ℳ hModels.1 target).mpr
    exact hEntails ℳ ((Structure.models_expand kernel ℳ theory).mp hModels)
/-- 在旧纯语言片段上，新核是旧核的双向保守扩张。 -/
theorem conservative_on_pure_fragment {σ : AtomSignature.{u}} (kernel : Kernel.{u, v} σ) (theory : Pure.Theory) (target : Pure.Sentence) :
    Definitional.SemanticallyEntails kernel (Theory.lift kernel.definitions theory) (Sentence.embed (σ := σ) target) ↔
