...
/-- 纯集合论项的解释。 -/
def eval {ℳ : SetTheory.Structure.{u}} {depth : Nat} (env : SetTheory.Env ℳ depth) : Term depth → ℳ.Domain
  | .bound entry => env.bound entry
  | .free id => env.free id
@[simp] theorem eval_weaken {ℳ : SetTheory.Structure.{u}} {depth : Nat} (env : SetTheory.Env ℳ depth) (value : ℳ.Domain) (term : Term depth) :
...
theorem freeSupport_bind_of_closed {sourceDepth targetDepth : Nat} (substitution : Fin sourceDepth → Term targetDepth)
    (hClosed : ∀ entry, (substitution entry).freeSupport = []) (formula : Formula sourceDepth) :
...
def substitute {ℳ : SetTheory.Structure.{u}}
    {sourceDepth targetDepth : Nat} (env : SetTheory.Env ℳ targetDepth) (substitution : Fin sourceDepth → Term targetDepth) :
...
private theorem substitute_liftSubstitution {ℳ : SetTheory.Structure.{u}}
  ...
  · rfl
/-- 纯集合论公式的 Tarski 满足关系。 -/
def satisfies {ℳ : SetTheory.Structure.{u}} {depth : Nat} (env : SetTheory.Env ℳ depth) : Formula depth → Prop
  | .falsum => False
  | .truth => True
...
@[simp] theorem satisfies_bind {ℳ : SetTheory.Structure.{u}}
  ...
  | .conj left right => by
      simpa [bind, satisfies] using
        and_congr (satisfies_bind env substitution left) (satisfies_bind env substitution right)
  | .disj left right => by
      simpa [bind, satisfies] using
        or_congr (satisfies_bind env substitution left) (satisfies_bind env substitution right)
  | .imp left right => by
      simpa [bind, satisfies] using
        imp_congr (satisfies_bind env substitution left) (satisfies_bind env substitution right)
  | .iff left right => by
      simpa [bind, satisfies] using
        iff_congr (satisfies_bind env substitution left) (satisfies_bind env substitution right)
  | .forallE body => by
      simp only [bind, satisfies]
      constructor
      · intro h value
        have hBody := (satisfies_bind (env.push value) (Term.liftSubstitution substitution) body).mp (h value)
        simpa only [substitute_liftSubstitution] using hBody
      · intro h value
        apply (satisfies_bind (env.push value) (Term.liftSubstitution substitution) body).mpr
  ...
  | .existsE body => by
      ...
      · rintro ⟨value, hBody⟩
        refine ⟨value, ?_⟩
        have hBody' := (satisfies_bind (env.push value) (Term.liftSubstitution substitution) body).mp hBody
        simpa only [substitute_liftSubstitution] using hBody'
      · rintro ⟨value, hBody⟩
        refine ⟨value, ?_⟩
        apply (satisfies_bind (env.push value) (Term.liftSubstitution substitution) body).mpr
...
namespace Structure
/-- 一个结构满足纯句子。 -/
def SatisfiesSentence (ℳ : SetTheory.Structure.{u}) (sentence : Sentence) : Prop :=
