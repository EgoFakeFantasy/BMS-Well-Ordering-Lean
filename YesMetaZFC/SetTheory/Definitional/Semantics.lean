...
def substitute {ℳ : SetTheory.Structure.{v}} {sourceDepth targetDepth : Nat} (env : SetTheory.Env ℳ targetDepth)
...
namespace Term
/-- 新核集合论项的解释。 -/
def eval {ℳ : SetTheory.Structure.{v}} {depth : Nat} (env : SetTheory.Env ℳ depth) : Term depth → ℳ.Domain
  | .bound entry => env.bound entry
  | .free id => env.free id
/-- bound-variable 重命名等价于环境反向重索引。 -/
@[simp] theorem eval_rename {ℳ : SetTheory.Structure.{v}}
    {sourceDepth targetDepth : Nat} (env : SetTheory.Env ℳ targetDepth) (indexMap : Fin sourceDepth → Fin targetDepth) (term : Term sourceDepth) :
...
@[simp] theorem eval_bind {ℳ : SetTheory.Structure.{v}}
    {sourceDepth targetDepth : Nat} (env : SetTheory.Env ℳ targetDepth) (substitution : Fin sourceDepth → Term targetDepth) (term : Term sourceDepth) :
...
/-- 参数向量在当前环境中的取值。 -/
def eval {ℳ : SetTheory.Structure.{v}} {depth : Nat} (env : SetTheory.Env ℳ depth) {count : Nat} (arguments : TermVector count depth) :
    Fin count → ℳ.Domain :=
  fun entry => Term.eval env (arguments entry)
/-- 项参数向量在当前环境中形成的 schema 参数环境。 -/
def evalEnv {ℳ : SetTheory.Structure.{v}} {count depth : Nat} (parameters : TermVector count depth) (env : SetTheory.Env ℳ depth) :
...
structure Interpretation (σ : AtomSignature.{u}) where
  atom :
    {ℳ : SetTheory.Structure.{v}} → (symbol : σ.Symbol) → (Fin (σ.arity symbol) → ℳ.Domain) → (FreeVarId → ℳ.Domain) →
        Prop
namespace Semantics
/-- 原子参数在当前环境中的取值构成其定义体环境。 -/
def atomEnv {σ : AtomSignature.{u}} {ℳ : SetTheory.Structure.{v}}
    {depth : Nat} (env : SetTheory.Env ℳ depth)
    {symbol : σ.Symbol} (arguments : TermVector (σ.arity symbol) depth) :
...
private theorem atomEnv_bind {σ : AtomSignature.{u}}
    {ℳ : SetTheory.Structure.{v}} {sourceDepth targetDepth : Nat} (env : SetTheory.Env ℳ targetDepth) (substitution : Fin sourceDepth → Term targetDepth)
    {symbol : σ.Symbol} (arguments : TermVector (σ.arity symbol) sourceDepth) :
...
@[simp] theorem satisfies_bind {σ : AtomSignature.{u}} (interpretation : Interpretation.{u, v} σ)
    {ℳ : SetTheory.Structure.{v}}
    {availableStage sourceDepth targetDepth : Nat} (env : SetTheory.Env ℳ targetDepth) (substitution : Fin sourceDepth → Term targetDepth) :
  ...
  | .atom symbol hStage arguments => by
      ...
      change
        interpretation.atom symbol (atomEnv env (arguments.bind substitution)).bound (atomEnv env (arguments.bind substitution)).free ↔
          interpretation.atom symbol (atomEnv (Env.substitute env substitution) arguments).bound (atomEnv (Env.substitute env substitution) arguments).free
      rw [atomEnv_bind]
  | .neg formula => by
  ...
  | .conj left right => by
      simpa [Formula.bind, satisfies] using
        and_congr (satisfies_bind interpretation env substitution left) (satisfies_bind interpretation env substitution right)
  | .disj left right => by
      simpa [Formula.bind, satisfies] using
        or_congr (satisfies_bind interpretation env substitution left) (satisfies_bind interpretation env substitution right)
  | .imp left right => by
      simpa [Formula.bind, satisfies] using
        imp_congr (satisfies_bind interpretation env substitution left) (satisfies_bind interpretation env substitution right)
  | .iff left right => by
      simpa [Formula.bind, satisfies] using
        iff_congr (satisfies_bind interpretation env substitution left) (satisfies_bind interpretation env substitution right)
  | .forallE body => by
      simp only [Formula.bind, satisfies]
      constructor
      · intro h value
...
-/
@[simp] theorem satisfies_rename {σ : AtomSignature.{u}} (interpretation : Interpretation.{u, v} σ)
    {ℳ : SetTheory.Structure.{v}}
    {availableStage sourceDepth targetDepth : Nat} (env : SetTheory.Env ℳ targetDepth) (indexMap : Fin sourceDepth → Fin targetDepth)
    (formula : Formula σ availableStage sourceDepth) :
    satisfies interpretation env (formula.rename indexMap) ↔
      satisfies interpretation (SetTheory.Env.reindex env indexMap) formula := by
  rw [Formula.rename, satisfies_bind]
  rfl
/-- 全称闭合等价于任意 bound 参数赋值下满足原公式。 -/
theorem satisfies_forallClosure_iff {σ : AtomSignature.{u}} (interpretation : Interpretation.{u, v} σ)
    {ℳ : SetTheory.Structure.{v}} {availableStage depth : Nat} (free : FreeVarId → ℳ.Domain) (formula : Formula σ availableStage depth) :
    satisfies interpretation
        { bound := Fin.elim0, free := free } (Formula.forallClosure depth formula) ↔
  ...
  | succ depth ih =>
      ...
      · intro h bound
        ...
        have hEnv : ({ bound := tail, free := free } :
                SetTheory.Env ℳ depth).push (bound 0) = ({ bound := bound, free := free } :
      ...
      · intro h bound value
        ...
        have hEnv : ({ bound := bound, free := free } :
                SetTheory.Env ℳ depth).push value = ({ bound := extended, free := free } :
                SetTheory.Env ℳ (depth + 1)) := by
          rfl
        simpa only [hEnv] using hBody
end Semantics
...
structure Kernel (σ : AtomSignature.{u}) where
  ...
  atom_iff :
    ∀ {ℳ : SetTheory.Structure.{v}} (symbol : σ.Symbol) (_hExtensional : Extensional ℳ) (arguments : Fin (σ.arity symbol) → ℳ.Domain)
