...
structure OrderedPairConvention where
  ...
  freeClosed_code :
    ∀ {depth : Nat} (pair left right : Term depth),
      pair.freeSupport = [] →
      left.freeSupport = [] →
      right.freeSupport = [] → (code pair left right).FreeClosed
namespace OrderedPairConvention
@[simp] theorem code_freeClosed (𝒞 : OrderedPairConvention)
    {depth : Nat} (pair left right : Term depth) (hPair : pair.freeSupport = []) (hLeft : left.freeSupport = []) (hRight : right.freeSupport = []) :
...
def isComposition (𝒞 : OrderedPairConvention)
  ...
  .forallE <| .forallE <|
    .iff (orderedPairMem 𝒞 (.bound 1) (.bound 0)
        composition.weaken.weaken) (.existsE <|
