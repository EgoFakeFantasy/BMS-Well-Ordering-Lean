  ...
  .conj (isSequenceOfLength 𝒞 sequence length) <|
    .existsE <|
      .conj (isSuccessor Term.newest length.weaken) <|
        .conj (isSequenceOfLength 𝒞 extension.weaken Term.newest) <|
...
def isContinuousOrdinalSequence (𝒞 : OrderedPairConvention)
  ...
  .conj (isOrdinalValuedSequence 𝒞 sequence length) <|
    Formula.forallMem length <| .forallE <|
      .imp (.conj (isLimitOrdinal (.bound 1)) (orderedPairMem 𝒞 (.bound 1) (.bound 0)
...
`parameters` 显式提供。
-/
def obeysRecursion {parameterCount depth : Nat} (𝒞 : OrderedPairConvention) (operator : BinarySchema parameterCount)
    (parameters : TermVector parameterCount depth) (sequence length : Term depth) : Formula 1 depth :=
  Formula.forallMem length <| .forallE <|
    .imp (orderedPairMem 𝒞 (.bound 1) (.bound 0)
        sequence.weaken.weaken) <|
      .existsE <|
...
`target`。
-/
def isRecursionValueIn {parameterCount depth : Nat} (𝒞 : OrderedPairConvention) (operator : BinarySchema parameterCount)
    (parameters : TermVector parameterCount depth) (α value target : Term depth) : Formula 1 depth :=
  .existsE <|
...
-/
def isRecursionValue {parameterCount depth : Nat} (𝒞 : OrderedPairConvention) (operator : BinarySchema parameterCount)
    (parameters : TermVector parameterCount depth) (α value : Term depth) : Formula 1 depth :=
  .existsE <|
...
def isClassFunctionOnSequencesInBelow {parameterCount depth : Nat} (𝒞 : OrderedPairConvention) (operator : BinarySchema parameterCount)
  ...
  .forallE <|
    .imp (isSequenceInBelow 𝒞 Term.newest
...
def isIncreasingOnOrdinals {parameterCount depth : Nat} (function : BinarySchema parameterCount)
    (parameters : TermVector parameterCount depth) : Formula 1 depth :=
  .forallE <| .forallE <|
    .imp (.conj (isOrdinal (.bound 1)) <|
        .conj (isOrdinal Term.newest) (.mem (.bound 1) Term.newest)) <|
      .forallE <| .forallE <|
...
-/
def isContinuousOnOrdinals {parameterCount depth : Nat} (function : BinarySchema parameterCount)
    (parameters : TermVector parameterCount depth) : Formula 1 depth :=
  .forallE <| .forallE <|
    .imp (.conj (isLimitOrdinal (.bound 1)) (related function parameters.weaken.weaken (.bound 1) Term.newest)) <|
      .existsE <|
        .conj (.forallE <|
...
def recursiveSequenceInExistence (𝒞 : OrderedPairConvention)
...
-/
def recursiveSequenceInUniqueness (𝒞 : OrderedPairConvention)
    {parameterCount : Nat} (operator : BinarySchema parameterCount) :
    UnarySchema (parameterCount + 1) where
  body :=
    .forallE <| .forallE <|
