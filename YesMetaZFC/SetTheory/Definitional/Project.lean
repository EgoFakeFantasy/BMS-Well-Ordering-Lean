...
universe v
namespace Term
@[simp] theorem eval_bound {ℳ : Structure.{v}} {depth : Nat} (env : Env ℳ depth) (entry : Fin depth) :
    Definitional.Term.eval env (.bound entry) = env.bound entry :=
  rfl
@[simp] theorem eval_free {ℳ : Structure.{v}} {depth : Nat} (env : Env ℳ depth) (id : FreeVarId) :
    Definitional.Term.eval env (.free id : Term depth) = env.free id :=
  rfl
@[simp] theorem eval_bound_zero_push {ℳ : Structure.{v}} {depth : Nat} (env : Env ℳ depth) (value : ℳ.Domain) :
    Definitional.Term.eval (env.push value) (.bound 0) = value :=
  rfl
@[simp] theorem eval_bound_one_push {ℳ : Structure.{v}} {depth : Nat} (env : Env ℳ (depth + 1)) (value : ℳ.Domain) :
    Definitional.Term.eval (env.push value) (.bound 1) =
      Definitional.Term.eval env (.bound 0) :=
  rfl
@[simp] theorem eval_bound_two_push {ℳ : Structure.{v}} {depth : Nat} (env : Env ℳ (depth + 2)) (value : ℳ.Domain) :
    Definitional.Term.eval (env.push value) (.bound 2) =
      Definitional.Term.eval env (.bound 1) :=
  rfl
@[simp] theorem eval_bound_three_push {ℳ : Structure.{v}} {depth : Nat} (env : Env ℳ (depth + 3)) (value : ℳ.Domain) :
    Definitional.Term.eval (env.push value) (.bound 3) =
      Definitional.Term.eval env (.bound 2) :=
  rfl
/-- 一次压栈后，index `4` 指向此前的 index `3`。 -/
@[simp] theorem eval_bound_four_push {ℳ : Structure.{v}} {depth : Nat} (env : Env ℳ (depth + 4)) (value : ℳ.Domain) :
    Definitional.Term.eval (env.push value) (.bound 4) =
      Definitional.Term.eval env (.bound 3) :=
  rfl
/-- 一次压栈后，index `5` 指向此前的 index `4`。 -/
@[simp] theorem eval_bound_five_push {ℳ : Structure.{v}} {depth : Nat} (env : Env ℳ (depth + 5)) (value : ℳ.Domain) :
    Definitional.Term.eval (env.push value) (.bound 5) =
      Definitional.Term.eval env (.bound 4) :=
  rfl
/-- 一次压栈后，index `6` 指向此前的 index `5`。 -/
@[simp] theorem eval_bound_six_push {ℳ : Structure.{v}} {depth : Nat} (env : Env ℳ (depth + 6)) (value : ℳ.Domain) :
    Definitional.Term.eval (env.push value) (.bound 6) =
      Definitional.Term.eval env (.bound 5) :=
  rfl
/-- 一次压栈后，index `7` 指向此前的 index `6`。 -/
@[simp] theorem eval_bound_seven_push {ℳ : Structure.{v}} {depth : Nat} (env : Env ℳ (depth + 7)) (value : ℳ.Domain) :
    Definitional.Term.eval (env.push value) (.bound 7) =
      Definitional.Term.eval env (.bound 6) :=
  rfl
end Term
namespace Semantics
private def argument {ℳ : Structure} (arguments : Fin 2 → ℳ.Domain) (index : Nat) (hIndex : index < 2) : ℳ.Domain :=
...
def kernel : Kernel.{0, v} coreSignature where
  ...
  atom_iff := by
    ...
    | extensionalEq =>
        ...
        change (∀ value, ℳ.mem value (arguments 0) ↔
    ...
    | subset =>
        ...
        rfl
namespace Formula
def satisfies {ℳ : Structure.{v}} {depth : Nat} (env : Env ℳ depth) (formula : Formula 1 depth) : Prop :=
...
@[deep_rfl 2000] theorem satisfies_mem_iff
    {ℳ : Structure.{v}} {depth : Nat} (env : Env ℳ depth) (left right : Term depth) :
    satisfies env (.mem left right) ↔
      ℳ.mem (Definitional.Term.eval env left) (Definitional.Term.eval env right) := by
  simp only [satisfies, Definitional.Semantics.satisfies]
@[deep_rfl 2000] theorem satisfies_neg_iff
    {ℳ : Structure.{v}} {depth : Nat} (env : Env ℳ depth) (formula : Formula 1 depth) :
    satisfies env (.neg formula) ↔ ¬ satisfies env formula := by
  simp only [satisfies, Definitional.Semantics.satisfies]
@[deep_rfl 2000] theorem satisfies_conj_iff
    {ℳ : Structure.{v}} {depth : Nat} (env : Env ℳ depth) (left right : Formula 1 depth) :
...
@[deep_rfl 2000] theorem satisfies_imp_iff
    {ℳ : Structure.{v}} {depth : Nat} (env : Env ℳ depth) (left right : Formula 1 depth) :
    satisfies env (.imp left right) ↔ (satisfies env left → satisfies env right) := by
  simp only [satisfies, Definitional.Semantics.satisfies]
@[deep_rfl 2000] theorem satisfies_iff_iff
    {ℳ : Structure.{v}} {depth : Nat} (env : Env ℳ depth) (left right : Formula 1 depth) :
    satisfies env (.iff left right) ↔ (satisfies env left ↔ satisfies env right) := by
  simp only [satisfies, Definitional.Semantics.satisfies]
@[deep_rfl 2000] theorem satisfies_forall_iff
    {ℳ : Structure.{v}} {depth : Nat} (env : Env ℳ depth) (body : Formula 1 (depth + 1)) :
...
theorem satisfies_bind
    {ℳ : Structure.{v}} {sourceDepth targetDepth : Nat} (env : Env ℳ targetDepth) (substitution : Fin sourceDepth → Term targetDepth)
    (formula : Formula 1 sourceDepth) :
...
theorem satisfies_forallClosure_iff
    {ℳ : Structure.{v}} {depth : Nat} (free : FreeVarId → ℳ.Domain) (formula : Formula 1 depth) :
    satisfies ({ bound := Fin.elim0, free := free } : Env ℳ 0) (Formula.forallClosure depth formula) ↔
    ∀ bound : Fin depth → ℳ.Domain,
        satisfies ({ bound := bound, free := free } : Env ℳ depth) formula :=
  by
    simpa [satisfies, Formula.forallClosure] using (Definitional.Semantics.satisfies_forallClosure_iff
        Semantics.interpretation free formula)
/-- 任意参数向量上的外延等同原子语义。 -/
theorem satisfies_atom_extensionalEq_iff
    {ℳ : Structure.{v}} {depth : Nat} (env : Env ℳ depth) (hStage : coreSignature.stage CoreAtom.extensionalEq < 1) (arguments : TermVector 2 depth) :
...
theorem satisfies_atom_subset_iff
    {ℳ : Structure.{v}} {depth : Nat} (env : Env ℳ depth) (hStage : coreSignature.stage CoreAtom.subset < 1) (arguments : TermVector 2 depth) :
