...
def empty (capacity : Nat := 16) : ST ω (Builder ω σ) := do
  return {
    table := ← Automation.Data.InternTable.Builder.empty (σ := ω) (tag := AtomTag) (α := σ.Symbol) capacity
  }
...
def decodeTerm (graph : Graph σ) (depth : Nat) (id : TermId) :
  ...
  | .free freeId =>
      return .free freeId
private def decodeTermArray (graph : Graph σ) (depth : Nat) (ids : Array TermId) : Option (Array (Term depth)) :=
  ids.mapM (graph.decodeTerm depth)
/-- 带 fuel 解码公式；恶意循环或越界节点安全地返回 `none`。 -/
def decodeFormulaWithFuel (graph : Graph σ) : (fuel availableStage depth : Nat) → FormulaId →
  ...
  | fuel + 1, availableStage, depth, id =>
      ...
      | some (.mem left right) => do
          return .mem (← graph.decodeTerm depth left) (← graph.decodeTerm depth right)
      | some (.atom symbolId argumentIds) => do
          ...
          if hStage : σ.stage symbol < availableStage then
            ...
            none
      | some (.neg body) => do
          return .neg (← graph.decodeFormulaWithFuel fuel availableStage depth body)
      | some (.conj left right) => do
          return .conj (← graph.decodeFormulaWithFuel fuel availableStage depth left) (← graph.decodeFormulaWithFuel fuel availableStage depth right)
      | some (.disj left right) => do
          return .disj (← graph.decodeFormulaWithFuel fuel availableStage depth left) (← graph.decodeFormulaWithFuel fuel availableStage depth right)
      | some (.imp left right) => do
          return .imp (← graph.decodeFormulaWithFuel fuel availableStage depth left) (← graph.decodeFormulaWithFuel fuel availableStage depth right)
      | some (.iff left right) => do
          return .iff (← graph.decodeFormulaWithFuel fuel availableStage depth left) (← graph.decodeFormulaWithFuel fuel availableStage depth right)
      | some (.forallE body) => do
          return .forallE (← graph.decodeFormulaWithFuel fuel availableStage (depth + 1) body)
      | some (.existsE body) => do
          return .existsE (← graph.decodeFormulaWithFuel fuel availableStage (depth + 1) body)
/-- 使用图大小给出的保守 fuel 解码根公式。 -/
def decodeFormula (graph : Graph σ) (availableStage depth : Nat) (root : FormulaId) : Option (Formula σ availableStage depth) :=
  graph.decodeFormulaWithFuel (graph.formulaCount + 1)
    availableStage depth root
/-- 解码成功后的可检查可信边界。 -/
structure CheckedFormula (graph : Graph σ) (availableStage depth : Nat) (root : FormulaId) where
...
def checkFormula (graph : Graph σ) (availableStage depth : Nat) (root : FormulaId) :
...
def empty (capacity : Nat := 64) : ST ω (Builder ω σ) := do
  return {
    ...
    terms := ← Automation.Data.InternTable.Builder.empty (σ := ω) (tag := TermTag) (α := TermNode) capacity
    formulas := ← Automation.Data.InternTable.Builder.empty (σ := ω) (tag := FormulaTag) (α := FormulaNode) capacity
  }
...
def internTerm (builder : Builder ω σ) {depth : Nat} (term : Term depth) : ST ω TermId :=
...
private def weakenTermId (builder : Builder ω σ) (amount : Nat) (termId : TermId) : ST ω (Option TermId) := do
  ...
  | none =>
      return none
private def bindTermId (builder : Builder ω σ) (substitution : Array TermId) (binderLift : Nat) (termId : TermId) : ST ω (Option TermId) := do
  match ← builder.terms.get? termId with
  | some (.free _) =>
      return some termId
  | some (.bound index) =>
  ...
  | none =>
      return none
private def bindTermIds (builder : Builder ω σ) (substitution : Array TermId) (binderLift : Nat) (termIds : Array TermId) : ST ω (Option (Array TermId)) := do
  ...
  return some result
private def bindFormulaWithFuel (builder : Builder ω σ) (memo : ST.Ref ω (Std.HashMap (FormulaId × Nat) FormulaId))
...
structure BindResult where
  ...
  visitedNodes : Nat
  deriving Repr
def bindFormula (builder : Builder ω σ) (fuel : Nat) (root : FormulaId) (substitution : Array TermId) : ST ω (Option BindResult) := do
  let memo ← ST.mkRef {}
...
def ofFormula {availableStage depth : Nat} (formula : Formula σ availableStage depth) : Graph σ × FormulaId :=
  runST fun ω => do
...
def bindWithStats {sourceDepth targetDepth : Nat} (graph : Graph σ) (root : FormulaId) (substitution : Fin sourceDepth → Term targetDepth) :
    Option (BindResult σ) :=
  runST fun ω => do
...
def bind {sourceDepth targetDepth : Nat} (graph : Graph σ) (root : FormulaId) (substitution : Fin sourceDepth → Term targetDepth) :
...
def rename {sourceDepth targetDepth : Nat} (graph : Graph σ) (root : FormulaId) (indexMap : Fin sourceDepth → Fin targetDepth) :
