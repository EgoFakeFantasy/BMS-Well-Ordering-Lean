...
structure Interpretation (𝒞 : OrderedPairConvention) (ℳ : Structure.{u}) where
  Codes : ℳ.Domain → ℳ.Domain → ℳ.Domain → Prop
  realizes :
    ∀ {depth : Nat} (env : Env ℳ depth) (pair left right : Term depth),
...
@[simp] theorem satisfies_code_iff
    {𝒞 : OrderedPairConvention} {ℳ : Structure.{u}} (𝕀 : 𝒞.Interpretation ℳ)
    {depth : Nat} (env : Env ℳ depth) (pair left right : Term depth) :
    Formula.satisfies env (𝒞.code pair left right) ↔
      𝕀.Codes (pair.eval env) (left.eval env) (right.eval env) :=
...
theorem eq {ℳ : Structure.{u}}
    ...
    {left right relation : ℳ.Domain} (hLeft : ℳ.IsDomainOf 𝕀 left relation) (hRight : ℳ.IsDomainOf 𝕀 right relation) :
    left = right := by
...
theorem eq {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ} (hExt : Extensional ℳ)
    {left right relation : ℳ.Domain} (hLeft : ℳ.IsRangeOf 𝕀 left relation) (hRight : ℳ.IsRangeOf 𝕀 right relation) :
    left = right := by
  apply hExt.eq_of_same_members
  intro output
...
theorem input_mem_of_pairMember {ℳ : Structure.{u}}
    ...
    {function source target input output : ℳ.Domain} (hFunction :
      ℳ.IsSetFunctionFromTo 𝕀 function source target) (hPair : ℳ.PairMember 𝕀 input output function) :
    ℳ.mem input source := by
  exact (hFunction.2.1 input).mpr ⟨output, hPair⟩
/-- 函数图中的每个输出都属于给定目标集。 -/
...
theorem mem_iff_pairMember {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ}
    {relation pair : ℳ.Domain} (hRelation : ℳ.IsSetRelation 𝕀 relation) :
    ℳ.mem pair relation ↔
      ∃ left right,
...
theorem eq_of_pairMember_iff {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ} (hExt : Extensional ℳ) {left right : ℳ.Domain} (hLeft : ℳ.IsSetRelation 𝕀 left) (hRight : ℳ.IsSetRelation 𝕀 right)
    (hPairs : ∀ input output,
...
theorem isSetFunction {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ}
    {restriction function source : ℳ.Domain} (hRestriction :
  ...
  · intro input first second hFirst hSecond
    ...
    · exact (hRestriction.2 input second).mp hSecond |>.2
/-- 函数在其定义域子集上的限制以该子集为精确定义域。 -/
theorem isDomainOf {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ}
    {restriction function source domain : ℳ.Domain} (hRestriction :
      ℳ.IsRestrictionOf 𝕀 restriction function source) (hDomain : ℳ.IsDomainOf 𝕀 domain function)
    (hSubset : ℳ.MemberSubset source domain) :
    ℳ.IsDomainOf 𝕀 source restriction := by
  intro input
  constructor
  · intro hInput
    rcases (hDomain input).mp (hSubset input hInput) with
      ⟨output, hPair⟩
    exact
      ⟨output, (hRestriction.2 input output).mpr ⟨hInput, hPair⟩⟩
  · rintro ⟨output, hPair⟩
    exact (hRestriction.2 input output).mp hPair |>.1
/-- 函数限制到定义域子集后仍取值于原目标集。 -/
theorem isSetFunctionFromTo {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ}
    {restriction function source domain target : ℳ.Domain} (hRestriction :
      ℳ.IsRestrictionOf 𝕀 restriction function source) (hFunction :
  ...
  exact ⟨output, hOutput, (hRestriction.2 input output).mpr ⟨hInput, hPair⟩⟩
/-- 同一函数在同一源集上的两个限制相等。 -/
theorem eq {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ} (hExt : Extensional ℳ)
    {left right function source : ℳ.Domain} (hLeft : ℳ.IsRestrictionOf 𝕀 left function source) (hRight : ℳ.IsRestrictionOf 𝕀 right function source) :
    left = right := by
  apply IsSetRelation.eq_of_pairMember_iff hExt hLeft.1 hRight.1
  intro input output
  exact
    (hLeft.2 input output).trans (hRight.2 input output).symm
/-- 先限制到外层源集、再限制到其子集，等于直接限制原函数。 -/
theorem trans {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ}
    {outerRestriction innerRestriction function outer inner : ℳ.Domain} (hOuter :
      ℳ.IsRestrictionOf 𝕀 outerRestriction function outer) (hInner :
  ...
  · intro input output
    constructor
    · intro hPair
      have hInnerPair := (hInner.2 input output).mp hPair
      exact
        ⟨hInnerPair.1,
          (hOuter.2 input output).mpr
            ⟨hSubset input hInnerPair.1, hInnerPair.2⟩⟩
    · intro hPair
      exact
...
theorem satisfies_related_iff
    ...
    satisfies env (related schema parameters input output) ↔
      schema.denote (parameters.evalEnv env) (input.eval env) (output.eval env) := by
  unfold related BinarySchema.instantiate BinarySchema.denote
  rw [satisfies_bind]
  have hEnv :
      Definitional.Env.substitute env (Fin.cases output (fun previous => Fin.cases input parameters.get previous)) =
        ((parameters.evalEnv env).push (input.eval env)).push (output.eval env) := by
    rw [Env.mk.injEq]
    constructor
    · funext entry
...
theorem satisfies_orderedPairMem_iff
    ...
    {depth : Nat} (env : Env ℳ depth) (left right relation : Term depth) :
    satisfies env (orderedPairMem 𝒞 left right relation) ↔
      ℳ.PairMember 𝕀 (left.eval env) (right.eval env) (relation.eval env) := by
...
theorem satisfies_isFunction_iff
  ...
  · rintro ⟨hRelation, hSingle⟩
    refine ⟨hRelation, ?_⟩
    intro input first second hFirst hSecond
    exact hExt.eq_of_same_members first second <| (satisfies_extensionalEq_iff (((env.push input).push first).push second) (.bound 1) (.bound 0)).mp
          (hSingle input first second ⟨hFirst, hSecond⟩)
  · rintro ⟨hRelation, hSingle⟩
    refine ⟨hRelation, ?_⟩
    intro input first second hPairs
    apply (satisfies_extensionalEq_iff (((env.push input).push first).push second) (.bound 1) (.bound 0)).mpr
    have hFirst :
        ℳ.PairMember 𝕀 input first (function.eval env) := by
      simpa using hPairs.1
    have hSecond :
        ℳ.PairMember 𝕀 input second (function.eval env) := by
...
theorem satisfies_isDomain_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {depth : Nat} (env : Env ℳ depth) (domain relation : Term depth) :
    satisfies env (isDomain 𝒞 domain relation) ↔
      ℳ.IsDomainOf 𝕀 (domain.eval env) (relation.eval env) := by
...
theorem satisfies_isRange_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {depth : Nat} (env : Env ℳ depth) (range relation : Term depth) :
    satisfies env (isRange 𝒞 range relation) ↔
      ℳ.IsRangeOf 𝕀 (range.eval env) (relation.eval env) := by
...
theorem satisfies_isFunctionFromTo_iff
    ...
    satisfies env (isFunctionFromTo 𝒞 function source target) ↔
      ℳ.IsSetFunctionFromTo 𝕀 (function.eval env) (source.eval env) (target.eval env) := by
...
theorem satisfies_isSurjectiveOnto_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {depth : Nat} (env : Env ℳ depth) (function source target : Term depth) :
    satisfies env (isSurjectiveOnto 𝒞 function source target) ↔
      ℳ.IsSetSurjectiveOnto 𝕀 (function.eval env) (source.eval env) (target.eval env) := by
...
theorem satisfies_isRestriction_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {depth : Nat} (env : Env ℳ depth) (restriction function source : Term depth) :
    satisfies env (isRestriction 𝒞 restriction function source) ↔
      ℳ.IsRestrictionOf 𝕀 (restriction.eval env) (function.eval env) (source.eval env) := by
