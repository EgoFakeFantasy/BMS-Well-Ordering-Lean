...
namespace Axioms
/-- 基础公理的项目核语义。 -/
theorem foundation_sat_iff_d {ℳ : Structure.{u}} (free : FreeVarId → ℳ.Domain) :
...
theorem mem_minimal_exists_d {ℳ : Structure.{u}} (hKP : ℳ.Models SetTheory.KP) {set : ℳ.Domain} (hNonempty : ∃ value, ℳ.mem value set) :
    ∃ minimal, ℳ.mem minimal set ∧
      ∀ value, ℳ.mem value set → ¬ ℳ.mem value minimal := by
  ...
  exact (Axioms.foundation_sat_iff_d free).mp hFoundation
      set hNonempty
/-- Foundation 排除集合的自隶属。 -/
theorem mem_irrefl_d {ℳ : Structure.{u}} (hKP : ℳ.Models SetTheory.KP) (set : ℳ.Domain) :
    ¬ ℳ.mem set set := by
  intro hSelf
  ...
  rcases mem_minimal_exists_d hKP hNonempty with
    ⟨minimal, hMinimal, hDisjoint⟩
  have hMinimalEq : minimal = set := (hSingleton minimal).mp hMinimal
  subst minimal
  exact hDisjoint set ((hSingleton set).mpr rfl) hSelf
/-- Foundation 排除包含所有对象的全集。 -/
theorem no_universal_set_d {ℳ : Structure.{u}} (hKP : ℳ.Models SetTheory.KP) :
    ¬ ∃ universal, ∀ value, ℳ.mem value universal := by
  rintro ⟨universal, hUniversal⟩
  exact mem_irrefl_d hKP universal (hUniversal universal)
end KP
end SetTheory
end YesMetaZFC
