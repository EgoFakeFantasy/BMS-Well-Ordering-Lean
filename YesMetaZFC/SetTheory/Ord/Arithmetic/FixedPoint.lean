...
def IsOrdinalTotalization (function : ℳ.Domain → ℳ.Domain → Prop) (input output : ℳ.Domain) : Prop := (ℳ.IsOrdinal input ∧ function input output) ∨
    (¬ ℳ.IsOrdinal input ∧ ∀ value, ¬ ℳ.mem value output)
/-- 从零序数开始迭代序数类函数，极限步取此前值域的并。 -/
def IsOrdinalIterationStep (𝕀 : 𝒞.Interpretation ℳ) (function : ℳ.Domain → ℳ.Domain → Prop) (sequence output : ℳ.Domain) : Prop :=
  ℳ.IsZeroSuccessorLimitStep 𝕀 (fun initial => ∀ value, ¬ ℳ.mem value initial) (ℳ.IsOrdinalTotalization function) sequence output
/-- `value` 是从零序数开始迭代 `function` 在 `α` 处的值。 -/
def IsOrdinalIteration (𝕀 : 𝒞.Interpretation ℳ) (function : ℳ.Domain → ℳ.Domain → Prop) (α value : ℳ.Domain) : Prop :=
  ℳ.IsRecursionValue 𝕀 (ℳ.IsOrdinalIterationStep 𝕀 function) α value
/-- `α` 是以 `ω` 为底的序数幂不动点。 -/
def IsEpsilonNumber (𝕀 : 𝒞.Interpretation ℳ) (ω α : ℳ.Domain) : Prop :=
  ℳ.IsOrdinal α ∧
    ℳ.IsOrdinalExponentiation 𝕀 α ω α
/-- `epsilon` 是最小的以 `ω` 为底的序数幂不动点。 -/
def IsEpsilonZero (𝕀 : 𝒞.Interpretation ℳ) (ω epsilon : ℳ.Domain) : Prop :=
...
def ordinalIteration (𝒞 : OrderedPairConvention)
  ...
  transfiniteRecursion 𝒞 (ordinalIterationOperator 𝒞 function)
end BinarySchema
namespace Formula
/-- `value` 是从零序数开始迭代 `function` 在 `α` 处的值。 -/
def isOrdinalIteration (𝒞 : OrderedPairConvention)
    {parameterCount depth : Nat} (function : BinarySchema parameterCount) (parameters : TermVector parameterCount depth)
    (α value : Term depth) : Formula 1 depth :=
  related (function.ordinalIteration 𝒞)
    parameters α value
/-- `α` 是以 `ω` 为底的序数幂不动点。 -/
def isEpsilonNumber (𝒞 : OrderedPairConvention)
...
def isEpsilonZero (𝒞 : OrderedPairConvention)
  ...
  .conj (isEpsilonNumber 𝒞 ω epsilon) <|
    .forallE <| .imp (isEpsilonNumber 𝒞 ω.weaken Term.newest) <|
      .disj (extensionalEq epsilon.weaken Term.newest) (.mem epsilon.weaken Term.newest)
end Formula
...
theorem denote_ordinalIterationOperator_iff (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ)
    ...
    (function.ordinalIterationOperator 𝒞).denote
        env sequence output ↔
      ℳ.IsOrdinalIterationStep 𝕀 (function.denote env) sequence output := by
...
theorem denote_ordinalIteration_iff (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ)
    ...
    (function.ordinalIteration 𝒞).denote env α value ↔
      ℳ.IsOrdinalIteration 𝕀 (function.denote env) α value := by
  have hOperator : (function.ordinalIterationOperator 𝒞).denote env =
        ℳ.IsOrdinalIterationStep 𝕀 (function.denote env) := by
  ...
  rw [Formula.denote_transfiniteRecursion_iff
    𝕀 hExt env (function.ordinalIterationOperator 𝒞) α value]
  rw [hOperator]
  rfl
end BinarySchema
namespace Formula
...
theorem satisfies_isOrdinalIteration_iff (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ)
    ...
    (α value : Term depth) :
    satisfies env (isOrdinalIteration 𝒞 function
          parameters α value) ↔
      ℳ.IsOrdinalIteration 𝕀 (function.denote (parameters.evalEnv env)) (α.eval env) (value.eval env) := by
...
theorem satisfies_isEpsilonNumber_iff (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ)
    ...
    satisfies env (isEpsilonNumber 𝒞 ω α) ↔
      ℳ.IsEpsilonNumber 𝕀 (ω.eval env) (α.eval env) := by
...
theorem satisfies_isEpsilonZero_iff (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ)
    ...
    satisfies env (isEpsilonZero 𝒞 ω epsilon) ↔
      ℳ.IsEpsilonZero 𝕀 (ω.eval env) (epsilon.eval env) := by
...
theorem ordinalTotalization_isClassFunction (hZF : ℳ.Models SetTheory.ZF)
    {function : ℳ.Domain → ℳ.Domain → Prop} (hFunction : ℳ.IsClassFunctionOnOrdinals function) :
    ∀ input,
      ∃ output,
...
theorem ordinalIterationStep_isClassFunctionOnTransfiniteSequences (hZF : ℳ.Models SetTheory.ZF) (𝕀 : 𝒞.Interpretation ℳ)
    {function : ℳ.Domain → ℳ.Domain → Prop} (hFunction : ℳ.IsClassFunctionOnOrdinals function) :
    ℳ.IsClassFunctionOnTransfiniteSequences 𝕀 (ℳ.IsOrdinalIterationStep 𝕀 function) := by
  rcases KP.exists_empty (ZF.modelsKP hZF) with
    ⟨zero, hZero⟩
  exact
    zeroSuccessorLimitStep_isClassFunctionOnTransfiniteSequences
      hZF 𝕀 (fun initial => ∀ value, ¬ ℳ.mem value initial) (ℳ.IsOrdinalTotalization function)
      ⟨zero, hZero, fun other hOther =>
        hZF.1.eq_of_same_members other zero (fun value => iff_of_false (hOther value) (hZero value))⟩ (ordinalTotalization_isClassFunction hZF hFunction)
/-- 迭代算子的 schema 解释在所有超限序列上全定义且单值。 -/
theorem ordinalIterationOperator_isClassFunctionOnTransfiniteSequences (hZF : ℳ.Models SetTheory.ZF) (𝕀 : 𝒞.Interpretation ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (function : Definitional.Project.BinarySchema parameterCount) (hFunction :
      ℳ.IsClassFunctionOnOrdinals (function.denote env)) :
    ℳ.IsClassFunctionOnTransfiniteSequences 𝕀 ((function.ordinalIterationOperator 𝒞).denote env) := by
  have hStep :=
  ...
  rcases hStep sequence hSequence with
    ⟨output, hOutput, hUnique⟩
  refine ⟨output, ?_, ?_⟩
  · exact (BinarySchema.denote_ordinalIterationOperator_iff
...
theorem ordinalIteration_isClassFunctionOnOrdinals (hZF : ℳ.Models SetTheory.ZF) (𝕀 : 𝒞.Interpretation ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (function : Definitional.Project.BinarySchema parameterCount) (hFunction :
      ℳ.IsClassFunctionOnOrdinals (function.denote env)) :
    ℳ.IsClassFunctionOnOrdinals ((function.ordinalIteration 𝒞).denote env) := by
  simpa [BinarySchema.ordinalIteration] using
    transfiniteRecursion_isClassFunctionOnOrdinals
      hZF 𝕀 env (function.ordinalIterationOperator 𝒞) (ordinalIterationOperator_isClassFunctionOnTransfiniteSequences
        hZF 𝕀 env function hFunction)
/-- 从零序数开始的迭代关系在每个序数处存在唯一值。 -/
theorem ordinalIteration_existsUnique (hZF : ℳ.Models SetTheory.ZF) (𝕀 : 𝒞.Interpretation ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (function : Definitional.Project.BinarySchema parameterCount) (hFunction :
      ℳ.IsClassFunctionOnOrdinals (function.denote env))
    {α : ℳ.Domain} (hα : ℳ.IsOrdinal α) :
    ∃ value,
      ℳ.IsOrdinalIteration 𝕀 (function.denote env) α value ∧
        ∀ other,
          ℳ.IsOrdinalIteration 𝕀 (function.denote env) α other →
            other = value := by
...
theorem ordinalIteration_isOrdinalClassFunction (hZF : ℳ.Models SetTheory.ZF) (𝕀 : 𝒞.Interpretation ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (function : Definitional.Project.BinarySchema parameterCount) (hNormal :
      ℳ.IsNormalOrdinalFunction (function.denote env)) :
    ℳ.IsOrdinalClassFunction ((function.ordinalIteration 𝒞).denote env) := by
  have hClass :=
  ...
  have hIteration :
      ℳ.IsOrdinalIteration 𝕀 (function.denote env) α value := (BinarySchema.denote_ordinalIteration_iff
      𝕀 hZF.1 env function α value).mp
      hValue
  ...
  · rcases hSuccessor with
    ...
    have hPredecessorMem : ℳ.mem predecessor α := (hSuccessor predecessor).mpr (Or.inr fun _ => Iff.rfl)
    ...
    have hPreviousIteration :
        ℳ.IsOrdinalIteration 𝕀 (function.denote env) predecessor previous :=
      hSequence.recursionValue_of_pairMember
        hPredecessorMem hPreviousPair
    have hPreviousOrdinal : ℳ.IsOrdinal previous :=
      hPrevious predecessor hPredecessorMem previous <| (BinarySchema.denote_ordinalIteration_iff
  ...
  · rcases exists_limitLengthSequenceWithUnion
    ...
    · rcases hOutput with
      ...
      apply Structure.IsOrdinal.of_union (ZF.modelsKP hZF) hUnion
      intro member hMember
      rcases (hRange member).mp hMember with
        ⟨index, hMemberPair⟩
      have hIndex : ℳ.mem index α := (hSequence.1.2.2 index).mpr
          ⟨member, hMemberPair⟩
      have hMemberIteration :
          ℳ.IsOrdinalIteration 𝕀 (function.denote env) index member :=
        hSequence.recursionValue_of_pairMember
          hIndex hMemberPair
      exact hPrevious index hIndex member <| (BinarySchema.denote_ordinalIteration_iff
          𝕀 hZF.1 env function
          index member).mpr hMemberIteration
/-- 正规序数函数的每个序数阶段迭代值都是序数。 -/
theorem ordinalIteration_isOrdinal (hZF : ℳ.Models SetTheory.ZF) (𝕀 : 𝒞.Interpretation ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (function : Definitional.Project.BinarySchema parameterCount) (hNormal :
      ℳ.IsNormalOrdinalFunction (function.denote env))
    {α value : ℳ.Domain} (hα : ℳ.IsOrdinal α) (hValue :
      ℳ.IsOrdinalIteration 𝕀 (function.denote env) α value) :
    ℳ.IsOrdinal value := by
  exact (ordinalIteration_isOrdinalClassFunction
      hZF 𝕀 env function hNormal).2
      α value hα <| (BinarySchema.denote_ordinalIteration_iff
          𝕀 hZF.1 env function
          α value).mpr hValue
/-- 迭代的零步方程：零阶段仍取零序数。 -/
theorem ordinalIteration_zero_iff (hZF : ℳ.Models SetTheory.ZF) (𝕀 : 𝒞.Interpretation ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (function : Definitional.Project.BinarySchema parameterCount) (hFunction :
      ℳ.IsClassFunctionOnOrdinals (function.denote env))
    {zero value : ℳ.Domain} (hZero : ∀ member, ¬ ℳ.mem member zero) :
    ℳ.IsOrdinalIteration 𝕀 (function.denote env) zero value ↔
      ∀ member, ¬ ℳ.mem member value := by
  have iterationValue_empty {selected : ℳ.Domain} (hSelected :
        ℳ.IsOrdinalIteration 𝕀 (function.denote env) zero selected) :
  ...
  · intro hValue
    rcases ordinalIteration_existsUnique
        hZF 𝕀 env function hFunction (Structure.IsOrdinal.of_no_members hZero) with
      ⟨selected, hSelected, _⟩
    have hSelectedEmpty := iterationValue_empty hSelected
    have hSelectedEq := by
      apply hZF.1.eq_of_same_members
      intro member
      exact iff_of_false (hSelectedEmpty member) (hValue member)
    simpa [hSelectedEq] using hSelected
/-- 迭代的后继步方程：下一阶段应用一次原正规函数。 -/
theorem ordinalIteration_successor_iff (hZF : ℳ.Models SetTheory.ZF) (𝕀 : 𝒞.Interpretation ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (function : Definitional.Project.BinarySchema parameterCount) (hNormal :
      ℳ.IsNormalOrdinalFunction (function.denote env))
    {predecessor successor value : ℳ.Domain} (hPredecessor : ℳ.IsOrdinal predecessor) (hSuccessor : ℳ.SuccessorOf successor predecessor) :
    ℳ.IsOrdinalIteration 𝕀 (function.denote env) successor value ↔
      ∃ previous,
        ℳ.IsOrdinalIteration 𝕀 (function.denote env) predecessor previous ∧
          function.denote env previous value := by
  have successorIterationData {selected : ℳ.Domain} (hSelected :
        ℳ.IsOrdinalIteration 𝕀 (function.denote env) successor selected) :
      ∃ previous,
        ℳ.IsOrdinalIteration 𝕀 (function.denote env) predecessor previous ∧
          function.denote env previous selected := by
    rcases hSelected with ⟨sequence, hSequence, hOutput⟩
    have hPredecessorMem : ℳ.mem predecessor successor := (hSuccessor predecessor).mpr (Or.inr fun _ => Iff.rfl)
    rcases (hSequence.1.2.2 predecessor).mp
        hPredecessorMem with
      ⟨previous, hPreviousPair⟩
    have hSuccessorLength :
  ...
  · rintro ⟨previous, hPrevious, hValue⟩
    have hSuccessorOrdinal :=
      KP.successor_isOrdinal (ZF.modelsKP hZF) hPredecessor hSuccessor
    rcases ordinalIteration_existsUnique
        hZF 𝕀 env function hNormal.1.1
    ...
    rcases ordinalIteration_existsUnique
        ...
        hPredecessor with
      ⟨_, _, hPreviousUnique⟩
    have hPreviousEq : selectedPrevious = previous := (hPreviousUnique selectedPrevious hSelectedPrevious).trans (hPreviousUnique previous hPrevious).symm
    subst selectedPrevious
    rcases hNormal.1.1 previous (ordinalIteration_isOrdinal
          hZF 𝕀 env function hNormal
          hPredecessor hPrevious) with
      ⟨_, _, hValueUnique⟩
    have hSelectedEq : selected = value := (hValueUnique selected hSelectedValue).trans (hValueUnique value hValue).symm
    simpa [hSelectedEq] using hSelected
/-- 迭代的极限步方程：极限阶段取此前迭代值集合的并。 -/
theorem ordinalIteration_limit_iff (hZF : ℳ.Models SetTheory.ZF) (𝕀 : 𝒞.Interpretation ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (function : Definitional.Project.BinarySchema parameterCount) (hFunction :
      ℳ.IsClassFunctionOnOrdinals (function.denote env))
    {limit value : ℳ.Domain} (hLimit : ℳ.IsLimitOrdinal limit) :
    ℳ.IsOrdinalIteration 𝕀 (function.denote env) limit value ↔
      ∃ range, (∀ member,
          ℳ.mem member range ↔
            ∃ index, ℳ.mem index limit ∧
              ℳ.IsOrdinalIteration 𝕀 (function.denote env) index member) ∧
          ℳ.IsUnionOf value range := by
  have limitIterationData {selected : ℳ.Domain} (hSelected :
        ℳ.IsOrdinalIteration 𝕀 (function.denote env) limit selected) :
      ∃ range, (∀ member,
          ℳ.mem member range ↔
            ∃ index, ℳ.mem index limit ∧
              ℳ.IsOrdinalIteration 𝕀 (function.denote env) index member) ∧
    ...
    · intro index hIndex first second hFirst hSecond
      rcases ordinalIteration_existsUnique
          hZF 𝕀 env function hFunction (hLimit.1.mem hIndex) with
        ⟨_, _, hUnique⟩
...
-/
theorem ordinalIteration_isIncreasing_on_omega (hZF : ℳ.Models SetTheory.ZF) (𝕀 : 𝒞.Interpretation ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (function : Definitional.Project.BinarySchema parameterCount) (hNormal :
      ℳ.IsNormalOrdinalFunction (function.denote env))
    {ω : ℳ.Domain} (hω : ℳ.IsOmega ω) (hInitialGrowth : ∀ zero, (∀ member, ¬ ℳ.mem member zero) →
        ∀ next, function.denote env zero next →
          ℳ.mem zero next) :
    ∀ α, ℳ.mem α ω →
      ∀ predecessor, ℳ.mem predecessor α →
        ∀ predecessorValue αValue,
          ℳ.IsOrdinalIteration 𝕀 (function.denote env) predecessor predecessorValue →
            ℳ.IsOrdinalIteration 𝕀 (function.denote env) α αValue →
  ...
  have hProperty :
    ...
    · rcases separation_exists_d hZF
    ...
    · intro predecessor hPredecessorOmega hPredecessorProperty
      ...
      have hSuccessorOrdinal :=
        KP.successor_isOrdinal (ZF.modelsKP hZF) hPredecessorOrdinal hSuccessor
      have hSuccessorIteration :
          ℳ.IsOrdinalIteration 𝕀 (function.denote env) successor successorValue := (BinarySchema.denote_ordinalIteration_iff
          𝕀 hZF.1 env function
          successor successorValue).mp hSuccessorValue
      rcases (ordinalIteration_successor_iff
            hZF 𝕀 env function hNormal
            hPredecessorOrdinal hSuccessor).mp
            hSuccessorIteration with
        ⟨previousValue, hPreviousIteration, hSuccessorFunction⟩
      have hPreviousValue :
          iteration.denote env predecessor previousValue := (BinarySchema.denote_ordinalIteration_iff
          𝕀 hZF.1 env function
      ...
      have hPreviousSuccessor :
        ...
        · rcases hω.exists_predecessor_of_mem_of_nonempty
          ...
          rcases (ordinalIteration_successor_iff
                hZF 𝕀 env function hNormal (hω.members_areOrdinals hZF
                  prior hPriorOmega)
                hPredecessorSuccessor).mp hPreviousIteration with
            ⟨priorValue, hPriorIteration, hPreviousFunction⟩
          have hPriorValue :
              iteration.denote env prior priorValue := (BinarySchema.denote_ordinalIteration_iff
              𝕀 hZF.1 env function
              prior priorValue).mpr hPriorIteration
          have hPriorPredecessor :
              ℳ.mem prior predecessor := (hPredecessorSuccessor prior).mpr (Or.inr fun _ => Iff.rfl)
          ...
          exact hNormal.2.1 priorValue previousValue (ordinalIteration_isOrdinal
              hZF 𝕀 env function hNormal (hω.members_areOrdinals hZF prior hPriorOmega)
              hPriorIteration)
        ...
        · have hPredecessorEmpty :
          ...
          have hPreviousEmpty := (ordinalIteration_zero_iff
              hZF 𝕀 env function hNormal.1.1
      ...
      · have hEarlierEq :=
        ...
        have hEarlierIteration :
            ℳ.IsOrdinalIteration 𝕀 (function.denote env) predecessor predecessorValue := (BinarySchema.denote_ordinalIteration_iff
            𝕀 hZF.1 env function
            predecessor predecessorValue).mp hEarlierValue
        have hValueEq : predecessorValue = previousValue := (hUnique predecessorValue hEarlierIteration).trans (hUnique previousValue hPreviousIteration).symm
  ...
  exact hProperty α hα predecessor hPredecessor
    predecessorValue αValue ((BinarySchema.denote_ordinalIteration_iff
      𝕀 hZF.1 env function
      predecessor predecessorValue).mpr hPredecessorValue) ((BinarySchema.denote_ordinalIteration_iff
      𝕀 hZF.1 env function
...
交换。
-/
theorem normalFunction_exists_limitFixedPoint (hZF : ℳ.Models SetTheory.ZF) (𝕀 : 𝒞.Interpretation ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (function : Definitional.Project.BinarySchema parameterCount) (hNormal :
      ℳ.IsNormalOrdinalFunction (function.denote env))
    {ω : ℳ.Domain} (hω : ℳ.IsOmega ω) (hInitialGrowth : ∀ zero, (∀ member, ¬ ℳ.mem member zero) →
        ∀ next, function.denote env zero next →
          ℳ.mem zero next) :
    ∃ fixed,
      ℳ.IsOrdinalIteration 𝕀 (function.denote env) ω fixed ∧
  ...
  rcases (ordinalIteration_limit_iff
        hZF 𝕀 env function hNormal.1.1
        hωLimit).mp hFixedIteration with
    ⟨range, hRange, hUnion⟩
  -- 每个有限阶段都能推进到下一阶段，并由严格递增性得到当前值属于下一值。
  have exists_nextIterationValue
      {index current : ℳ.Domain} (hIndex : ℳ.mem index ω) (hCurrent :
        ℳ.IsOrdinalIteration 𝕀 (function.denote env) index current) :
      ∃ successor next,
        ℳ.mem successor ω ∧
          ℳ.SuccessorOf successor index ∧
          ℳ.IsOrdinalIteration 𝕀 (function.denote env) successor next ∧
    ...
    have hSuccessorOrdinal :=
      KP.successor_isOrdinal (ZF.modelsKP hZF) hIndexOrdinal hSuccessor
    rcases ordinalIteration_existsUnique
        hZF 𝕀 env function hNormal.1.1
        hSuccessorOrdinal with
      ⟨next, hNext, _⟩
    ...
    rcases ordinalIteration_existsUnique
        ...
        hIndexOrdinal with
      ⟨_, _, hUnique⟩
    have hPreviousEq : previous = current := (hUnique previous hPrevious).trans (hUnique current hCurrent).symm
    subst previous
    have hIndexSuccessor : ℳ.mem index successor := (hSuccessor index).mpr (Or.inr fun _ => Iff.rfl)
    exact
  ...
  have hFixedLimit : ℳ.IsLimitOrdinal fixed := by
    ...
    · rcases hω.1.1 with
      ...
      rcases ordinalIteration_existsUnique
          hZF 𝕀 env function hNormal.1.1 (Structure.IsOrdinal.of_no_members hZero) with
    ...
    · intro member hMember
      ...
      exact ⟨current, (hUnion current).mpr
          ⟨next, (hRange next).mpr
            ⟨successor, hSuccessorOmega, hNext⟩,
            hCurrentNext⟩,
        hMemberCurrent⟩
  ...
  have hImageEq : image = fixed := by
    ...
    · intro hMember
      ...
      have hFunctionValueNext :
        ...
        hNormal.2.1 input current (hFixedOrdinal.mem hInputFixed) (ordinalIteration_isOrdinal
            hZF 𝕀 env function hNormal (hω.members_areOrdinals hZF index hIndex)
            hCurrent)
          hInputCurrent functionValue next
          hInputFunction hCurrentFunction
      have hNextOrdinal :=
        ordinalIteration_isOrdinal
          hZF 𝕀 env function hNormal (hω.members_areOrdinals
    ...
    · intro hMember
      ...
      have hCurrentFixed : ℳ.mem current fixed := (hUnion current).mpr
          ⟨next, (hRange next).mpr
            ⟨successor, hSuccessorOmega, hNext⟩,
            hCurrentNext⟩
      have hNextRange : ℳ.mem next imageRange := (hImageRange next).mpr
...
theorem exists_epsilonNumber (hZF : ℳ.Models SetTheory.ZF) (𝕀 : 𝒞.Interpretation ℳ)
  ...
  have hNormal :
      ℳ.IsNormalOrdinalFunction ((BinarySchema.ordinalExponentiation 𝒞).denote env) := by
    have hRelation : (BinarySchema.ordinalExponentiation 𝒞).denote env =
          fun exponent power =>
  ...
  have hInitialGrowth :
      ∀ zero, (∀ member, ¬ ℳ.mem member zero) →
        ∀ next, (BinarySchema.ordinalExponentiation 𝒞).denote
              env zero next →
            ℳ.mem zero next := by
    intro zero hZero next hNext
    have hNextSemantic :
        ℳ.IsOrdinalExponentiation 𝕀
          next ω zero := (BinarySchema.denote_ordinalExponentiation_iff
        𝕀 hZF.1 env zero next).mp hNext
    have hNextOne := (ordinalExponentiation_zero_iff
    ...
    subst oneZero
    exact (hNextSuccessor zero).mpr (Or.inr fun _ => Iff.rfl)
  rcases normalFunction_exists_limitFixedPoint
      hZF 𝕀 env (BinarySchema.ordinalExponentiation 𝒞)
      hNormal hω hInitialGrowth with
    ⟨epsilon, hIteration, _, hFixed⟩
  have hFixedExponentiation :
      ℳ.IsOrdinalExponentiation 𝕀
        epsilon ω epsilon := (BinarySchema.denote_ordinalExponentiation_iff
...
theorem epsilonZero_exists (hZF : ℳ.Models SetTheory.ZF) (𝕀 : 𝒞.Interpretation ℳ)
    {ω : ℳ.Domain} (hω : ℳ.IsOmega ω) :
    ∃ epsilon,
  ...
  rcases KP.exists_successor (ZF.modelsKP hZF) upper with
    ⟨bound, hBound⟩
  have hBoundOrdinal :=
    KP.successor_isOrdinal (ZF.modelsKP hZF) hUpper.1 hBound
  let env : Env ℳ 1 := {
    bound := fun _ => ω
    free := fun _ => Classical.choice ℳ.nonempty
  }
  rcases separation_exists_d hZF (UnarySchema.epsilonNumber 𝒞) env bound with
  ...
  have hEpsilonData := (hCandidatesSemantic epsilon).mp hEpsilonCandidates
  refine ⟨epsilon, hEpsilonData.2, ?_⟩
  intro candidate hCandidate
  rcases Structure.IsOrdinal.trichotomy hZF.1
      hEpsilonData.2.1 hCandidate.1 (KP.difference_exists_d (ZF.modelsKP hZF)) (KP.intersection_exists_d (ZF.modelsKP hZF)
  ...
  · have hCandidateBound : ℳ.mem candidate bound :=
    ...
    have hCandidateCandidates : ℳ.mem candidate candidates := (hCandidatesSemantic candidate).mpr
...
theorem epsilonZero_unique (𝕀 : 𝒞.Interpretation ℳ)
    {ω first second : ℳ.Domain} (hFirst : ℳ.IsEpsilonZero 𝕀 ω first) (hSecond : ℳ.IsEpsilonZero 𝕀 ω second) :
    first = second := by
...
theorem epsilonZero_existsUnique (hZF : ℳ.Models SetTheory.ZF) (𝕀 : 𝒞.Interpretation ℳ)
    {ω : ℳ.Domain} (hω : ℳ.IsOmega ω) :
    ∃ epsilon,
