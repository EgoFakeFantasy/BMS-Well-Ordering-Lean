...
/-- 序数二确实是序数。 -/
theorem isOrdinal {ℳ : Structure.{u}} (hKP : ℳ.Models SetTheory.KP)
    {two : ℳ.Domain} (hTwo : ℳ.IsOrdinalTwo two) :
    ℳ.IsOrdinal two := by
  rcases hTwo with ⟨one, hOne, hSuccessor⟩
  exact KP.successor_isOrdinal hKP (KP.ordinalOne_isOrdinal hKP hOne) hSuccessor
/-- 序数二非空。 -/
...
namespace KP
/-- KP 中存在序数二。 -/
theorem exists_ordinalTwo {ℳ : Structure.{u}} (hKP : ℳ.Models SetTheory.KP) :
...
-/
private def ordinalMultiplicationTwoBounded (𝒞 : OrderedPairConvention) : UnarySchema 2 where
...
private theorem satisfies_ordinalAdditionOneSuccessor_iff
    {ℳ : Structure.{u}}
    {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 1) (α : ℳ.Domain) :
    satisfies (env.push α) (UnarySchema.ordinalAdditionOneSuccessor 𝒞).body ↔
      ∀ sum,
...
private theorem satisfies_ordinalMultiplicationTwoBounded_iff
    {ℳ : Structure.{u}}
    {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 2) (α : ℳ.Domain) :
    satisfies (env.push α) (UnarySchema.ordinalMultiplicationTwoBounded 𝒞).body ↔
      ∀ product,
        ℳ.IsOrdinalMultiplication 𝕀
            product (env.bound 0) α →
          ℳ.mem product (env.bound 1) ∧ (ℳ.SameMembers α product ∨
              ℳ.mem α product) := by
...
theorem ordinalAddition_one_iff
    ...
    {sum left one : ℳ.Domain} (hOne : ℳ.IsOrdinalOne one) :
  ...
  rw [ordinalAddition_successor_iff
    hZF 𝕀 (Structure.IsOrdinal.of_no_members hZero) hSuccessor]
  constructor
  · rintro ⟨previous, hPrevious, hSum⟩
    have hPreviousEq := (ordinalAddition_zero_iff
        hZF 𝕀 hZero).mp hPrevious
    simpa [hPreviousEq] using hSum
  · intro hSum
    exact ⟨left, (ordinalAddition_zero_iff
        hZF 𝕀 hZero).mpr rfl,
      hSum⟩
/-- 加二等价于连续取两次集合后继。 -/
theorem ordinalAddition_two_iff
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {sum left two : ℳ.Domain} (hTwo : ℳ.IsOrdinalTwo two) :
  ...
  rw [ordinalAddition_successor_iff
    hZF 𝕀 (KP.ordinalOne_isOrdinal (ZF.modelsKP hZF) hOne)
    hTwoSuccessor]
  constructor
  · rintro ⟨middle, hMiddle, hSum⟩
    exact ⟨middle, (ordinalAddition_one_iff
...
theorem ordinalAddition_one_isSuccessor_on_omega
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω one : ℳ.Domain} (hω : ℳ.IsOmega ω) (hOne : ℳ.IsOrdinalOne one) :
    ∀ α, ℳ.mem α ω →
      ∀ sum,
  ...
  apply hω.induction (fun α => ∀ sum,
        ℳ.IsOrdinalAddition 𝕀 sum one α →
          ℳ.SuccessorOf sum α)
  · rcases separation_exists_d hZF (Definitional.Project.UnarySchema.ordinalAdditionOneSuccessor 𝒞)
  ...
  · intro empty hEmpty sum hSum
    have hSumEq := (ordinalAddition_zero_iff
...
theorem ordinalAddition_one_omega
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω one sum : ℳ.Domain} (hω : ℳ.IsOmega ω) (hOne : ℳ.IsOrdinalOne one) (hSum : ℳ.IsOrdinalAddition 𝕀 sum one ω) :
    sum = ω := by
  have hFinite :=
  ...
  · intro hValue
    ...
    exact (hUnion value).mpr
      ⟨successor, (hRange successor).mpr
          ⟨value, hValue, hSuccessorValue⟩, (hSuccessor value).mpr (Or.inr fun _ => Iff.rfl)⟩
/-- `ω + one` 是 `ω` 的后继，因此不等于 `ω`。 -/
theorem ordinalAddition_omega_one_ne_omega
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω one sum : ℳ.Domain} (hω : ℳ.IsOmega ω) (hOne : ℳ.IsOrdinalOne one) (hSum : ℳ.IsOrdinalAddition 𝕀 sum ω one) :
    sum ≠ ω := by
  have hSuccessor := (ordinalAddition_one_iff
      hZF 𝕀 hOne).mp hSum
  intro hEq
  subst sum
  have hSelf : ℳ.mem ω ω := (hSuccessor ω).mpr (Or.inr fun _ => Iff.rfl)
...
theorem ordinalAddition_not_commutative_at_omega
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω one onePlusOmega omegaPlusOne : ℳ.Domain} (hω : ℳ.IsOmega ω) (hOne : ℳ.IsOrdinalOne one) (hOnePlusOmega :
      ℳ.IsOrdinalAddition 𝕀 onePlusOmega one ω) (hωPlusOne :
...
theorem ordinalMultiplication_two_bounded_on_omega
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω two : ℳ.Domain} (hω : ℳ.IsOmega ω) (hTwo : ℳ.IsOrdinalTwo two) :
    ∀ α, ℳ.mem α ω →
      ∀ product,
        ℳ.IsOrdinalMultiplication 𝕀
            product two α →
          ℳ.mem product ω ∧ (ℳ.SameMembers α product ∨
              ℳ.mem α product) := by
  let env : Env ℳ 2 := {
    bound := fun index =>
  ...
  apply hω.induction (fun α => ∀ product,
        ℳ.IsOrdinalMultiplication 𝕀
            product two α →
          ℳ.mem product ω ∧ (ℳ.SameMembers α product ∨
              ℳ.mem α product))
  · rcases separation_exists_d hZF (Definitional.Project.UnarySchema.ordinalMultiplicationTwoBounded 𝒞)
        env ω with ⟨closed, hClosed⟩
    refine ⟨closed, fun α => ?_⟩
    rw [hClosed α]
    exact and_congr_right fun _ =>
  ...
  · intro empty hEmpty product hProduct
    ...
    have hProductEq : product = omegaEmpty := by
      apply hZF.1.eq_of_same_members
      intro value
      exact iff_of_false (hProductEmpty value) (hωEmpty value)
  ...
  · intro predecessor hPredecessor hPrevious
    ...
    have hMiddleMemProduct : ℳ.mem middle product := (hProductSuccessor middle).mpr (Or.inr fun _ => Iff.rfl)
    have hPreviousMemMiddle : ℳ.mem previous middle := (hMiddleSuccessor previous).mpr (Or.inr fun _ => Iff.rfl)
    ...
    · have hPreviousOrdinal :=
      ...
      rcases Structure.IsOrdinal.trichotomy hZF.1
          hSuccessorOrdinal hPreviousOrdinal (KP.difference_exists_d (ZF.modelsKP hZF)) (KP.intersection_exists_d (ZF.modelsKP hZF)
...
theorem ordinalMultiplication_two_omega
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω two product : ℳ.Domain} (hω : ℳ.IsOmega ω) (hTwo : ℳ.IsOrdinalTwo two) (hProduct :
  ...
  rcases (ordinalMultiplication_limit_iff
      hZF 𝕀 hTwoOrdinal (hω.isLimitOrdinal hZF)).mp hProduct with
    ⟨range, hRange, hUnion⟩
  apply hZF.1.eq_of_same_members
  intro value
  constructor
  · intro hValue
    ...
    rcases (hRange rangeValue).mp hRangeValue with
      ⟨index, hIndex, hIndexValue⟩
    have hRangeOmega := (hFinite index hIndex rangeValue hIndexValue).1
    exact (hω.transitive hZF)
  ...
  · intro hValue
    rcases hω.1.2 value hValue with
      ⟨successor, hSuccessor, hSuccessorOmega⟩
    have hSuccessorOrdinal :=
      KP.successor_isOrdinal (ZF.modelsKP hZF) (hω.members_areOrdinals hZF value hValue)
        hSuccessor
    ...
    have hValueSuccessor : ℳ.mem value successor := (hSuccessor value).mpr (Or.inr fun _ => Iff.rfl)
...
theorem ordinalAddition_omega_self_ne_omega
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω sum : ℳ.Domain} (hω : ℳ.IsOmega ω) (hSum :
  ...
  have hωPlusZero :
      ℳ.IsOrdinalAddition 𝕀 ω ω zero := (ordinalAddition_zero_iff
      hZF 𝕀 hZero).mpr rfl
  have hωMemSum :=
    ordinalAddition_isIncreasingOnOrdinals
      hZF 𝕀 hωOrdinal
      zero ω (Structure.IsOrdinal.of_no_members hZero)
...
theorem ordinalMultiplication_omega_two_eq_addition
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω two product sum : ℳ.Domain} (hω : ℳ.IsOmega ω) (hTwo : ℳ.IsOrdinalTwo two) (hProduct :
      ℳ.IsOrdinalMultiplication 𝕀 product ω two) (hSum :
      ℳ.IsOrdinalAddition 𝕀 sum ω ω) :
    product = sum := by
  rcases hTwo with ⟨one, hOne, hTwoSuccessor⟩
  have hωOrdinal := hω.isOrdinal hZF
  rcases (ordinalMultiplication_successor_iff
      hZF 𝕀 hωOrdinal (KP.ordinalOne_isOrdinal (ZF.modelsKP hZF) hOne)
      hTwoSuccessor).mp hProduct with
    ⟨omegaTimesOne, hωTimesOne, hProductAddition⟩
  rcases hOne with ⟨zero, hZero, hOneSuccessor⟩
  rcases (ordinalMultiplication_successor_iff
      hZF 𝕀 hωOrdinal (Structure.IsOrdinal.of_no_members hZero)
      hOneSuccessor).mp hωTimesOne with
    ⟨omegaTimesZero, hωTimesZero, hωTimesOneAddition⟩
  have hωTimesZeroEmpty := (ordinalMultiplication_zero_iff
  ...
  exact (hUnique product hProductAddition).trans (hUnique sum hSum).symm
/-- 乘法在 `two` 与 `ω` 上不交换。 -/
theorem ordinalMultiplication_not_commutative_at_omega
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω two twoTimesOmega omegaTimesTwo
      omegaPlusOmega : ℳ.Domain} (hω : ℳ.IsOmega ω) (hTwo : ℳ.IsOrdinalTwo two) (hTwoTimesOmega :
      ℳ.IsOrdinalMultiplication 𝕀
        twoTimesOmega two ω) (hωTimesTwo :
      ℳ.IsOrdinalMultiplication 𝕀
        omegaTimesTwo ω two) (hωPlusOmega :
      ℳ.IsOrdinalAddition 𝕀
        omegaPlusOmega ω ω) :
    twoTimesOmega = ω ∧
...
theorem exists_ordinalAddition_noncommutative
  ...
  rcases ordinalAddition_existsUnique hZF 𝕀
      ω (KP.ordinalOne_isOrdinal (ZF.modelsKP hZF) hOne) with
