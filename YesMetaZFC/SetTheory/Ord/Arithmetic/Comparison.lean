...
theorem right_isOrdinal
    ...
    {sum left right : ℳ.Domain} (hSum : ℳ.IsOrdinalAddition 𝕀 sum left right) :
    ℳ.IsOrdinal right := by
  rcases hSum with ⟨sequence, hSequence, _⟩
  exact hSequence.1.1
...
theorem right_isOrdinal
    ...
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ}
    {product left right : ℳ.Domain} (hProduct :
      ℳ.IsOrdinalMultiplication 𝕀 product left right) :
    ℳ.IsOrdinal right := by
  rcases hProduct with ⟨sequence, hSequence, _⟩
  exact hSequence.1.1
...
theorem exponent_isOrdinal
    ...
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ}
    {power base exponent : ℳ.Domain} (hPower :
      ℳ.IsOrdinalExponentiation 𝕀 power base exponent) :
    ℳ.IsOrdinal exponent := by
  rcases hPower with ⟨sequence, hSequence, _⟩
  exact hSequence.1.1
...
private def ordinalAdditionMembership (𝒞 : OrderedPairConvention) : UnarySchema 1 where
...
private theorem satisfies_ordinalAdditionMembership_iff
    {ℳ : Structure.{u}}
    {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 1) (right : ℳ.Domain) :
    satisfies (env.push right) (UnarySchema.ordinalAdditionMembership 𝒞).body ↔
      ∀ sum,
...
private theorem satisfies_ordinalMultiplicationMembership_iff
    {ℳ : Structure.{u}}
    {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 1) (right : ℳ.Domain) :
    satisfies (env.push right) (UnarySchema.ordinalMultiplicationMembership 𝒞).body ↔
      ∀ product,
...
private theorem satisfies_ordinalAdditionUpperBound_iff
    {ℳ : Structure.{u}}
    {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 2) (candidate : ℳ.Domain) :
    satisfies (env.push candidate) (UnarySchema.ordinalAdditionUpperBound 𝒞).body ↔
      ∃ sum,
        ℳ.IsOrdinalAddition 𝕀
            sum (env.bound 0) candidate ∧ (env.bound 1 = sum ∨ ℳ.mem (env.bound 1) sum) := by
...
private theorem satisfies_ordinalMultiplicationUpperBlock_iff
    {ℳ : Structure.{u}}
    {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 2) (candidate : ℳ.Domain) :
    satisfies (env.push candidate) (UnarySchema.ordinalMultiplicationUpperBlock 𝒞).body ↔
      ∃ successor,
...
theorem ordinalAddition_mem_iff
    ...
    {sum left right value : ℳ.Domain} (hRight : ℳ.IsOrdinal right) (hSum : ℳ.IsOrdinalAddition 𝕀 sum left right) :
    ℳ.mem value sum ↔
      ℳ.mem value left ∨
        ∃ index, ℳ.mem index right ∧
  ...
  have hProperty : property right := by
    ...
    · rcases separation_exists_d hZF (Definitional.Project.UnarySchema.ordinalAdditionMembership 𝒞).neg
          env right with
    ...
    · intro current hCurrent hPrevious currentSum hCurrentSum member
      ...
      · have hCurrentSumEq := (ordinalAddition_zero_iff
      ...
      · rcases hSuccessor with
        ...
        have hPredecessorMem : ℳ.mem predecessor current := (hSuccessor predecessor).mpr (Or.inr fun _ => Iff.rfl)
        ...
        · intro hMember
          rcases hMember with
            hMemberLeft | ⟨index, hIndex, hIndexValue⟩
          · exact (hCurrentSuccessor member).mpr <| Or.inl <| (hPreviousMembership member).mpr (Or.inl hMemberLeft)
          · rcases (hSuccessor index).mp hIndex with
              hIndexPredecessor | hSame
            · exact (hCurrentSuccessor member).mpr <| Or.inl <| (hPreviousMembership member).mpr <| Or.inr
            ...
            · have hIndexEq :=
              ...
              rcases ordinalAddition_existsUnique hZF
                ...
                ⟨_, _, hUnique⟩
              have hMemberEq : member = previousSum := (hUnique member hIndexValue).trans (hUnique previousSum hPreviousSum).symm
              subst member
              exact (hCurrentSuccessor previousSum).mpr (Or.inr fun _ => Iff.rfl)
      · rcases (ordinalAddition_limit_iff
        ...
        · intro hMember
          ...
          · rcases hLimit.2.1 with ⟨index, hIndex⟩
            ...
            have hMemberRange :
                ℳ.mem member rangeValue := (hPrevious index hIndex
                rangeValue hRangeValue member).mpr (Or.inl hMemberLeft)
            exact (hUnion member).mpr
              ⟨rangeValue, (hRange rangeValue).mpr
          ...
          · rcases hLimit.2.2 index hIndex with
            ...
            rcases ordinalAddition_existsUnique hZF
              ...
              ⟨rangeValue, hRangeValue, _⟩
            have hMemberRange :
                ℳ.mem member rangeValue := (hPrevious larger hLarger
...
theorem ordinalAddition_left_eq_or_mem
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {sum left right : ℳ.Domain} (hRight : ℳ.IsOrdinal right) (hSum : ℳ.IsOrdinalAddition 𝕀 sum left right) :
    left = sum ∨ ℳ.mem left sum := by
  by_cases hRightEmpty : ∀ value, ¬ ℳ.mem value right
  · exact Or.inl <| ((ordinalAddition_zero_iff
  ...
  · have hRightNonempty : ∃ value, ℳ.mem value right := by
    ...
    have hZeroRight : ℳ.mem zero right :=
      hRight.empty_mem_of_nonempty (ZF.modelsKP hZF) hRightNonempty hZero
    have hLeftPlusZero :
        ℳ.IsOrdinalAddition 𝕀 left left zero := (ordinalAddition_zero_iff
        hZF 𝕀 hZero).mpr rfl
...
theorem ordinalAddition_right_injective
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {sum left first second : ℳ.Domain} (hLeft : ℳ.IsOrdinal left) (hFirst : ℳ.IsOrdinal first) (hSecond : ℳ.IsOrdinal second) (hFirstSum :
      ℳ.IsOrdinalAddition 𝕀 sum left first) (hSecondSum :
      ℳ.IsOrdinalAddition 𝕀 sum left second) :
    first = second := by
  rcases Structure.IsOrdinal.trichotomy hZF.1
      hFirst hSecond (KP.difference_exists_d (ZF.modelsKP hZF)) (KP.intersection_exists_d (ZF.modelsKP hZF)
        first second) with
  ...
  · have hSelf :=
    ...
    exact False.elim <| (ordinalAddition_isOrdinal hZF 𝕀
...
theorem ordinalAddition_left_member_mem_value
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {left index value member : ℳ.Domain} (hLeft : ℳ.IsOrdinal left) (hIndex : ℳ.IsOrdinal index) (hMember : ℳ.mem member left) (hValue :
...
theorem ordinalAddition_values_mem_iff
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {left firstIndex secondIndex firstValue secondValue : ℳ.Domain} (hLeft : ℳ.IsOrdinal left) (hFirstIndex : ℳ.IsOrdinal firstIndex)
    (hSecondIndex : ℳ.IsOrdinal secondIndex) (hFirstValue :
      ℳ.IsOrdinalAddition 𝕀 firstValue left firstIndex) (hSecondValue :
  ...
  · intro hFirstSecond
    rcases Structure.IsOrdinal.trichotomy hZF.1
        hFirstIndex hSecondIndex (KP.difference_exists_d (ZF.modelsKP hZF)) (KP.intersection_exists_d (ZF.modelsKP hZF)
    ...
    · have hIndexEq :=
      ...
      rcases ordinalAddition_existsUnique hZF
        ...
        ⟨_, _, hUnique⟩
      have hValueEq : firstValue = secondValue := (hUnique firstValue hFirstValue).trans (hUnique secondValue hSecondValue).symm
      subst secondValue
      exact False.elim <| (ordinalAddition_isOrdinal hZF 𝕀
    ...
    · have hSecondFirst :=
      ...
      have hSelf := (ordinalAddition_isOrdinal hZF 𝕀
...
theorem ordinalExponentiation_exponent_injective
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {power base one first second : ℳ.Domain} (hBase : ℳ.IsOrdinal base) (hOne : ℳ.IsOrdinalOne one) (hOneBase : ℳ.mem one base)
    (hFirst : ℳ.IsOrdinal first) (hSecond : ℳ.IsOrdinal second) (hFirstPower :
      ℳ.IsOrdinalExponentiation 𝕀 power base first) (hSecondPower :
  ...
  · have hSelf :=
    ...
    exact False.elim <| (ordinalExponentiation_isOrdinal hZF 𝕀
...
theorem ordinalExponentiation_values_mem_iff
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {base one first second firstPower secondPower : ℳ.Domain} (hBase : ℳ.IsOrdinal base) (hOne : ℳ.IsOrdinalOne one) (hOneBase : ℳ.mem one base)
    (hFirst : ℳ.IsOrdinal first) (hSecond : ℳ.IsOrdinal second) (hFirstPower :
      ℳ.IsOrdinalExponentiation 𝕀 firstPower base first) (hSecondPower :
  ...
  · intro hPowerOrder
    ...
    · have hIndexEq :=
      ...
      rcases ordinalExponentiation_existsUnique hZF
          𝕀 hBase hFirst with
        ⟨_, _, hUnique⟩
      have hPowerEq : firstPower = secondPower := (hUnique firstPower hFirstPower).trans (hUnique secondPower hSecondPower).symm
      subst secondPower
      exact False.elim <| (ordinalExponentiation_isOrdinal hZF 𝕀
          hBase hFirst hFirstPower).wellOrder.linear.irrefl
    ...
    · have hReversePower :=
      ...
      have hSelf := (ordinalExponentiation_isOrdinal hZF 𝕀
...
theorem ordinalAddition_existsUnique_rightRemainder
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {left right : ℳ.Domain} (hLeft : ℳ.IsOrdinal left) (hRight : ℳ.IsOrdinal right) (hLeftRight : ℳ.mem left right) :
    ∃ remainder,
  ...
  rcases separation_exists_d hZF (Definitional.Project.UnarySchema.ordinalAdditionUpperBound 𝒞)
      ...
      ∀ candidate,
        ℳ.mem candidate candidates ↔
          ℳ.mem candidate rightSuccessor ∧
            ∃ sum,
              ℳ.IsOrdinalAddition 𝕀 sum left candidate ∧ (right = sum ∨ ℳ.mem right sum) := by
    intro candidate
    rw [hCandidates candidate]
    constructor
    · rintro ⟨hBound, hCandidate⟩
      exact ⟨hBound, (Definitional.Project.Formula.satisfies_ordinalAdditionUpperBound_iff
  ...
  have hCandidatesNonempty :
    ...
    have hRightLeSum :=
      ordinalAddition_right_eq_or_mem hZF
        𝕀 hLeft hRight hSum
    exact
      ⟨right, (hCandidatesSemantic right).mpr
          ⟨(hRightSuccessor right).mpr (Or.inr fun _ => Iff.rfl),
  ...
  have hRemainderData := (hCandidatesSemantic remainder).mp hRemainderCandidate
  ...
  · subst sum
    ...
    exact ordinalAddition_right_injective
      hZF 𝕀 hLeft (Structure.IsOrdinalAddition.right_isOrdinal hOther) (Structure.IsOrdinalAddition.right_isOrdinal hSum)
      hOther hSum
  · rcases (ordinalAddition_mem_iff
        hZF 𝕀 (Structure.IsOrdinalAddition.right_isOrdinal hSum)
    ...
    · have hEarlierCandidate : ℳ.mem earlier candidates := (hCandidatesSemantic earlier).mpr
      ...
      · have hSelf :=
          hRightSuccessorOrdinal.wellOrder.linear.trans
            ...
            earlier ((hCandidatesSemantic earlier).mp
                hEarlierCandidate).1
            remainder hRemainderData.1
...
theorem ordinalMultiplication_mem_iff
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {product left right value : ℳ.Domain} (hLeft : ℳ.IsOrdinal left) (hRight : ℳ.IsOrdinal right) (hProduct :
      ℳ.IsOrdinalMultiplication 𝕀 product left right) :
    ℳ.mem value product ↔
      ∃ index, ℳ.mem index right ∧
        ∃ block,
  ...
  have hProperty : property right := by
    apply hRight.induction property
    · rcases separation_exists_d hZF (Definitional.Project.UnarySchema.ordinalMultiplicationMembership 𝒞).neg
          env right with
        ⟨counterexamples, hCounterexamples⟩
      refine ⟨counterexamples, fun current => ?_⟩
      rw [hCounterexamples current]
    ...
    · intro current hCurrent hPrevious currentProduct
      ...
      rcases hCurrent.classify hZF.1 with
        hEmpty | hSuccessor | hLimit
      · have hCurrentProductEmpty := (ordinalMultiplication_zero_iff
      ...
      · rcases hSuccessor with
        ...
        have hCurrentMembership :
          ...
          ordinalAddition_mem_iff hZF 𝕀 (value := member) hLeft hCurrentAddition
        constructor
        · intro hMember
        ...
        · rintro ⟨index, hIndex, block, hBlock,
          ...
          · have hIndexEq :=
            ...
            rcases ordinalMultiplication_existsUnique hZF
                𝕀 left hPredecessorOrdinal with
              ⟨_, _, hUnique⟩
            have hBlockEq : block = previousProduct := (hUnique block hBlock).trans (hUnique previousProduct hPreviousProduct).symm
            subst block
      ...
      · rcases (ordinalMultiplication_limit_iff
        ...
        · rintro ⟨index, hIndex, block, hBlock,
          ...
          rcases ordinalMultiplication_existsUnique hZF
              𝕀 left (hLimit.1.mem hLarger) with
            ⟨rangeValue, hRangeValue, _⟩
          have hMemberRange : ℳ.mem member rangeValue := (hPrevious larger hLarger
...
theorem eqOrMem_trans
    {ℳ : Structure.{u}} {left middle right : ℳ.Domain} (hRight : ℳ.IsOrdinal right) (hLeftMiddle : left = middle ∨ ℳ.mem left middle)
    (hMiddleRight : middle = right ∨ ℳ.mem middle right) :
    left = right ∨ ℳ.mem left right := by
  rcases hLeftMiddle with hLeftMiddle | hLeftMiddle
  · rcases hMiddleRight with hMiddleRight | hMiddleRight
    · exact Or.inl (hLeftMiddle.trans hMiddleRight)
    · exact Or.inr (hLeftMiddle ▸ hMiddleRight)
  · rcases hMiddleRight with hMiddleRight | hMiddleRight
    · exact Or.inr (hMiddleRight ▸ hLeftMiddle)
    · exact Or.inr <| hRight.transitive middle hMiddleRight left hLeftMiddle
...
theorem ordinalMultiplication_block_lt_of_index_mem
    ...
    {left smallerIndex largerIndex smallerBlock largerBlock
      ...
      smallerValue largerValue : ℳ.Domain} (hLeft : ℳ.IsOrdinal left) (hLeftNonempty : ∃ member, ℳ.mem member left) (hSmallerIndex : ℳ.IsOrdinal smallerIndex)
    (hLargerIndex : ℳ.IsOrdinal largerIndex) (hIndexOrder : ℳ.mem smallerIndex largerIndex) (hSmallerBlock :
      ℳ.IsOrdinalMultiplication 𝕀
        smallerBlock left smallerIndex) (hLargerBlock :
      ℳ.IsOrdinalMultiplication 𝕀
        largerBlock left largerIndex) (hSmallerRemainder : ℳ.mem smallerRemainder left) (hLargerRemainder : ℳ.mem largerRemainder left) (hSmallerValue :
      ℳ.IsOrdinalAddition 𝕀
        smallerValue smallerBlock smallerRemainder) (hLargerValue :
      ℳ.IsOrdinalAddition 𝕀
  ...
  have hSmallerIndexSuccessor :
      ℳ.mem smallerIndex smallerSuccessor := (hSmallerSuccessor smallerIndex).mpr (Or.inr fun _ => Iff.rfl)
  have hSmallerValueSuccessorProduct :
      ℳ.mem smallerValue successorProduct := (ordinalMultiplication_mem_iff hZF 𝕀
  ...
  have hSuccessorLeLarger :
    ...
    · exact Or.inr <| Structure.IsOrdinal.mem_of_properSubset
        ...
        ⟨hSuccessorSubset, hSame⟩ (KP.difference_exists_d (ZF.modelsKP hZF)
          smallerSuccessor largerIndex)
  ...
  have hSuccessorProductLeBlock :
    ...
    · subst largerIndex
      rcases ordinalMultiplication_existsUnique hZF
          𝕀 left hSmallerSuccessorOrdinal with
        ⟨_, _, hUnique⟩
      exact Or.inl <| (hUnique successorProduct hSuccessorProduct).trans (hUnique largerBlock hLargerBlock).symm
    · exact Or.inr <|
...
theorem ordinalMultiplication_block_values_mem_iff
    ...
    {left firstIndex secondIndex firstBlock secondBlock
      firstRemainder secondRemainder firstValue secondValue : ℳ.Domain} (hLeft : ℳ.IsOrdinal left) (hLeftNonempty : ∃ member, ℳ.mem member left)
    (hFirstIndex : ℳ.IsOrdinal firstIndex) (hSecondIndex : ℳ.IsOrdinal secondIndex) (hFirstBlock :
      ℳ.IsOrdinalMultiplication 𝕀
        firstBlock left firstIndex) (hSecondBlock :
      ℳ.IsOrdinalMultiplication 𝕀
        secondBlock left secondIndex) (hFirstRemainder : ℳ.mem firstRemainder left) (hSecondRemainder : ℳ.mem secondRemainder left) (hFirstValue :
      ℳ.IsOrdinalAddition 𝕀
        firstValue firstBlock firstRemainder) (hSecondValue :
      ℳ.IsOrdinalAddition 𝕀
        secondValue secondBlock secondRemainder) :
    ℳ.mem firstValue secondValue ↔
      ℳ.mem firstIndex secondIndex ∨ (firstIndex = secondIndex ∧
  ...
  · intro hValueOrder
    ...
    · have hIndexEq :=
      ...
      rcases ordinalMultiplication_existsUnique hZF
          𝕀 left hFirstIndex with
        ⟨_, _, hUnique⟩
      have hBlockEq : firstBlock = secondBlock := (hUnique firstBlock hFirstBlock).trans (hUnique secondBlock hSecondBlock).symm
      subst secondBlock
      exact Or.inr ⟨rfl, (ordinalAddition_values_mem_iff
          hZF 𝕀 (ordinalMultiplication_isOrdinal hZF 𝕀
            hLeft hFirstIndex hFirstBlock) (hLeft.mem hFirstRemainder) (hLeft.mem hSecondRemainder)
    ...
    · have hReverseValue :=
      ...
      have hSecondValueOrdinal :=
        ordinalAddition_isOrdinal hZF 𝕀 (ordinalMultiplication_isOrdinal hZF 𝕀
            hLeft hSecondIndex hSecondBlock) (hLeft.mem hSecondRemainder) hSecondValue
      have hSelf :=
...
theorem ordinalMultiplication_block_coordinates_unique
    ...
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {left firstIndex secondIndex firstBlock secondBlock
      firstRemainder secondRemainder value : ℳ.Domain} (hLeft : ℳ.IsOrdinal left) (hLeftNonempty : ∃ member, ℳ.mem member left)
    (hFirstIndex : ℳ.IsOrdinal firstIndex) (hSecondIndex : ℳ.IsOrdinal secondIndex) (hFirstBlock :
      ℳ.IsOrdinalMultiplication 𝕀
        firstBlock left firstIndex) (hSecondBlock :
      ℳ.IsOrdinalMultiplication 𝕀
        secondBlock left secondIndex) (hFirstRemainder : ℳ.mem firstRemainder left) (hSecondRemainder : ℳ.mem secondRemainder left) (hFirstValue :
      ℳ.IsOrdinalAddition 𝕀
        value firstBlock firstRemainder) (hSecondValue :
      ℳ.IsOrdinalAddition 𝕀
  ...
  · have hSelf :=
      ordinalMultiplication_block_lt_of_index_mem
        ...
        hFirstRemainder hSecondRemainder
        hFirstValue hSecondValue
    exact False.elim <| (ordinalAddition_isOrdinal hZF 𝕀 (ordinalMultiplication_isOrdinal hZF 𝕀
          hLeft hFirstIndex hFirstBlock) (hLeft.mem hFirstRemainder) hFirstValue).wellOrder.linear.irrefl
        value hSelf hSelf
  · have hSelf :=
      ordinalMultiplication_block_lt_of_index_mem
        hZF 𝕀 hLeft hLeftNonempty
...
theorem ordinalDivision_existsUnique_pair
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {dividend divisor : ℳ.Domain} (hDividend : ℳ.IsOrdinal dividend) (hDivisor : ℳ.IsOrdinal divisor) (hDivisorNonempty : ∃ member, ℳ.mem member divisor) :
    ∃ quotient remainder,
  ...
  rcases separation_exists_d hZF (Definitional.Project.UnarySchema.ordinalMultiplicationUpperBlock 𝒞)
  ...
  have hCandidatesSemantic :
    ...
    constructor
    · rintro ⟨hBound, hCandidate⟩
      exact ⟨hBound, (Definitional.Project.Formula.satisfies_ordinalMultiplicationUpperBlock_iff
          𝕀 hZF.1 env candidate).mp hCandidate⟩
    · rintro ⟨hBound, hCandidate⟩
      exact ⟨hBound, (Definitional.Project.Formula.satisfies_ordinalMultiplicationUpperBlock_iff
          𝕀 hZF.1 env candidate).mpr hCandidate⟩
  ...
  have hCandidatesNonempty :
    ...
    have hDividendBlockSuccessor :
      ...
      ordinalMultiplication_isIncreasingOnOrdinals
        ...
        hDividend hDividendSuccessorOrdinal ((hDividendSuccessor dividend).mpr (Or.inr fun _ => Iff.rfl))
    ...
    exact
      ⟨dividend, (hCandidatesSemantic dividend).mpr
          ⟨(hDividendSuccessor dividend).mpr (Or.inr fun _ => Iff.rfl),
            dividendSuccessor, hDividendSuccessor,
  ...
  have hQuotientData := (hCandidatesSemantic quotient).mp hQuotientCandidate
  ...
  have hIndexEq : index = quotient := by
    ...
    · rcases KP.exists_successor (ZF.modelsKP hZF) index with
      ...
      have hDividendIndexSuccessor :
          ℳ.mem dividend indexSuccessorProduct := (ordinalMultiplication_mem_iff
          hZF 𝕀 hDivisor hIndexSuccessorOrdinal
          hIndexSuccessorProduct).mpr
          ⟨index, (hIndexSuccessorRelation index).mpr (Or.inr fun _ => Iff.rfl),
            block, hBlock, remainder, hRemainder,
            hDividendDecomposition⟩
      have hIndexCandidate : ℳ.mem index candidates := (hCandidatesSemantic index).mpr
      ...
      · have hSelf :=
          hDividendSuccessorOrdinal.wellOrder.linear.trans
            ...
            index ((hCandidatesSemantic index).mp hIndexCandidate).1
            quotient hQuotientData.1
