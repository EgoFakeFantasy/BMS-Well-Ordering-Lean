...
structure StrictOrderIsomorphism
    {Source : Type u} {Target : Type v} (sourceRelation : Source → Source → Prop) (targetRelation : Target → Target → Prop) where
  toFun : Source → Target
...
def OrdinalMembershipRelation (ℳ : Structure.{u})
    {α : ℳ.Domain} (first second : ℳ.OrdinalSegment α) : Prop :=
  ℳ.mem first.1 second.1
/-- 两个序数成员序的不交序和关系。 -/
def OrdinalSumRelation (ℳ : Structure.{u}) (left right : ℳ.Domain) (first second :
...
/-- 两个序数成员序的第二坐标优先右字典序。 -/
def OrdinalRightLexicographicRelation (ℳ : Structure.{u}) (left right : ℳ.Domain) (first second :
      ℳ.OrdinalSegment left × ℳ.OrdinalSegment right) : Prop :=
  ℳ.mem first.2.1 second.2.1 ∨ (first.2.1 = second.2.1 ∧
...
noncomputable def ordinalAddition_strictOrderIsomorphism
    ...
    StrictOrderIsomorphism (ℳ.OrdinalSumRelation left right) (ℳ.OrdinalMembershipRelation (α := sum)) := by
  classical
  let tailExistence :
          ...
          ℳ.IsOrdinalAddition 𝕀 value left index.1 ∧
            ∀ other,
              ℳ.IsOrdinalAddition 𝕀 other left index.1 →
                other = value :=
    fun index =>
      ordinalAddition_existsUnique hZF 𝕀 left (hRight.mem index.2)
  ...
  have hTailValue (index : ℳ.OrdinalSegment right) :
      ℳ.IsOrdinalAddition 𝕀 (tailValue index) left index.1 := (Classical.choose_spec (tailExistence index)).1
  let forward :
    ...
    fun point =>
      match point with
      | .inl member =>
          ⟨member.1, (ordinalAddition_mem_iff
              hZF 𝕀 hRight hSum).mpr (Or.inl member.2)⟩
      | .inr index =>
          ⟨tailValue index, (ordinalAddition_mem_iff
              hZF 𝕀 hRight hSum).mpr (Or.inr ⟨index.1, index.2, hTailValue index⟩)⟩
  have hForwardSurjective :
    ...
    · let sourceIndex : ℳ.OrdinalSegment right := ⟨index, hIndex⟩
      have hTargetEq :
          target.1 = tailValue sourceIndex := (Classical.choose_spec (tailExistence sourceIndex)).2
  ...
  have hForwardRelation :
    ...
    · change ℳ.mem first.1 second.1 ↔
      ...
      exact (ordinalAddition_values_mem_iff
        hZF 𝕀 hLeft (hRight.mem first.2) (hRight.mem second.2) (hTailValue first) (hTailValue second)).symm
...
noncomputable def ordinalMultiplication_strictOrderIsomorphism
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {product left right : ℳ.Domain} (hLeft : ℳ.IsOrdinal left) (hRight : ℳ.IsOrdinal right) (hProduct :
      ℳ.IsOrdinalMultiplication 𝕀 product left right) :
    StrictOrderIsomorphism (ℳ.OrdinalRightLexicographicRelation left right) (ℳ.OrdinalMembershipRelation (α := product)) := by
  classical
  ...
  · have hLeftNonempty : ∃ member, ℳ.mem member left := by
    ...
    have hBlockValue (index : ℳ.OrdinalSegment right) :
        ℳ.IsOrdinalMultiplication 𝕀 (blockValue index) left index.1 := (Classical.choose_spec (blockExistence index)).1
    let pointExistence :
            ...
            ℳ.IsOrdinalAddition 𝕀
              ...
              ∀ other,
                ℳ.IsOrdinalAddition 𝕀
                  ...
                  other = value :=
      fun point =>
        ordinalAddition_existsUnique hZF 𝕀 (blockValue point.2) (hLeft.mem point.1.2)
    let pointValue :
        ℳ.OrdinalSegment left × ℳ.OrdinalSegment right → ℳ.Domain :=
      fun point => Classical.choose (pointExistence point)
    have hPointValue (point : ℳ.OrdinalSegment left × ℳ.OrdinalSegment right) :
        ℳ.IsOrdinalAddition 𝕀 (pointValue point) (blockValue point.2) point.1.1 := (Classical.choose_spec (pointExistence point)).1
    let forward :
        ℳ.OrdinalSegment left × ℳ.OrdinalSegment right →
          ℳ.OrdinalSegment product :=
      fun point =>
        ⟨pointValue point, (ordinalMultiplication_mem_iff
    ...
    have hForwardSurjective :
      ...
      let source :
          ℳ.OrdinalSegment left × ℳ.OrdinalSegment right := (⟨remainder, hRemainder⟩, ⟨index, hIndex⟩)
      have hBlockEq :
          block = blockValue source.2 := (Classical.choose_spec (blockExistence source.2)).2
          block hBlock
      ...
      have hTargetEq :
          target.1 = pointValue source := (Classical.choose_spec (pointExistence source)).2
    ...
    have hForwardInjective :
      ...
      have hValueEq : pointValue first = pointValue second := by
        simpa [forward] using congrArg Subtype.val hEqual
      have hSecondAtFirst :
          ℳ.IsOrdinalAddition 𝕀 (pointValue first) (blockValue second.2) second.1.1 := by
        simpa [hValueEq] using hPointValue second
      have hCoordinates :=
        ordinalMultiplication_block_coordinates_unique
          hZF 𝕀 hLeft hLeftNonempty (hRight.mem first.2.2) (hRight.mem second.2.2) (hBlockValue first.2) (hBlockValue second.2)
          first.1.2 second.1.2 (hPointValue first) hSecondAtFirst
      apply Prod.ext
    ...
    have hForwardRelation :
        ∀ first second,
          ℳ.OrdinalRightLexicographicRelation left right first second ↔
            ℳ.OrdinalMembershipRelation (forward first) (forward second) := by
      intro first second
      change (ℳ.mem first.2.1 second.2.1 ∨ (first.2.1 = second.2.1 ∧
            ℳ.mem first.1.1 second.1.1)) ↔
          ℳ.mem (pointValue first) (pointValue second)
      exact (ordinalMultiplication_block_values_mem_iff
        hZF 𝕀 hLeft hLeftNonempty (hRight.mem first.2.2) (hRight.mem second.2.2) (hBlockValue first.2) (hBlockValue second.2)
        first.1.2 second.1.2 (hPointValue first) (hPointValue second)).symm
