...
-/
def isSuccessorLengthSequenceWithLast (𝒞 : OrderedPairConvention)
...
-/
def isLimitLengthSequenceWithUnion (𝒞 : OrderedPairConvention)
...
-/
def ordinalAdditionOperator (𝒞 : OrderedPairConvention) : BinarySchema 1 where
  body :=
...
-/
def ordinalMultiplicationOperator (𝒞 : OrderedPairConvention) : BinarySchema 1 where
  body :=
    .disj (.conj (.neg <| Formula.isOrdinal (.bound 2)) (Formula.isEmpty (.bound 0))) <|
...
def ordinalMultiplication (𝒞 : OrderedPairConvention) :
    BinarySchema 1 :=
  transfiniteRecursion 𝒞 (ordinalMultiplicationOperator 𝒞)
end BinarySchema
namespace Formula
...
-/
def ordinalExponentiationOperator (𝒞 : OrderedPairConvention) : BinarySchema 1 where
  body :=
    .disj (.conj (Formula.isZeroLengthSequence 𝒞 (.bound 1)) (Formula.isOrdinalOne (.bound 0))) <|
      .disj (.existsE <|
...
def ordinalExponentiation (𝒞 : OrderedPairConvention) :
    BinarySchema 1 :=
  transfiniteRecursion 𝒞 (ordinalExponentiationOperator 𝒞)
end BinarySchema
namespace Formula
...
def isOrdinalDivision (𝒞 : OrderedPairConvention)
  ...
  .conj (isOrdinal quotient) <| .conj (.mem remainder divisor) <|
    .existsE <| .conj (isOrdinalMultiplication 𝒞
        Term.newest divisor.weaken quotient.weaken) (isOrdinalAddition 𝒞
...
def isSetCodedLinearOrder (𝒞 : OrderedPairConvention)
  ...
  .conj (isRelation 𝒞 relation) (isLinearOrderRelation 𝒞 relation carrier)
/-- `sumRelation` 精确实现不相交线性序的序和关系。 -/
def isLinearOrderSumRelation (𝒞 : OrderedPairConvention)
    {depth : Nat} (sumRelation leftCarrier leftRelation rightCarrier rightRelation :
      Term depth) : Formula 1 depth :=
  .forallE <| .forallE <|
    .iff (orderedPairMem 𝒞 (.bound 1) Term.newest
        sumRelation.weaken.weaken) <|
      .disj (.conj (.mem (.bound 1) leftCarrier.weaken.weaken) <|
          .conj (.mem Term.newest leftCarrier.weaken.weaken) <|
            orderedPairMem 𝒞 (.bound 1) Term.newest
              leftRelation.weaken.weaken) <|
        .disj (.conj (.mem (.bound 1) rightCarrier.weaken.weaken) <|
            .conj (.mem Term.newest rightCarrier.weaken.weaken) <|
              orderedPairMem 𝒞 (.bound 1) Term.newest
                rightRelation.weaken.weaken) (.conj (.mem (.bound 1) leftCarrier.weaken.weaken) (.mem Term.newest rightCarrier.weaken.weaken))
/-- `sumCarrier, sumRelation` 是两个不相交线性序的序和。 -/
def isLinearOrderSum (𝒞 : OrderedPairConvention)
    {depth : Nat} (sumCarrier sumRelation leftCarrier leftRelation
      rightCarrier rightRelation : Term depth) : Formula 1 depth :=
  .conj (isSetCodedLinearOrder 𝒞 leftRelation leftCarrier) <|
  .conj (isSetCodedLinearOrder 𝒞 rightRelation rightCarrier) <|
  .conj (isDisjoint leftCarrier rightCarrier) <|
  .conj (isUnionOfTwo sumCarrier leftCarrier rightCarrier) <|
  .conj (isRelation 𝒞 sumRelation) (isLinearOrderSumRelation 𝒞 sumRelation
      leftCarrier leftRelation rightCarrier rightRelation)
/--
`productRelation` 精确实现第二坐标优先的右字典序关系。
-/
def isRightLexicographicProductRelation (𝒞 : OrderedPairConvention)
    {depth : Nat} (productRelation leftCarrier leftRelation
      rightCarrier rightRelation : Term depth) : Formula 1 depth :=
  .forallE <| .forallE <|
    .iff (orderedPairMem 𝒞 (.bound 1) Term.newest
        ...
        Formula.existsMem rightCarrier.weaken.weaken.weaken <|
          Formula.existsMem leftCarrier.weaken.weaken.weaken.weaken <|
            Formula.existsMem
              ...
              .conj (𝒞.code (.bound 4) (.bound 1) (.bound 0)) <|
                .disj (orderedPairMem 𝒞 (.bound 2) (.bound 0)
                    rightRelation.weaken.weaken.weaken.weaken.weaken.weaken) (.conj (extensionalEq (.bound 2) (.bound 0))
...
def isLinearOrderProduct (𝒞 : OrderedPairConvention)
    {depth : Nat} (productCarrier productRelation leftCarrier leftRelation
      rightCarrier rightRelation : Term depth) : Formula 1 depth :=
  .conj (isSetCodedLinearOrder 𝒞 leftRelation leftCarrier) <|
  .conj (isSetCodedLinearOrder 𝒞 rightRelation rightCarrier) <|
  .conj (isCartesianProduct 𝒞 productCarrier
      leftCarrier rightCarrier) <|
  .conj (isRelation 𝒞 productRelation) (isRightLexicographicProductRelation 𝒞 productRelation
      leftCarrier leftRelation rightCarrier rightRelation)
end Formula
end Project
end Definitional
