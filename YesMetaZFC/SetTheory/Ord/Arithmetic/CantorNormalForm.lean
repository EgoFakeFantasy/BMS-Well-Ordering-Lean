...
private def cantorNormalFormBelowUniqueness (𝒞 : OrderedPairConvention) : UnarySchema 1 where
  body := .forallE <| .forallE <| .forallE <| .forallE <|
    ...
    .forallE <| .imp (.conj (Formula.isCantorNormalFormBelow 𝒞 (.bound 10) (.bound 8) (.bound 9) (.bound 7) (.bound 6) (.bound 5) (.bound 4))
        (Formula.isCantorNormalFormBelow 𝒞 (.bound 10) (.bound 8) (.bound 9) (.bound 3) (.bound 2) (.bound 1) Term.newest)) <|
...
private theorem satisfies_cantorNormalFormExistence_iff
    {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 1) (α : ℳ.Domain) :
    satisfies (env.push α) (UnarySchema.cantorNormalFormExistence 𝒞).body ↔
      ∃ length exponents coefficients values,
        ℳ.IsCantorNormalForm 𝕀 (env.bound 0) α length
...
private theorem satisfies_ordinalExponentiationStrictUpperBound_iff
    {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 2) (candidate : ℳ.Domain) :
    satisfies (env.push candidate) (UnarySchema.ordinalExponentiationStrictUpperBound
          𝒞).body ↔
      ∃ power,
        ℳ.IsOrdinalExponentiation 𝕀
...
private theorem satisfies_cantorNormalFormBelowUniqueness_iff
    {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 1) (α : ℳ.Domain) :
    satisfies (env.push α) (UnarySchema.cantorNormalFormBelowUniqueness
          𝒞).body ↔
      ∀ bound
          firstLength firstExponents firstCoefficients firstValues
          secondLength secondExponents secondCoefficients secondValues, (ℳ.IsCantorNormalFormBelow 𝕀 (env.bound 0) bound α firstLength
            firstExponents firstCoefficients firstValues ∧
          ℳ.IsCantorNormalFormBelow 𝕀 (env.bound 0) bound α secondLength
...
theorem exists_cantorNormalForm_of_empty
    ...
    {ω zero : ℳ.Domain} (hω : ℳ.IsOmega ω) (hZero : ∀ value, ¬ ℳ.mem value zero) :
    ∃ length exponents coefficients values,
  ...
  rcases Structure.IsSequenceOfLength.exists_append (ZF.modelsKP hZF) 𝕀
  ...
  have hValuesOrdinal :
    ...
    rcases (hValuesPairs index value).mp hValue with
      hOld | ⟨_, hValueZero⟩
    · have hIndexZero := (hZeroSequence.2.2 index).mpr ⟨value, hOld⟩
      exact False.elim (hZero index hIndexZero)
    · subst value
      exact hZeroOrdinal
  have hZeroValue :
      ℳ.PairMember 𝕀 zero zero values := (hValuesPairs zero zero).mpr (Or.inr ⟨rfl, rfl⟩)
  exact
...
theorem ordinalExponentiation_exists_leadingExponent
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω α : ℳ.Domain} (hω : ℳ.IsOmega ω) (hα : ℳ.IsOrdinal α) (hαNonempty : ∃ value, ℳ.mem value α) :
    ∃ exponent successor power successorPower,
      ℳ.IsOrdinal exponent ∧
        ...
        ℳ.IsOrdinalExponentiation 𝕀
          power ω exponent ∧
        ℳ.IsOrdinalExponentiation 𝕀
          successorPower ω successor ∧ (power = α ∨ ℳ.mem power α) ∧
  ...
  rcases separation_exists_d hZF (Definitional.Project.UnarySchema.ordinalExponentiationStrictUpperBound 𝒞)
  ...
  have hCandidatesSemantic :
    ...
    · rintro ⟨hCandidateBound, hCandidate⟩
      exact ⟨hCandidateBound, (Definitional.Project.Formula.satisfies_ordinalExponentiationStrictUpperBound_iff
  ...
  have hCandidatesNonempty :
    ...
    have hαSuccessorBound :
        ℳ.mem ordinalSuccessor candidateBound := (hCandidateBound ordinalSuccessor).mpr (Or.inr fun _ => Iff.rfl)
    exact
      ⟨ordinalSuccessor, (hCandidatesSemantic ordinalSuccessor).mpr
          ⟨hαSuccessorBound,
  ...
  · have hLeastPowerOne := (ordinalExponentiation_zero_iff
  ...
  · rcases hLeastSuccessor with
    ...
    have hExponentLeast : ℳ.mem exponent least := (hLeastSuccessor exponent).mpr (Or.inr fun _ => Iff.rfl)
    rcases ordinalExponentiation_existsUnique hZF
    ...
    have hNotOrdinalPower : ¬ ℳ.mem α power := by
      ...
      have hExponentCandidate : ℳ.mem exponent candidates := (hCandidatesSemantic exponent).mpr
    ...
    have hPowerLeOrdinal :
      ...
      rcases Structure.IsOrdinal.trichotomy hZF.1
          hPowerOrdinal hα (KP.difference_exists_d (ZF.modelsKP hZF)) (KP.intersection_exists_d (ZF.modelsKP hZF)
            power α) with
  ...
  · rcases (ordinalExponentiation_limit_iff
    ...
    have hEarlierCandidate : ℳ.mem earlier candidates := (hCandidatesSemantic earlier).mpr
    ...
    · have hLeastEq :=
      ...
      subst least
      exact False.elim <| (hLeastLimit.1.mem hEarlierLeast).wellOrder.linear.irrefl
...
theorem ordinalExponentiation_leadingExponent_unique
    ...
    {ω α
      ...
      secondSuccessorPower : ℳ.Domain} (hω : ℳ.IsOmega ω) (hFirstExponent : ℳ.IsOrdinal firstExponent) (hFirstSuccessor :
      ℳ.SuccessorOf firstSuccessor firstExponent) (hFirstPower :
      ℳ.IsOrdinalExponentiation 𝕀
        firstPower ω firstExponent) (hFirstSuccessorPower :
      ℳ.IsOrdinalExponentiation 𝕀
        firstSuccessorPower ω firstSuccessor) (hFirstPowerLe :
      firstPower = α ∨ ℳ.mem firstPower α) (hαFirstSuccessorPower :
      ℳ.mem α firstSuccessorPower) (hSecondExponent : ℳ.IsOrdinal secondExponent) (hSecondSuccessor :
      ℳ.SuccessorOf secondSuccessor secondExponent) (hSecondPower :
      ℳ.IsOrdinalExponentiation 𝕀
        secondPower ω secondExponent) (hSecondSuccessorPower :
      ℳ.IsOrdinalExponentiation 𝕀
        secondSuccessorPower ω secondSuccessor) (hSecondPowerLe :
      secondPower = α ∨ ℳ.mem secondPower α) (hαSecondSuccessorPower :
  ...
  have hContradictionOfFirstSecond (hFirstSecond : ℳ.mem firstExponent secondExponent) :
      False := by
    have hFirstSuccessorLeSecond :
        firstSuccessor = secondExponent ∨
          ℳ.mem firstSuccessor secondExponent := by
      rcases Structure.IsOrdinal.trichotomy hZF.1
          hFirstSuccessorOrdinal hSecondExponent (KP.difference_exists_d (ZF.modelsKP hZF)) (KP.intersection_exists_d (ZF.modelsKP hZF)
    ...
    have hFirstSuccessorPowerLeSecondPower :
      ...
      · subst secondExponent
        rcases ordinalExponentiation_existsUnique hZF
          ...
          ⟨_, _, hUnique⟩
        exact Or.inl <| (hUnique firstSuccessorPower
              hFirstSuccessorPower).trans (hUnique secondPower hSecondPower).symm
      · exact Or.inr <|
  ...
  have hContradictionOfSecondFirst (hSecondFirst : ℳ.mem secondExponent firstExponent) :
      False := by
    have hSecondSuccessorLeFirst :
        secondSuccessor = firstExponent ∨
          ℳ.mem secondSuccessor firstExponent := by
      rcases Structure.IsOrdinal.trichotomy hZF.1
          hSecondSuccessorOrdinal hFirstExponent (KP.difference_exists_d (ZF.modelsKP hZF)) (KP.intersection_exists_d (ZF.modelsKP hZF)
    ...
    have hSecondSuccessorPowerLeFirstPower :
      ...
      · subst firstExponent
        rcases ordinalExponentiation_existsUnique hZF
          ...
          ⟨_, _, hUnique⟩
        exact Or.inl <| (hUnique secondSuccessorPower
              hSecondSuccessorPower).trans (hUnique firstPower hFirstPower).symm
      · exact Or.inr <|
          ordinalExponentiation_isIncreasingOnOrdinals
            hZF 𝕀 hωOrdinal
            hOne hOneOmega
  ...
  rcases Structure.IsOrdinal.trichotomy hZF.1
      hFirstExponent hSecondExponent (KP.difference_exists_d (ZF.modelsKP hZF)) (KP.intersection_exists_d (ZF.modelsKP hZF)
...
theorem ordinalDivision_omegaPower_quotient_mem_omega
    ...
    {ω α exponent successor power successorPower
      quotient remainder : ℳ.Domain} (hω : ℳ.IsOmega ω) (hExponent : ℳ.IsOrdinal exponent) (hSuccessor : ℳ.SuccessorOf successor exponent) (hPower :
      ℳ.IsOrdinalExponentiation 𝕀 power ω exponent) (hSuccessorPower :
      ℳ.IsOrdinalExponentiation 𝕀
        successorPower ω successor) (hPowerLeOrdinal : power = α ∨ ℳ.mem power α) (hαSuccessorPower : ℳ.mem α successorPower) (hDivision :
  ...
  rcases ordinalExponentiation_existsUnique hZF
    ...
    ⟨_, _, hUniquePower⟩
  have hPreviousPowerEq : previousPower = power := (hUniquePower previousPower hPreviousPower).trans (hUniquePower power hPower).symm
  subst previousPower
  have hQuotientOmega : ℳ.mem quotient ω := by
    rcases Structure.IsOrdinal.trichotomy hZF.1
        hQuotientOrdinal hωOrdinal (KP.difference_exists_d (ZF.modelsKP hZF)) (KP.intersection_exists_d (ZF.modelsKP hZF)
          quotient ω) with
    ...
    · have hQuotientEq :=
      ...
      rcases ordinalMultiplication_existsUnique hZF
        ...
        ⟨_, _, hUniqueProduct⟩
      have hProductEq : product = successorPower := (hUniqueProduct product hProduct).trans (hUniqueProduct successorPower hSuccessorProduct).symm
      exact False.elim <|
        hαNotMemProduct <| by
  ...
  have hQuotientNonempty :
    ...
    have hProductEmpty := (ordinalMultiplication_zero_iff
...
theorem cantorNormalFormBelow_length_nonempty
    ...
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ}
    {ω bound α length exponents coefficients values : ℳ.Domain} (hαNonempty : ∃ member, ℳ.mem member α) (hForm :
      ℳ.IsCantorNormalFormBelow 𝕀
        ω bound α length exponents coefficients values) :
...
theorem cantorNormalForm_length_nonempty
    ...
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ}
    {ω α length exponents coefficients values : ℳ.Domain} (hαNonempty : ∃ member, ℳ.mem member α) (hForm :
      ℳ.IsCantorNormalForm 𝕀
        ω α length exponents coefficients values) :
    ∃ index, ℳ.mem index length :=
...
theorem cantorNormalFormBelow_exists_leadingTerm_of_length_nonempty
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω bound α length exponents coefficients values : ℳ.Domain} (hω : ℳ.IsOmega ω) (hLengthNonempty : ∃ index, ℳ.mem index length) (hForm :
      ℳ.IsCantorNormalFormBelow 𝕀
        ω bound α length exponents coefficients values) :
    ∃ previousLength exponent coefficient previous
      ...
      ℳ.mem previousLength ω ∧
        ...
        ℳ.IsOrdinal exponent ∧
        ℳ.mem coefficient ω ∧ (∃ member, ℳ.mem member coefficient) ∧
        ℳ.IsOrdinalExponentiation 𝕀
          power ω exponent ∧
        ℳ.IsOrdinalDivision 𝕀
  ...
  have hPreviousLengthLength :
      ℳ.mem previousLength length := (hLengthSuccessor previousLength).mpr (Or.inr fun _ => Iff.rfl)
  ...
  have hLengthValueLength :
      ℳ.mem length valueLength := (hValueLengthSuccessor length).mpr (Or.inr fun _ => Iff.rfl)
  ...
  have hPowerOrdinal :=
    ordinalExponentiation_isOrdinal hZF 𝕀
      hωOrdinal hExponentOrdinal hPower
  have hαSuccessorPower :
      ℳ.mem α successorPower := (ordinalMultiplication_mem_iff
      hZF 𝕀 hPowerOrdinal
...
theorem cantorNormalFormBelow_exists_leadingTerm
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω bound α length exponents coefficients values : ℳ.Domain} (hω : ℳ.IsOmega ω) (hαNonempty : ∃ member, ℳ.mem member α) (hForm :
      ℳ.IsCantorNormalFormBelow 𝕀
        ω bound α length exponents coefficients values) :
    ∃ previousLength exponent coefficient previous
        power successor successorPower,
  ...
  cantorNormalFormBelow_exists_leadingTerm_of_length_nonempty
    hZF 𝕀 hω (cantorNormalFormBelow_length_nonempty
        hZF hαNonempty hForm)
      hForm
...
theorem cantorNormalForm_exists_leadingTerm
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω α length exponents coefficients values : ℳ.Domain} (hω : ℳ.IsOmega ω) (hαNonempty : ∃ member, ℳ.mem member α) (hForm :
      ℳ.IsCantorNormalForm 𝕀
        ω α length exponents coefficients values) :
    ∃ previousLength exponent coefficient previous
        power successor successorPower,
...
theorem cantorNormalFormBelow_exists_prefix
  ...
  have hPreviousExponentsOrdinalValued :
    ...
    exact hExponents.1.2 index hIndexLength value ((hPreviousExponentsRestriction.2 index value).mp hValue).2
  have hPreviousExponents :
    ...
    exact hExponents.2 left hLeftLength right hRightLength
      hLeftRight leftValue rightValue ((hPreviousExponentsRestriction.2
        left leftValue).mp hLeftValue).2 ((hPreviousExponentsRestriction.2
  ...
  have hPreviousCoefficients :
    ...
    exact
      ⟨value, hValueOmega, (hPreviousCoefficientsRestriction.2 index value).mpr
          ⟨hIndex, hValue⟩⟩
  ...
  have hPreviousValues :
    ...
    exact hValues.2 index hIndexValueLength value ((hPreviousValuesRestriction.2 index value).mp hValue).2
  have hPreviousInitial :
    ...
    have hZeroLength :
        ℳ.mem zero length :=
      hLengthOrdinal.empty_mem_of_nonempty (ZF.modelsKP hZF)
        ⟨previousLength, hPreviousLengthLength⟩ hZero
    exact
      ⟨zero, hZero, (hPreviousValuesRestriction.2 zero zero).mpr
          ⟨hZeroLength, hZeroValue⟩⟩
  have hPreviousFinal :
      ℳ.PairMember 𝕀
        previousLength previous previousValues := (hPreviousValuesRestriction.2
  ...
  have hPreviousSteps :
    ...
    have hOldExponent' := (hPreviousExponentsRestriction.2
        index oldExponent).mp hOldExponent |>.2
    have hOldCoefficient' := (hPreviousCoefficientsRestriction.2
        index oldCoefficient).mp hOldCoefficient |>.2
    have hOldPrevious' := (hPreviousValuesRestriction.2
    ...
    have hNextLePreviousLength :
      ...
      rcases Structure.IsOrdinal.trichotomy hZF.1
          hNextOrdinal hPreviousLengthOrdinal (KP.difference_exists_d (ZF.modelsKP hZF)) (KP.intersection_exists_d (ZF.modelsKP hZF)
        ...
        · have hPreviousEq :=
          ...
          exact False.elim <|
            hIndexOrdinal.wellOrder.linear.irrefl
              index hIndex hIndex
    have hNextLength : ℳ.mem next length := (hLengthSuccessor next).mpr <| by
    ...
    exact
      ⟨hOldCoefficientNonempty,
        ...
        hNext, hOldPower, hOldPreviousPower,
        hOldPowerBound, hMonomial, hOutput,
        hOldPowerOutput, (hPreviousValuesRestriction.2 next output).mpr
...
theorem cantorNormalFormBelow_length_empty_of_ordinal_empty
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω bound α length exponents coefficients values : ℳ.Domain} (hω : ℳ.IsOmega ω) (hαEmpty : ∀ member, ¬ ℳ.mem member α) (hForm :
      ℳ.IsCantorNormalFormBelow 𝕀
        ω bound α length exponents coefficients values) :
    ∀ index, ¬ ℳ.mem index length := by
  intro index hIndex
...
theorem cantorNormalFormBelow_unique_of_ordinal_empty
    ...
    {ω bound α
      ...
      secondLength secondExponents secondCoefficients secondValues :
        ℳ.Domain} (hω : ℳ.IsOmega ω) (hαEmpty : ∀ member, ¬ ℳ.mem member α) (hFirst :
      ℳ.IsCantorNormalFormBelow 𝕀
        ω bound α firstLength
        firstExponents firstCoefficients firstValues) (hSecond :
      ℳ.IsCantorNormalFormBelow 𝕀
  ...
  have hLengthEq : firstLength = secondLength := by
    apply hZF.1.eq_of_same_members
    intro member
    exact iff_of_false (hFirstLengthEmpty member) (hSecondLengthEmpty member)
  subst secondLength
  have hExponentsEq : firstExponents = secondExponents := by
    ...
    constructor
    · intro hValue
      have hInput := (hFirstExponents.1.1.2.2 input).mpr
          ⟨output, hValue⟩
      exact False.elim (hFirstLengthEmpty input hInput)
    · intro hValue
      have hInput := (hSecondExponents.1.1.2.2 input).mpr
          ⟨output, hValue⟩
  ...
  have hCoefficientsEq :
    ...
    constructor
    · intro hValue
      have hInput := (hFirstCoefficients.2.2.1 input).mpr
          ⟨output, hValue⟩
      exact False.elim (hFirstLengthEmpty input hInput)
    · intro hValue
      have hInput := (hSecondCoefficients.2.2.1 input).mpr
          ⟨output, hValue⟩
      exact False.elim (hSecondLengthEmpty input hInput)
  have hValuesEq : firstValues = secondValues := by
    ...
    intro input output
    have transfer
        {left right leftValueLength : ℳ.Domain} (hLeftValues :
          ℳ.IsOrdinalValuedSequence 𝕀
            left leftValueLength) (hLeftValueLengthSuccessor :
          ℳ.SuccessorOf leftValueLength firstLength) (hLeftFinal :
          ℳ.PairMember 𝕀
            firstLength α left) (hRightFinal :
          ℳ.PairMember 𝕀
            firstLength α right) (hValue :
          ℳ.PairMember 𝕀 input output left) :
        ℳ.PairMember 𝕀 input output right := by
      have hInputValueLength := (hLeftValues.1.2.2 input).mpr
          ⟨output, hValue⟩
      rcases (hLeftValueLengthSuccessor input).mp
          hInputValueLength with
        hInputLength | hInputSame
      · exact False.elim (hFirstLengthEmpty input hInputLength)
      · have hInputEq :=
          hZF.1.eq_of_same_members
...
theorem exists_cantorNormalForm_of_remainder
    ...
    {ω α remainder length exponents coefficients values
      exponent successor power successorPower quotient : ℳ.Domain} (hω : ℳ.IsOmega ω) (hRemainderForm :
      ℳ.IsCantorNormalForm 𝕀
        ω remainder length exponents coefficients values) (hExponent : ℳ.IsOrdinal exponent) (hSuccessor : ℳ.SuccessorOf successor exponent) (hPower :
      ℳ.IsOrdinalExponentiation 𝕀 power ω exponent) (hSuccessorPower :
      ℳ.IsOrdinalExponentiation 𝕀
        successorPower ω successor) (hPowerLeOrdinal : power = α ∨ ℳ.mem power α) (hαSuccessorPower : ℳ.mem α successorPower) (hDivision :
      ℳ.IsOrdinalDivision 𝕀
  ...
  have hOldExponentMem :
    ...
    have hIndexNewLength :
        ℳ.mem index newLength := (hNewLengthSuccessor index).mpr (Or.inl hIndex)
    ...
    exact (ordinalExponentiation_values_mem_iff
        hZF 𝕀 hωOrdinal hOne hOneOmega (hExponents.1.2 index hIndex oldExponent hOldExponent)
  ...
  have hNewExponentsOrdinalValued :
    ...
    rcases (hNewExponentsPairs index value).mp hValue with
      hOld | ⟨_, hValueEq⟩
    · have hIndexLength := (hExponents.1.1.2.2 index).mpr ⟨value, hOld⟩
  ...
  have hNewExponents :
    ...
    · have hLeftLength := (hExponents.1.1.2.2 left).mpr
          ⟨leftValue, hLeftOld⟩
      have hRightLength := (hExponents.1.1.2.2 right).mpr
  ...
  have hNewCoefficients :
    ...
    rcases (hNewCoefficientsPairs index value).mp hValue with
      hOld | ⟨_, hValueEq⟩
    · have hIndexLength := (coefficientSequence.2.2 index).mpr ⟨value, hOld⟩
      rcases hCoefficients.2.2.2 index hIndexLength with
  ...
  have hNewValuesOrdinal :
    ...
    rcases (hNewValuesPairs index value).mp hValue with
      hOld | ⟨_, hValueEq⟩
    · have hIndexNewLength := (hValues.1.2.2 index).mpr ⟨value, hOld⟩
  ...
  have hNewInitial :
    ...
    rcases hInitial with ⟨zero, hZero, hZeroValue⟩
    exact
      ⟨zero, hZero, (hNewValuesPairs zero zero).mpr (Or.inl hZeroValue)⟩
  have hNewFinal :
      ℳ.PairMember 𝕀 newLength α newValues := (hNewValuesPairs newLength α).mpr (Or.inr ⟨rfl, rfl⟩)
  have hLiftPowerBound
      {oldPower : ℳ.Domain} (hOldPowerLeRemainder :
  ...
  have hNewSteps :
    ...
    · intro oldExponent coefficient previous
      ...
      have hIndexNewLength :
          ℳ.mem index newLength := (hNewLengthSuccessor index).mpr (Or.inl hIndexLength)
      have hPrevious' :=
        Structure.pairMember_old_of_append
      ...
      exact
        ⟨hCoefficientNonempty,
          ...
          hMonomial, hOutput, hPowerLeOutput, (hNewValuesPairs next output).mpr (Or.inl hNextOutput)⟩
    · have hIndexEq :=
      ...
      have hLengthNewLength :
          ℳ.mem length newLength := (hNewLengthSuccessor length).mpr (Or.inr fun _ => Iff.rfl)
      have hPreviousOld :=
        Structure.pairMember_old_of_append
...
theorem cantorNormalForm_exists
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω α : ℳ.Domain} (hω : ℳ.IsOmega ω) (hα : ℳ.IsOrdinal α) :
    ∃ length exponents coefficients values,
      ℳ.IsCantorNormalForm 𝕀
  ...
  · rcases separation_exists_d hZF (Definitional.Project.UnarySchema.cantorNormalFormExistence 𝒞).neg
        env α with
...
theorem cantorNormalFormBelow_unique
  ...
  let property : ℳ.Domain → Prop := fun current =>
    ∀ bound
        firstLength firstExponents firstCoefficients firstValues
        secondLength secondExponents secondCoefficients secondValues, (ℳ.IsCantorNormalFormBelow 𝕀
  ...
  have hProperty : property α := by
    apply hα.induction property
    · rcases separation_exists_d hZF (Definitional.Project.UnarySchema.cantorNormalFormBelowUniqueness
            𝒞).neg env α with
        ⟨counterexamples, hCounterexamples⟩
      refine ⟨counterexamples, fun current => ?_⟩
      rw [hCounterexamples current]
      ...
      · have hCurrentNonempty :
        ...
        have hPowerEq : firstPower = secondPower := by
          rcases ordinalExponentiation_existsUnique hZF
            ...
            ⟨_, _, hUniquePower⟩
          exact (hUniquePower firstPower hFirstPower).trans (hUniquePower secondPower hSecondPower).symm
        subst secondPower
        have hPowerOrdinal :=
          ordinalExponentiation_isOrdinal hZF 𝕀
...
theorem cantorNormalFormBelow_coefficient_nonempty
    ...
    {𝕀 : 𝒞.Interpretation ℳ}
    {ω bound α length exponents coefficients values
      index coefficient : ℳ.Domain} (hForm :
      ℳ.IsCantorNormalFormBelow 𝕀
        ω bound α length exponents coefficients values) (hIndex : ℳ.mem index length) (hCoefficient :
      ℳ.PairMember 𝕀 index coefficient coefficients) :
    ∃ member, ℳ.mem member coefficient := by
  rcases hForm with
  ...
  rcases (hValues.1.2.2 index).mp hIndexValueLength with
    ⟨previous, hPrevious⟩
  exact (hSteps index hIndex exponent coefficient previous
...
theorem cantorNormalForm_existsUnique_of_nonempty
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω α : ℳ.Domain} (hω : ℳ.IsOmega ω) (hα : ℳ.IsOrdinal α) (hαNonempty : ∃ member, ℳ.mem member α) :
    ∃ length exponents coefficients values,
      ℳ.IsCantorNormalForm 𝕀
          ω α length exponents coefficients values ∧ (∃ index, ℳ.mem index length) ∧ (∀ index, ℳ.mem index length →
