...
theorem isSetCodedLinearOrder
    ...
    {sumCarrier sumRelation leftCarrier leftRelation
      rightCarrier rightRelation : ℳ.Domain} (hSum : ℳ.IsLinearOrderSum 𝕀
      sumCarrier sumRelation leftCarrier leftRelation
  ...
  · intro left hLeftCarrier right hRightCarrier
    ...
    · rcases (hCarrier right).mp hRightCarrier with
      ...
      · rcases hLeft.2.2 left hLeftInLeft
        ...
        · exact Or.inl hSame
        · exact Or.inr <| Or.inl <| (hRelation left right).mpr <| Or.inl
              ⟨hLeftInLeft, hRightInLeft, hLess⟩
        · exact Or.inr <| Or.inr <| (hRelation right left).mpr <| Or.inl
              ⟨hRightInLeft, hLeftInLeft, hGreater⟩
      · exact Or.inr <| Or.inl <| (hRelation left right).mpr <| Or.inr <| Or.inr
            ⟨hLeftInLeft, hRightInRight⟩
    · rcases (hCarrier right).mp hRightCarrier with
        hRightInLeft | hRightInRight
      · exact Or.inr <| Or.inr <| (hRelation right left).mpr <| Or.inr <| Or.inr
            ⟨hRightInLeft, hLeftInRight⟩
      · rcases hRight.2.2 left hLeftInRight
            right hRightInRight with
          hSame | hLess | hGreater
        · exact Or.inl hSame
        · exact Or.inr <| Or.inl <| (hRelation left right).mpr <| Or.inr <| Or.inl
              ⟨hLeftInRight, hRightInRight, hLess⟩
        · exact Or.inr <| Or.inr <| (hRelation right left).mpr <| Or.inr <| Or.inl
...
theorem isSetCodedLinearOrder
    ...
    {productCarrier productRelation leftCarrier leftRelation
      rightCarrier rightRelation : ℳ.Domain} (hProduct : ℳ.IsLinearOrderProduct 𝕀
      productCarrier productRelation leftCarrier leftRelation
      rightCarrier rightRelation) :
  ...
  have lex_iff
      {firstPair secondPair firstLeft firstRight
        secondLeft secondRight : ℳ.Domain} (hFirstCode :
        𝕀.Codes firstPair firstLeft firstRight) (hSecondCode :
        𝕀.Codes secondPair secondLeft secondRight) (hFirstLeft : ℳ.mem firstLeft leftCarrier) (hFirstRight : ℳ.mem firstRight rightCarrier)
      (hSecondLeft : ℳ.mem secondLeft leftCarrier) (hSecondRight : ℳ.mem secondRight rightCarrier) :
  ...
  · intro firstPair hFirstPair secondPair hSecondPair
    ...
    · have hRightEq :=
      ...
      · exact Or.inr <| Or.inl <| (lex_iff hFirstCode hSecondCode
            hFirstLeft hFirstRight hSecondLeft hSecondRight).mpr <| Or.inr
            ⟨fun _ => Iff.rfl, hLeftLess⟩
      · exact Or.inr <| Or.inr <| (lex_iff hSecondCode hFirstCode
