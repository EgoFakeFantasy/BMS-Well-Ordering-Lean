...
theorem ordinalAddition_isOrdinalClassFunction
    ...
    ℳ.IsOrdinalClassFunction (fun right sum =>
  ...
  have hSchemaClass :
      ℳ.IsOrdinalClassFunction ((Definitional.Project.BinarySchema.ordinalAddition 𝒞).denote env) := by
    apply ordinalClassFunction_of_progressive hZF env (Definitional.Project.BinarySchema.ordinalAddition 𝒞)
    · rw [hRelation]
    ...
    · intro α hα hPrevious value hValue
      ...
      · have hValueEq := (ordinalAddition_zero_iff
      ...
      · rcases hSuccessor with
        ...
        exact KP.successor_isOrdinal (ZF.modelsKP hZF) (hPrevious predecessor hPredecessorMem
...
theorem ordinalAddition_isOrdinal
    ...
    ℳ.IsOrdinal sum := (ordinalAddition_isOrdinalClassFunction
...
theorem ordinalMultiplication_isOrdinalClassFunction
    ...
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {left : ℳ.Domain} (hLeft : ℳ.IsOrdinal left) :
    ℳ.IsOrdinalClassFunction (fun right product =>
  ...
  have hSchemaClass :
      ℳ.IsOrdinalClassFunction ((Definitional.Project.BinarySchema.ordinalMultiplication 𝒞).denote env) := by
    apply ordinalClassFunction_of_progressive hZF env (Definitional.Project.BinarySchema.ordinalMultiplication 𝒞)
    · rw [hRelation]
...
theorem ordinalMultiplication_isOrdinal
    ...
    ℳ.IsOrdinal product := (ordinalMultiplication_isOrdinalClassFunction
...
theorem ordinalExponentiation_isOrdinalClassFunction
    ...
    ℳ.IsOrdinalClassFunction (fun exponent power =>
  ...
  have hSchemaClass :
      ℳ.IsOrdinalClassFunction ((Definitional.Project.BinarySchema.ordinalExponentiation 𝒞).denote env) := by
    apply ordinalClassFunction_of_progressive hZF env (Definitional.Project.BinarySchema.ordinalExponentiation 𝒞)
    · rw [hRelation]
    ...
    · intro α hα hPrevious value hValue
      ...
      rcases hα.classify hZF.1 with
        hEmpty | hSuccessor | hLimit
      · have hValueOne := (ordinalExponentiation_zero_iff
            hZF 𝕀 hBase hEmpty).mp hValue
        exact KP.ordinalOne_isOrdinal (ZF.modelsKP hZF) hValueOne
      · rcases hSuccessor with
          ⟨predecessor, hPredecessorOrdinal, hSuccessor⟩
...
theorem ordinalExponentiation_isOrdinal
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {power base exponent : ℳ.Domain} (hBase : ℳ.IsOrdinal base) (hExponent : ℳ.IsOrdinal exponent) (hPower :
      ℳ.IsOrdinalExponentiation 𝕀 power base exponent) :
    ℳ.IsOrdinal power := (ordinalExponentiation_isOrdinalClassFunction
