...
def IsZeroSuccessorLimitStep {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (initialCondition : ℳ.Domain → Prop)
    (successorOperation : ℳ.Domain → ℳ.Domain → Prop) (sequence output : ℳ.Domain) : Prop := (ℳ.IsZeroLengthSequence 𝕀 sequence ∧
      initialCondition output) ∨ (∃ previous,
...
theorem eq {ℳ : Structure.{u}} (hExt : Extensional ℳ)
    {left right predecessor : ℳ.Domain} (hLeft : ℳ.SuccessorOf left predecessor) (hRight : ℳ.SuccessorOf right predecessor) :
    left = right := by
...
theorem predecessor_eq {ℳ : Structure.{u}} (hExt : Extensional ℳ)
    ...
    first = second := by
  have hFirstInSuccessor :
      ℳ.mem first successor :=
    (hFirst first).mpr (Or.inr fun _ => Iff.rfl)
  have hSecondInSuccessor :
      ℳ.mem second successor :=
    (hSecond second).mpr (Or.inr fun _ => Iff.rfl)
  rcases (hSecond first).mp hFirstInSuccessor with
    hFirstSecond | hSame
  · rcases (hFirst second).mp hSecondInSuccessor with
      hSecondFirst | hSame'
    · have hSelf : ℳ.mem first first :=
        hFirstOrdinal.transitive second hSecondFirst
          first hFirstSecond
      exact False.elim <|
        hFirstOrdinal.wellOrder.linear.irrefl first
          hSelf hSelf
    · exact
...
/-- 任意两个序数一相等。 -/
theorem eq {ℳ : Structure.{u}} (hExt : Extensional ℳ)
    {left right : ℳ.Domain} (hLeft : ℳ.IsOrdinalOne left) (hRight : ℳ.IsOrdinalOne right) :
    left = right := by
  rcases hLeft with ⟨leftZero, hLeftEmpty, hLeftSuccessor⟩
  rcases hRight with ⟨rightZero, hRightEmpty, hRightSuccessor⟩
  have hZeroEq : leftZero = rightZero := by
    apply hExt.eq_of_same_members
    intro value
    constructor
    · intro hValue
      exact False.elim (hLeftEmpty value hValue)
    · intro hValue
...
theorem not_successorLength {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ} (hExt : Extensional ℳ)
    {sequence previous : ℳ.Domain} (hZero : ℳ.IsZeroLengthSequence 𝕀 sequence) (hSuccessor :
      ℳ.IsSuccessorLengthSequenceWithLast 𝕀
        sequence previous) :
    False := by
  rcases hZero with
    ⟨zero, hZeroSequence, hEmpty⟩
  rcases hSuccessor with
    ⟨predecessor, _, length, hLength, hSuccessorSequence, _⟩
  have hLengthEq :=
...
theorem not_limitLength {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ} (hExt : Extensional ℳ)
    {sequence limit : ℳ.Domain} (hZero : ℳ.IsZeroLengthSequence 𝕀 sequence) (hLimit :
      ℳ.IsLimitLengthSequenceWithUnion 𝕀 sequence limit) :
    False := by
  rcases hZero with
    ⟨zero, hZeroSequence, hEmpty⟩
  rcases hLimit with
    ⟨length, hLimitOrdinal, hLimitSequence, _⟩
  have hLengthEq :=
    IsSequenceOfLength.length_eq hExt hZeroSequence
      hLimitSequence
  subst length
  rcases hLimitOrdinal.2.1 with ⟨value, hValue⟩
  exact hEmpty value hValue
...
theorem last_eq {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ} (hExt : Extensional ℳ)
    {sequence first second : ℳ.Domain} (hFirst :
      ℳ.IsSuccessorLengthSequenceWithLast 𝕀
        sequence first) (hSecond :
    ℳ.IsSuccessorLengthSequenceWithLast 𝕀
        sequence second) :
    first = second := by
  rcases hFirst with
    ⟨firstPredecessor, hFirstOrdinal, firstLength,
      hFirstLength, hFirstSequence, hFirstValue⟩
  rcases hSecond with
    ⟨secondPredecessor, _, secondLength,
      hSecondLength, hSecondSequence, hSecondValue⟩
  have hLengthEq :=
    IsSequenceOfLength.length_eq hExt hFirstSequence hSecondSequence
  subst secondLength
  have hPredecessorEq :=
    SuccessorOf.predecessor_eq hExt hFirstOrdinal
      hFirstLength hSecondLength
...
theorem not_limitLength {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ} (hExt : Extensional ℳ)
    {sequence previous limit : ℳ.Domain} (hSuccessor :
      ℳ.IsSuccessorLengthSequenceWithLast 𝕀
        sequence previous) (hLimit :
      ℳ.IsLimitLengthSequenceWithUnion 𝕀 sequence limit) :
    False := by
  rcases hSuccessor with
    ⟨predecessor, hPredecessorOrdinal, length,
      hLength, hSuccessorSequence, _⟩
  rcases hLimit with
    ⟨limitLength, hLimitOrdinal, hLimitSequence, _⟩
  have hLengthEq :=
  ...
  rcases (hLength larger).mp hLargerMember with
    hLargerPredecessor | hSame
  · have hSelf : ℳ.mem predecessor predecessor :=
      hPredecessorOrdinal.transitive larger
        hLargerPredecessor predecessor hPredecessorLarger
    exact hPredecessorOrdinal.wellOrder.linear.irrefl
      predecessor hSelf hSelf
  · have hEq :=
      hExt.eq_of_same_members larger predecessor hSame
    subst larger
    exact hPredecessorOrdinal.wellOrder.linear.irrefl
      predecessor hPredecessorLarger hPredecessorLarger
...
theorem union_eq {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ} (hExt : Extensional ℳ)
    {sequence left right : ℳ.Domain} (hLeft :
      ℳ.IsLimitLengthSequenceWithUnion 𝕀 sequence left) (hRight :
    ℳ.IsLimitLengthSequenceWithUnion 𝕀 sequence right) :
    left = right := by
  rcases hLeft with
    ⟨leftLength, _, hLeftSequence, leftRange, hLeftRange, hLeftUnion⟩
  rcases hRight with
    ⟨rightLength, _, hRightSequence, rightRange, hRightRange, hRightUnion⟩
  have hLengthEq :=
    IsSequenceOfLength.length_eq hExt hLeftSequence hRightSequence
  subst rightLength
  have hRangeEq := IsRangeOf.eq hExt hLeftRange hRightRange
  subst rightRange
  apply hExt.eq_of_same_members
  intro value
...
theorem range_iff_recursionValue {ℳ : Structure.{u}}
    ...
    {sequence length range : ℳ.Domain} (hSequence :
      ℳ.IsRecursiveSequence 𝕀 operator sequence length) (hRange : ℳ.IsRangeOf 𝕀 range sequence) (hUnique : ∀ index, ℳ.mem index length →
      ∀ first second,
  ...
  constructor
  · rintro ⟨index, hValue⟩
    have hIndex : ℳ.mem index length := (hSequence.1.2.2 index).mpr ⟨value, hValue⟩
    exact
      ⟨index, hIndex,
...
theorem limit_range {ℳ : Structure.{u}}
    ...
    {𝕀 : 𝒞.Interpretation ℳ}
    {operator : ℳ.Domain → ℳ.Domain → Prop}
    {α value : ℳ.Domain} (hValue :
      ℳ.IsRecursionValue 𝕀 operator α value) (hLimitStep : ∀ {sequence output},
      ℳ.IsRecursiveSequence 𝕀 operator sequence α →
      operator sequence output →
        ℳ.IsLimitLengthSequenceWithUnion 𝕀
          sequence output) (hUnique : ∀ index, ℳ.mem index α →
      ∀ first second,
        ℳ.IsRecursionValue 𝕀 operator index first →
        ℳ.IsRecursionValue 𝕀 operator index second →
        first = second) :
...
namespace KP
/-- KP 模型中任意对象都有后继。 -/
theorem exists_successor {ℳ : Structure.{u}} (hKP : ℳ.Models SetTheory.KP) (predecessor : ℳ.Domain) :
...
/-- KP 模型中存在序数一。 -/
theorem exists_ordinalOne {ℳ : Structure.{u}} (hKP : ℳ.Models SetTheory.KP) :
...
-/
theorem successor_isOrdinal {ℳ : Structure.{u}} (hKP : ℳ.Models SetTheory.KP)
    {predecessor successor : ℳ.Domain} (hPredecessor : ℳ.IsOrdinal predecessor) (hSuccessor : ℳ.SuccessorOf successor predecessor) :
  ...
  · intro middle hMiddle value hValue
    ...
    · exact (hSuccessor value).mpr <| Or.inl <|
  ...
  · refine ⟨?_, ?_⟩
    ...
    · intro subset hSubset hNonempty
      ...
      · rcases intersection_exists_d hKP subset predecessor with
        ...
        have hCandidateData := (hIntersection candidate).mp hCandidate
        refine ⟨candidate, hCandidateData.1, ?_⟩
        intro value hValueSubset
        rcases (hSuccessor value).mp (hSubset value hValueSubset) with
          hValuePredecessor | hSame
        · exact hLeast value <| (hIntersection value).mpr
              ⟨hValueSubset, hValuePredecessor⟩
        · exact Or.inr <| (hSame candidate).mpr hCandidateData.2
      · rcases hNonempty with ⟨candidate, hCandidate⟩
        have hCandidateSame :
            ℳ.SameMembers candidate predecessor := by
          rcases (hSuccessor candidate).mp (hSubset candidate hCandidate) with
        ...
        have hValueSame :
          ...
          rcases (hSuccessor value).mp (hSubset value hValue) with
            hValuePredecessor | hSame
          · exact False.elim <|
              hHasPredecessorMember
...
/-- KP 模型中的序数一确实是序数。 -/
theorem ordinalOne_isOrdinal {ℳ : Structure.{u}} (hKP : ℳ.Models SetTheory.KP) {one : ℳ.Domain} (hOne : ℳ.IsOrdinalOne one) :
    ℳ.IsOrdinal one := by
  rcases hOne with ⟨zero, hZero, hSuccessor⟩
  exact successor_isOrdinal hKP (Structure.IsOrdinal.of_no_members hZero) hSuccessor
end KP
namespace ZF
/-- ZF 中，集合编码函数具有精确值域。 -/
theorem exists_range_of_setFunction {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {function domain : ℳ.Domain} (hFunction : ℳ.IsSetFunction 𝕀 function) (hDomain : ℳ.IsDomainOf 𝕀 domain function) :
  ...
  have hTotal : ∀ input, ℳ.mem input domain →
      ∃ output, (Definitional.Project.RelationSchema.setCoded 𝒞).denote
          env input output := by
    intro input hInput
    rcases (hDomain input).mp hInput with
      ⟨output, hOutput⟩
    refine ⟨output, ?_⟩
    apply (Definitional.Project.BinarySchema.denote_setCoded_iff
        𝕀 env input output).mpr
    simpa [env] using hOutput
  have hUnique : ∀ input, ℳ.mem input domain →
      ∀ first second, (Definitional.Project.RelationSchema.setCoded 𝒞).denote
            env input first → (Definitional.Project.RelationSchema.setCoded 𝒞).denote
            env input second →
          first = second := by
    intro input _ first second hFirst hSecond
    apply hFunction.2 input first second
    · have := (Definitional.Project.BinarySchema.denote_setCoded_iff
          𝕀 env input first).mp hFirst
      simpa [env] using this
    · have := (Definitional.Project.BinarySchema.denote_setCoded_iff
          𝕀 env input second).mp hSecond
      simpa [env] using this
  rcases exists_functionalImageOn hZF (Definitional.Project.RelationSchema.setCoded 𝒞) env domain
      hTotal hUnique with
    ⟨range, hRange⟩
  ...
  · rintro ⟨input, hOutput⟩
    have hInput : ℳ.mem input domain := (hDomain input).mpr ⟨output, hOutput⟩
    refine ⟨input, hInput, ?_⟩
    apply (Definitional.Project.BinarySchema.denote_setCoded_iff
        𝕀 env input output).mpr
    simpa [env] using hOutput
/-- ZF 中，极限长度函数序列具有值域之并。 -/
theorem exists_limitLengthSequenceWithUnion {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {sequence length : ℳ.Domain} (hSequence :
      ℳ.IsSequenceOfLength 𝕀 sequence length) (hLimit : ℳ.IsLimitOrdinal length) :
    ∃ union,
      ℳ.IsLimitLengthSequenceWithUnion 𝕀
        sequence union := by
...
theorem zeroSuccessorLimitStep_isClassFunctionOnTransfiniteSequences
    ...
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (initialCondition : ℳ.Domain → Prop)
    (successorOperation : ℳ.Domain → ℳ.Domain → Prop) (hInitialCondition :
      ∃ output, initialCondition output ∧
        ∀ other, initialCondition other → other = output) (hSuccessorOperation : ∀ previous,
      ∃ output, successorOperation previous output ∧
        ∀ other, successorOperation previous other →
          other = output) :
    ℳ.IsClassFunctionOnTransfiniteSequences 𝕀 (ℳ.IsZeroSuccessorLimitStep 𝕀 initialCondition
  ...
  · rcases hSuccessor with
    ...
    have hPredecessorMem : ℳ.mem predecessor length := (hLength predecessor).mpr (Or.inr fun _ => Iff.rfl)
...
theorem ordinalAdditionStep_isClassFunctionOnTransfiniteSequences
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (left : ℳ.Domain) :
    ℳ.IsClassFunctionOnTransfiniteSequences 𝕀 (ℳ.IsOrdinalAdditionStep 𝕀 left) := by
  have hStep :=
    zeroSuccessorLimitStep_isClassFunctionOnTransfiniteSequences
      hZF 𝕀 (fun output => output = left) (fun previous output => ℳ.SuccessorOf output previous)
...
theorem ordinalAdditionOperator_isClassFunctionOnTransfiniteSequences
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (env : Env ℳ 1) :
    ℳ.IsClassFunctionOnTransfiniteSequences 𝕀 ((Definitional.Project.BinarySchema.ordinalAdditionOperator 𝒞).denote env) := by
  have hStep :=
  ...
  refine ⟨output, ?_, ?_⟩
  · exact (Definitional.Project.BinarySchema.denote_ordinalAdditionOperator_iff
...
theorem ordinalAddition_isClassFunctionOnOrdinals
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (left : ℳ.Domain) :
    ℳ.IsClassFunctionOnOrdinals (fun right sum =>
  ...
  have hClass :
      ℳ.IsClassFunctionOnOrdinals ((Definitional.Project.BinarySchema.ordinalAddition 𝒞).denote env) := by
    simpa [Definitional.Project.BinarySchema.ordinalAddition] using
      transfiniteRecursion_isClassFunctionOnOrdinals
        hZF 𝕀 env (Definitional.Project.BinarySchema.ordinalAdditionOperator 𝒞) (ordinalAdditionOperator_isClassFunctionOnTransfiniteSequences
          hZF 𝕀 env)
  intro right hRight
  rcases hClass right hRight with
    ⟨sum, hSum, hUnique⟩
  refine ⟨sum, ?_, ?_⟩
  · have hSemantic := (Definitional.Project.BinarySchema.denote_ordinalAddition_iff
        𝕀 hZF.1 env right sum).mp hSum
    simpa [env] using hSemantic
  · intro other hOther
    apply hUnique other
    apply (Definitional.Project.BinarySchema.denote_ordinalAddition_iff
        𝕀 hZF.1 env right other).mpr
    simpa [env] using hOther
/-- 序数加法对任意左参数与序数右参数存在唯一。 -/
theorem ordinalAddition_existsUnique
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (left : ℳ.Domain) {right : ℳ.Domain} (hRight : ℳ.IsOrdinal right) :
    ∃ sum,
...
theorem ordinalMultiplicationStep_isClassFunctionOnTransfiniteSequences
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (left : ℳ.Domain) :
    ℳ.IsClassFunctionOnTransfiniteSequences 𝕀 (ℳ.IsOrdinalMultiplicationStep 𝕀 left) := by
  rcases KP.exists_empty (ZF.modelsKP hZF) with
    ⟨zero, hZero⟩
  by_cases hLeft : ℳ.IsOrdinal left
  · have hStep :=
      zeroSuccessorLimitStep_isClassFunctionOnTransfiniteSequences
        hZF 𝕀 (fun output => ∀ value, ¬ ℳ.mem value output) (fun previous output =>
...
theorem ordinalMultiplicationOperator_isClassFunctionOnTransfiniteSequences
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (env : Env ℳ 1) :
    ℳ.IsClassFunctionOnTransfiniteSequences 𝕀 ((Definitional.Project.BinarySchema.ordinalMultiplicationOperator 𝒞).denote
        env) := by
  have hStep :=
  ...
  rcases hStep sequence hSequence with
    ⟨output, hOutput, hUnique⟩
  refine ⟨output, ?_, ?_⟩
  · exact (Definitional.Project.BinarySchema.denote_ordinalMultiplicationOperator_iff
        𝕀 hZF.1 env sequence output).mpr hOutput
  · intro other hOther
    apply hUnique other
    exact (Definitional.Project.BinarySchema.denote_ordinalMultiplicationOperator_iff
...
theorem ordinalMultiplication_isClassFunctionOnOrdinals
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (left : ℳ.Domain) :
    ℳ.IsClassFunctionOnOrdinals (fun right product =>
  ...
  have hClass :
      ℳ.IsClassFunctionOnOrdinals ((Definitional.Project.BinarySchema.ordinalMultiplication 𝒞).denote env) := by
    simpa [Definitional.Project.BinarySchema.ordinalMultiplication] using
      transfiniteRecursion_isClassFunctionOnOrdinals
        hZF 𝕀 env (Definitional.Project.BinarySchema.ordinalMultiplicationOperator 𝒞) (ordinalMultiplicationOperator_isClassFunctionOnTransfiniteSequences
          hZF 𝕀 env)
  intro right hRight
  rcases hClass right hRight with
    ⟨product, hProduct, hUnique⟩
  refine ⟨product, ?_, ?_⟩
  · have hSemantic := (Definitional.Project.BinarySchema.denote_ordinalMultiplication_iff
        𝕀 hZF.1 env right product).mp hProduct
    simpa [env] using hSemantic
  · intro other hOther
    apply hUnique other
    apply (Definitional.Project.BinarySchema.denote_ordinalMultiplication_iff
        𝕀 hZF.1 env right other).mpr
    simpa [env] using hOther
...
theorem ordinalExponentiationStep_isClassFunctionOnTransfiniteSequences
    ...
    ℳ.IsClassFunctionOnTransfiniteSequences 𝕀 (ℳ.IsOrdinalExponentiationStep 𝕀 base) := by
  rcases KP.exists_ordinalOne (ZF.modelsKP hZF) with
    ⟨one, hOne⟩
  have hStep :=
    zeroSuccessorLimitStep_isClassFunctionOnTransfiniteSequences
      hZF 𝕀 ℳ.IsOrdinalOne (fun previous output =>
...
theorem ordinalExponentiationOperator_isClassFunctionOnTransfiniteSequences
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (env : Env ℳ 1) (hBase : ℳ.IsOrdinal (env.bound 0)) :
    ℳ.IsClassFunctionOnTransfiniteSequences 𝕀 ((Definitional.Project.BinarySchema.ordinalExponentiationOperator 𝒞).denote
        env) := by
  have hStep :=
  ...
  rcases hStep sequence hSequence with
    ⟨output, hOutput, hUnique⟩
  refine ⟨output, ?_, ?_⟩
  · exact (Definitional.Project.BinarySchema.denote_ordinalExponentiationOperator_iff
        𝕀 hZF.1 env sequence output).mpr hOutput
  · intro other hOther
    apply hUnique other
    exact (Definitional.Project.BinarySchema.denote_ordinalExponentiationOperator_iff
...
theorem ordinalExponentiation_isClassFunctionOnOrdinals
    ...
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {base : ℳ.Domain} (hBase : ℳ.IsOrdinal base) :
    ℳ.IsClassFunctionOnOrdinals (fun exponent power =>
  ...
  have hClass :
      ℳ.IsClassFunctionOnOrdinals ((Definitional.Project.BinarySchema.ordinalExponentiation 𝒞).denote env) := by
    simpa [Definitional.Project.BinarySchema.ordinalExponentiation] using
      transfiniteRecursion_isClassFunctionOnOrdinals
        hZF 𝕀 env (Definitional.Project.BinarySchema.ordinalExponentiationOperator 𝒞) (ordinalExponentiationOperator_isClassFunctionOnTransfiniteSequences
          hZF 𝕀 env hEnvBase)
  intro exponent hExponent
  rcases hClass exponent hExponent with
    ⟨power, hPower, hUnique⟩
  refine ⟨power, ?_, ?_⟩
  · have hSemantic := (Definitional.Project.BinarySchema.denote_ordinalExponentiation_iff
        𝕀 hZF.1 env exponent power).mp hPower
    simpa [env] using hSemantic
  · intro other hOther
    apply hUnique other
    apply (Definitional.Project.BinarySchema.denote_ordinalExponentiation_iff
...
theorem ordinalExponentiation_existsUnique
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {base exponent : ℳ.Domain} (hBase : ℳ.IsOrdinal base) (hExponent : ℳ.IsOrdinal exponent) :
    ∃ power,
...
theorem ordinalAddition_zero_iff
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {sum left zero : ℳ.Domain} (hZero : ∀ value, ¬ ℳ.mem value zero) :
    ℳ.IsOrdinalAddition 𝕀 sum left zero ↔
      sum = left := by
  have value_eq {value : ℳ.Domain} (hValue :
    ...
    exact (hUnique value hOutput).trans (hUnique left hLeftOutput).symm
  constructor
  · exact value_eq
  · intro hEq
    subst sum
    rcases ordinalAddition_existsUnique hZF 𝕀 left (Structure.IsOrdinal.of_no_members hZero) with
      ⟨value, hValue, _⟩
...
theorem ordinalMultiplication_zero_iff
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {product left zero : ℳ.Domain} (hLeft : ℳ.IsOrdinal left) (hZero : ∀ value, ¬ ℳ.mem value zero) :
    ℳ.IsOrdinalMultiplication 𝕀 product left zero ↔
      ∀ value, ¬ ℳ.mem value product := by
  have value_empty {value : ℳ.Domain} (hValue :
  ...
  · intro hProduct
    ...
    have hSelectedEq : selected = product := by
      apply hZF.1.eq_of_same_members
      intro value
      exact iff_of_false (hSelectedEmpty value) (hProduct value)
    simpa [hSelectedEq] using hSelected
/-- 幂的零步方程：序数底数的零次幂是序数一。 -/
theorem ordinalExponentiation_zero_iff
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {power base zero : ℳ.Domain} (hBase : ℳ.IsOrdinal base) (hZero : ∀ value, ¬ ℳ.mem value zero) :
    ℳ.IsOrdinalExponentiation 𝕀 power base zero ↔
      ℳ.IsOrdinalOne power := by
  have value_one {value : ℳ.Domain} (hValue :
...
theorem ordinalAddition_successor_iff
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {sum left predecessor successor : ℳ.Domain} (hPredecessor : ℳ.IsOrdinal predecessor) (hSuccessor : ℳ.SuccessorOf successor predecessor) :
    ℳ.IsOrdinalAddition 𝕀 sum left successor ↔
      ∃ previous,
        ℳ.IsOrdinalAddition 𝕀
            previous left predecessor ∧
          ℳ.SuccessorOf sum previous := by
  have characterize {value : ℳ.Domain} (hValue :
...
theorem ordinalMultiplication_successor_iff
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {product left predecessor successor : ℳ.Domain} (hLeft : ℳ.IsOrdinal left) (hPredecessor : ℳ.IsOrdinal predecessor)
  ...
  · rintro ⟨previous, hPrevious, hProduct⟩
    ...
    rcases ordinalAddition_existsUnique hZF 𝕀
        previous hLeft with
      ⟨_, _, hProductUnique⟩
    have hSelectedEq : selected = product := (hProductUnique selected hSelectedProduct).trans (hProductUnique product hProduct).symm
    simpa [hSelectedEq] using hSelected
/--
幂的后继步方程：
...
theorem ordinalExponentiation_successor_iff
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {power base predecessor successor : ℳ.Domain} (hBase : ℳ.IsOrdinal base) (hPredecessor : ℳ.IsOrdinal predecessor)
    (hSuccessor : ℳ.SuccessorOf successor predecessor) :
  ...
  · rintro ⟨previous, hPrevious, hPower⟩
    ...
    rcases ordinalMultiplication_existsUnique hZF 𝕀
        previous hBase with
      ⟨_, _, hPowerUnique⟩
    have hSelectedEq : selected = power := (hPowerUnique selected hSelectedPower).trans (hPowerUnique power hPower).symm
    simpa [hSelectedEq] using hSelected
/--
...
theorem ordinalAddition_limit_iff
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {sum left limit : ℳ.Domain} (hLimit : ℳ.IsLimitOrdinal limit) :
    ...
    · intro index hIndex first second hFirst hSecond
      ...
      change
        ℳ.IsOrdinalAddition 𝕀 second left index
          at hSecond
      rcases ordinalAddition_existsUnique hZF 𝕀 left (Structure.IsOrdinal.mem hLimit.1 hIndex) with
        ⟨_, _, hUnique⟩
...
theorem ordinalMultiplication_limit_iff
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {product left limit : ℳ.Domain} (hLeft : ℳ.IsOrdinal left) (hLimit : ℳ.IsLimitOrdinal limit) :
  ...
  have characterize {value : ℳ.Domain} (hValue :
    ...
    · intro index hIndex first second hFirst hSecond
      ...
      rcases ordinalMultiplication_existsUnique hZF
          𝕀 left (Structure.IsOrdinal.mem hLimit.1 hIndex) with
        ⟨_, _, hUnique⟩
      exact (hUnique first hFirst).trans (hUnique second hSecond).symm
  constructor
  · exact characterize
...
theorem ordinalExponentiation_limit_iff
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {power base limit : ℳ.Domain} (hBase : ℳ.IsOrdinal base) (hLimit : ℳ.IsLimitOrdinal limit) :
  ...
  have characterize {value : ℳ.Domain} (hValue :
    ...
    · intro index hIndex first second hFirst hSecond
      ...
      rcases ordinalExponentiation_existsUnique hZF
          𝕀 hBase (Structure.IsOrdinal.mem hLimit.1 hIndex) with
        ⟨_, _, hUnique⟩
      exact (hUnique first hFirst).trans (hUnique second hSecond).symm
  constructor
  · exact characterize
