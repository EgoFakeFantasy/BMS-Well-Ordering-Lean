...
theorem ordinalAddition_empty_left
  ...
  have hAgreement :=
    agreeOnOrdinals_of_progressive hZF env (Definitional.Project.BinarySchema.ordinalAddition 𝒞)
      Definitional.Project.BinarySchema.ordinalIdentity (fun α hα hPrevious
          sum identityValue hSum hIdentity => by
        rw [hRelation] at hPrevious hSum
        have hIdentityEq : α = identityValue := (Definitional.Project.BinarySchema.denote_ordinalIdentity_iff
        ...
        · have hSumEq := (ordinalAddition_zero_iff
          ...
          have hLeftEq : left = α := by
            ...
            intro value
            exact iff_of_false (hLeftEmpty value) (hEmpty value)
        ...
        · rcases hSuccessor with
          ...
          have hPreviousEq : previous = predecessor :=
            hPrevious predecessor hPredecessorMem
              previous predecessor hPreviousValue ((Definitional.Project.BinarySchema.denote_ordinalIdentity_iff
          ...
          have hSumEq : sum = α := by
            ...
            · intro hValue
              ...
              have hRangeValueEq : rangeValue = index :=
                hPrevious index hIndex
                  rangeValue index hIndexValue ((Definitional.Project.BinarySchema.denote_ordinalIdentity_iff
            ...
            · intro hValue
              ...
              rcases ordinalAddition_existsUnique
                  hZF 𝕀 left (hLimit.1.mem hLarger) with
                ⟨largerValue, hLargerValue, _⟩
              have hLargerValueEq : largerValue = larger :=
                hPrevious larger hLarger
                  largerValue larger hLargerValue ((Definitional.Project.BinarySchema.denote_ordinalIdentity_iff
  ...
  exact hAgreement right hRight sum right hSum ((Definitional.Project.BinarySchema.denote_ordinalIdentity_iff
      hZF.1 env right right).mpr rfl)
/-- 乘以序数一保持原序数。 -/
theorem ordinalMultiplication_one_right
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {product left one : ℳ.Domain} (hLeft : ℳ.IsOrdinal left) (hOne : ℳ.IsOrdinalOne one) (hProduct :
  ...
  rcases (ordinalMultiplication_successor_iff
      hZF 𝕀 hLeft (Structure.IsOrdinal.of_no_members hZero)
      hSuccessor).mp hProduct with
    ⟨previous, hPrevious, hProductAddition⟩
  have hPreviousEmpty := (ordinalMultiplication_zero_iff
