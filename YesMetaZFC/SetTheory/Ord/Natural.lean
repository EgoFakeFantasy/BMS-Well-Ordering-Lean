...
def IsInductive (ℳ : Structure.{u}) (set : ℳ.Domain) : Prop := (∃ empty, (∀ value, ¬ ℳ.mem value empty) ∧
        ℳ.mem empty set) ∧
...
namespace IsOmega
/-- 最小归纳集包含某个序数一。 -/
theorem exists_ordinalOne_mem {ℳ : Structure.{u}} {ω : ℳ.Domain} (hω : ℳ.IsOmega ω) :
...
-/
theorem induction {ℳ : Structure.{u}} {ω : ℳ.Domain} (hω : ℳ.IsOmega ω) (property : ℳ.Domain → Prop) (hPropertySet : ∃ propertySet, ∀ value,
      ℳ.mem value propertySet ↔
        ℳ.mem value ω ∧ property value) (hEmpty : ∀ empty, (∀ value, ¬ ℳ.mem value empty) → property empty) (hSuccessor : ∀ predecessor, ℳ.mem predecessor ω →
  ...
  have hPropertySetInductive : ℳ.IsInductive propertySet := by
    ...
    · rcases hω.1.1 with ⟨empty, hEmptySet, hEmptyOmega⟩
      exact ⟨empty, hEmptySet, (hPropertySet empty).mpr
          ⟨hEmptyOmega, hEmpty empty hEmptySet⟩⟩
    · intro predecessor hPredecessor
      have hPredecessorData := (hPropertySet predecessor).mp hPredecessor
      rcases hω.1.2 predecessor hPredecessorData.1 with
        ⟨successor, hSuccessorOf, hSuccessorOmega⟩
      exact ⟨successor, hSuccessorOf, (hPropertySet successor).mpr
          ⟨hSuccessorOmega,
            hSuccessor predecessor hPredecessorData.1
              hPredecessorData.2 successor hSuccessorOf⟩⟩
  intro value hValue
  exact (hPropertySet value).mp (hω.2 propertySet hPropertySetInductive value hValue) |>.2
end IsOmega
end Structure
...
namespace Axioms
/-- 无穷公理的项目核语义正是存在归纳集。 -/
theorem satisfies_infinity_iff {ℳ : Structure.{u}} (free : FreeVarId → ℳ.Domain) :
    Definitional.Project.Formula.satisfies ({ bound := Fin.elim0, free := free } : Env ℳ 0)
        infinity.formula ↔
      ∃ set, ℳ.IsInductive set := by
  unfold infinity
  dsimp only [Definitional.Project.Sentence.ofFormula]
  change
    Definitional.Project.Formula.satisfies ({ bound := Fin.elim0, free := free } : Env ℳ 0) (.existsE <| Definitional.Project.Formula.isInductive
...
namespace KP
/-- KP 的无穷公理给出一个归纳集。 -/
theorem exists_inductive {ℳ : Structure.{u}} (hKP : ℳ.Models SetTheory.KP) :
    ∃ set, ℳ.IsInductive set := by
...
theorem exists_omega {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF) :
  ...
  rcases separation_exists_d hZF omegaSchema env inductiveSet with
    ⟨ω, hω⟩
  have hOmegaSemantic :
      ∀ value, ℳ.mem value ω ↔
  ...
  have hOmegaInductive : ℳ.IsInductive ω := by
    ...
    · intro predecessor hPredecessor
      have hPredecessorData := (hOmegaSemantic predecessor).mp hPredecessor
      rcases hInductiveSet.2 predecessor hPredecessorData.1 with
        ⟨successor, hSuccessor, hSuccessorInductiveSet⟩
      refine ⟨successor, hSuccessor, (hOmegaSemantic successor).mpr
...
private theorem satisfies_ordinalAdditionClosedInOmegaSchema_iff
    {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 2) (right : ℳ.Domain) :
    Definitional.Project.Formula.satisfies (env.push right) (ordinalAdditionClosedInOmegaSchema 𝒞).body ↔
      ∀ sum,
...
private theorem satisfies_ordinalMultiplicationClosedInOmegaSchema_iff
    {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 2) (right : ℳ.Domain) :
    Definitional.Project.Formula.satisfies (env.push right) (ordinalMultiplicationClosedInOmegaSchema 𝒞).body ↔
      ∀ product,
  ...
  rfl
/-- 自然数幂封闭模式的模型语义。 -/
private theorem satisfies_ordinalExponentiationClosedInOmegaSchema_iff
    {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 2) (right : ℳ.Domain) :
    Definitional.Project.Formula.satisfies (env.push right) (ordinalExponentiationClosedInOmegaSchema 𝒞).body ↔
      ∀ power,
        ℳ.IsOrdinalExponentiation 𝕀
          power (env.bound 0) right →
          ℳ.mem power (env.bound 1) := by
  simp only [ordinalExponentiationClosedInOmegaSchema,
    Definitional.Project.Formula.satisfies_forall_iff,
    Definitional.Project.Formula.satisfies_imp_iff,
...
/-- 最小归纳集的每个成员都是序数。 -/
theorem members_areOrdinals {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
  ...
  apply hω.induction (fun value => ℳ.IsOrdinal value)
  · rcases ZF.separation_exists_d hZF ZF.ordinalSchema
        env ω with ⟨ordinals, hOrdinals⟩
    refine ⟨ordinals, fun value => ?_⟩
    rw [hOrdinals value]
    constructor
    · rintro ⟨hValue, hValueOrdinal⟩
      exact ⟨hValue, (Definitional.Project.Formula.satisfies_isOrdinal_iff (env.push value) (.bound 0)).mp hValueOrdinal⟩
    · rintro ⟨hValue, hValueOrdinal⟩
      exact ⟨hValue, (Definitional.Project.Formula.satisfies_isOrdinal_iff (env.push value) (.bound 0)).mpr hValueOrdinal⟩
  · intro empty hEmpty
...
/-- 最小归纳集是传递集。 -/
theorem transitive {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {ω : ℳ.Domain} (hω : ℳ.IsOmega ω) :
  ...
  have hMembersClosed :
    ...
    apply hω.induction (fun predecessor => ℳ.MemberSubset predecessor ω)
    · rcases ZF.separation_exists_d hZF
      ...
      · rintro ⟨hValue, hSubset⟩
        exact ⟨hValue, (Definitional.Project.Formula.satisfies_subset_iff (env.push value) (.bound 0) (.bound 1)).mp
            hSubset⟩
      · rintro ⟨hValue, hSubset⟩
        exact ⟨hValue, (Definitional.Project.Formula.satisfies_subset_iff (env.push value) (.bound 0) (.bound 1)).mpr
            hSubset⟩
...
theorem exists_predecessor_of_mem_of_nonempty
    ...
    {ω number : ℳ.Domain} (hω : ℳ.IsOmega ω) (hNumberOmega : ℳ.mem number ω) (hNumberNonempty : ∃ member, ℳ.mem member number) :
    ∃ predecessor,
      ℳ.mem predecessor ω ∧
...
theorem membershipWellOrder {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
  ...
  · refine {
    ...
    · intro left hLeft right hRight
      exact Structure.IsOrdinal.trichotomy hZF.1 (hMembersOrdinal left hLeft) (hMembersOrdinal right hRight) (KP.difference_exists_d (ZF.modelsKP hZF))
        (KP.intersection_exists_d (ZF.modelsKP hZF) left right)
  · intro subset hSubset hNonempty
    rcases KP.mem_minimal_exists_d (ZF.modelsKP hZF) hNonempty with
      ⟨minimal, hMinimalSubset, hMinimal⟩
    refine ⟨minimal, hMinimalSubset, ?_⟩
    intro value hValueSubset
    rcases Structure.IsOrdinal.trichotomy hZF.1 (hMembersOrdinal minimal (hSubset minimal hMinimalSubset)) (hMembersOrdinal value (hSubset value hValueSubset))
        (KP.difference_exists_d (ZF.modelsKP hZF)) (KP.intersection_exists_d (ZF.modelsKP hZF)
          minimal value) with
...
/-- 最小归纳集是序数。 -/
theorem isOrdinal {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {ω : ℳ.Domain} (hω : ℳ.IsOmega ω) :
    ℳ.IsOrdinal ω :=
  ⟨hω.transitive hZF,
    hω.membershipWellOrder hZF⟩
/-- 最小归纳集是非零极限序数。 -/
theorem isLimitOrdinal {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {ω : ℳ.Domain} (hω : ℳ.IsOmega ω) :
  ...
  · intro predecessor hPredecessor
    ...
    exact ⟨successor, hSuccessorOmega, (hSuccessor predecessor).mpr (Or.inr fun _ => Iff.rfl)⟩
/-- 每个非零极限序数都包含最小归纳集中的全部自然数。 -/
theorem subset_limitOrdinal {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {ω α : ℳ.Domain} (hω : ℳ.IsOmega ω) (hα : ℳ.IsLimitOrdinal α) :
    ℳ.MemberSubset ω α := by
  apply hω.2 α
  constructor
  · rcases KP.exists_empty (ZF.modelsKP hZF) with
      ⟨empty, hEmpty⟩
    rcases hα.2.1 with ⟨member, hMember⟩
    rcases Structure.IsOrdinal.trichotomy hZF.1 (Structure.IsOrdinal.of_no_members hEmpty) (hα.1.mem hMember) (KP.difference_exists_d (ZF.modelsKP hZF))
        (KP.intersection_exists_d (ZF.modelsKP hZF)
          empty member) with
      hSame | hEmptyMember | hMemberEmpty
    · have hEq :=
        hZF.1.eq_of_same_members empty member hSame
      exact ⟨empty, hEmpty, by simpa [hEq] using hMember⟩
    · exact ⟨empty, hEmpty,
        hα.1.transitive member hMember empty hEmptyMember⟩
    · exact False.elim <| hEmpty member hMemberEmpty
  · intro predecessor hPredecessor
    ...
    have hPredecessorOrdinal :
      ...
      hα.1.mem hPredecessor
    have hSuccessorOrdinal :
        ℳ.IsOrdinal successor :=
      KP.successor_isOrdinal (ZF.modelsKP hZF)
        hPredecessorOrdinal hSuccessor
    rcases hα.2.2 predecessor hPredecessor with
      ⟨larger, hLarger, hPredecessorLarger⟩
    have hLargerOrdinal :
        ℳ.IsOrdinal larger :=
      hα.1.mem hLarger
    rcases Structure.IsOrdinal.trichotomy hZF.1
        hSuccessorOrdinal hLargerOrdinal (KP.difference_exists_d (ZF.modelsKP hZF)) (KP.intersection_exists_d (ZF.modelsKP hZF)
          successor larger) with
      hSame | hSuccessorLarger | hLargerSuccessor
    · have hEq :=
        hZF.1.eq_of_same_members successor larger hSame
      exact ⟨successor, hSuccessor, by simpa [hEq] using hLarger⟩
    · exact ⟨successor, hSuccessor,
        hα.1.transitive larger hLarger
          successor hSuccessorLarger⟩
    · rcases (hSuccessor larger).mp hLargerSuccessor with
        hLargerPredecessor | hLargerEqPredecessor
      · have hSelf : ℳ.mem predecessor predecessor :=
          hα.1.wellOrder.linear.trans
            predecessor hPredecessor larger hLarger
            predecessor hPredecessor
            hPredecessorLarger hLargerPredecessor
        exact False.elim <|
          hα.1.wellOrder.linear.irrefl
            predecessor hPredecessor hSelf
      · have hEq :=
          hZF.1.eq_of_same_members
            larger predecessor hLargerEqPredecessor
        have hSelf : ℳ.mem predecessor predecessor := by
          simpa [hEq] using hPredecessorLarger
        exact False.elim <|
          hα.1.wellOrder.linear.irrefl
            predecessor hPredecessor hSelf
...
theorem ordinalAddition_mem_omega
    ...
    {ω left right sum : ℳ.Domain} (hω : ℳ.IsOmega ω) (hLeft : ℳ.mem left ω) (hRight : ℳ.mem right ω) (hSum : ℳ.IsOrdinalAddition 𝕀 sum left right) :
    ℳ.mem sum ω := by
  let env : Env ℳ 2 := {
      ...
      ∀ current, ℳ.mem current ω →
        ∀ value,
          ℳ.IsOrdinalAddition 𝕀 value left current →
            ℳ.mem value ω := by
    apply hω.induction (fun current =>
          ∀ value,
            ℳ.IsOrdinalAddition 𝕀 value left current →
              ℳ.mem value ω)
    · rcases separation_exists_d hZF (ordinalAdditionClosedInOmegaSchema 𝒞) env ω with
        ⟨closed, hClosed⟩
      refine ⟨closed, fun current => ?_⟩
      rw [hClosed current,
        satisfies_ordinalAdditionClosedInOmegaSchema_iff
          𝕀 hZF.1 env current]
      change (ℳ.mem current ω ∧
          ∀ value,
            ℳ.IsOrdinalAddition 𝕀 value left current →
              ℳ.mem value ω) ↔
        ℳ.mem current ω ∧
      ...
      rfl
    · intro empty hEmpty value hValue
      have hValueEq := (ordinalAddition_zero_iff hZF 𝕀 hEmpty).mp hValue
...
theorem ordinalMultiplication_mem_omega
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω left right product : ℳ.Domain} (hω : ℳ.IsOmega ω) (hLeft : ℳ.mem left ω) (hRight : ℳ.mem right ω)
  ...
  have hClosed :
    ...
    apply hω.induction (fun current =>
          ∀ value,
            ℳ.IsOrdinalMultiplication 𝕀 value left current →
              ℳ.mem value ω)
    · rcases separation_exists_d hZF (ordinalMultiplicationClosedInOmegaSchema 𝒞) env ω with
        ⟨closed, hClosed⟩
      refine ⟨closed, fun current => ?_⟩
      rw [hClosed current,
      ...
      rfl
    · intro empty hEmpty value hValue
      have hValueEmpty := (ordinalMultiplication_zero_iff
          hZF 𝕀 (hω.members_areOrdinals hZF left hLeft)
          hEmpty).mp hValue
      rcases hω.1.1 with
      ...
      have hValueEq : value = omegaEmpty := by
        apply hZF.1.eq_of_same_members
        intro member
        exact iff_of_false (hValueEmpty member) (hOmegaEmpty member)
...
/-- 两个自然数的序数幂值仍是自然数。 -/
theorem ordinalExponentiation_mem_omega
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ω base exponent power : ℳ.Domain} (hω : ℳ.IsOmega ω) (hBase : ℳ.mem base ω) (hExponent : ℳ.mem exponent ω)
    (hPower : ℳ.IsOrdinalExponentiation 𝕀 power base exponent) :
    ℳ.mem power ω := by
  let env : Env ℳ 2 := {
    bound := fun index => Fin.cases base (fun _ => ω) index
    free := fun _ => Classical.choice ℳ.nonempty
  }
  have hClosed :
      ∀ current, ℳ.mem current ω →
        ∀ value,
          ℳ.IsOrdinalExponentiation 𝕀 value base current →
            ℳ.mem value ω := by
    apply hω.induction (fun current =>
          ∀ value,
            ℳ.IsOrdinalExponentiation 𝕀 value base current →
              ℳ.mem value ω)
    · rcases separation_exists_d hZF (ordinalExponentiationClosedInOmegaSchema 𝒞) env ω with
        ⟨closed, hClosed⟩
      refine ⟨closed, fun current => ?_⟩
      rw [hClosed current,
        satisfies_ordinalExponentiationClosedInOmegaSchema_iff
          𝕀 hZF.1 env current]
      change (ℳ.mem current ω ∧
          ∀ value,
            ℳ.IsOrdinalExponentiation 𝕀 value base current →
      ...
      rfl
    · intro empty hEmpty value hValue
      have hValueOne := (ordinalExponentiation_zero_iff hZF 𝕀 (hω.members_areOrdinals hZF base hBase)
          hEmpty).mp hValue
      rcases hω.exists_ordinalOne_mem with
        ⟨one, hOne, hOneOmega⟩
      have hValueEq :=
        Structure.IsOrdinalOne.eq hZF.1 hValueOne hOne
      simpa [hValueEq] using hOneOmega
    · intro predecessor hPredecessor hPredecessorClosed
        successor hSuccessor value hValue
      have hBaseOrdinal :=
      ...
      rcases (ordinalExponentiation_successor_iff
          hZF 𝕀 hBaseOrdinal hPredecessorOrdinal hSuccessor).mp hValue with
        ⟨previous, hPrevious, hValueSuccessor⟩
      have hPreviousOmega :=
        hPredecessorClosed previous hPrevious
      exact ordinalMultiplication_mem_omega
        hZF 𝕀 hω hPreviousOmega hBase hValueSuccessor
  exact hClosed exponent hExponent power hPower
end ZF
end SetTheory
end YesMetaZFC
