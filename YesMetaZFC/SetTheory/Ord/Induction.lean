...
def SuccessorOf (ℳ : SetTheory.Structure.{u}) (successor predecessor : ℳ.Domain) : Prop :=
  ∀ value, ℳ.mem value successor ↔
    ℳ.mem value predecessor ∨ ℳ.SameMembers value predecessor
/-- 非零极限序数的纸面语义。 -/
def IsLimitOrdinal (ℳ : SetTheory.Structure.{u}) (α : ℳ.Domain) : Prop :=
  ℳ.IsOrdinal α ∧ (∃ value, ℳ.mem value α) ∧
...
/-- 序数上的最小反例归纳核。 -/
theorem induction {ℳ : SetTheory.Structure.{u}} {α : ℳ.Domain} (hα : Structure.IsOrdinal ℳ α) (property : ℳ.Domain → Prop)
    (hCounterexamples : ∃ counterexamples, ∀ value,
      ℳ.mem value counterexamples ↔ ℳ.mem value α ∧ ¬ property value) (hProgressive : ∀ β, Structure.IsOrdinal ℳ β →
  ...
  · have hSelf : ℳ.mem predecessor predecessor := (hSameMembers predecessor).mp hPredecessor
...
-/
theorem inductionWithin {ℳ : SetTheory.Structure.{u}} {α : ℳ.Domain} (hα : Structure.IsOrdinal ℳ α) (property : ℳ.Domain → Prop)
    (hCounterexamples : ∃ counterexamples, ∀ value,
      ℳ.mem value counterexamples ↔ ℳ.mem value α ∧ ¬ property value) (hProgressive : ∀ β, Structure.IsOrdinal ℳ β → (β = α ∨ ℳ.mem β α) →
...
theorem classify {ℳ : SetTheory.Structure.{u}} {α : ℳ.Domain} (hExt : Extensional ℳ) (hα : Structure.IsOrdinal ℳ α) : (∀ value, ¬ ℳ.mem value α) ∨
...
def limitCaseCore {parameterCount : Nat} (schema : UnarySchema parameterCount) : Formula 1 parameterCount :=
  .forallE <| .imp (Formula.isLimitOrdinal Term.newest) <|
    .imp (Formula.forallMem Term.newest <|
      ...
      schema.body
/-- 零、后继与非零极限三种闭包条件。 -/
def casesCore {parameterCount : Nat} (schema : UnarySchema parameterCount) : Formula 1 parameterCount :=
...
def caseInductionSentence {parameterCount : Nat} (schema : UnarySchema parameterCount) : Sentence :=
  Sentence.forallClosure (.imp (casesCore schema) (inductionCore schema)) <| by
      simpa [Formula.FreeClosed] using
        And.intro (casesCore_freeClosed schema) (inductionCore_freeClosed schema)
/-- 递进公式的语义正是序数上的递进性质。 -/
theorem satisfies_progressiveCore_iff {ℳ : Structure.{u}}
    {parameterCount : Nat} (env : Env ℳ parameterCount) (schema : UnarySchema parameterCount) :
    Formula.satisfies env (progressiveCore schema) ↔
      ∀ α, Structure.IsOrdinal ℳ α → (∀ predecessor, ℳ.mem predecessor α →
...
theorem satisfies_limitCaseCore_iff {ℳ : Structure.{u}}
    ...
    Formula.satisfies env (limitCaseCore schema) ↔
      ∀ α, ℳ.IsLimitOrdinal α → (∀ predecessor, ℳ.mem predecessor α →
          Formula.satisfies (env.push predecessor) schema.body) →
        Formula.satisfies (env.push α) schema.body := by
...
theorem satisfies_inductionCore_of_progressive {ℳ : Structure.{u}}
    {parameterCount : Nat} (env : Env ℳ parameterCount) (schema : UnarySchema parameterCount) (hCounterexamples : ∀ α, ∃ counterexamples, ∀ value,
      ℳ.mem value counterexamples ↔
        ℳ.mem value α ∧
          ¬ Formula.satisfies (env.push value) schema.body) (hProgressive : Formula.satisfies env (progressiveCore schema)) :
  ...
  intro α hα
  exact Structure.IsOrdinal.induction hα (fun value => Formula.satisfies (env.push value) schema.body) (hCounterexamples α) hProgressive
/-- 三类闭包与反例分离共同推出完整的超限归纳结论。 -/
theorem satisfies_inductionCore_of_cases {ℳ : Structure.{u}}
    {parameterCount : Nat} (env : Env ℳ parameterCount) (schema : UnarySchema parameterCount) (hExt : Extensional ℳ)
    (hCounterexamples : ∀ α, ∃ counterexamples, ∀ value,
      ℳ.mem value counterexamples ↔
        ℳ.mem value α ∧
          ¬ Formula.satisfies (env.push value) schema.body) (hCases : Formula.satisfies env (casesCore schema)) :
    Formula.satisfies env (inductionCore schema) := by
  ...
  rw [satisfies_inductionCore_iff]
  intro α hα
  apply Structure.IsOrdinal.induction hα (fun value => Formula.satisfies (env.push value) schema.body) (hCounterexamples α)
  ...
  · rcases hSuccessor with
    ...
    have hPredecessor : ℳ.mem predecessor current := (hSuccessor predecessor).mpr <| Or.inr fun value => Iff.rfl
    exact hCases.2.1 predecessor current hPredecessorOrdinal (hPredecessors predecessor hPredecessor) hSuccessor
  · exact hCases.2.2 current hLimit hPredecessors
end UnarySchema
namespace Delta0UnarySchema
/-- `Δ₀` 一元模式对否定封闭。 -/
def neg {parameterCount : Nat} (schema : Delta0UnarySchema parameterCount) :
...
private theorem ordinalCounterexamples {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF) {parameterCount : Nat}
    (schema : Definitional.Project.UnarySchema parameterCount) (env : Env ℳ parameterCount) :
  ...
  simpa only [Definitional.Project.UnarySchema.neg,
    ...
    ZF.separation_exists_d hZF schema.neg env α
/-- ZF 证明任意公式模式的超限归纳。 -/
theorem ordInduction {parameterCount : Nat} (schema : Definitional.Project.UnarySchema parameterCount) :
    SemanticallyEntails.{0} SetTheory.ZF schema.inductionSentence := by
  intro ℳ hZF free
  change Definitional.Project.Formula.satisfies
    { bound := Fin.elim0, free := free } (Definitional.Project.Formula.forallClosure parameterCount (.imp schema.progressiveCore schema.inductionCore))
  ...
  change Definitional.Project.Formula.satisfies env (.imp schema.progressiveCore schema.inductionCore)
  rw [Definitional.Project.Formula.satisfies_imp_iff]
  exact (@Definitional.Project.UnarySchema.satisfies_inductionCore_of_progressive
      ℳ parameterCount env schema) (ordinalCounterexamples hZF schema env)
/-- ZF 中按零、后继与极限三类序数表述的超限归纳。 -/
theorem ordInductionCases {parameterCount : Nat} (schema : Definitional.Project.UnarySchema parameterCount) :
    SemanticallyEntails.{0} SetTheory.ZF
      schema.caseInductionSentence := by
  intro ℳ hZF free
  change Definitional.Project.Formula.satisfies
    { bound := Fin.elim0, free := free } (Definitional.Project.Formula.forallClosure parameterCount (.imp schema.casesCore schema.inductionCore))
  rw [Definitional.Project.Formula.satisfies_forallClosure_iff]
  intro bound
  let env : Env ℳ parameterCount := {
    bound := bound
  ...
  change Definitional.Project.Formula.satisfies env (.imp schema.casesCore schema.inductionCore)
  rw [Definitional.Project.Formula.satisfies_imp_iff]
  exact (@Definitional.Project.UnarySchema.satisfies_inductionCore_of_cases
      ℳ parameterCount env schema hZF.1) (ordinalCounterexamples hZF schema env)
end ZF
namespace KP
/-- KP 的 `Δ₀` 分离收集给定序数中的全部模式反例。 -/
private theorem delta0OrdinalCounterexamples {ℳ : Structure.{u}} (hKP : ℳ.Models SetTheory.KP) {parameterCount : Nat}
    (schema : Definitional.Project.Delta0UnarySchema parameterCount) (env : Env ℳ parameterCount) :
    ∀ α, ∃ counterexamples, ∀ value,
      ℳ.mem value counterexamples ↔
        ℳ.mem value α ∧
          ¬ Definitional.Project.Formula.satisfies (env.push value) schema.toUnarySchema.body := by
  intro α
  simpa only [Definitional.Project.Delta0UnarySchema.neg,
    Definitional.Project.UnarySchema.neg,
    Definitional.Project.Formula.satisfies_neg_iff] using
      KP.separation_exists_d hKP schema.neg env α
/-- KP 证明 `Δ₀` 公式模式的超限归纳。 -/
theorem delta0OrdInduction {parameterCount : Nat} (schema : Definitional.Project.Delta0UnarySchema parameterCount) :
    SemanticallyEntails.{0} SetTheory.KP
      schema.toUnarySchema.inductionSentence := by
  intro ℳ hKP free
  change Definitional.Project.Formula.satisfies
    { bound := Fin.elim0, free := free } (Definitional.Project.Formula.forallClosure parameterCount (.imp schema.toUnarySchema.progressiveCore
  ...
  change Definitional.Project.Formula.satisfies env (.imp schema.toUnarySchema.progressiveCore
      schema.toUnarySchema.inductionCore)
  rw [Definitional.Project.Formula.satisfies_imp_iff]
  exact (@Definitional.Project.UnarySchema.satisfies_inductionCore_of_progressive
      ℳ parameterCount env schema.toUnarySchema) (delta0OrdinalCounterexamples hKP schema env)
/-- KP 中按零、后继与极限三类序数表述的 `Δ₀` 超限归纳。 -/
theorem delta0OrdInductionCases {parameterCount : Nat} (schema : Definitional.Project.Delta0UnarySchema parameterCount) :
    SemanticallyEntails.{0} SetTheory.KP
      schema.toUnarySchema.caseInductionSentence := by
  intro ℳ hKP free
  change Definitional.Project.Formula.satisfies
    { bound := Fin.elim0, free := free } (Definitional.Project.Formula.forallClosure parameterCount (.imp schema.toUnarySchema.casesCore
        schema.toUnarySchema.inductionCore))
  rw [Definitional.Project.Formula.satisfies_forallClosure_iff]
  intro bound
  let env : Env ℳ parameterCount := {
  ...
  change Definitional.Project.Formula.satisfies env (.imp schema.toUnarySchema.casesCore
      schema.toUnarySchema.inductionCore)
  rw [Definitional.Project.Formula.satisfies_imp_iff]
  exact (@Definitional.Project.UnarySchema.satisfies_inductionCore_of_cases
      ℳ parameterCount env schema.toUnarySchema hKP.1) (delta0OrdinalCounterexamples hKP schema env)
end KP
end SetTheory
end YesMetaZFC
