...
def IsOrdinalSequenceLimit {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (limit sequence length : ℳ.Domain) : Prop :=
  ℳ.IsOrdinal limit ∧
    ℳ.IsOrdinalValuedSequence 𝕀 sequence length ∧
      ∃ range,
...
def IsOrdinalClassFunction (ℳ : Structure.{u}) (function : ℳ.Domain → ℳ.Domain → Prop) : Prop :=
  ℳ.IsClassFunctionOnOrdinals function ∧
    ∀ α value,
      ℳ.IsOrdinal α → function α value →
        ℳ.IsOrdinal value
/-- `function` 在序数类上严格递增。 -/
def IsIncreasingOnOrdinals (ℳ : Structure.{u}) (function : ℳ.Domain → ℳ.Domain → Prop) : Prop :=
  ∀ left right,
...
def IsContinuousOnOrdinals (ℳ : Structure.{u}) (function : ℳ.Domain → ℳ.Domain → Prop) : Prop :=
  ∀ α value,
    ℳ.IsLimitOrdinal α ∧ function α value →
      ∃ range, (∀ member, ℳ.mem member range ↔
          ∃ input, ℳ.mem input α ∧ function input member) ∧
        ℳ.IsUnionOf value range
/-- `function` 是序数类上的正规函数。 -/
def IsNormalOrdinalFunction (ℳ : Structure.{u}) (function : ℳ.Domain → ℳ.Domain → Prop) : Prop :=
...
theorem limit_value {ℳ : Structure.{u}}
    {function : ℳ.Domain → ℳ.Domain → Prop} (hNormal : ℳ.IsNormalOrdinalFunction function)
    {α value : ℳ.Domain} (hLimit : ℳ.IsLimitOrdinal α) (hValue : function α value) :
  ...
  · rcases hLimit.2.1 with ⟨input, hInput⟩
    ...
    have hInputValueLarger : ℳ.mem inputValue largerValue :=
      hNormal.2.1 input larger (hLimit.1.mem hInput) (hLimit.1.mem hLarger)
        hInputLarger inputValue largerValue
        hInputValue hLargerValue
    exact ⟨inputValue, (hUnion inputValue).mpr
      ⟨largerValue, (hRange largerValue).mpr
  ...
  · intro predecessor hPredecessor
    ...
    exact ⟨container, (hUnion container).mpr
        ⟨largerValue, (hRange largerValue).mpr
            ⟨larger, hLarger, hLargerValue⟩,
          hContainerLarger⟩,
...
等于外层函数在内层极限值处的值。
-/
theorem compose_limit_union_eq {ℳ : Structure.{u}} (hExt : Extensional ℳ)
    {outer inner : ℳ.Domain → ℳ.Domain → Prop} (hOuter : ℳ.IsNormalOrdinalFunction outer) (hInner : ℳ.IsNormalOrdinalFunction inner)
    {limit innerLimit outerLimit range composed : ℳ.Domain} (hLimit : ℳ.IsLimitOrdinal limit) (hInnerLimit : inner limit innerLimit)
    (hOuterLimit : outer innerLimit outerLimit) (hRange : ∀ member, ℳ.mem member range ↔
      ∃ index, ℳ.mem index limit ∧
        ∃ innerValue,
          inner index innerValue ∧
            outer innerValue member) (hUnion : ℳ.IsUnionOf composed range) :
  ...
  · intro hValue
    ...
    have hInnerValueLarger : ℳ.mem innerValue largerValue :=
      hInner.2.1 index larger (hLimit.1.mem hIndex) (hLimit.1.mem hLarger)
        hIndexLarger innerValue largerValue
        hInnerValue hLargerValue
    have hInnerValueLimit : ℳ.mem innerValue innerLimit := (hInnerUnion innerValue).mpr
        ⟨largerValue, (hInnerRange largerValue).mpr
            ⟨larger, hLarger, hLargerValue⟩,
          hInnerValueLarger⟩
    exact (hOuterUnion value).mpr
      ⟨composedValue, (hOuterRange composedValue).mpr
  ...
  · intro hValue
    ...
    rcases hOuter.1.1 innerValue (hInner.1.2 index innerValue (hLimit.1.mem hIndex) hInnerValue) with
      ⟨composedValue, hComposedValue, _⟩
    have hOuterValueComposed : ℳ.mem outerValue composedValue :=
      hOuter.2.1 innerInput innerValue (hInnerLimitOrdinal.1.mem hInnerInputLimit) (hInner.1.2 index innerValue (hLimit.1.mem hIndex) hInnerValue)
        hInnerInputValue outerValue composedValue
        hOuterValue hComposedValue
    have hComposedOrdinal : ℳ.IsOrdinal composedValue :=
      hOuter.1.2 innerValue composedValue (hInner.1.2 index innerValue (hLimit.1.mem hIndex) hInnerValue)
        hComposedValue
    exact (hUnion value).mpr
      ⟨composedValue, (hRange composedValue).mpr
...
theorem compose_limit_member_of_pointwise_eq
    ...
    {firstOuter firstInner secondOuter secondInner :
      ℳ.Domain → ℳ.Domain → Prop} (hFirstOuter : ℳ.IsNormalOrdinalFunction firstOuter) (hFirstInner : ℳ.IsNormalOrdinalFunction firstInner)
    (hSecondOuter : ℳ.IsNormalOrdinalFunction secondOuter) (hSecondInner : ℳ.IsNormalOrdinalFunction secondInner)
    {limit firstInnerLimit firstLimit
      secondInnerLimit secondLimit : ℳ.Domain} (hLimit : ℳ.IsLimitOrdinal limit) (hFirstInnerLimit : firstInner limit firstInnerLimit)
    (hFirstLimit : firstOuter firstInnerLimit firstLimit) (hSecondInnerLimit : secondInner limit secondInnerLimit)
    (hSecondLimit : secondOuter secondInnerLimit secondLimit) (hPointwise : ∀ index, ℳ.mem index limit →
  ...
  have hFirstInnerValueOrdinal : ℳ.IsOrdinal firstInnerValue :=
    hFirstInner.1.2 index firstInnerValue (hLimit.1.mem hIndex) hFirstInnerValue
  ...
  have hFirstOuterValueFirst : ℳ.mem firstOuterValue firstValue :=
    hFirstOuter.2.1 firstInnerInput firstInnerValue (hFirstInnerLimitOrdinal.1.mem hFirstInnerInputLimit)
  ...
  have hSecondInnerValueOrdinal : ℳ.IsOrdinal secondInnerValue :=
    hSecondInner.1.2 index secondInnerValue (hLimit.1.mem hIndex) hSecondInnerValue
  ...
  have hSecondInnerValueLimit :
    ...
    hSecondInner.2.1 index limit (hLimit.1.mem hIndex) hLimit.1 hIndex
...
theorem satisfies_isOrdinalValuedSequence_iff
    ...
    satisfies env (isOrdinalValuedSequence 𝒞 sequence length) ↔
      ℳ.IsOrdinalValuedSequence 𝕀 (sequence.eval env) (length.eval env) := by
...
theorem satisfies_isNondecreasingOrdinalSequence_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ)
    {depth : Nat} (env : Env ℳ depth) (sequence length : Term depth) :
    satisfies env (isNondecreasingOrdinalSequence 𝒞 sequence length) ↔
      ℳ.IsNondecreasingOrdinalSequence 𝕀 (sequence.eval env) (length.eval env) := by
...
theorem satisfies_isIncreasingOrdinalSequence_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ)
    {depth : Nat} (env : Env ℳ depth) (sequence length : Term depth) :
    satisfies env (isIncreasingOrdinalSequence 𝒞 sequence length) ↔
      ℳ.IsIncreasingOrdinalSequence 𝕀 (sequence.eval env) (length.eval env) := by
...
theorem satisfies_isOrdinalSequenceLimit_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ)
    {depth : Nat} (env : Env ℳ depth) (limit sequence length : Term depth) :
    satisfies env (isOrdinalSequenceLimit 𝒞 limit sequence length) ↔
      ℳ.IsOrdinalSequenceLimit 𝕀 (limit.eval env) (sequence.eval env) (length.eval env) := by
...
theorem satisfies_isLimitOfNondecreasingOrdinalSequence_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ)
    {depth : Nat} (env : Env ℳ depth) (limit sequence length : Term depth) :
    satisfies env (isLimitOfNondecreasingOrdinalSequence 𝒞
          limit sequence length) ↔
      ℳ.IsLimitOfNondecreasingOrdinalSequence 𝕀 (limit.eval env) (sequence.eval env) (length.eval env) := by
...
theorem satisfies_isContinuousOrdinalSequence_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ)
    {depth : Nat} (env : Env ℳ depth) (sequence length : Term depth) :
    satisfies env (isContinuousOrdinalSequence 𝒞 sequence length) ↔
      ℳ.IsContinuousOrdinalSequence 𝕀 (sequence.eval env) (length.eval env) := by
...
theorem satisfies_isNormalOrdinalSequence_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ)
    {depth : Nat} (env : Env ℳ depth) (sequence length : Term depth) :
    satisfies env (isNormalOrdinalSequence 𝒞 sequence length) ↔
      ℳ.IsNormalOrdinalSequence 𝕀 (sequence.eval env) (length.eval env) := by
...
theorem satisfies_isClassFunctionOnOrdinals_iff
    ...
    satisfies env (isClassFunctionOnOrdinals function parameters) ↔
      ℳ.IsClassFunctionOnOrdinals (function.denote (parameters.evalEnv env)) := by
...
theorem satisfies_isOrdinalClassFunction_iff
    {ℳ : Structure.{u}} (hExt : Extensional ℳ)
    {parameterCount depth : Nat} (env : Env ℳ depth) (function : BinarySchema parameterCount) (parameters : TermVector parameterCount depth) :
    satisfies env (isOrdinalClassFunction function parameters) ↔
      ℳ.IsOrdinalClassFunction (function.denote (parameters.evalEnv env)) := by
...
theorem satisfies_isIncreasingOnOrdinals_iff
    {ℳ : Structure.{u}}
    {parameterCount depth : Nat} (env : Env ℳ depth) (function : BinarySchema parameterCount) (parameters : TermVector parameterCount depth) :
    satisfies env (isIncreasingOnOrdinals function parameters) ↔
      ℳ.IsIncreasingOnOrdinals (function.denote (parameters.evalEnv env)) := by
...
theorem satisfies_isContinuousOnOrdinals_iff
    {ℳ : Structure.{u}}
    {parameterCount depth : Nat} (env : Env ℳ depth) (function : BinarySchema parameterCount) (parameters : TermVector parameterCount depth) :
    satisfies env (isContinuousOnOrdinals function parameters) ↔
      ℳ.IsContinuousOnOrdinals (function.denote (parameters.evalEnv env)) := by
...
theorem satisfies_isNormalOrdinalFunction_iff
    {ℳ : Structure.{u}} (hExt : Extensional ℳ)
    {parameterCount depth : Nat} (env : Env ℳ depth) (function : BinarySchema parameterCount) (parameters : TermVector parameterCount depth) :
    satisfies env (isNormalOrdinalFunction function parameters) ↔
      ℳ.IsNormalOrdinalFunction (function.denote (parameters.evalEnv env)) := by
