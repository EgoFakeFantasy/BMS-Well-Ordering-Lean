...
def IsOrdinalMaximum {ℳ : Structure.{u}} (left right maximum : ℳ.Domain) : Prop := (maximum = left ∧ (right = left ∨ ℳ.mem right left)) ∨
    (maximum = right ∧ ℳ.mem left right)
/-- 两个有序数坐标编码之间的典范次序。 -/
def CanonicalOrdinalPairLess {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (first second : ℳ.Domain) : Prop :=
    ...
    𝕀.Codes first firstLeft firstRight ∧
      𝕀.Codes second secondLeft secondRight ∧
        ℳ.IsOrdinalMaximum firstLeft firstRight firstMaximum ∧
          ℳ.IsOrdinalMaximum secondLeft secondRight secondMaximum ∧ (ℳ.mem firstMaximum secondMaximum ∨ (firstMaximum = secondMaximum ∧
                (ℳ.mem firstLeft secondLeft ∨ (firstLeft = secondLeft ∧
...
def canonicalOrdinalPairLess (𝒞 : OrderedPairConvention) {depth : Nat} (first second : Term depth) : Formula 1 depth :=
  .existsE <| .existsE <| .existsE <| .existsE <| .existsE <| .existsE <|
    .conj (𝒞.code first.weaken.weaken.weaken.weaken.weaken.weaken (.bound 5) (.bound 4)) <|
    .conj (𝒞.code second.weaken.weaken.weaken.weaken.weaken.weaken (.bound 3) (.bound 2)) <|
    .conj (isOrdinalMaximum (.bound 5) (.bound 4) (.bound 1)) <|
...
theorem satisfies_isOrdinalMaximum_iff
    ...
    {depth : Nat} (env : Env ℳ depth) (left right maximum : Term depth) :
    satisfies env (isOrdinalMaximum left right maximum) ↔
      ℳ.IsOrdinalMaximum (left.eval env) (right.eval env) (maximum.eval env) := by
...
theorem denote_canonicalOrdinalPairLess_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 0) (first second : ℳ.Domain) :
...
theorem denote_orderedPairFirst_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (env : Env ℳ 0) (pair first : ℳ.Domain) :
...
theorem denote_orderedPairSecond_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (env : Env ℳ 0) (pair second : ℳ.Domain) :
...
theorem denote_ordinalPairMaximum_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 0) (pair maximum : ℳ.Domain) :
...
theorem satisfies_hasOrdinalPairMaximum_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 1) (pair : ℳ.Domain) :
    satisfies (env.push pair) (UnarySchema.hasOrdinalPairMaximum 𝒞).body ↔
      ∃ left right,
        𝕀.Codes pair left right ∧
...
theorem satisfies_hasOrderedPairFirst_iff
    ...
    satisfies (env.push pair) (UnarySchema.hasOrderedPairFirst 𝒞).body ↔
...
theorem exists_of_mem
    {ℳ : Structure.{u}} (hExt : Extensional ℳ)
    {α left right : ℳ.Domain} (hα : ℳ.IsOrdinal α) (hLeft : ℳ.mem left α) (hRight : ℳ.mem right α) :
...
/-- 最大值仍是同一序数载体的成员。 -/
theorem mem
    {ℳ : Structure.{u}} {α left right maximum : ℳ.Domain} (hMaximum : ℳ.IsOrdinalMaximum left right maximum) (hLeft : ℳ.mem left α) (hRight : ℳ.mem right α) :
    ℳ.mem maximum α := by
...
theorem eq
    {ℳ : Structure.{u}}
    {α left right first second : ℳ.Domain} (hα : ℳ.IsOrdinal α) (hLeft : ℳ.mem left α) (hRight : ℳ.mem right α) (hFirst : ℳ.IsOrdinalMaximum left right first)
...
theorem exists_canonicalOrdinalPairRelation
    ...
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (carrier : ℳ.Domain) :
    ∃ relation,
  ...
  rcases exists_setRelationOn_of_denote hZF 𝕀 (Definitional.Project.BinarySchema.canonicalOrdinalPairLess 𝒞)
      env carrier with
...
def IsCanonicalOrdinalPairOrder {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (relation carrier α : ℳ.Domain) : Prop :=
...
private theorem pairMember_iff
    ...
    {relation carrier α first second
      firstLeft firstRight secondLeft secondRight : ℳ.Domain} (hOrder : ℳ.IsCanonicalOrdinalPairOrder 𝕀 relation carrier α)
    (hFirstCode : 𝕀.Codes first firstLeft firstRight) (hSecondCode : 𝕀.Codes second secondLeft secondRight) (hFirst : ℳ.mem first carrier)
...
theorem isSetCodedLinearOrder
    ...
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ}
    {relation carrier α : ℳ.Domain} (hα : ℳ.IsOrdinal α) (hOrder : ℳ.IsCanonicalOrdinalPairOrder 𝕀 relation carrier α) :
    ℳ.IsSetCodedLinearOrder 𝕀 relation carrier := by
  have coordinate_data
      {pair : ℳ.Domain} (hPair : ℳ.mem pair carrier) :
      ∃ left, ℳ.mem left α ∧
        ∃ right, ℳ.mem right α ∧ 𝕀.Codes pair left right := (hOrder.1 pair).mp hPair
  ...
  · intro pair hPair hSelf
    ...
    · exact hα.wellOrder.linear.irrefl firstMaximum (hFirstMaximum.mem hLeft hRight) hMaximumSelf
  ...
  · intro first hFirst second hSecond third hThird hFirstSecond hSecondThird
    ...
    · rcases hSecondLess with hSecondMaximumLess | ⟨hSecondMaximumEq, _⟩
      · exact Or.inl <| hα.wellOrder.linear.trans
          ...
          firstSecondMaximum (hFirstSecondMaximum.mem hSecondLeft hSecondRight)
  ...
  · intro first hFirst second hSecond
    ...
    · have hMaximumEq :=
      ...
      · have hLeftEq := hExt.eq_of_same_members firstLeft secondLeft hSameLeft
        ...
        · exact Or.inr <| Or.inl <| (pairMember_iff hOrder hFirstCode hSecondCode hFirst hSecond).mpr
              ⟨firstMaximum, firstMaximum,
                hFirstMaximum, hSecondMaximum, Or.inr
                  ⟨rfl, Or.inr ⟨rfl, hFirstRightLess⟩⟩⟩
        · exact Or.inr <| Or.inr <| (pairMember_iff hOrder hSecondCode hFirstCode hSecond hFirst).mpr
              ⟨firstMaximum, firstMaximum,
...
theorem isSetCodedWellOrder
  ...
  have hMaximumTotal :
      ∀ pair, ℳ.mem pair subset →
        ∃ maximum, (Definitional.Project.BinarySchema.ordinalPairMaximum 𝒞).denote
    ...
    rcases Structure.IsOrdinalMaximum.exists_of_mem hZF.1 hα
        hLeft hRight with
      ⟨maximum, hMaximum⟩
    exact ⟨maximum, (Definitional.Project.Formula.denote_ordinalPairMaximum_iff
        𝕀 hZF.1 emptyEnv pair maximum).mpr
        ⟨left, right, hCode, hMaximum⟩⟩
  have hMaximumUnique :
      ∀ pair, ℳ.mem pair subset → ∀ first second, (Definitional.Project.BinarySchema.ordinalPairMaximum 𝒞).denote
            emptyEnv pair first → (Definitional.Project.BinarySchema.ordinalPairMaximum 𝒞).denote
  ...
  rcases ZF.exists_functionalImageOn hZF (Definitional.Project.BinarySchema.ordinalPairMaximum 𝒞)
  ...
  rcases ZF.separation_exists_d hZF (Definitional.Project.UnarySchema.hasOrdinalPairMaximum 𝒞)
  ...
  have hFirstTotal :
      ∀ pair, ℳ.mem pair maximumPairs →
        ∃ first, (Definitional.Project.BinarySchema.orderedPairFirst 𝒞).denote
            emptyEnv pair first := by
    intro pair hPair
    rcases (hMaximumPairs pair).mp hPair with
      ⟨_, left, right, hCode, _⟩
    exact ⟨left, (Definitional.Project.Formula.denote_orderedPairFirst_iff
        𝕀 emptyEnv pair left).mpr ⟨right, hCode⟩⟩
  have hFirstUnique :
      ∀ pair, ℳ.mem pair maximumPairs → ∀ first second, (Definitional.Project.BinarySchema.orderedPairFirst 𝒞).denote
            emptyEnv pair first → (Definitional.Project.BinarySchema.orderedPairFirst 𝒞).denote
            emptyEnv pair second →
          first = second := by
  ...
  rcases ZF.exists_functionalImageOn hZF (Definitional.Project.BinarySchema.orderedPairFirst 𝒞)
  ...
  have hFirstsSubset : ℳ.MemberSubset firsts α := by
    ...
    rcases coordinate_data (hSubset pair ((hMaximumPairs pair).mp hPair).1) with
  ...
  rcases ZF.separation_exists_d hZF (Definitional.Project.UnarySchema.hasOrderedPairFirst 𝒞)
  ...
  have hSecondTotal :
      ∀ pair, ℳ.mem pair firstPairs →
        ∃ second, (Definitional.Project.BinarySchema.orderedPairSecond 𝒞).denote
            emptyEnv pair second := by
    intro pair hPair
    rcases (hFirstPairs pair).mp hPair with
      ⟨_, second, hCode⟩
    exact ⟨second, (Definitional.Project.Formula.denote_orderedPairSecond_iff
        𝕀 emptyEnv pair second).mpr ⟨leastFirst, hCode⟩⟩
  have hSecondUnique :
      ∀ pair, ℳ.mem pair firstPairs → ∀ first second, (Definitional.Project.BinarySchema.orderedPairSecond 𝒞).denote
            emptyEnv pair first → (Definitional.Project.BinarySchema.orderedPairSecond 𝒞).denote
            emptyEnv pair second →
          first = second := by
    intro pair _ first second hFirst hSecond
  ...
  rcases ZF.exists_functionalImageOn hZF (Definitional.Project.BinarySchema.orderedPairSecond 𝒞)
  ...
  have hSecondsSubset : ℳ.MemberSubset seconds α := by
    ...
    have hPairSubset := (hMaximumPairs pair).mp ((hFirstPairs pair).mp hPair).1 |>.1
  ...
  have hValueMaximumMember : ℳ.mem valueMaximum maximums := (hMaximums valueMaximum).mpr
      ⟨value, hValueSubset, (Definitional.Project.Formula.denote_ordinalPairMaximum_iff
  ...
  · have hMaximumEq :=
    ...
    subst valueMaximum
    have hValueMaximumPairs : ℳ.mem value maximumPairs := (hMaximumPairs value).mpr
        ⟨hValueSubset, valueLeft, valueRight,
          hValueCode, hValueMaximum⟩
    have hValueLeftMember : ℳ.mem valueLeft firsts := (hFirsts valueLeft).mpr
        ⟨value, hValueMaximumPairs, (Definitional.Project.Formula.denote_orderedPairFirst_iff
    ...
    · have hFirstEq :=
      ...
      subst valueLeft
      have hValueFirstPairs : ℳ.mem value firstPairs := (hFirstPairs value).mpr
          ⟨hValueMaximumPairs, valueRight, hValueCode⟩
      have hValueRightMember : ℳ.mem valueRight seconds := (hSeconds valueRight).mpr
          ⟨value, hValueFirstPairs, (Definitional.Project.Formula.denote_orderedPairSecond_iff
      ...
      · have hSecondEq :=
        ...
        exact Or.inl <| by
          intro member
          simp [hPairEq]
      · exact Or.inr <| (pairMember_iff hOrder hCandidateCode hValueCode
...
theorem predecessorSet_boundedByMaximumSquare
    ...
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ}
    {relation carrier α current predecessors : ℳ.Domain} (hα : ℳ.IsOrdinal α) (hOrder : ℳ.IsCanonicalOrdinalPairOrder 𝕀 relation carrier α)
    (hCurrent : ℳ.mem current carrier) (hPredecessors :
  ...
  have coordinate_mem_successor
      {coordinate : ℳ.Domain} (hCoordinate :
...
theorem exists_canonicalOrdinalPairLinearOrder
    ...
    {α carrier : ℳ.Domain} (hα : ℳ.IsOrdinal α) (hCarrier : ℳ.IsCartesianProduct 𝕀 carrier α α) :
    ∃ relation,
...
def IsCanonicalOrdinalPairOrderType {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (ordinal α : ℳ.Domain) : Prop :=
  ∃ carrier relation,
    ℳ.IsCanonicalOrdinalPairOrder 𝕀 relation carrier α ∧
...
theorem predecessorSet_equinumerous
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {relation carrier current value : ℳ.Domain} (hOrder : ℳ.IsSetCodedWellOrder 𝕀 relation carrier) (hValue :
  ...
  have hInjective : ℳ.IsSetInjective 𝕀 function := by
    ...
    have hFirstDomain :
        ℳ.mem first predecessors := (hFunction.2.1 first).mpr ⟨output, hFirst⟩
    have hSecondDomain :
        ℳ.mem second predecessors := (hFunction.2.1 second).mpr ⟨output, hSecond⟩
    have hFirstCarrier :=
    ...
    · have hSelf : ℳ.mem output output := (hFunction.2.2.2 second hSecondDomain
          output hSecond output).mpr
          ⟨first, hFirstDomain, hFirstSecond, hFirst⟩
      exact False.elim <|
        KP.mem_irrefl_d (ZF.modelsKP hZF) output hSelf
    · have hSelf : ℳ.mem output output := (hFunction.2.2.2 first hFirstDomain
...
theorem canonicalOrdinalPairOrderType_existsUnique
  ...
  rcases wellOrderType_existsUnique hZF 𝕀 hWellOrder with
    ⟨selected, _, hUnique⟩
  exact (hUnique other hOtherType).trans (hUnique ordinal hType).symm
end ZF
...
theorem isOrdinal
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ordinal α : ℳ.Domain} (hType : ℳ.IsCanonicalOrdinalPairOrderType 𝕀 ordinal α) :
...
theorem member_equinumerous_predecessorSet_boundedByMaximumSquare
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {ordinal α member : ℳ.Domain} (hα : ℳ.IsOrdinal α) (hType : ℳ.IsCanonicalOrdinalPairOrderType 𝕀 ordinal α) (hMember : ℳ.mem member ordinal) :
  ...
  have hCurrent :
      ℳ.mem current carrier := (hFunction.2.1 current).mpr ⟨member, hCurrentValue⟩
  have hCollapseValue :=
