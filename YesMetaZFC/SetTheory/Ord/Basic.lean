...
namespace Structure
/-- `α` 是传递且由隶属关系良序的对象。 -/
structure IsOrdinal (ℳ : SetTheory.Structure.{u}) (α : ℳ.Domain) : Prop where
  transitive : ℳ.TransitiveSet α
  wellOrder : ℳ.MembershipWellOrder α
namespace IsOrdinal
/-- 传递子集继承序数上的隶属良序。 -/
theorem of_transitive_subset {ℳ : SetTheory.Structure.{u}}
    {α subset : ℳ.Domain} (hα : IsOrdinal ℳ α) (hSubset : ℳ.MemberSubset subset α) (hTransitive : ℳ.TransitiveSet subset) :
  ...
  · intro part hPartSubset hNonempty
    exact hα.wellOrder.least part (fun value hValue => hSubset value (hPartSubset value hValue))
      hNonempty
/--
没有成员的对象满足序数定义。
...
theorem of_no_members {ℳ : SetTheory.Structure.{u}} {empty : ℳ.Domain} (hEmpty : ∀ value, ¬ ℳ.mem value empty) :
  ...
  · refine ⟨?_, ?_⟩
    · exact {
        irrefl := fun value hValue =>
          False.elim (hEmpty value hValue)
        trans := fun left hLeft =>
          False.elim (hEmpty left hLeft)
        compare := fun left hLeft =>
          False.elim (hEmpty left hLeft)
      }
...
-/
theorem mem {ℳ : SetTheory.Structure.{u}} {α β : ℳ.Domain} (hα : IsOrdinal ℳ α) (hβ : ℳ.mem β α) :
    IsOrdinal ℳ β := by
...
theorem mem_of_properSubset {ℳ : SetTheory.Structure.{u}}
    {α β : ℳ.Domain} (hExt : Extensional ℳ) (hα : IsOrdinal ℳ α) (hβ : IsOrdinal ℳ β) (hProper : ℳ.MemberSubset α β ∧
      ¬ ℳ.SameMembers α β) (hDifference : ∃ difference, ∀ value,
  ...
  have hNonempty : ∃ value, ℳ.mem value difference := by
    ...
    · intro hValueβ
      apply Classical.byContradiction
      intro hValueα
      exact hEmpty ⟨value, (hDifference value).2 ⟨hValueβ, hValueα⟩⟩
  ...
  have hγβ : ℳ.mem γ β := (hDifference γ).1 hγDifference |>.1
  have hγNotα : ¬ ℳ.mem γ α := (hDifference γ).1 hγDifference |>.2
  have hSameMembers : ℳ.SameMembers α γ := by
    ...
    · intro hValueα
      ...
      · exact hValueγ
      · exact False.elim (hγNotα (hα.transitive value hValueα γ hγValue))
    · intro hValueγ
      have hValueβ :=
        hβ.transitive γ hγβ value hValueγ
      apply Classical.byContradiction
      intro hValueα
      have hValueDifference := (hDifference value).2 ⟨hValueβ, hValueα⟩
  ...
  simpa [hEq] using hγβ
/-- 任意两个序数相等，或其中一个属于另一个。 -/
theorem trichotomy {ℳ : SetTheory.Structure.{u}} {α β : ℳ.Domain} (hExt : Extensional ℳ) (hα : IsOrdinal ℳ α) (hβ : IsOrdinal ℳ β)
    (hDifference : ∀ left right, ∃ difference, ∀ value,
      ℳ.mem value difference ↔
        ℳ.mem value right ∧ ¬ ℳ.mem value left) (hIntersection : ∃ intersection, ∀ value,
  ...
  · have hIntersectionMemα :=
      mem_of_properSubset hExt hIntersectionOrdinal hα
        ⟨hIntersectionα, hSameα⟩ (hDifference intersection α)
    ...
    · have hEqβ :=
      ...
      · exact Or.inr (Or.inr <|
          mem_of_properSubset hExt hβ hα
            ⟨hβSubsetα,
              fun h => hSame (fun value => (h value).symm)⟩ (hDifference β α))
    · have hIntersectionMemβ :=
        mem_of_properSubset hExt hIntersectionOrdinal hβ
          ⟨hIntersectionβ, hSameβ⟩ (hDifference intersection β)
      have hSelf : ℳ.mem intersection intersection := (hIntersection intersection).2
          ⟨hIntersectionMemα, hIntersectionMemβ⟩
      exact False.elim (hIntersectionOrdinal.wellOrder.linear.irrefl
          intersection hSelf hSelf)
/-- 任一空对象都属于每个非空序数。 -/
theorem empty_mem_of_nonempty
    {ℳ : SetTheory.Structure.{u}} (hKP : ℳ.Models SetTheory.KP)
    {α empty : ℳ.Domain} (hα : IsOrdinal ℳ α) (hNonempty : ∃ value, ℳ.mem value α) (hEmpty : ∀ value, ¬ ℳ.mem value empty) :
    ℳ.mem empty α := by
  have hEmptyOrdinal : IsOrdinal ℳ empty :=
    of_no_members hEmpty
  rcases trichotomy hKP.1 hEmptyOrdinal hα (KP.difference_exists_d hKP) (KP.intersection_exists_d hKP empty α) with
...
/-- 序数族的并仍是序数。 -/
theorem of_union {ℳ : SetTheory.Structure.{u}} (hKP : ℳ.Models SetTheory.KP)
    {union family : ℳ.Domain} (hUnion : ℳ.IsUnionOf union family) (hFamily : ∀ member, ℳ.mem member family → ℳ.IsOrdinal member) :
  ...
  · intro middle hMiddle value hValue
    ...
    exact (hUnion value).mpr
      ⟨member, hMemberFamily, (hFamily member hMemberFamily).transitive
  ...
  · refine ⟨?_, ?_⟩
    · refine {
      ...
      · intro left hLeft right hRight
        exact trichotomy hKP.1 (memberOrdinal hLeft) (memberOrdinal hRight) (KP.difference_exists_d hKP) (KP.intersection_exists_d hKP left right)
    · intro subset hSubset hNonempty
      ...
      rcases KP.intersection_exists_d hKP subset container with
      ...
      have hCandidateData := (hIntersection candidate).mp hCandidateIntersection
      ...
      rcases trichotomy hKP.1
          hCandidateOrdinal hValueOrdinal (KP.difference_exists_d hKP) (KP.intersection_exists_d hKP candidate value) with
      ...
      · have hValueContainer : ℳ.mem value container :=
        ...
        have hValueIntersection : ℳ.mem value intersection := (hIntersection value).mpr ⟨hValueSubset, hValueContainer⟩
...
theorem subsetMem :
  ...
  rw [Definitional.Project.Formula.satisfies_isOrdinal_iff] at h
  rw [Definitional.Project.Formula.proper_subset_sat_iff_d] at h
...
theorem trichotomy :
  ...
  have hCompare := Structure.IsOrdinal.trichotomy hKP.1
    h.1 h.2 (KP.difference_exists_d hKP) (KP.intersection_exists_d hKP α β)
