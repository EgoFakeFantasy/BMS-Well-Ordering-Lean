...
theorem ordinalClassFunction_of_progressive
    ...
    {parameterCount : Nat} (env : Env ℳ parameterCount) (function : Definitional.Project.BinarySchema parameterCount)
    (hClass : ℳ.IsClassFunctionOnOrdinals (function.denote env)) (hProgressive : ∀ α, ℳ.IsOrdinal α → (∀ predecessor, ℳ.mem predecessor α →
...
theorem increasingOnOrdinals_of_progressive
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (function : Definitional.Project.BinarySchema parameterCount) (hProgressive : ∀ α, ℳ.IsOrdinal α →
      (∀ predecessor, ℳ.mem predecessor α →
  ...
  · intro current hCurrent hPrevious currentValue hCurrentValue
    ...
    · by_cases hValueEmpty : ∀ member, ¬ ℳ.mem member currentValue
      ...
      · have hValueNonempty : ∃ member, ℳ.mem member currentValue := by
        ...
        exact Or.inr <|
          hCurrentValueOrdinal.empty_mem_of_nonempty (ZF.modelsKP hZF) hValueNonempty hEmpty
    · rcases hSuccessor with
      ...
      · exact Or.inr <| Structure.IsOrdinal.mem_of_properSubset
          ...
          ⟨hSubset, hSame⟩ (KP.difference_exists_d (ZF.modelsKP hZF)
    ...
    · rcases hNormal.2.2 current currentValue
      ...
      have hSubset : ℳ.MemberSubset current currentValue := by
        ...
        rcases hNormal.1.1 member (hLimit.1.mem hMember) with
        ...
        have hMemberValueLarger :
          ...
          hNormal.2.1 member larger (hLimit.1.mem hMember) (hLimit.1.mem hLarger)
        ...
        have hMemberLargerValue : ℳ.mem member largerValue := by
          ...
          · have hLargerValueOrdinal :=
              hNormal.1.2 larger largerValue (hLimit.1.mem hLarger) hLargerValue
...
theorem agreeOnOrdinals_of_progressive
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (first second : Definitional.Project.BinarySchema parameterCount) (hProgressive : ∀ α, ℳ.IsOrdinal α →
      (∀ predecessor, ℳ.mem predecessor α →
        ∀ firstValue secondValue,
  ...
  let property : ℳ.Domain → Prop := fun current =>
    ∀ firstValue secondValue,
      ...
      second.denote env current secondValue →
        firstValue = secondValue
  apply hα.induction property
  · rcases separation_exists_d hZF (first.agreeAt second).neg env α with
      ⟨counterexamples, hCounterexamples⟩
    refine ⟨counterexamples, fun current => ?_⟩
    rw [hCounterexamples current]
    simp [property, Definitional.Project.UnarySchema.neg,
