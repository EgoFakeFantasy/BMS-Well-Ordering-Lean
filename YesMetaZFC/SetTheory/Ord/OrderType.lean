...
structure IsSetCodedWellOrder {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (relation carrier : ℳ.Domain) : Prop where
  linear : ℳ.IsSetCodedLinearOrder 𝕀 relation carrier
  least :
    ∀ subset, ℳ.MemberSubset subset carrier → (∃ value, ℳ.mem value subset) →
...
def IsPredecessorSet {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (predecessors relation carrier current : ℳ.Domain) : Prop :=
  ∀ value, ℳ.mem value predecessors ↔
    ℳ.mem value carrier ∧
      ℳ.PairMember 𝕀 value current relation
def IsRelationInitialSegment {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (segment relation carrier : ℳ.Domain) : Prop :=
...
def IsWellOrderCollapseFunction {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (function relation carrier domain : ℳ.Domain) : Prop :=
...
def IsWellOrderCollapseValue {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (relation carrier current value : ℳ.Domain) : Prop :=
...
def isWellOrderCollapseFunction (𝒞 : OrderedPairConvention)
    ...
    Formula 1 depth :=
  .conj (isFunction 𝒞 function) <| .conj (isDomain 𝒞 domain function) <| .conj (isRelationInitialSegment 𝒞 domain relation carrier) <|
      forallMem domain <| .forallE <| .imp (orderedPairMem 𝒞 (.bound 1) Term.newest
          function.weaken.weaken) <|
        .forallE <| .iff (.mem Term.newest (.bound 1)) <|
          existsMem domain.weaken.weaken.weaken <| .conj (orderedPairMem 𝒞 Term.newest (.bound 3)
...
def isWellOrderCollapseValue (𝒞 : OrderedPairConvention)
    ...
    Formula 1 depth :=
  .existsE <| .existsE <| .conj (isPredecessorSet 𝒞 (.bound 1)
...
theorem satisfies_isPredecessorSet_iff
    ...
    satisfies env (isPredecessorSet 𝒞
          predecessors relation carrier current) ↔
      ℳ.IsPredecessorSet 𝕀 (predecessors.eval env) (relation.eval env) (carrier.eval env) (current.eval env) := by
...
theorem satisfies_isSetCodedWellOrder_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {depth : Nat} (env : Env ℳ depth) (relation carrier : Term depth) :
    satisfies env (isSetCodedWellOrder 𝒞 relation carrier) ↔
      ℳ.IsSetCodedWellOrder 𝕀 (relation.eval env) (carrier.eval env) := by
  ...
  · rintro ⟨hRelation, hWellOrder⟩
    ...
    have hLinearSemantic :
        ℳ.IsSetCodedLinearOrder 𝕀 (relation.eval env) (carrier.eval env) := by
      apply (satisfies_isSetCodedLinearOrder_iff
              ...
              ∃ candidate, ℳ.mem candidate subset ∧
                ∀ other, ℳ.mem other subset →
                  ℳ.SameMembers candidate other ∨
                    ℳ.PairMember 𝕀 candidate other (relation.eval env) := by
...
theorem satisfies_isRelationInitialSegment_iff
    ...
    satisfies env (isRelationInitialSegment 𝒞 segment relation carrier) ↔
      ℳ.IsRelationInitialSegment 𝕀 (segment.eval env) (relation.eval env) (carrier.eval env) := by
...
theorem satisfies_isWellOrderCollapseFunction_iff
    ...
    {depth : Nat} (env : Env ℳ depth) (function relation carrier domain : Term depth) :
    satisfies env (isWellOrderCollapseFunction 𝒞
          function relation carrier domain) ↔
      ℳ.IsWellOrderCollapseFunction 𝕀 (function.eval env) (relation.eval env) (carrier.eval env) (domain.eval env) := by
...
theorem satisfies_isWellOrderCollapseValue_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ)
    {depth : Nat} (env : Env ℳ depth) (relation carrier current value : Term depth) :
    satisfies env (isWellOrderCollapseValue 𝒞
          relation carrier current value) ↔
      ℳ.IsWellOrderCollapseValue 𝕀 (relation.eval env) (carrier.eval env) (current.eval env) (value.eval env) := by
...
theorem denote_transportedOrdinalMembership_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (env : Env ℳ 1) (left right : ℳ.Domain) :
...
theorem satisfies_wellOrderPredecessorMembership_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (env : Env ℳ 2) (value : ℳ.Domain) :
    satisfies (env.push value) (UnarySchema.wellOrderPredecessorMembership 𝒞).body ↔
      ℳ.PairMember 𝕀 value (env.bound 1) (env.bound 0) := by
  change
    satisfies (env.push value) (Formula.orderedPairMem 𝒞
          Term.newest (.bound 2) (.bound 1)) ↔
...
theorem satisfies_wellOrderRangePreimageMembership_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (env : Env ℳ 2) (input : ℳ.Domain) :
    satisfies (env.push input) (UnarySchema.wellOrderRangePreimageMembership 𝒞).body ↔
      ∃ output,
        ℳ.mem output (env.bound 1) ∧
          ℳ.PairMember 𝕀 input output (env.bound 0) := by
  change
    satisfies (env.push input) (Formula.existsMem (.bound 2) (Formula.orderedPairMem 𝒞 (.bound 1) Term.newest (.bound 2))) ↔
      ∃ output,
        ℳ.mem output (env.bound 1) ∧
...
theorem denote_wellOrderCollapseValue_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 2) (current value : ℳ.Domain) :
    (BinarySchema.wellOrderCollapseValue 𝒞).denote
        env current value ↔
      ℳ.IsWellOrderCollapseValue 𝕀 (env.bound 0) (env.bound 1) current value := by
  change
    satisfies ((env.push current).push value) (Formula.isWellOrderCollapseValue 𝒞 (.bound 2) (.bound 3) (.bound 1) (.bound 0)) ↔
      ℳ.IsWellOrderCollapseValue 𝕀 (env.bound 0) (env.bound 1) current value
  rw [satisfies_isWellOrderCollapseValue_iff 𝕀 hExt]
  rfl
/-- 坍缩值存在唯一模式的纸面语义。 -/
theorem satisfies_wellOrderCollapseValueExistsUnique_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ) (env : Env ℳ 2) (current : ℳ.Domain) :
    satisfies (env.push current) (UnarySchema.wellOrderCollapseValueExistsUnique 𝒞).body ↔
      ∃ value,
        ℳ.IsWellOrderCollapseValue 𝕀 (env.bound 0) (env.bound 1) current value ∧
          ∀ other,
  ...
  change ((∃ value,
        ℳ.IsWellOrderCollapseValue 𝕀 (env.bound 0) (env.bound 1) current value) ∧
      ∀ first second,
        ℳ.IsWellOrderCollapseValue 𝕀 (env.bound 0) (env.bound 1) current first →
          ℳ.IsWellOrderCollapseValue 𝕀 (env.bound 0) (env.bound 1) current second →
            first = second) ↔
      ∃ value,
        ℳ.IsWellOrderCollapseValue 𝕀 (env.bound 0) (env.bound 1) current value ∧
          ∀ other,
...
theorem irrefl {ℳ : Structure.{u}}
    ...
    {relation carrier value : ℳ.Domain} (hOrder : ℳ.IsSetCodedWellOrder 𝕀 relation carrier) (hValue : ℳ.mem value carrier) :
...
theorem trans {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ}
    {relation carrier left middle right : ℳ.Domain} (hOrder : ℳ.IsSetCodedWellOrder 𝕀 relation carrier) (hLeft : ℳ.mem left carrier)
    (hMiddle : ℳ.mem middle carrier) (hRight : ℳ.mem right carrier) (hLeftMiddle : ℳ.PairMember 𝕀 left middle relation)
...
theorem compare {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ}
    {relation carrier left right : ℳ.Domain} (hOrder : ℳ.IsSetCodedWellOrder 𝕀 relation carrier) (hLeft : ℳ.mem left carrier) (hRight : ℳ.mem right carrier) :
...
theorem induction
    ...
    {relation carrier : ℳ.Domain} (hOrder : ℳ.IsSetCodedWellOrder 𝕀 relation carrier) (property : ℳ.Domain → Prop) (hCounterexamples :
      ∃ counterexamples, ∀ value,
        ℳ.mem value counterexamples ↔
          ℳ.mem value carrier ∧ ¬ property value) (hProgressive :
      ∀ current, ℳ.mem current carrier → (∀ predecessor, ℳ.mem predecessor carrier →
          ℳ.PairMember 𝕀 predecessor current relation →
  ...
  have hCurrentCounterexample :
      ℳ.mem current counterexamples := (hCounterexamples current).mpr
  ...
  have hLeastData := (hCounterexamples least).mp hLeastCounterexample
  apply hLeastData.2
  ...
  have hPredecessorCounterexample :
      ℳ.mem predecessor counterexamples := (hCounterexamples predecessor).mpr
...
theorem eq {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ} (hExt : Extensional ℳ)
    {left right relation carrier current : ℳ.Domain} (hLeft :
      ℳ.IsPredecessorSet 𝕀 left relation carrier current) (hRight :
...
theorem isInitialSegment {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ}
    {predecessors relation carrier current : ℳ.Domain} (hOrder : ℳ.IsSetCodedWellOrder 𝕀 relation carrier) (hCurrent : ℳ.mem current carrier) (hPredecessors :
      ℳ.IsPredecessorSet 𝕀
        predecessors relation carrier current) :
  ...
  · intro current' hCurrent' predecessor hPredecessor hPair
    apply (hPredecessors predecessor).mpr
    refine ⟨hPredecessor, ?_⟩
    have hCurrentCarrier := (hPredecessors current').mp hCurrent' |>.1
    exact hOrder.trans hPredecessor hCurrentCarrier hCurrent
      hPair ((hPredecessors current').mp hCurrent').2
end Structure.IsPredecessorSet
namespace Structure.IsRelationInitialSegment
/-- 整个载体是自身的向下闭初段。 -/
theorem refl {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (relation carrier : ℳ.Domain) :
    ℳ.IsRelationInitialSegment 𝕀 carrier relation carrier := by
  refine ⟨?_, ?_⟩
  · intro value hValue
    exact hValue
  · prove_auto
...
theorem exists_wellOrderPredecessorSet
    ...
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (relation carrier current : ℳ.Domain) :
    ∃ predecessors,
      ℳ.IsPredecessorSet 𝕀
  ...
  rcases separation_exists_d hZF (Definitional.Project.UnarySchema.wellOrderPredecessorMembership 𝒞)
      env carrier with
...
theorem restriction
    ...
    {function relation carrier domain current predecessors
      restricted : ℳ.Domain} (hOrder :
      ℳ.IsSetCodedWellOrder 𝕀 relation carrier) (hFunction :
      ℳ.IsWellOrderCollapseFunction 𝕀
        function relation carrier domain) (hCurrent : ℳ.mem current domain) (hPredecessors :
      ℳ.IsPredecessorSet 𝕀
        predecessors relation carrier current) (hRestriction :
  ...
  have hValueFunction := ((hRestriction.2 index value).mp hValue).2
  ...
  · rintro ⟨predecessor, hPredecessorDomain,
    ...
    have hPredecessorPredecessors := (hPredecessors predecessor).mpr
        ⟨hPredecessorCarrier, hPredecessorCurrent⟩
    exact ⟨predecessor, hPredecessorPredecessors,
      hPredecessorIndex, (hRestriction.2 predecessor member).mpr
  ...
  · rintro ⟨predecessor, hPredecessorPredecessors,
    ...
    exact ⟨predecessor,
      ...
      hPredecessorIndex, ((hRestriction.2 predecessor member).mp
        hPredecessorValue).2⟩
/-- 坍缩函数在定义域内一点的函数值正是该点的坍缩值。 -/
theorem collapseValue_of_pairMember
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {function relation carrier domain current value : ℳ.Domain} (hOrder :
      ℳ.IsSetCodedWellOrder 𝕀 relation carrier) (hFunction :
      ℳ.IsWellOrderCollapseFunction 𝕀
        function relation carrier domain) (hCurrent : ℳ.mem current domain) (hValue : ℳ.PairMember 𝕀 current value function) :
  ...
  · rintro ⟨predecessor, hPredecessorDomain,
    ...
    have hPredecessorCarrier :=
      hFunction.2.2.1.1 predecessor hPredecessorDomain
    exact ⟨predecessor, (hRestriction.2 predecessor member).mpr
        ⟨(hPredecessors predecessor).mpr
          ⟨hPredecessorCarrier, hPredecessorCurrent⟩,
          hPredecessorValue⟩⟩
  · rintro ⟨predecessor, hPredecessorValue⟩
    have hPredecessorData := (hRestriction.2 predecessor member).mp hPredecessorValue
    have hPredecessorSetData := (hPredecessors predecessor).mp hPredecessorData.1
    exact ⟨predecessor,
...
theorem eq_of_collapseValue_unique
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {left right relation carrier domain : ℳ.Domain} (hOrder :
      ℳ.IsSetCodedWellOrder 𝕀 relation carrier) (hLeft :
      ℳ.IsWellOrderCollapseFunction 𝕀
        left relation carrier domain) (hRight :
      ℳ.IsWellOrderCollapseFunction 𝕀
        right relation carrier domain) (hUnique :
      ∀ current, ℳ.mem current domain →
        ∀ first second,
          ℳ.IsWellOrderCollapseValue 𝕀
  ...
  intro current value
  have transfer
      {first second : ℳ.Domain} (hFirst :
        ℳ.IsWellOrderCollapseFunction 𝕀
          first relation carrier domain) (hSecond :
        ℳ.IsWellOrderCollapseFunction 𝕀
          second relation carrier domain) (hPair : ℳ.PairMember 𝕀 current value first) :
      ℳ.PairMember 𝕀 current value second := by
    have hCurrent :
        ℳ.mem current domain := (hFirst.2.1 current).mpr ⟨value, hPair⟩
...
theorem isSetInjective
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {function relation carrier : ℳ.Domain} (hOrder :
      ℳ.IsSetCodedWellOrder 𝕀 relation carrier) (hFunction :
      ℳ.IsWellOrderCollapseFunction 𝕀
  ...
  have hFirstCarrier :
      ℳ.mem first carrier := (hFunction.2.1 first).mpr ⟨output, hFirst⟩
  have hSecondCarrier :
      ℳ.mem second carrier := (hFunction.2.1 second).mpr ⟨output, hSecond⟩
  ...
  · have hSelf : ℳ.mem output output := (hFunction.2.2.2 second hSecondCarrier
        output hSecond output).mpr
        ⟨first, hFirstCarrier, hFirstSecond, hFirst⟩
    exact False.elim <|
      KP.mem_irrefl_d (ZF.modelsKP hZF) output hSelf
  · have hSelf : ℳ.mem output output := (hFunction.2.2.2 first hFirstCarrier
...
theorem relation_iff_mem
    ...
    {function relation carrier first second firstValue secondValue :
      ...
      ℳ.IsSetCodedWellOrder 𝕀 relation carrier) (hFunction :
      ℳ.IsWellOrderCollapseFunction 𝕀
        function relation carrier carrier) (hFirst :
      ℳ.PairMember 𝕀 first firstValue function) (hSecond :
      ℳ.PairMember 𝕀 second secondValue function) :
    ℳ.PairMember 𝕀 first second relation ↔
      ℳ.mem firstValue secondValue := by
  have hFirstCarrier :
      ℳ.mem first carrier := (hFunction.2.1 first).mpr ⟨firstValue, hFirst⟩
  have hSecondCarrier :
      ℳ.mem second carrier := (hFunction.2.1 second).mpr ⟨secondValue, hSecond⟩
  constructor
  · intro hFirstSecond
    exact (hFunction.2.2.2 second hSecondCarrier
        secondValue hSecond firstValue).mpr
        ⟨first, hFirstCarrier, hFirstSecond, hFirst⟩
  · intro hMember
    rcases (hFunction.2.2.2 second hSecondCarrier
...
theorem exists_wellOrderCollapseFunction
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {relation carrier domain : ℳ.Domain} (hOrder :
      ℳ.IsSetCodedWellOrder 𝕀 relation carrier) (hInitial :
      ℳ.IsRelationInitialSegment 𝕀 domain relation carrier) (hValues :
      ∀ current, ℳ.mem current domain →
        ∃ value,
          ℳ.IsWellOrderCollapseValue 𝕀
              relation carrier current value ∧
  ...
  have hGraphTotal :
      ∀ current, ℳ.mem current domain →
        ∃ pair, (valueSchema.functionGraph 𝒞).denote
            env current pair := by
    intro current hCurrent
    rcases hValues current hCurrent with
      ⟨value, hValue, _⟩
    rcases 𝕀.total current value with ⟨pair, hCode⟩
    exact ⟨pair, (Definitional.Project.Formula.denote_functionGraph_iff
        𝕀 valueSchema env current pair).mpr
          ⟨value, (Definitional.Project.Formula.denote_wellOrderCollapseValue_iff
              𝕀 hZF.1 env current value).mpr hValue,
            hCode⟩⟩
  have hGraphUnique :
      ∀ current, ℳ.mem current domain →
        ∀ first second, (valueSchema.functionGraph 𝒞).denote
              env current first → (valueSchema.functionGraph 𝒞).denote
              env current second →
            first = second := by
    intro current hCurrent first second hFirst hSecond
    rcases (Definitional.Project.Formula.denote_functionGraph_iff
    ...
    have hValueEq :=
      hUnique secondValue <| (Definitional.Project.Formula.denote_wellOrderCollapseValue_iff
          𝕀 hZF.1 env current secondValue).mp hSecondValue
    have hFirstEq :=
      hUnique firstValue <| (Definitional.Project.Formula.denote_wellOrderCollapseValue_iff
  ...
  rcases exists_functionalImageOn hZF (valueSchema.functionGraph 𝒞) env domain
  ...
  have hPairMember (current value : ℳ.Domain) :
    ...
    · rintro ⟨pair, hCode, hPair⟩
      ...
      subst source
      subst output
      exact ⟨hSource, (Definitional.Project.Formula.denote_wellOrderCollapseValue_iff
          𝕀 hZF.1 env current value).mp hOutput⟩
    · rintro ⟨hCurrent, hValue⟩
      rcases 𝕀.total current value with ⟨pair, hCode⟩
      refine ⟨pair, hCode, (hFunctionMembers pair).mpr ?_⟩
      exact ⟨current, hCurrent, (Definitional.Project.Formula.denote_functionGraph_iff
          𝕀 valueSchema env current pair).mpr
            ⟨value, (Definitional.Project.Formula.denote_wellOrderCollapseValue_iff
                𝕀 hZF.1 env current value).mpr hValue,
              hCode⟩⟩
  ...
  have hFunction : ℳ.IsSetFunction 𝕀 function := by
    ...
    rcases hValues current hCurrent with
      ⟨selected, hSelected, hUnique⟩
    exact (hUnique first ((hPairMember current first).mp hFirst).2).trans <| (hUnique second ((hPairMember current second).mp hSecond).2).symm
  have hDomain : ℳ.IsDomainOf 𝕀 domain function := by
    intro current
    constructor
    · intro hCurrent
      rcases hValues current hCurrent with ⟨value, hValue, _⟩
      exact ⟨value, (hPairMember current value).mpr ⟨hCurrent, hValue⟩⟩
    · rintro ⟨value, hValue⟩
  ...
  have hValueCollapse := ((hPairMember current value).mp hValue).2
  ...
  · rintro ⟨predecessor, hPredecessorValue⟩
    have hPredecessorPredecessors :
        ℳ.mem predecessor predecessors := (hPrior.2.1 predecessor).mpr
        ⟨member, hPredecessorValue⟩
    have hPredecessorData := (hPredecessors predecessor).mp
        hPredecessorPredecessors
    ...
    exact ⟨predecessor, hPredecessorDomain,
      hPredecessorData.2, (hPairMember predecessor member).mpr
  ...
  · rintro ⟨predecessor, hPredecessorDomain,
    ...
    have hCurrentCollapse := ((hPairMember predecessor member).mp
        hPredecessorValue).2
    have hPriorCollapse :=
      hPrior.collapseValue_of_pairMember
...
def IsWellOrderType {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (relation carrier ordinal : ℳ.Domain) : Prop :=
  ∃ function,
    ℳ.IsWellOrderCollapseFunction 𝕀
        function relation carrier carrier ∧
...
theorem satisfies_isWellOrderType_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ)
    {depth : Nat} (env : Env ℳ depth) (relation carrier ordinal : Term depth) :
    satisfies env (isWellOrderType 𝒞 relation carrier ordinal) ↔
      ℳ.IsWellOrderType 𝕀 (relation.eval env) (carrier.eval env) (ordinal.eval env) := by
...
theorem wellOrderCollapseValue_existsUnique
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {relation carrier : ℳ.Domain} (hOrder :
  ...
  · rcases separation_exists_d hZF (Definitional.Project.UnarySchema.wellOrderCollapseValueExistsUnique
          𝒞).neg
        env carrier with
      ⟨counterexamples, hCounterexamples⟩
  ...
  · intro current hCurrent hPrevious
    ...
    rcases exists_wellOrderCollapseFunction hZF 𝕀 hOrder (hPredecessors.isInitialSegment hOrder hCurrent)
...
theorem wellOrderType_existsUnique
  ...
  rcases exists_wellOrderCollapseFunction hZF 𝕀 hOrder (Structure.IsRelationInitialSegment.refl
        𝕀 relation carrier)
      hValues with
    ⟨function, hFunction⟩
  rcases exists_range_of_setFunction hZF 𝕀
...
theorem equinumerous
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {relation carrier ordinal : ℳ.Domain} (hOrder :
      ℳ.IsSetCodedWellOrder 𝕀 relation carrier) (hType :
      ℳ.IsWellOrderType 𝕀 relation carrier ordinal) :
  ...
  have hSurjective :
    ...
    rcases (hRange output).mp hOutput with
      ⟨input, hInput⟩
    exact ⟨input, (hFunction.2.1 input).mpr ⟨output, hInput⟩,
      hInput⟩
  exact ⟨function,
...
theorem isOrdinal
  ...
  have hTransitive : ℳ.TransitiveSet ordinal := by
    ...
    have hCurrent :
        ℳ.mem current carrier := (hFunction.2.1 current).mpr
        ⟨middle, hCurrentValue⟩
    rcases (hFunction.2.2.2 current hCurrent
  ...
  · refine {
    ...
    · intro value _ hSelf
      exact KP.mem_irrefl_d (ZF.modelsKP hZF) value hSelf
    · intro left hLeft middle hMiddle right hRight
      ...
      have hLeftMiddleRelation := (hFunction.relation_iff_mem hZF 𝕀 hOrder
          hLeftValue hMiddleValue).mpr hLeftMiddle
      have hMiddleRightRelation := (hFunction.relation_iff_mem hZF 𝕀 hOrder
          hMiddleValue hRightValue).mpr hMiddleRight
      have hLeftInputCarrier := (hFunction.2.1 leftInput).mpr
          ⟨left, hLeftValue⟩
      have hMiddleInputCarrier := (hFunction.2.1 middleInput).mpr
          ⟨middle, hMiddleValue⟩
      have hRightInputCarrier := (hFunction.2.1 rightInput).mpr
          ⟨right, hRightValue⟩
      exact (hFunction.relation_iff_mem hZF 𝕀 hOrder
    ...
    · intro left hLeft right hRight
      ...
      · have hInputEq :=
        ...
        exact Or.inl <| by
          intro member
          simp [hValueEq]
      · exact Or.inr <| Or.inl <| (hFunction.relation_iff_mem hZF 𝕀 hOrder
            hLeftValue hRightValue).mp hLeftRight
      · exact Or.inr <| Or.inr <| (hFunction.relation_iff_mem hZF 𝕀 hOrder
  ...
  · intro subset hSubset ⟨value, hValue⟩
    ...
    rcases ZF.separation_exists_d hZF (Definitional.Project.UnarySchema.wellOrderRangePreimageMembership
          𝒞)
        env carrier with
      ⟨preimage, hPreimage⟩
    ...
    have hOtherInputPreimage :
        ℳ.mem otherInput preimage := (hPreimageSemantic otherInput).mpr
    ...
    · have hInputEq :=
      ...
      exact Or.inl <| by
        intro member
        simp [hValueEq]
    · exact Or.inr <| (hFunction.relation_iff_mem hZF 𝕀 hOrder
...
theorem exists_wellOrderRealization_of_ordinalInjection
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {α source function : ℳ.Domain} (hα : ℳ.IsOrdinal α) (hInjection :
  ...
  rcases exists_setRelationOn_of_denote hZF 𝕀 (Definitional.Project.BinarySchema.transportedOrdinalMembership 𝒞)
      env carrier with
  ...
  have hOrder :
      ...
      · intro value hValue hSelf
        ...
        have hOrdinalEq :=
          hInverse.1.1.1.2 value leftOrdinal rightOrdinal
            hLeftValue hRightValue
        subst rightOrdinal
        exact hα.wellOrder.linear.irrefl leftOrdinal (hInverse.1.1.output_mem_of_pairMember hLeftValue)
          hMember
      · intro left hLeft middle hMiddle right hRight
          hLeftMiddle hMiddleRight
        ...
        have hTransitive :=
          hα.wellOrder.linear.trans leftOrdinal (hInverse.1.1.output_mem_of_pairMember hLeftValue)
            firstMiddleOrdinal (hInverse.1.1.output_mem_of_pairMember
              hFirstMiddleValue)
            rightOrdinal (hInverse.1.1.output_mem_of_pairMember hRightValue)
    ...
    · intro subset hSubset ⟨selected, hSelected⟩
      ...
      have hLeastValue :
          ℳ.PairMember 𝕀 least leastOrdinal inverse := (hRestricted.2 least leastOrdinal).mp
      ...
      have hOtherOrdinalImage :
          ℳ.mem otherOrdinal image := (hImageRange otherOrdinal).mpr
          ⟨other, hOtherValueRestricted⟩
      have hOtherValue :
          ℳ.PairMember 𝕀 other otherOrdinal inverse := (hRestricted.2 other otherOrdinal).mp
  ...
  have hCollapse :
    ...
    · intro hMember
      ...
      exact ⟨predecessor, hPredecessor, (hRelation predecessor current).mpr
