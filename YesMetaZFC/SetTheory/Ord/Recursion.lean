...
def IsSequenceInBelow {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (sequence bound target : ℳ.Domain) : Prop :=
  ℳ.IsOrdinal bound ∧
...
def IsTransfiniteSequence {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (sequence : ℳ.Domain) : Prop :=
...
def IsRecursionValueIn {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (operator : ℳ.Domain → ℳ.Domain → Prop) (α value target : ℳ.Domain) : Prop :=
  ∃ sequence,
...
def IsRecursionValue {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (operator : ℳ.Domain → ℳ.Domain → Prop) (α value : ℳ.Domain) : Prop :=
  ∃ sequence,
...
def IsClassFunctionOnTransfiniteSequences {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (operator : ℳ.Domain → ℳ.Domain → Prop) : Prop :=
...
def IsClassFunctionOnSequencesInBelow {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (operator : ℳ.Domain → ℳ.Domain → Prop) (bound target : ℳ.Domain) : Prop :=
  ∀ sequence,
...
def IsClassFunctionOnOrdinals (ℳ : Structure.{u}) (function : ℳ.Domain → ℳ.Domain → Prop) : Prop :=
  ∀ α, ℳ.IsOrdinal α →
...
`(length, value)`。
-/
theorem exists_append {ℳ : Structure.{u}} (hKP : ℳ.Models SetTheory.KP)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {sequence length successor value : ℳ.Domain} (hSequence :
      ℳ.IsSequenceOfLength 𝕀 sequence length) (hSuccessorOrdinal : ℳ.IsOrdinal successor) (hSuccessor : ℳ.SuccessorOf successor length) :
    ∃ appended,
      ℳ.IsSequenceOfLength 𝕀 appended successor ∧
        ∀ input output,
          ℳ.PairMember 𝕀 input output appended ↔
            ℳ.PairMember 𝕀 input output sequence ∨ (input = length ∧ output = value) := by
  ...
  have hPairMember (input output : ℳ.Domain) :
    ...
    · rintro (hPairSequence | ⟨rfl, rfl⟩)
      · rcases hPairSequence with
        ...
        exact
          ⟨pair, hPairCode, (hAppended pair).mpr (Or.inl hPairSequence)⟩
      · exact
          ⟨lastPair, hLastPair, (hAppended lastPair).mpr (Or.inr rfl)⟩
  ...
  have hFunction : ℳ.IsSetFunction 𝕀 appended := by
    ...
    · subst input
      have hSelf : ℳ.mem length length := (hSequence.2.2 length).mpr ⟨first, hFirstOld⟩
      exact False.elim <|
        hSequence.1.wellOrder.linear.irrefl length hSelf hSelf
    · subst input
      have hSelf : ℳ.mem length length := (hSequence.2.2 length).mpr ⟨second, hSecondOld⟩
      exact False.elim <|
        hSequence.1.wellOrder.linear.irrefl length hSelf hSelf
  ...
  have hDomain : ℳ.IsDomainOf 𝕀 successor appended := by
    ...
    · intro hInput
      ...
      · have hInputEq :=
        ...
        subst input
        exact
          ⟨value, (hPairMember length value).mpr (Or.inr ⟨rfl, rfl⟩)⟩
    · rintro ⟨output, hOutput⟩
      rcases (hPairMember input output).mp hOutput with
        hOutputOld | ⟨hInputLength, _⟩
      · exact (hSuccessor input).mpr <| Or.inl <| (hSequence.2.2 input).mpr ⟨output, hOutputOld⟩
      · subst input
...
theorem restriction {ℳ : Structure.{u}}
    ...
    {sequence length index restricted : ℳ.Domain} (hSequence :
      ℳ.IsSequenceOfLength 𝕀 sequence length) (hIndex : ℳ.mem index length) (hRestriction :
  ...
  exact ⟨hIndexOrdinal, hRestrictedFunction, hRestrictedDomain⟩
/--
...
theorem eq_of_restriction_eq_of_last
    ...
    {first second firstLength secondLength previous
      firstRestriction secondRestriction lastValue : ℳ.Domain} (hFirst :
      ℳ.IsSequenceOfLength 𝕀 first firstLength) (hSecond :
      ℳ.IsSequenceOfLength 𝕀 second secondLength) (hFirstLength : ℳ.SuccessorOf firstLength previous) (hSecondLength : ℳ.SuccessorOf secondLength previous)
    (hFirstRestriction :
      ℳ.IsRestrictionOf 𝕀
        firstRestriction first previous) (hSecondRestriction :
      ℳ.IsRestrictionOf 𝕀
        secondRestriction second previous) (hRestrictionEq : firstRestriction = secondRestriction) (hFirstLast :
      ℳ.PairMember 𝕀 previous lastValue first) (hSecondLast :
  ...
  intro input output
  have transfer
      {left right leftRestriction rightRestriction : ℳ.Domain} (hLeft :
        ℳ.IsSequenceOfLength 𝕀 left firstLength) (hRight :
        ℳ.IsSequenceOfLength 𝕀 right firstLength) (hLeftRestriction :
        ℳ.IsRestrictionOf 𝕀
          leftRestriction left previous) (hRightRestriction :
        ℳ.IsRestrictionOf 𝕀
          rightRestriction right previous) (hRestrictionEq : leftRestriction = rightRestriction) (hLeftLast :
        ℳ.PairMember 𝕀 previous lastValue left) (hRightLast :
        ℳ.PairMember 𝕀 previous lastValue right) (hValue :
        ℳ.PairMember 𝕀 input output left) :
      ℳ.PairMember 𝕀 input output right := by
    have hInputLength :
        ℳ.mem input firstLength := (hLeft.2.2 input).mpr ⟨output, hValue⟩
    rcases (hFirstLength input).mp hInputLength with
      hInputPrevious | hInputSame
    · have hRestricted :
          ℳ.PairMember 𝕀
            input output leftRestriction := (hLeftRestriction.2 input output).mpr
          ⟨hInputPrevious, hValue⟩
      subst rightRestriction
      exact ((hRightRestriction.2 input output).mp hRestricted).2
    · have hInputEq :=
...
theorem restriction {ℳ : Structure.{u}}
    ...
    {sequence length index restricted : ℳ.Domain} (hSequence :
      ℳ.IsRecursiveSequence 𝕀 operator sequence length) (hIndex : ℳ.mem index length) (hRestriction :
      ℳ.IsRestrictionOf 𝕀 restricted sequence index) :
  ...
  have hRestrictedRecursion :
    ...
    have hValueSequence :
        ℳ.PairMember 𝕀 predecessor value sequence := ((hRestriction.2 predecessor value).mp hValue).2
...
theorem recursionValue_of_pairMember {ℳ : Structure.{u}}
    ...
    {𝕀 : 𝒞.Interpretation ℳ}
    {operator : ℳ.Domain → ℳ.Domain → Prop}
    {sequence length index value : ℳ.Domain} (hSequence :
      ℳ.IsRecursiveSequence 𝕀 operator sequence length) (hIndex : ℳ.mem index length) (hValue :
...
theorem eq_of_predecessor_uniqueness {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ}
    {operator : ℳ.Domain → ℳ.Domain → Prop} (hExt : Extensional ℳ) (hOperator :
      ℳ.IsClassFunctionOnTransfiniteSequences 𝕀 operator)
    {α left right : ℳ.Domain} (hLeft :
      ℳ.IsRecursiveSequence 𝕀 operator left α) (hRight :
      ℳ.IsRecursiveSequence 𝕀 operator right α) (hPrevious : ∀ index, ℳ.mem index α →
      ∀ first second,
  ...
  intro index value
  have transfer {first second : ℳ.Domain} (hFirst :
        ℳ.IsRecursiveSequence 𝕀 operator first α) (hSecond :
        ℳ.IsRecursiveSequence 𝕀 operator second α) (hValueFirst :
        ℳ.PairMember 𝕀 index value first) :
      ℳ.PairMember 𝕀 index value second := by
    have hIndex : ℳ.mem index α := (hFirst.1.2.2 index).mpr ⟨value, hValueFirst⟩
...
theorem toRecursiveSequence {ℳ : Structure.{u}}
    ...
    {𝕀 : 𝒞.Interpretation ℳ}
    {operator : ℳ.Domain → ℳ.Domain → Prop}
    {sequence length target : ℳ.Domain} (hSequence :
      ℳ.IsRecursiveSequenceIn 𝕀
        operator sequence length target) :
    ℳ.IsRecursiveSequence 𝕀 operator sequence length := by
  exact
    ⟨⟨hSequence.1.1, hSequence.1.2.1, hSequence.1.2.2.1⟩,
      hSequence.2⟩
...
theorem restriction {ℳ : Structure.{u}}
    ...
    {𝕀 : 𝒞.Interpretation ℳ}
    {operator : ℳ.Domain → ℳ.Domain → Prop}
    {sequence length target index restricted : ℳ.Domain} (hSequence :
      ℳ.IsRecursiveSequenceIn 𝕀
        operator sequence length target) (hIndex : ℳ.mem index length) (hRestriction :
      ℳ.IsRestrictionOf 𝕀 restricted sequence index) :
    ℳ.IsRecursiveSequenceIn 𝕀
  ...
  have hIntoTarget :
    ...
    exact
      ⟨output, hOutputTarget, (hRestriction.2 input output).mpr
...
theorem recursionValue_of_pairMember {ℳ : Structure.{u}}
    ...
    {𝕀 : 𝒞.Interpretation ℳ}
    {operator : ℳ.Domain → ℳ.Domain → Prop}
    {sequence length target index value : ℳ.Domain} (hSequence :
      ℳ.IsRecursiveSequenceIn 𝕀
        operator sequence length target) (hIndex : ℳ.mem index length) (hValue :
      ℳ.PairMember 𝕀 index value sequence) :
...
theorem eq_of_predecessor_uniqueness {ℳ : Structure.{u}}
    {𝒞 : Definitional.Project.OrderedPairConvention}
    {𝕀 : 𝒞.Interpretation ℳ}
    {operator : ℳ.Domain → ℳ.Domain → Prop} (hExt : Extensional ℳ)
    {bound target current left right : ℳ.Domain} (hBound : ℳ.IsOrdinal bound) (hCurrentWithin : current = bound ∨ ℳ.mem current bound) (hOperator :
      ℳ.IsClassFunctionOnSequencesInBelow 𝕀
        operator bound target) (hLeft :
      ℳ.IsRecursiveSequenceIn 𝕀
        operator left current target) (hRight :
      ℳ.IsRecursiveSequenceIn 𝕀
        operator right current target) (hPrevious : ∀ index, ℳ.mem index current →
      ∀ first second,
        ℳ.IsRecursiveSequenceIn 𝕀
  ...
  have transfer {first second : ℳ.Domain} (hFirst :
        ℳ.IsRecursiveSequenceIn 𝕀
          operator first current target) (hSecond :
        ℳ.IsRecursiveSequenceIn 𝕀
          operator second current target) (hValueFirst :
        ℳ.PairMember 𝕀 index value first) :
      ℳ.PairMember 𝕀 index value second := by
    have hIndex : ℳ.mem index current := (hFirst.1.2.2.1 index).mpr ⟨value, hValueFirst⟩
...
theorem satisfies_isSequenceOfLength_iff
    ...
    {depth : Nat} (env : Env ℳ depth) (sequence length : Term depth) :
    satisfies env (isSequenceOfLength 𝒞 sequence length) ↔
      ℳ.IsSequenceOfLength 𝕀 (sequence.eval env) (length.eval env) := by
...
theorem satisfies_isSequenceIn_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ)
    {depth : Nat} (env : Env ℳ depth) (sequence length target : Term depth) :
    satisfies env (isSequenceIn 𝒞 sequence length target) ↔
      ℳ.IsSequenceIn 𝕀 (sequence.eval env) (length.eval env) (target.eval env) := by
...
theorem satisfies_isSequenceInBelow_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ)
    {depth : Nat} (env : Env ℳ depth) (sequence bound target : Term depth) :
    satisfies env (isSequenceInBelow 𝒞 sequence bound target) ↔
      ℳ.IsSequenceInBelow 𝕀 (sequence.eval env) (bound.eval env) (target.eval env) := by
  ...
  · rintro ⟨hBound, hExists⟩
    ...
    simpa using (satisfies_isSequenceIn_iff 𝕀 hExt (env.push length) sequence.weaken Term.newest
        target.weaken).mp hSequence
  · rintro ⟨hBound, length, hLength, hSequence⟩
    refine ⟨hBound, ?_⟩
    rw [satisfies_existsMem_iff]
    refine ⟨length, hLength, ?_⟩
    apply (satisfies_isSequenceIn_iff 𝕀 hExt (env.push length) sequence.weaken Term.newest
        target.weaken).mpr
...
theorem satisfies_obeysRecursion_iff
    ...
    {parameterCount depth : Nat} (env : Env ℳ depth) (operator : BinarySchema parameterCount) (parameters : TermVector parameterCount depth)
    (sequence length : Term depth) :
    satisfies env (obeysRecursion 𝒞 operator parameters sequence length) ↔
      ℳ.ObeysRecursion 𝕀 (operator.denote (parameters.evalEnv env)) (sequence.eval env) (length.eval env) := by
...
theorem satisfies_isRecursiveSequence_iff
    ...
    {parameterCount depth : Nat} (env : Env ℳ depth) (operator : BinarySchema parameterCount) (parameters : TermVector parameterCount depth)
    (sequence length : Term depth) :
    satisfies env (isRecursiveSequence 𝒞 operator parameters
          sequence length) ↔
      ℳ.IsRecursiveSequence 𝕀 (operator.denote (parameters.evalEnv env)) (sequence.eval env) (length.eval env) := by
...
theorem satisfies_isRecursiveSequenceIn_iff
    ...
    satisfies env (isRecursiveSequenceIn 𝒞 operator parameters
          sequence length target) ↔
      ℳ.IsRecursiveSequenceIn 𝕀 (operator.denote (parameters.evalEnv env)) (sequence.eval env) (length.eval env) (target.eval env) := by
...
theorem satisfies_isRecursionValueIn_iff
    ...
    satisfies env (isRecursionValueIn 𝒞 operator parameters
          α value target) ↔
      ℳ.IsRecursionValueIn 𝕀 (operator.denote (parameters.evalEnv env)) (α.eval env) (value.eval env) (target.eval env) := by
...
theorem satisfies_isRecursionValue_iff
    ...
    {parameterCount depth : Nat} (env : Env ℳ depth) (operator : BinarySchema parameterCount) (parameters : TermVector parameterCount depth)
    (α value : Term depth) :
    satisfies env (isRecursionValue 𝒞 operator parameters α value) ↔
      ℳ.IsRecursionValue 𝕀 (operator.denote (parameters.evalEnv env)) (α.eval env) (value.eval env) := by
...
theorem satisfies_recursiveSequenceInExistence_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (operator : BinarySchema parameterCount) (target α : ℳ.Domain) :
    satisfies ((env.push target).push α) (operator.recursiveSequenceInExistence 𝒞).body ↔
      ∃ sequence,
        ℳ.IsRecursiveSequenceIn 𝕀 (operator.denote env) sequence α target := by
...
theorem satisfies_recursiveSequenceInUniqueness_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (operator : BinarySchema parameterCount) (target α : ℳ.Domain) :
    satisfies ((env.push target).push α) (operator.recursiveSequenceInUniqueness 𝒞).body ↔
      ∀ left right,
        ℳ.IsRecursiveSequenceIn 𝕀 (operator.denote env) left α target →
          ℳ.IsRecursiveSequenceIn 𝕀 (operator.denote env) right α target →
          left = right := by
...
theorem denote_recursionGraphPairIn_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (operator : BinarySchema parameterCount) (target α pair : ℳ.Domain) :
    (operator.recursionGraphPairIn 𝒞).denote (env.push target) α pair ↔
      ∃ value,
        ℳ.IsRecursionValueIn 𝕀 (operator.denote env) α value target ∧
...
theorem satisfies_recursiveSequenceExistence_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (operator : BinarySchema parameterCount) (α : ℳ.Domain) :
    satisfies (env.push α) (operator.recursiveSequenceExistence 𝒞).body ↔
      ∃ sequence,
        ℳ.IsRecursiveSequence 𝕀 (operator.denote env) sequence α := by
...
theorem satisfies_recursiveSequenceUniqueness_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (operator : BinarySchema parameterCount) (α : ℳ.Domain) :
    satisfies (env.push α) (operator.recursiveSequenceUniqueness 𝒞).body ↔
      ∀ left right,
        ℳ.IsRecursiveSequence 𝕀 (operator.denote env) left α →
          ℳ.IsRecursiveSequence 𝕀 (operator.denote env) right α →
          left = right := by
...
theorem denote_recursionGraphPair_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (operator : BinarySchema parameterCount) (α pair : ℳ.Domain) : (operator.recursionGraphPair 𝒞).denote
        env α pair ↔
      ∃ value,
        ℳ.IsRecursionValue 𝕀 (operator.denote env) α value ∧
          𝕀.Codes pair α value := by
...
theorem denote_transfiniteRecursion_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (hExt : Extensional ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (operator : BinarySchema parameterCount) (α value : ℳ.Domain) :
    (operator.transfiniteRecursion 𝒞).denote
        env α value ↔
      ℳ.IsRecursionValue 𝕀 (operator.denote env) α value := by
...
theorem satisfies_isClassFunctionOnSequencesInBelow_iff
    ...
    satisfies env (isClassFunctionOnSequencesInBelow 𝒞 operator
          parameters bound target) ↔
      ℳ.IsClassFunctionOnSequencesInBelow 𝕀 (operator.denote (parameters.evalEnv env)) (bound.eval env) (target.eval env) := by
...
theorem satisfies_mapsSequencesInBelowInto_iff
    ...
    {parameterCount depth : Nat} (env : Env ℳ depth) (operator : BinarySchema parameterCount) (parameters : TermVector parameterCount depth)
    (bound target : Term depth) :
    satisfies env (mapsSequencesInBelowInto 𝒞 operator
          parameters bound target) ↔
      ℳ.MapsSequencesInBelowInto 𝕀 (operator.denote (parameters.evalEnv env)) (bound.eval env) (target.eval env) := by
...
-/
theorem recursiveSequenceIn_unique {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (operator : Definitional.Project.BinarySchema parameterCount)
    {bound target α left right : ℳ.Domain} (hBound : ℳ.IsOrdinal bound) (hOrdinalWithin : α = bound ∨ ℳ.mem α bound) (hOperator :
      ℳ.IsClassFunctionOnSequencesInBelow 𝕀 (operator.denote env) bound target) (hLeft :
      ℳ.IsRecursiveSequenceIn 𝕀 (operator.denote env) left α target) (hRight :
      ℳ.IsRecursiveSequenceIn 𝕀 (operator.denote env) right α target) :
    left = right := by
  let property : ℳ.Domain → Prop := fun current =>
    ∀ first second,
      ℳ.IsRecursiveSequenceIn 𝕀 (operator.denote env) first current target →
        ℳ.IsRecursiveSequenceIn 𝕀 (operator.denote env) second current target →
        first = second
  have hProperty : property α := by
    apply Structure.IsOrdinal.inductionWithin
      hLeft.1.1 property
    · rcases separation_exists_d hZF (operator.recursiveSequenceInUniqueness 𝒞).neg (env.push target) α with
...
theorem recursionValueIn_unique {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (operator : Definitional.Project.BinarySchema parameterCount)
    {bound target α first second : ℳ.Domain} (hBound : ℳ.IsOrdinal bound) (hOrdinalBound : ℳ.mem α bound) (hOperator :
      ℳ.IsClassFunctionOnSequencesInBelow 𝕀 (operator.denote env) bound target) (hFirst :
      ℳ.IsRecursionValueIn 𝕀 (operator.denote env) α first target) (hSecond :
      ℳ.IsRecursionValueIn 𝕀 (operator.denote env) α second target) :
    first = second := by
  rcases hFirst with
...
theorem recursiveSequence_unique {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (operator : Definitional.Project.BinarySchema parameterCount) (hOperator :
      ℳ.IsClassFunctionOnTransfiniteSequences 𝕀 (operator.denote env))
    {α left right : ℳ.Domain} (hLeft :
      ℳ.IsRecursiveSequence 𝕀 (operator.denote env) left α) (hRight :
      ℳ.IsRecursiveSequence 𝕀 (operator.denote env) right α) :
    left = right := by
  let property : ℳ.Domain → Prop := fun current =>
    ∀ first second,
      ℳ.IsRecursiveSequence 𝕀 (operator.denote env) first current →
        ℳ.IsRecursiveSequence 𝕀 (operator.denote env) second current →
        first = second
  have hProperty : property α := by
    apply Structure.IsOrdinal.induction hLeft.1.1 property
    · rcases separation_exists_d hZF (operator.recursiveSequenceUniqueness 𝒞).neg
          env α with
        ⟨counterexamples, hCounterexamples⟩
      refine ⟨counterexamples, fun value => ?_⟩
      rw [hCounterexamples value]
...
theorem recursionValue_unique {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (operator : Definitional.Project.BinarySchema parameterCount) (hOperator :
      ℳ.IsClassFunctionOnTransfiniteSequences 𝕀 (operator.denote env))
    {α first second : ℳ.Domain} (hFirst :
      ℳ.IsRecursionValue 𝕀 (operator.denote env) α first) (hSecond :
      ℳ.IsRecursionValue 𝕀 (operator.denote env) α second) :
    first = second := by
  rcases hFirst with
...
-/
theorem recursiveSequenceIn_of_predecessors {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (operator : Definitional.Project.BinarySchema parameterCount)
    {bound target current : ℳ.Domain} (hBound : ℳ.IsOrdinal bound) (hCurrent : ℳ.IsOrdinal current) (hCurrentWithin : current = bound ∨ ℳ.mem current bound)
    (hOperator :
      ℳ.IsClassFunctionOnSequencesInBelow 𝕀 (operator.denote env) bound target) (hMaps :
      ℳ.MapsSequencesInBelowInto 𝕀 (operator.denote env) bound target) (hPrevious : ∀ index, ℳ.mem index current →
      ∃ sequence,
        ℳ.IsRecursiveSequenceIn 𝕀 (operator.denote env) sequence index target) :
    ∃ sequence,
      ℳ.IsRecursiveSequenceIn 𝕀 (operator.denote env) sequence current target := by
  have hIndexBound {index : ℳ.Domain} (hIndex : ℳ.mem index current) :
      ℳ.mem index bound := by
  ...
  have hGraphTotal : ∀ index, ℳ.mem index current →
      ∃ pair, (operator.recursionGraphPairIn 𝒞).denote (env.push target) index pair := by
    intro index hIndex
    ...
    rcases 𝕀.total index value with
      ⟨pair, hPair⟩
    refine ⟨pair, ?_⟩
    apply (Definitional.Project.Formula.denote_recursionGraphPairIn_iff
  ...
  have hGraphUnique : ∀ index, ℳ.mem index current →
      ∀ first second, (operator.recursionGraphPairIn 𝒞).denote (env.push target) index first → (operator.recursionGraphPairIn 𝒞).denote
            (env.push target) index second →
          first = second := by
    intro index hIndex first second hFirst hSecond
    rcases (Definitional.Project.Formula.denote_recursionGraphPairIn_iff
          𝕀 hZF.1 env operator
  ...
  rcases exists_functionalImageOn hZF (operator.recursionGraphPairIn 𝒞) (env.push target) current hGraphTotal hGraphUnique with
    ⟨sequence, hSequenceMembers⟩
  have hPairMember (index value : ℳ.Domain) :
      ℳ.PairMember 𝕀 index value sequence ↔
        ℳ.mem index current ∧
          ℳ.IsRecursionValueIn 𝕀 (operator.denote env) index value target := by
    constructor
  ...
  have hFunction :
    ...
    exact
      recursionValueIn_unique hZF 𝕀 env operator
        hBound (hIndexBound hIndex) hOperator ((hPairMember index first).mp hFirst).2 ((hPairMember index second).mp hSecond).2
  have hDomain :
  ...
  have hRecursion :
      ℳ.ObeysRecursion 𝕀 (operator.denote env) sequence current := by
    ...
    constructor
    · intro hOutputPrior
      have hInput : ℳ.mem input index := (hPrior.1.2.2.1 input).mpr
          ⟨output, hOutputPrior⟩
    ...
    · rintro ⟨hInput, hOutputSequence⟩
      ...
      have hSequenceValue := ((hPairMember input output).mp hOutputSequence).2
      have hInputBound :
          ℳ.mem input bound :=
...
-/
theorem recursiveSequenceIn_exists {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (operator : Definitional.Project.BinarySchema parameterCount)
    {bound target α : ℳ.Domain} (hBound : ℳ.IsOrdinal bound) (hα : ℳ.IsOrdinal α) (hOrdinalWithin : α = bound ∨ ℳ.mem α bound) (hOperator :
      ℳ.IsClassFunctionOnSequencesInBelow 𝕀 (operator.denote env) bound target) (hMaps :
      ℳ.MapsSequencesInBelowInto 𝕀 (operator.denote env) bound target) :
    ∃ sequence,
      ℳ.IsRecursiveSequenceIn 𝕀 (operator.denote env) sequence α target := by
  let property : ℳ.Domain → Prop := fun current =>
    ∃ sequence,
      ℳ.IsRecursiveSequenceIn 𝕀 (operator.denote env) sequence current target
  apply Structure.IsOrdinal.inductionWithin hα property
  · rcases separation_exists_d hZF (operator.recursiveSequenceInExistence 𝒞).neg (env.push target) α with
      ⟨counterexamples, hCounterexamples⟩
    refine ⟨counterexamples, fun value => ?_⟩
    rw [hCounterexamples value]
    simp [property, Definitional.Project.UnarySchema.neg,
...
`target` 值递归序列。
-/
theorem recursiveSequenceIn_existsUnique {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (operator : Definitional.Project.BinarySchema parameterCount)
    {bound target : ℳ.Domain} (hBound : ℳ.IsOrdinal bound) (hOperator :
      ℳ.IsClassFunctionOnSequencesInBelow 𝕀 (operator.denote env) bound target) (hMaps :
      ℳ.MapsSequencesInBelowInto 𝕀 (operator.denote env) bound target) :
    ∃ sequence,
      ℳ.IsRecursiveSequenceIn 𝕀 (operator.denote env) sequence bound target ∧
        ∀ other,
          ℳ.IsRecursiveSequenceIn 𝕀 (operator.denote env) other bound target →
            other = sequence := by
...
-/
theorem recursiveSequence_of_predecessors {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (operator : Definitional.Project.BinarySchema parameterCount) (hOperator :
      ℳ.IsClassFunctionOnTransfiniteSequences 𝕀 (operator.denote env))
    {α : ℳ.Domain} (hα : ℳ.IsOrdinal α) (hPrevious : ∀ index, ℳ.mem index α →
      ∃ sequence,
        ℳ.IsRecursiveSequence 𝕀 (operator.denote env) sequence index) :
    ∃ sequence,
      ℳ.IsRecursiveSequence 𝕀 (operator.denote env) sequence α := by
  have hGraphTotal : ∀ index, ℳ.mem index α →
      ∃ pair, (operator.recursionGraphPair 𝒞).denote
          env index pair := by
    intro index hIndex
    rcases hPrevious index hIndex with
      ⟨prior, hPrior⟩
    ...
    rcases 𝕀.total index value with
      ⟨pair, hPair⟩
    refine ⟨pair, ?_⟩
    apply (Definitional.Project.Formula.denote_recursionGraphPair_iff
        𝕀 hZF.1 env operator index pair).mpr
    exact ⟨value, ⟨prior, hPrior, hValue⟩, hPair⟩
  have hGraphUnique : ∀ index first second, (operator.recursionGraphPair 𝒞).denote
          env index first → (operator.recursionGraphPair 𝒞).denote
          env index second →
        first = second := by
    intro index first second hFirst hSecond
    rcases (Definitional.Project.Formula.denote_recursionGraphPair_iff
    ...
    subst secondValue
    exact 𝕀.unique hFirstCode hSecondCode
  rcases exists_functionalImage hZF (operator.recursionGraphPair 𝒞) env α
      hGraphTotal hGraphUnique with
    ⟨sequence, hSequenceMembers⟩
  have hPairMember (index value : ℳ.Domain) :
      ℳ.PairMember 𝕀 index value sequence ↔
        ℳ.mem index α ∧
          ℳ.IsRecursionValue 𝕀 (operator.denote env) index value := by
    constructor
    · rintro ⟨pair, hPairCode, hPairMem⟩
      rcases (hSequenceMembers pair).mp hPairMem with
        ⟨source, hSource, hGraph⟩
  ...
  have hFunction :
    ...
    exact
      recursionValue_unique hZF 𝕀 env operator
        hOperator ((hPairMember index first).mp hFirst).2 ((hPairMember index second).mp hSecond).2
  have hDomain :
    ...
    · rintro ⟨value, hValue⟩
      exact ((hPairMember index value).mp hValue).1
  have hRecursion :
      ℳ.ObeysRecursion 𝕀 (operator.denote env) sequence α := by
    intro index hIndex value hValue
    rcases ((hPairMember index value).mp hValue).2 with
    ...
    constructor
    · intro hOutputPrior
      have hInput : ℳ.mem input index := (hPrior.1.2.2 input).mpr ⟨output, hOutputPrior⟩
...
theorem recursiveSequence_exists {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (operator : Definitional.Project.BinarySchema parameterCount) (hOperator :
      ℳ.IsClassFunctionOnTransfiniteSequences 𝕀 (operator.denote env))
  ...
  let property : ℳ.Domain → Prop := fun current =>
    ∃ sequence,
      ℳ.IsRecursiveSequence 𝕀 (operator.denote env) sequence current
  apply Structure.IsOrdinal.induction hα property
  · rcases separation_exists_d hZF (operator.recursiveSequenceExistence 𝒞).neg
        env α with
      ⟨counterexamples, hCounterexamples⟩
    refine ⟨counterexamples, fun value => ?_⟩
    rw [hCounterexamples value]
...
theorem recursionValue_exists {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {parameterCount : Nat} (env : Env ℳ parameterCount) (operator : Definitional.Project.BinarySchema parameterCount) (hOperator :
      ℳ.IsClassFunctionOnTransfiniteSequences 𝕀 (operator.denote env))
    {α : ℳ.Domain} (hα : ℳ.IsOrdinal α) :
...
theorem transfiniteRecursion_isClassFunctionOnOrdinals
    ...
    {parameterCount : Nat} (env : Env ℳ parameterCount) (operator : Definitional.Project.BinarySchema parameterCount) (hOperator :
      ℳ.IsClassFunctionOnTransfiniteSequences 𝕀 (operator.denote env)) :
    ℳ.IsClassFunctionOnOrdinals ((operator.transfiniteRecursion 𝒞).denote env) := by
  intro α hα
  rcases recursionValue_exists hZF 𝕀 env operator
      hOperator hα with
    ⟨value, hValue⟩
  refine ⟨value, ?_, ?_⟩
  · exact (Definitional.Project.Formula.denote_transfiniteRecursion_iff
