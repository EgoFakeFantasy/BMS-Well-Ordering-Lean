...
theorem separation_sat_iff_d {ℳ : Structure.{u}}
    {parameterCount : Nat} (env : Env ℳ parameterCount) (schema : Definitional.Project.UnarySchema parameterCount) :
...
theorem separation_exists_d {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {parameterCount : Nat} (schema : Definitional.Project.UnarySchema parameterCount) (env : Env ℳ parameterCount) (source : ℳ.Domain) :
    ∃ subset, ∀ value,
      ℳ.mem value subset ↔
        ℳ.mem value source ∧
          Definitional.Project.Formula.satisfies (env.push value) schema.body := by
  have hSentence :=
    hZF.2 (Axioms.Schema.separation schema) (Axiom.separation schema) env.free
  have hCore :
      Definitional.Project.Formula.satisfies env (Axioms.Schema.separationCore schema) := by
    have hAll := (Definitional.Project.Formula.satisfies_forallClosure_iff env.free (Axioms.Schema.separationCore schema)).mp hSentence
    simpa only using hAll env.bound
  exact (Axioms.Schema.separation_sat_iff_d env schema).mp
      hCore source
end ZF
namespace KP
/-- KP 分离带 `Δ₀` 证据的公式模式。 -/
theorem separation_exists_d {ℳ : Structure.{u}} (hKP : ℳ.Models SetTheory.KP)
    {parameterCount : Nat} (schema : Definitional.Project.Delta0UnarySchema parameterCount) (env : Env ℳ parameterCount) (source : ℳ.Domain) :
    ∃ subset, ∀ value,
      ℳ.mem value subset ↔
        ℳ.mem value source ∧
          Definitional.Project.Formula.satisfies (env.push value) schema.body := by
  have hSentence :=
    hKP.2 (Axioms.Schema.separation schema.toUnarySchema) (Axiom.separation schema) env.free
  have hCore :
      Definitional.Project.Formula.satisfies env (Axioms.Schema.separationCore schema.toUnarySchema) := by
    have hAll := (Definitional.Project.Formula.satisfies_forallClosure_iff env.free (Axioms.Schema.separationCore schema.toUnarySchema)).mp
          hSentence
    simpa only using hAll env.bound
  exact (Axioms.Schema.separation_sat_iff_d
