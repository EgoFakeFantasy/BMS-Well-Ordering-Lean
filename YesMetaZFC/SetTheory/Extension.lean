...
/-- 扩张关系可传递。 -/
theorem trans {strong middle weak : Theory} (hStrong : Extends.{u} strong middle) (hMiddle : Extends.{u} middle weak) :
    Extends.{u} strong weak := by
  intro sentence hSentence ℳ hModels
  apply hMiddle sentence hSentence ℳ
  exact ⟨hModels.1, fun sentence hSentence => hStrong sentence hSentence ℳ hModels⟩
end Extends
/--
定理继承主定理。
...
theorem entails_of_extends {strong weak : Theory}
    {target : Definitional.Project.Sentence} (hExtends : Extends.{u} strong weak) (hTheorem : SemanticallyEntails.{u} weak target) :
    SemanticallyEntails.{u} strong target := by
  intro ℳ hModels
  apply hTheorem ℳ
  exact ⟨hModels.1, fun sentence hSentence => hExtends sentence hSentence ℳ hModels⟩
end Theory
namespace ZF
/-- ZF 扩张带无穷的 KP。 -/
...
theorem modelsKP {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF) :
...
/-- ZF 自动继承 KP 的每个定理。 -/
theorem zf_inherits_kp {target : Definitional.Project.Sentence} (hTheorem : SemanticallyEntails.{u} KP target) :
    SemanticallyEntails.{u} ZF target :=
  Theory.entails_of_extends ZF.extendsKP hTheorem
/-- ZFC 自动继承 ZF 的每个定理。 -/
theorem zfc_inherits_zf {target : Definitional.Project.Sentence} (hTheorem : SemanticallyEntails.{u} ZF target) :
    SemanticallyEntails.{u} ZFC target :=
  Theory.entails_of_extends ZFC.extendsZF hTheorem
/-- ZFC 自动继承 KP 的每个定理。 -/
theorem zfc_inherits_kp {target : Definitional.Project.Sentence} (hTheorem : SemanticallyEntails.{u} KP target) :
    SemanticallyEntails.{u} ZFC target :=
