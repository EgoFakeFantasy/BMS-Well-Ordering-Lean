...
def constantValue : BinarySchema 1 where
  ...
  freeClosed := by
    simp [Formula.extensionalEq, Formula.FreeClosed]
/-- 给定函数图的逆值关系。 -/
def inverseValue (𝒞 : OrderedPairConvention) : BinarySchema 1 where
...
theorem denote_identityValue_iff
    {ℳ : Structure.{u}} (hExt : Extensional ℳ) (env : Env ℳ 0) (input output : ℳ.Domain) :
...
theorem denote_inverseValue_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (env : Env ℳ 1) (input output : ℳ.Domain) :
...
private theorem reindex_functionGraphValue
    {ℳ : Structure.{u}} {parameterCount : Nat} (env : Env ℳ parameterCount) (input pair output : ℳ.Domain) : (((env.push input).push pair).push output).reindex
        (FunctionGraphEmbedding.value (parameterCount := parameterCount)) = (env.push input).push output := by
  rw [Env.mk.injEq]
  constructor
  · funext entry
...
theorem denote_functionGraph_iff
    ...
    {parameterCount : Nat} (schema : BinarySchema parameterCount) (env : Env ℳ parameterCount) (input pair : ℳ.Domain) :
...
def relationMember (𝒞 : OrderedPairConvention)
    {parameterCount : Nat} (schema : BinarySchema parameterCount) :
...
private theorem reindex_relationGraphValue
    {ℳ : Structure.{u}} {parameterCount : Nat} (env : Env ℳ parameterCount) (pair left right : ℳ.Domain) : (((env.push pair).push left).push right).reindex
        (RelationGraphEmbedding.value (parameterCount := parameterCount)) = (env.push left).push right := by
  rw [Env.mk.injEq]
  constructor
  · funext entry
    refine Fin.cases ?_ (fun previous => ?_) entry
...
theorem satisfies_relationMember_iff
    {ℳ : Structure.{u}} {𝒞 : OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {parameterCount : Nat} (schema : BinarySchema parameterCount) (env : Env ℳ parameterCount) (pair : ℳ.Domain) :
    satisfies (env.push pair) (UnarySchema.relationMember 𝒞 schema).body ↔
      ∃ left right,
...
theorem satisfies_restrictionMember_iff
    ...
    satisfies (env.push pair) (UnarySchema.restrictionMember 𝒞).body ↔
      ∃ input output,
...
theorem exists_setRelationOn_of_denote
    ...
    {parameterCount : Nat} (schema : Definitional.Project.BinarySchema parameterCount) (env : Env ℳ parameterCount) (carrier : ℳ.Domain) :
    ∃ relation,
  ...
  rcases exists_cartesianProduct hZF 𝕀 carrier carrier with
    ⟨product, hProduct⟩
  rcases separation_exists_d hZF (Definitional.Project.UnarySchema.relationMember 𝒞 schema) env product with
    ⟨relation, hRelation⟩
...
theorem exists_restriction {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (relation source : ℳ.Domain) :
    ∃ restriction,
      ℳ.IsRestrictionOf 𝕀
  ...
  rcases separation_exists_d hZF (Definitional.Project.UnarySchema.restrictionMember 𝒞) env relation with
  ...
  · rintro ⟨hInput, pair, hCode, hPair⟩
    exact
      ⟨pair, hCode, (hRestrictionSemantic pair).mpr
          ⟨hPair, input, output, hCode, hInput⟩⟩
/-- 全定义且单值的可定义二元关系可收集为模型内部函数图。 -/
theorem exists_setFunctionFromTo_of_denote
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {parameterCount : Nat} (schema : Definitional.Project.BinarySchema parameterCount) (env : Env ℳ parameterCount)
    {source target : ℳ.Domain} (hTotal : ∀ input, ℳ.mem input source →
      ∃ output, schema.denote env input output) (hUnique : ∀ input, ℳ.mem input source → ∀ first second,
      schema.denote env input first →
      schema.denote env input second →
      first = second) (hTarget : ∀ input output,
      ℳ.mem input source →
      schema.denote env input output →
      ℳ.mem output target) :
    ∃ function,
  ...
  have hGraphUnique :
      ∀ input, ℳ.mem input source → ∀ first second, (schema.functionGraph 𝒞).denote env input first → (schema.functionGraph 𝒞).denote env input second →
        first = second := by
  ...
  rcases exists_functionalImageOn hZF (schema.functionGraph 𝒞) env source hGraphTotal hGraphUnique with
  ...
  have hPairMember (input output : ℳ.Domain) :
    ...
    · rintro ⟨hInput, hValue⟩
      ...
      exact ⟨input, hInput, (Definitional.Project.Formula.denote_functionGraph_iff
          𝕀 schema env input pair).mpr
  ...
  have hSetFunction : ℳ.IsSetFunction 𝕀 function := by
    ...
    exact hUnique input ((hPairMember input first).mp hFirst).1 first second ((hPairMember input first).mp hFirst).2 ((hPairMember input second).mp hSecond).2
  ...
  have hIntoTarget :
    ...
    intro input hInput
    rcases hTotal input hInput with ⟨output, hOutput⟩
    exact ⟨output, hTarget input output hInput hOutput, (hPairMember input output).mpr ⟨hInput, hOutput⟩⟩
  exact ⟨function, ⟨⟨hSetFunction, hDomain, hIntoTarget⟩, hPairMember⟩⟩
/-- 两个模型内部函数的复合仍可收集为模型内部函数。 -/
theorem exists_compositionFunction
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {first second source middle target : ℳ.Domain} (hFirst :
      ℳ.IsSetFunctionFromTo 𝕀 first source middle) (hSecond :
  ...
  rcases exists_setFunctionFromTo_of_denote
      hZF 𝕀 (Definitional.Project.BinarySchema.compositionValue 𝒞) env (by
        intro input hInput
        ...
        exact ⟨output, (Definitional.Project.Formula.denote_compositionValue_iff
            𝕀 env input output).mpr
              ⟨middleValue, hFirstPair, hSecondPair⟩⟩) (by
...
theorem exists_compositionInjection
    ...
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {first second source middle target : ℳ.Domain} (hFirst :
      ℳ.IsSetInjectionFromTo 𝕀 first source middle) (hSecond :
...
theorem exists_setInjectionFromTo_of_denote
    ...
    {source target : ℳ.Domain} (hTotal : ∀ input, ℳ.mem input source →
      ...
      ℳ.mem input source →
      schema.denote env input output →
      ℳ.mem output target) (hInjective : ∀ first second output,
      ℳ.mem first source →
      ℳ.mem second source →
  ...
  exact hInjective first second output ((hPairMember first output).mp hFirst).1 ((hPairMember second output).mp hSecond).1
    ((hPairMember first output).mp hFirst).2 ((hPairMember second output).mp hSecond).2
...
theorem exists_setBijectionFromTo_of_denote
    ...
    {source target : ℳ.Domain} (hTotal : ∀ input, ℳ.mem input source →
      ...
      schema.denote env first output →
      schema.denote env second output →
      first = second) (hSurjective : ∀ output, ℳ.mem output target →
      ∃ input, ℳ.mem input source ∧
        schema.denote env input output) :
    ∃ function,
  ...
  · intro output hOutput
    ...
    exact ⟨input, hInput, (hPairMember input output).mpr ⟨hInput, hValue⟩⟩
/-- 任意集合上的恒等关系可收集为模型内部双射。 -/
theorem exists_identityBijection
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ) (set : ℳ.Domain) :
    ∃ function,
  ...
  · intro output hOutput
    exact ⟨output, hOutput, (Definitional.Project.Formula.denote_identityValue_iff
        hZF.1 env output output).mpr rfl⟩
/-- 子集包含映射可由恒等函数图实现为模型内部单射。 -/
theorem exists_inclusionInjection
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {source target : ℳ.Domain} (hSubset : ∀ value, ℳ.mem value source → ℳ.mem value target) :
    ∃ function,
      ℳ.IsSetInjectionFromTo 𝕀 function source target := by
...
theorem exists_constantFunction
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {source target value : ℳ.Domain} (hValue : ℳ.mem value target) :
    ∃ function,
      ℳ.IsSetFunctionFromTo 𝕀 function source target ∧
        ∀ input output,
          ℳ.PairMember 𝕀 input output function ↔
  ...
  rcases exists_setFunctionFromTo_of_denote
      hZF 𝕀 Definitional.Project.BinarySchema.constantValue env (by
        intro input _
        exact ⟨value, (Definitional.Project.Formula.denote_constantValue_iff
            hZF.1 env input value).mpr rfl⟩) (by
        intro input _ first second hFirst hSecond
        rw [Definitional.Project.Formula.denote_constantValue_iff hZF.1]
          at hFirst hSecond
        exact hFirst.trans hSecond.symm) (by
...
theorem exists_bijectionBetweenSingletons
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {source target sourceValue targetValue : ℳ.Domain} (hSource : ℳ.IsSingletonOf source sourceValue) (hTarget : ℳ.IsSingletonOf target targetValue) :
    ∃ function,
      ℳ.IsSetBijectionFromTo 𝕀 function source target := by
  have hTargetValue : ℳ.mem targetValue target := (hTarget targetValue).mpr rfl
  ...
  · intro first second output hFirst hSecond
    ...
    exact ((hSource first).mp hFirstSource).trans <| ((hSource second).mp hSecondSource).symm
  · intro output hOutput
    have hOutputEq := (hTarget output).mp hOutput
    have hSourceValue : ℳ.mem sourceValue source := (hSource sourceValue).mpr rfl
...
theorem exists_inverseBijectionWithPairs
    {ℳ : Structure.{u}} (hZF : ℳ.Models SetTheory.ZF)
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {function source target : ℳ.Domain} (hFunction :
  ...
  rcases exists_setFunctionFromTo_of_denote
      hZF 𝕀 (Definitional.Project.BinarySchema.inverseValue 𝒞) env (by
        intro output hOutput
        rcases hFunction.2 output hOutput with
          ⟨input, _, hPair⟩
        exact ⟨input, (Definitional.Project.Formula.denote_inverseValue_iff
            𝕀 env output input).mpr hPair⟩) (by
        intro output _ first second hFirst hSecond
        rw [Definitional.Project.Formula.denote_inverseValue_iff 𝕀]
          at hFirst hSecond
        exact hFunction.1.2 first second output hFirst hSecond) (by
  ...
  have hSurjective :
    ...
    exact ⟨output, hOutput, (hPairs output input).mpr
      ⟨hOutput, (Definitional.Project.Formula.denote_inverseValue_iff
...
theorem exists_compositionBijection
    ...
    {𝒞 : Definitional.Project.OrderedPairConvention} (𝕀 : 𝒞.Interpretation ℳ)
    {first second source middle target : ℳ.Domain} (hFirst :
      ℳ.IsSetBijectionFromTo 𝕀 first source middle) (hSecond :
  ...
  · intro input output _ hValue
    rw [Definitional.Project.Formula.denote_compositionValue_iff 𝕀] at hValue
    rcases hValue with ⟨middleValue, _, hOutput⟩
    rcases hSecond.1.1.2.2 middleValue ((hSecond.1.1.2.1 middleValue).mpr ⟨output, hOutput⟩) with
  ...
  · intro output hOutput
    ...
    exact ⟨input, hInput, (Definitional.Project.Formula.denote_compositionValue_iff
        𝕀 env input output).mpr
          ⟨middleValue, hFirstPair, hSecondPair⟩⟩
end ZF
...
theorem functionFromTo_of_disjoint
    ...
    {function leftFunction rightFunction
      source left right target : ℳ.Domain} (hFunctionUnion :
      ℳ.IsUnionOfTwo function leftFunction rightFunction) (hSourceUnion :
      ℳ.IsUnionOfTwo source left right) (hDisjoint : ℳ.IsDisjoint left right) (hLeft :
      ℳ.IsSetFunctionFromTo 𝕀 leftFunction left target) (hRight :
  ...
  have hPairs (input output : ℳ.Domain) :
    ...
    · intro hPair
      ...
      · rcases hPairLeft with ⟨pair, hCode, hPair⟩
        exact ⟨pair, hCode, (hFunctionUnion pair).mpr <| Or.inl hPair⟩
      · rcases hPairRight with ⟨pair, hCode, hPair⟩
        exact ⟨pair, hCode, (hFunctionUnion pair).mpr <| Or.inr hPair⟩
  have hRelation : ℳ.IsSetRelation 𝕀 function := by
    intro pair hPair
    rcases (hFunctionUnion pair).mp hPair with
      hPairLeft | hPairRight
  ...
  · rcases hLeft.2.2 input hInputLeft with
      ⟨output, hOutput, hPair⟩
    exact ⟨output, hOutput, (hPairs input output).mpr <| Or.inl hPair⟩
  · rcases hRight.2.2 input hInputRight with
      ⟨output, hOutput, hPair⟩
    exact ⟨output, hOutput, (hPairs input output).mpr <| Or.inr hPair⟩
