...
def empty {σ : Signature.{u, v, w}} : Scope σ :=
  fun _ => 0
def push {σ : Signature.{u, v, w}} [DecidableEq σ.SortSymbol] (ctx : Scope σ) (s : σ.SortSymbol) : Scope σ :=
...
mutual
  inductive TermWellSorted {σ : Signature.{u, v, w}} :
    ...
    | app (f : σ.FuncSymbol) {args : List (Term σ)} (hArgs : ArgsWellSorted args (σ.funcDomain f)) :
  ...
  inductive ArgsWellSorted {σ : Signature.{u, v, w}} :
    ...
    | cons {term : Term σ} {terms : List (Term σ)}
        {sort : σ.SortSymbol} {sorts : List σ.SortSymbol} (hTerm : TermWellSorted term sort) (hRest : ArgsWellSorted terms sorts) :
...
inductive FormulaWellFormed {σ : Signature.{u, v, w}} : Formula σ → Prop where
  ...
  | rel (r : σ.RelSymbol) {args : List (Term σ)} (hArgs : ArgsWellSorted args (σ.relDomain r)) :
      FormulaWellFormed (.rel r args)
  | equal {left right : Term σ} {s : σ.SortSymbol} (hLeft : TermWellSorted left s) (hRight : TermWellSorted right s) :
      FormulaWellFormed (.equal left right)
  | neg {φ : Formula σ} (h : FormulaWellFormed φ) :
      FormulaWellFormed (.neg φ)
  | conj {φ ψ : Formula σ} (hφ : FormulaWellFormed φ) (hψ : FormulaWellFormed ψ) :
      FormulaWellFormed (.conj φ ψ)
  | disj {φ ψ : Formula σ} (hφ : FormulaWellFormed φ) (hψ : FormulaWellFormed ψ) :
      FormulaWellFormed (.disj φ ψ)
  | imp {φ ψ : Formula σ} (hφ : FormulaWellFormed φ) (hψ : FormulaWellFormed ψ) :
      FormulaWellFormed (.imp φ ψ)
  | iff {φ ψ : Formula σ} (hφ : FormulaWellFormed φ) (hψ : FormulaWellFormed ψ) :
      FormulaWellFormed (.iff φ ψ)
  | forallE (s : σ.SortSymbol) {body : Formula σ} (hBody : FormulaWellFormed body) :
      FormulaWellFormed (.forallE s body)
  | existsE (s : σ.SortSymbol) {body : Formula σ} (hBody : FormulaWellFormed body) :
...
inductive TermScoped {σ : Signature.{u, v, w}} [DecidableEq σ.SortSymbol] :
  ...
  | app {ctx : Scope σ} (f : σ.FuncSymbol) (args : List (Term σ)) (hArgs : ∀ term, term ∈ args → TermScoped ctx term) :
...
inductive FormulaScoped {σ : Signature.{u, v, w}} [DecidableEq σ.SortSymbol] :
  ...
  | rel {ctx : Scope σ} (r : σ.RelSymbol) (args : List (Term σ)) (hArgs : ∀ term, term ∈ args → TermScoped ctx term) :
      FormulaScoped ctx (.rel r args)
  | equal {ctx : Scope σ} {left right : Term σ} (hLeft : TermScoped ctx left) (hRight : TermScoped ctx right) :
      FormulaScoped ctx (.equal left right)
  | neg {ctx : Scope σ} {φ : Formula σ} (h : FormulaScoped ctx φ) :
      FormulaScoped ctx (.neg φ)
  | conj {ctx : Scope σ} {φ ψ : Formula σ} (hφ : FormulaScoped ctx φ) (hψ : FormulaScoped ctx ψ) :
      FormulaScoped ctx (.conj φ ψ)
  | disj {ctx : Scope σ} {φ ψ : Formula σ} (hφ : FormulaScoped ctx φ) (hψ : FormulaScoped ctx ψ) :
      FormulaScoped ctx (.disj φ ψ)
  | imp {ctx : Scope σ} {φ ψ : Formula σ} (hφ : FormulaScoped ctx φ) (hψ : FormulaScoped ctx ψ) :
      FormulaScoped ctx (.imp φ ψ)
  | iff {ctx : Scope σ} {φ ψ : Formula σ} (hφ : FormulaScoped ctx φ) (hψ : FormulaScoped ctx ψ) :
      FormulaScoped ctx (.iff φ ψ)
  | forallE {ctx : Scope σ} (s : σ.SortSymbol) {body : Formula σ} (hBody : FormulaScoped (Scope.push ctx s) body) :
      FormulaScoped ctx (.forallE s body)
  | existsE {ctx : Scope σ} (s : σ.SortSymbol) {body : Formula σ} (hBody : FormulaScoped (Scope.push ctx s) body) :
      FormulaScoped ctx (.existsE s body)
def FormulaClosed {σ : Signature.{u, v, w}} [DecidableEq σ.SortSymbol] (φ : Formula σ) : Prop :=
  FormulaScoped Scope.empty φ
namespace Term
/-- 在指定 sort 的第 `depth` 个 binder 处打开项。replacement 通常要求 locally closed。 -/
def openAt {σ : Signature.{u, v, w}} [DecidableEq σ.SortSymbol] (target : σ.SortSymbol) (depth : Nat) (replacement : Term σ) :
    Term σ → Term σ
  | var (.bvar s idx) =>
      if s = target then
        if idx = depth then
...
def closeFreeAt {σ : Signature.{u, v, w}} [DecidableEq σ.SortSymbol] (target : σ.SortSymbol) (id : FreeVarId) (depth : Nat) :
    Term σ → Term σ
  | var (.bvar s idx) =>
...
/-- 穿过同 sort binder 时递增局部深度。 -/
def next_depth {σ : Signature.{u, v, w}} [DecidableEq σ.SortSymbol] (target binder : σ.SortSymbol) (depth : Nat) : Nat :=
...
def openAt {σ : Signature.{u, v, w}} [DecidableEq σ.SortSymbol] (target : σ.SortSymbol) (depth : Nat) (replacement : Term σ) :
  ...
  | equal left right =>
      equal (Term.openAt target depth replacement left) (Term.openAt target depth replacement right)
  ...
  | forallE s body => forallE s (openAt target (next_depth target s depth) replacement body)
  | existsE s body => existsE s (openAt target (next_depth target s depth) replacement body)
/-- 关闭公式中的 free 变量。 -/
def closeFreeAt {σ : Signature.{u, v, w}} [DecidableEq σ.SortSymbol] (target : σ.SortSymbol) (id : FreeVarId) (depth : Nat) :
    Formula σ → Formula σ
  | falsum => falsum
  ...
  | equal left right =>
      equal (Term.closeFreeAt target id depth left) (Term.closeFreeAt target id depth right)
  ...
  | forallE s body => forallE s (closeFreeAt target id (next_depth target s depth) body)
  | existsE s body => existsE s (closeFreeAt target id (next_depth target s depth) body)
end Formula
end FirstOrder
end Logic
end YesMetaZFC
