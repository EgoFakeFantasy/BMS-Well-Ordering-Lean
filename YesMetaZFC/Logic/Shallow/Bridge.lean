...
structure TermView {σ : Signature.{u, v, w}} (M : Structure.{u, v, w, x} σ) (sort : σ.SortSymbol) where
  deep : Term σ
...
/-- bound 变量桥接。 -/
def bvar {σ : Signature.{u, v, w}} (M : Structure.{u, v, w, x} σ) (sort : σ.SortSymbol) (idx : Nat) : TermView M sort where
  ...
  sound := by
    ...
    simp [Term.eval]
/-- free 变量桥接。 -/
def fvar {σ : Signature.{u, v, w}} (M : Structure.{u, v, w, x} σ) (sort : σ.SortSymbol) (id : FreeVarId) : TermView M sort where
...
structure ArgsView {σ : Signature.{u, v, w}} (M : Structure.{u, v, w, x} σ) (sorts : List σ.SortSymbol) where
  deep : List (Term σ)
...
def cons {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ}
    {sort : σ.SortSymbol} {sorts : List σ.SortSymbol} (head : TermView M sort) (tail : ArgsView M sorts) :
...
namespace TermView
/-- 函数应用桥接。 -/
def app {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ} (f : σ.FuncSymbol) (args : ArgsView M (σ.funcDomain f)) :
...
structure FormulaView {σ : Signature.{u, v, w}} [DecidableEq σ.SortSymbol] (M : Structure.{u, v, w, x} σ) where
  deep : Formula σ
  wellFormed : FormulaWellFormed deep
  prop : Env M → Prop
  sound : ∀ env, prop env ↔ Formula.satisfies env deep
namespace FormulaView
def falsum {σ : Signature.{u, v, w}} [DecidableEq σ.SortSymbol] (M : Structure.{u, v, w, x} σ) : FormulaView M where
  deep := Formula.falsum
  ...
  sound := by
    intro env
    rfl
def truth {σ : Signature.{u, v, w}} [DecidableEq σ.SortSymbol] (M : Structure.{u, v, w, x} σ) : FormulaView M where
  deep := Formula.truth
...
def rel {σ : Signature.{u, v, w}} [DecidableEq σ.SortSymbol]
    {M : Structure.{u, v, w, x} σ} (r : σ.RelSymbol) (args : ArgsView M (σ.relDomain r)) :
    FormulaView M where
...
def equal {σ : Signature.{u, v, w}} [DecidableEq σ.SortSymbol]
    {M : Structure.{u, v, w, x} σ} {sort : σ.SortSymbol} (left right : TermView M sort) : FormulaView M where
...
def forallE {σ : Signature.{u, v, w}} [DecidableEq σ.SortSymbol]
    {M : Structure.{u, v, w, x} σ} (sort : σ.SortSymbol) (body : FormulaView M) : FormulaView M where
...
structure BridgeResult {σ : Signature.{u, v, w}} [DecidableEq σ.SortSymbol] (M : Structure.{u, v, w, x} σ) where
  spec : BridgeSpec σ
