...
def empty {σ : Signature.{u, v, w}} : RelContext σ :=
  fun _ => Empty
def extend {σ : Signature.{u, v, w}} (Δ : RelContext σ) (domains : List σ.SortSymbol) : RelContext σ :=
...
inductive Formula (σ : Signature.{u, v, w}) :
  ...
  | relBound : (domains : List σ.SortSymbol) → Δ domains → List (Term σ Γ) → Formula σ Γ Δ
  | relFree : (domains : List σ.SortSymbol) → RelVarId → List (Term σ Γ) → Formula σ Γ Δ
  ...
  | forallObj : (sort : σ.SortSymbol) →
        Formula σ (Infinitary.BoundContext.extend Γ (oneSort sort)) Δ →
          Formula σ Γ Δ
  | existsObj : (sort : σ.SortSymbol) →
        Formula σ (Infinitary.BoundContext.extend Γ (oneSort sort)) Δ →
          Formula σ Γ Δ
  | forallRel : (domains : List σ.SortSymbol) →
        Formula σ Γ (RelContext.extend Δ domains) → Formula σ Γ Δ
  | existsRel : (domains : List σ.SortSymbol) →
        Formula σ Γ (RelContext.extend Δ domains) → Formula σ Γ Δ
namespace Henkin
...
/-- Henkin 语义环境。 -/
structure Env {σ : Signature.{u, v, w}} (H : Structure.{u, v, w, x, y} σ) (Γ : BoundContext.{u, v, w, i} σ) (Δ : RelContext.{u, v, w, i} σ) where
...
def pushObj {σ : Signature.{u, v, w}} {H : Structure.{u, v, w, x, y} σ}
    {Γ : BoundContext.{u, v, w, i} σ} {Δ : RelContext.{u, v, w, i} σ} (env : Env H Γ Δ) (sort : σ.SortSymbol) (value : H.base.Domain)
    (hValue : H.base.sortInterp sort value) :
    Env H (Infinitary.BoundContext.extend Γ (oneSort sort)) Δ where
  firstOrder :=
    env.firstOrder.pushBlock (oneSort sort) (fun _ => value) (fun _ => hValue)
  boundRel := env.boundRel
  freeRel := env.freeRel
def pushRel {σ : Signature.{u, v, w}} {H : Structure.{u, v, w, x, y} σ}
    {Γ : BoundContext.{u, v, w, i} σ} {Δ : RelContext.{u, v, w, i} σ} (env : Env H Γ Δ) (domains : List σ.SortSymbol) (rel : H.relDomain domains) :
...
def satisfies {σ : Signature.{u, v, w}} {H : Structure.{u, v, w, x, y} σ}
    {Γ : BoundContext.{u, v, w, i} σ} {Δ : RelContext.{u, v, w, i} σ} (env : Env H Γ Δ) : _root_.YesMetaZFC.Logic.SecondOrder.Formula σ Γ Δ → Prop
  | .falsum => False
  | .truth => True
  ...
  | .relBound domains rel args =>
      H.relInterp domains (env.boundRel domains rel) (args.map (Infinitary.Term.eval env.firstOrder))
  | .relFree domains rel args =>
      H.relInterp domains (env.freeRel domains rel) (args.map (Infinitary.Term.eval env.firstOrder))
...
abbrev Relation {σ : Signature.{u, v, w}} (M : FirstOrder.Structure.{u, v, w, x} σ) (_domains : List σ.SortSymbol) :=
  List M.Domain → Prop
structure Env {σ : Signature.{u, v, w}} (M : FirstOrder.Structure.{u, v, w, x} σ) (Γ : BoundContext.{u, v, w, i} σ) (Δ : RelContext.{u, v, w, i} σ) where
...
def pushObj {σ : Signature.{u, v, w}} {M : FirstOrder.Structure.{u, v, w, x} σ}
    {Γ : BoundContext.{u, v, w, i} σ} {Δ : RelContext.{u, v, w, i} σ} (env : Env M Γ Δ) (sort : σ.SortSymbol) (value : M.Domain)
...
def pushRel {σ : Signature.{u, v, w}} {M : FirstOrder.Structure.{u, v, w, x} σ}
    {Γ : BoundContext.{u, v, w, i} σ} {Δ : RelContext.{u, v, w, i} σ} (env : Env M Γ Δ) (domains : List σ.SortSymbol) (rel : Relation M domains) :
...
def satisfies {σ : Signature.{u, v, w}} {M : FirstOrder.Structure.{u, v, w, x} σ}
    {Γ : BoundContext.{u, v, w, i} σ} {Δ : RelContext.{u, v, w, i} σ} (env : Env M Γ Δ) : _root_.YesMetaZFC.Logic.SecondOrder.Formula σ Γ Δ → Prop
  | .falsum => False
  | .truth => True
