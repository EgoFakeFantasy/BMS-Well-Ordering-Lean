...
/-!
# 基本一阶元数理定理
本模块提供后续元数学和自动化共同消费的基础逻辑定理。机械命题模式由
`derive_prop` 重放，但最终证明项只依赖 `Derives` 可信核及其通用派生接口。
自动化层不导入本模块，因此这些定理以后可以安全注册为资源，不会形成自举循环。
文献中把合取写成 `¬ₘ (φ ⟶ₘ ¬ₘ ψ)`。新核保留原生 `∧ₘ`，同时在本模块给出
两种表示之间的推导桥；有限公式级别和线性证明长度不进入定理接口。
-/
...
theorem imp_exchange {σ : Signature.{u, v, w}}
    ...
    {φ θ ψ : Formula σ} (hφ : Formula.Admissible φ) (hθ : Formula.Admissible θ) (hψ : Formula.Admissible ψ) :
...
theorem imp_trans {σ : Signature.{u, v, w}}
    [DecidableEq σ.SortSymbol] {T : Theory σ} {Γ : Context σ}
    {φ ψ θ : Formula σ} (hFirst : Γ ⊢ₘ[T] φ ⟶ₘ ψ) (hSecond : Γ ⊢ₘ[T] ψ ⟶ₘ θ) :
    Γ ⊢ₘ[T] φ ⟶ₘ θ := by
  derive_prop
/-- 合取可以推出“由左侧推出右侧”的蕴含。 -/
...
theorem conj_imp_mono {σ : Signature.{u, v, w}}
    [DecidableEq σ.SortSymbol] {T : Theory σ} {Γ : Context σ}
    {φ₁ φ₂ ψ₁ ψ₂ : Formula σ} (hφ₁ : Formula.Admissible φ₁) (hφ₂ : Formula.Admissible φ₂) (hψ₁ : Formula.Admissible ψ₁) (hψ₂ : Formula.Admissible ψ₂) :
    Γ ⊢ₘ[T] (φ₁ ⟶ₘ ψ₁) ⟶ₘ ((φ₂ ⟶ₘ ψ₂) ⟶ₘ ((φ₁ ∧ₘ φ₂) ⟶ₘ (ψ₁ ∧ₘ ψ₂))) := by
  derive_prop
/--
...
theorem of_neg_assumption_contradiction {σ : Signature.{u, v, w}}
    [DecidableEq σ.SortSymbol] {T : Theory σ} {Γ : Context σ}
    {φ ψ : Formula σ} (hφ : Formula.Admissible φ) (hPositive : (¬ₘ φ :: Γ) ⊢ₘ[T] ψ) (hNegative : (¬ₘ φ :: Γ) ⊢ₘ[T] ¬ₘ ψ) :
    Γ ⊢ₘ[T] φ := by
  derive_prop
/--
...
theorem by_formula_cases {σ : Signature.{u, v, w}}
    [DecidableEq σ.SortSymbol] {T : Theory σ} {Γ : Context σ}
    {φ ψ : Formula σ} (hφ : Formula.Admissible φ) (hNegative : (¬ₘ φ :: Γ) ⊢ₘ[T] ψ) (hPositive : (φ :: Γ) ⊢ₘ[T] ψ) :
    Γ ⊢ₘ[T] ψ := by
  derive_prop
/-- 双重否定消去的蕴含形式。 -/
...
theorem encoded_conj_intro_under {σ : Signature.{u, v, w}}
    [DecidableEq σ.SortSymbol] {T : Theory σ} {Γ : Context σ}
    {φ θ ψ : Formula σ} (hφ : Formula.Admissible φ) (hθ : Formula.Admissible θ) (hψ : Formula.Admissible ψ) :
    Γ ⊢ₘ[T] (θ ⟶ₘ φ) ⟶ₘ ((θ ⟶ₘ ψ) ⟶ₘ (θ ⟶ₘ ¬ₘ (φ ⟶ₘ ¬ₘ ψ))) := by
  derive_prop
/-- 否定一个蕴含会推出其后件的否定。 -/
