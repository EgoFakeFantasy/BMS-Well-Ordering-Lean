...
import YesMetaZFC.Logic.FirstOrder.FormalSystem.Metatheory.ProofT.ZFCRep.ProofCodeRejection
...
条件的否定。
-/
theorem ProofT.ZFCRep.code_neg_of_terminal_unchecked
    (number : Nat)
    {formula : SetFormula}
    {code : SetTerm}
  ...
  cases hReplay :
      fs_replay_code
        ProofCode.fs_zfc_replacement_support_enumeration number with
  | none =>
      exact
        ProofT.ZFCRep.code_neg_of_replay_failure
          number code hCode
          (fs_replay_code_failure_of_none
            ProofCode.fs_zfc_replacement_support_enumeration number hReplay)
  | some state =>
      have hTargetLastNot :
          state.proof.getLast? ≠
      ...
      | some certificates =>
          ...
          have hWitnessAdmissible :
              Formula.Admissible
                (ProofT.ZFCRep.witness number code) :=
          ...
          have hTerminal :
              Γ ⊢ₘ[fs_zfc_support_raw_theory]
                proof_sequence_terminal_condition
                  (ProofT.ZFCRep.proof_sequence number) code :=
          ...
          simpa [ProofT.ZFCRep.proof_sequence, decoded] using
