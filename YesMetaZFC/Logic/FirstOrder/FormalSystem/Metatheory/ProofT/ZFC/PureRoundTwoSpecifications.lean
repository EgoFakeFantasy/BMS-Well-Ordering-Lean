...
逐类消费旧阶段的已证规格，核验原正文翻译在四个函数替换后仍然相同。
-/
namespace YesMetaZFC.Logic.FirstOrder.FormalSystem.ProofT.ZFC.PureRoundTwoSpecifications
open PureModel PureNaturalInduction PureRoundTwoStage
...
theorem arithmetic_specification (hℳ : Theory.Models ℳ theory) (operation : PureArithmeticRecurrence.Operation)
  ...
  have hOld := PureNaturalDifferenceStage.arithmetic_specification hℳ operation hLeft hRight output
  have hTransfer := prior_transfer hℳ (PureArithmeticRecurrence.specification operation) (by cases operation <;> rfl) (.cons output (.cons left (.cons right .nil)))
...
theorem hierarchy_specification (hℳ : Theory.Models ℳ theory) :
  ...
  rw [prior_function hℳ .finiteHierarchy rfl .nil]
...
theorem minimum_definition (hℳ : Theory.Models ℳ theory)
  ...
  (prior_transfer hℳ _ rfl args).mp (PureNaturalDifferenceStage.minimum_definition hℳ args)
...
end YesMetaZFC.Logic.FirstOrder.FormalSystem.ProofT.ZFC.PureRoundTwoSpecifications
