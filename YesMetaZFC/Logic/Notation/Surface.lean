...
def checkedApp {σ : Signature} (function : σ.FuncSymbol) (arguments : List (Term σ)) (_arity : arguments.length = σ.funcArity function := by decide) : Term σ :=
...
private partial def compileTerm (binders : List Binder) (termSyntax : Syntax) :
  ...
  | `(foTerm| $identifier:ident) =>
      ...
      | some binder =>
          `(FirstOrder.Term.var (FirstOrder.Var.fvar $(binder.sort) $(quote binder.id)))
      | none =>
