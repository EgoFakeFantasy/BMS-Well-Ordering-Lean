...
inductive Formula (κ : Kappa.{i}) (σ : Signature.{u, v, w}) :
  ...
  | existsBlock {Γ : BoundContext σ} {I : Type i} :
      κ.Small I → (sortOf : I → σ.SortSymbol) →
        Formula κ σ (BoundContext.extend Γ sortOf) → Formula κ σ Γ
/-- 上下文索引语义环境。 -/
structure Env {σ : Signature.{u, v, w}} (M : FirstOrder.Structure.{u, v, w, x} σ) (Γ : BoundContext.{u, v, w, i} σ) where
...
def pushBlock {σ : Signature.{u, v, w}}
    ...
    {I : Type i} (sortOf : I → σ.SortSymbol) (values : I → M.Domain) (hValues : ∀ idx, M.sortInterp (sortOf idx) (values idx)) :
