...
mutual
  ...
  inductive TermWellSorted {σ : Signature.{u, v, w}} :
    ...
    | bvar {context : Context σ} {sort : SimpleType σ.BaseSort} {index : Nat} (hLookup : Context.lookup? context index = some sort) :
    ...
    | app {context : Context σ} (symbol : σ.FuncSymbol) {arguments : List (Term σ)} (hArguments : ArgsWellSorted context arguments (σ.funcDomain symbol)) :
        TermWellSorted context (.app symbol arguments) (σ.funcCodomain symbol)
    | apply {context : Context σ} {function argument : Term σ}
        {domain codomain : SimpleType σ.BaseSort} (hFunction : TermWellSorted context function (.arrow domain codomain))
        (hArgument : TermWellSorted context argument domain) :
        TermWellSorted context (.apply function argument) codomain
    | lam {context : Context σ} {domain codomain : SimpleType σ.BaseSort} {body : Term σ} (hBody : TermWellSorted (domain :: context) body codomain) :
  ...
  inductive ArgsWellSorted {σ : Signature.{u, v, w}} :
    ...
    | cons {context : Context σ} {term : Term σ} {terms : List (Term σ)}
        {sort : SimpleType σ.BaseSort} {sorts : List (SimpleType σ.BaseSort)} (hTerm : TermWellSorted context term sort)
...
inductive FormulaWellFormed {σ : Signature.{u, v, w}} :
  ...
  | rel {context : Context σ} (symbol : σ.RelSymbol) {arguments : List (Term σ)} (hArguments : ArgsWellSorted context arguments (σ.relDomain symbol)) :
      FormulaWellFormed context (.rel symbol arguments)
  | equal {context : Context σ} {sort : SimpleType σ.BaseSort} {left right : Term σ} (hLeft : TermWellSorted context left sort)
      (hRight : TermWellSorted context right sort) :
      FormulaWellFormed context (.equal sort left right)
  | neg {context : Context σ} {body : Formula σ} (hBody : FormulaWellFormed context body) :
      FormulaWellFormed context (.neg body)
  | conj {context : Context σ} {left right : Formula σ} (hLeft : FormulaWellFormed context left) (hRight : FormulaWellFormed context right) :
      FormulaWellFormed context (.conj left right)
  | disj {context : Context σ} {left right : Formula σ} (hLeft : FormulaWellFormed context left) (hRight : FormulaWellFormed context right) :
      FormulaWellFormed context (.disj left right)
  | imp {context : Context σ} {left right : Formula σ} (hLeft : FormulaWellFormed context left) (hRight : FormulaWellFormed context right) :
      FormulaWellFormed context (.imp left right)
  | iff {context : Context σ} {left right : Formula σ} (hLeft : FormulaWellFormed context left) (hRight : FormulaWellFormed context right) :
      FormulaWellFormed context (.iff left right)
  | forallE {context : Context σ} {sort : SimpleType σ.BaseSort} {body : Formula σ} (hBody : FormulaWellFormed (sort :: context) body) :
      FormulaWellFormed context (.forallE sort body)
  | existsE {context : Context σ} {sort : SimpleType σ.BaseSort} {body : Formula σ} (hBody : FormulaWellFormed (sort :: context) body) :
      FormulaWellFormed context (.existsE sort body)
mutual
  /-- 在绑定上下文中推断项的简单类型。失败同时表示 scope 或 sort 错误。 -/
  def Term.inferSortWith {σ : Signature.{u, v, w}} [DecidableEq σ.BaseSort] (context : Context σ) : Term σ → Option (SimpleType σ.BaseSort)
    | .var (.bvar sort index) =>
  ...
  /-- 推断参数列表的逐项类型。 -/
  def Term.inferSortListWith {σ : Signature.{u, v, w}} [DecidableEq σ.BaseSort] (context : Context σ) : List (Term σ) → Option (List (SimpleType σ.BaseSort))
    | [] => some []
    | term :: rest => do
...
namespace Term
/-- 在空绑定上下文中检查闭项并返回其类型。 -/
def inferSort? {σ : Signature.{u, v, w}} [DecidableEq σ.BaseSort] (term : Term σ) : Option (SimpleType σ.BaseSort) :=
  term.inferSortWith []
/-- 闭项的可计算类型检查。 -/
...
mutual
  ...
  theorem Term.inferSortWith_sound {σ : Signature.{u, v, w}} [DecidableEq σ.BaseSort]
      {context : Context σ} {term : Term σ} {sort : SimpleType σ.BaseSort} (hCheck : term.inferSortWith context = some sort) :
            ...
            | some argumentSort =>
                ...
                | arrow domain codomain =>
                    ...
                    · simp [Term.inferSortWith, hFunction, hArgument, hDomain] at hCheck
                      subst argumentSort
                      subst sort
                      exact .apply (Term.inferSortWith_sound hFunction) (Term.inferSortWith_sound hArgument)
                    · simp [Term.inferSortWith, hFunction, hArgument, hDomain] at hCheck
  ...
  theorem Term.inferSortListWith_sound {σ : Signature.{u, v, w}}
      ...
      {sorts : List (SimpleType σ.BaseSort)} (hCheck : Term.inferSortListWith context terms = some sorts) :
    ...
    | cons term rest =>
        ...
        | some sort =>
            ...
            | some restSorts =>
                simp [Term.inferSortListWith, hTerm, hRest] at hCheck
                subst sorts
                exact .cons (Term.inferSortWith_sound hTerm) (Term.inferSortListWith_sound hRest)
end
namespace Formula
/-- 公式的可计算 scope 与 sort 检查。 -/
def checkWith {σ : Signature.{u, v, w}} [DecidableEq σ.BaseSort] (context : Context σ) : Formula σ → Bool
  | .falsum | .truth => true
...
/-- 闭公式的可计算检查。 -/
def check {σ : Signature.{u, v, w}} [DecidableEq σ.BaseSort] (formula : Formula σ) : Bool :=
  formula.checkWith []
/-- 公式 checker 的正确性。 -/
theorem checkWith_sound {σ : Signature.{u, v, w}} [DecidableEq σ.BaseSort]
    {context : Context σ} {formula : Formula σ} (hCheck : formula.checkWith context = true) :
  ...
  | equal sort left right =>
      simp [checkWith] at hCheck
      exact .equal (Term.inferSortWith_sound hCheck.1) (Term.inferSortWith_sound hCheck.2)
  | neg body ih =>
      exact .neg (ih hCheck)
...
mutual
  ...
  def Term.instantiateAt {σ : Signature.{u, v, w}} (depth : Nat) (replacement : Term σ) : Term σ → Term σ
    | .var (.bvar sort index) =>
        if index < depth then
          .var (.bvar sort index)
    ...
    | .apply function argument =>
        .apply (Term.instantiateAt depth replacement function) (Term.instantiateAt depth replacement argument)
    | .lam domain codomain body =>
        .lam domain codomain (Term.instantiateAt (depth + 1) replacement body)
  /-- 对项列表执行局部无名替换。 -/
  def Term.instantiateListAt {σ : Signature.{u, v, w}} (depth : Nat) (replacement : Term σ) : List (Term σ) → List (Term σ)
    | [] => []
    | term :: rest =>
...
def lookup {σ : Signature.{u, v, w}} [DecidableEq σ.BaseSort] (substitution : TermSubstitution σ) (sort : SimpleType σ.BaseSort)
...
def keyAbsent {σ : Signature.{u, v, w}} [DecidableEq σ.BaseSort] (sort : SimpleType σ.BaseSort) (id : FreeVarId) : TermSubstitution σ → Bool
  | [] => true
  | binding :: rest =>
...
def Admissible {σ : Signature.{u, v, w}} [DecidableEq σ.BaseSort] (substitution : TermSubstitution σ) : Prop :=
  ∀ sort id replacement,
...
theorem check_sound {σ : Signature.{u, v, w}} [DecidableEq σ.BaseSort] :
  ...
  | binding :: rest, hCheck => by
      have hFields : (decide (binding.replacement.inferSort? = some binding.sort) = true ∧
...
mutual
  ...
  def Term.applySubstitution {σ : Signature.{u, v, w}} [DecidableEq σ.BaseSort] (substitution : TermSubstitution σ) : Term σ → Term σ
    ...
    | .apply function argument =>
        .apply (function.applySubstitution substitution) (argument.applySubstitution substitution)
    | .lam domain codomain body =>
