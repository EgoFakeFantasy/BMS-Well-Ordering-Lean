...
structure Structure (σ : Signature.{u, v, w}) where
  ...
  lambdaSort :
    ∀ domain codomain body, (∀ value, sortInterp domain value → sortInterp codomain (body value)) →
...
structure ExtensionalContract {σ : Signature.{u, v, w}} (M : Structure.{u, v, w, x} σ) where
  lambdaCongr :
    ∀ domain codomain left right, (∀ value, M.sortInterp domain value → left value = right value) →
        M.lambdaInterp domain codomain left = M.lambdaInterp domain codomain right
  beta :
    ∀ domain codomain body argument, (∀ value, M.sortInterp domain value → M.sortInterp codomain (body value)) →
  ...
  eta :
    ∀ domain codomain functionValue,
      ...
      M.lambdaInterp domain codomain (fun argument => M.applyInterp functionValue argument) =
        functionValue
...
普通参数函数误认成 `diff`。
-/
structure ExtensionalWitnessContract {σ : Signature.{u, v, w}} (M : Structure.{u, v, w, x} σ) where
  distinguishes :
...
def push {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ} (env : Env M) (value : M.Domain) : Env M where
  ...
  freeVal := env.freeVal
/-- 删除 bound 栈前面的若干值。 -/
def drop {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ} (amount : Nat) (env : Env M) : Env M where
  boundVal := fun index => env.boundVal (index + amount)
  freeVal := env.freeVal
/-- 解释 `shiftAbove amount cutoff` 时跳过环境中的同一段。 -/
def skip {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ} (amount cutoff : Nat) (env : Env M) : Env M where
  boundVal := fun index =>
    if index < cutoff then env.boundVal index else env.boundVal (index + amount)
  freeVal := env.freeVal
/-- 在 bound 栈的指定深度插入一个值。 -/
def insertAt {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ} (depth : Nat) (value : M.Domain) (env : Env M) : Env M where
  boundVal := fun index =>
    if index < depth then
      env.boundVal index
  ...
  freeVal := env.freeVal
@[simp]
theorem push_bound_zero {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ} (env : Env M) (value : M.Domain) : (env.push value).boundVal 0 = value :=
  rfl
@[simp]
theorem push_bound_succ {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ} (env : Env M) (value : M.Domain) (index : Nat) :
    (env.push value).boundVal (index + 1) = env.boundVal index :=
  rfl
@[simp]
theorem push_free {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ} (env : Env M) (value : M.Domain) (sort : SimpleType σ.BaseSort) (id : FreeVarId) :
    (env.push value).freeVal sort id = env.freeVal sort id :=
  rfl
@[simp]
theorem drop_bound {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ} (amount : Nat) (env : Env M) (index : Nat) :
    (env.drop amount).boundVal index = env.boundVal (index + amount) :=
  rfl
@[simp]
theorem drop_free {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ} (amount : Nat) (env : Env M) (sort : SimpleType σ.BaseSort) (id : FreeVarId) :
    (env.drop amount).freeVal sort id = env.freeVal sort id :=
  rfl
theorem skip_zero_bound {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ} (amount : Nat) (env : Env M) (index : Nat) :
    (env.skip amount 0).boundVal index = (env.drop amount).boundVal index := by
  simp [skip, drop]
theorem skip_push_bound {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ} (amount cutoff : Nat) (env : Env M) (value : M.Domain) (index : Nat) :
    ((env.push value).skip amount (cutoff + 1)).boundVal index = ((env.skip amount cutoff).push value).boundVal index := by
  cases index with
  | zero =>
      rfl
  | succ previous =>
...
theorem drop_push_bound {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ} (amount : Nat) (env : Env M) (value : M.Domain) (index : Nat) :
    ((env.push value).drop (amount + 1)).boundVal index = (env.drop amount).boundVal index := by
...
theorem insertAt_push_bound {σ : Signature.{u, v, w}}
    {M : Structure.{u, v, w, x} σ} (depth : Nat) (env : Env M) (inserted value : M.Domain) (index : Nat) :
    ((env.push value).insertAt (depth + 1) inserted).boundVal index = ((env.insertAt depth inserted).push value).boundVal index := by
  cases index with
  | zero =>
      rfl
  | succ previous =>
...
def WellSorted {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ} (context : Context σ) (env : Env M) : Prop :=
  (∀ index sort, Context.lookup? context index = some sort →
    M.sortInterp sort (env.boundVal index)) ∧ (∀ sort id, M.sortInterp sort (env.freeVal sort id))
/-- 压入一个对应 sort 的值后，环境仍尊重扩展后的上下文。 -/
theorem wellSorted_push {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ}
    {context : Context σ} {env : Env M} {sort : SimpleType σ.BaseSort} (hEnv : WellSorted context env) {value : M.Domain} (hValue : M.sortInterp sort value) :
...
noncomputable def canonical {σ : Signature.{u, v, w}} (M : Structure.{u, v, w, x} σ) : Env M where
  boundVal := fun _ => Classical.choice M.nonempty
  freeVal := fun sort _ => Classical.choose (M.sortNonempty sort)
/-- 规范环境在空 bound 上下文下类型正确。 -/
theorem canonical_wellSorted {σ : Signature.{u, v, w}} (M : Structure.{u, v, w, x} σ) : (canonical M).WellSorted [] := by
  constructor
...
def Satisfies {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ} (env : Env M) : Formula σ → Prop
  | .falsum => False
  | .truth => True
...
mutual
  ...
  theorem Term.eval_eq_of_wellSorted_env {σ : Signature.{u, v, w}}
      ...
      {env₁ env₂ : Env M} {term : Term σ} {sort : SimpleType σ.BaseSort} (hTerm : TermWellSorted context term sort) (hBound :
        ∀ index target,
  ...
  theorem Term.evalList_eq_of_argsWellSorted_env {σ : Signature.{u, v, w}}
      ...
      {sorts : List (SimpleType σ.BaseSort)} (hTerms : ArgsWellSorted context terms sorts) (hBound :
        ∀ index target,
          Context.lookup? context index = some target →
            env₁.boundVal index = env₂.boundVal index) (hFree : ∀ target id, env₁.freeVal target id = env₂.freeVal target id) :
      terms.map (Term.eval env₁) = terms.map (Term.eval env₂) := by
...
theorem Formula.satisfies_iff_of_wellFormed_env
    ...
    {context : Context σ} {env₁ env₂ : Env M} {formula : Formula σ} (hFormula : FormulaWellFormed context formula) (hBound :
      ∀ index target,
        Context.lookup? context index = some target →
          env₁.boundVal index = env₂.boundVal index) (hFree : ∀ target id, env₁.freeVal target id = env₂.freeVal target id) :
...
def EnvMatches {σ : Signature.{u, v, w}} [DecidableEq σ.BaseSort]
    {M : Structure.{u, v, w, x} σ} (substitution : TermSubstitution σ) (sourceEnv targetEnv : Env M) : Prop :=
...
def semanticEnv {σ : Signature.{u, v, w}} [DecidableEq σ.BaseSort]
    {M : Structure.{u, v, w, x} σ} (substitution : TermSubstitution σ) (sourceEnv : Env M) : Env M where
...
theorem push {σ : Signature.{u, v, w}} [DecidableEq σ.BaseSort]
    {M : Structure.{u, v, w, x} σ} {substitution : TermSubstitution σ}
    {sourceEnv targetEnv : Env M} (hAdmissible : substitution.Admissible) (hEnv : EnvMatches substitution sourceEnv targetEnv) (value : M.Domain) :
  ...
  · intro sort id
    ...
    | some replacement =>
        ...
        have hStable :
          ...
          apply Term.eval_eq_of_wellSorted_env (Term.inferSortWith_sound (by simpa [Term.inferSort?] using hSort))
...
/-- 自由变量平移对应的环境投影关系。 -/
def EnvMatches {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ} (offset : Nat) (sourceEnv targetEnv : Env M) : Prop :=
  (∀ index, targetEnv.boundVal index = sourceEnv.boundVal index) ∧
    ∀ sort id, targetEnv.freeVal sort id = sourceEnv.freeVal sort (id + offset)
/-- 从平移后的源环境构造原编号可消费的目标环境。 -/
def semanticEnv {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ} (offset : Nat) (sourceEnv : Env M) : Env M where
  boundVal := sourceEnv.boundVal
  freeVal := fun sort id => sourceEnv.freeVal sort (id + offset)
/-- 自由变量投影环境保持空上下文类型正确性。 -/
theorem semanticEnv_wellSorted {σ : Signature.{u, v, w}}
    {M : Structure.{u, v, w, x} σ} {offset : Nat} {sourceEnv : Env M} (hSource : sourceEnv.WellSorted []) : (semanticEnv offset sourceEnv).WellSorted [] := by
  constructor
  · intro index sort hLookup
    simp [Context.lookup?] at hLookup
  · intro sort id
...
theorem push {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ}
    {offset : Nat} {sourceEnv targetEnv : Env M} (hEnv : EnvMatches offset sourceEnv targetEnv) (value : M.Domain) :
...
mutual
  ...
  theorem Term.eval_applySubstitution_eq_of_envMatches
      ...
      {M : Structure.{u, v, w, x} σ} {substitution : TermSubstitution σ}
      {sourceEnv targetEnv : Env M} (hAdmissible : substitution.Admissible) (hEnv : TermSubstitution.EnvMatches substitution sourceEnv targetEnv) :
      ∀ term : Term σ,
    ...
    | .var (.fvar sort id) => by
        cases hLookup : substitution.lookup sort id with
        | none =>
            simpa [Term.applySubstitution, Term.eval, hLookup] using (hEnv.2 sort id).symm
        | some replacement =>
            simpa [Term.applySubstitution, Term.eval, hLookup] using (hEnv.2 sort id).symm
    ...
    | .lam domain codomain body => by
        ...
        congr 1
        funext value
        exact Term.eval_applySubstitution_eq_of_envMatches hAdmissible (TermSubstitution.EnvMatches.push hAdmissible hEnv value) body
  /-- typed substitution 的项列表语义。 -/
  theorem Term.evalList_applySubstitution_eq_of_envMatches
      {σ : Signature.{u, v, w}} [DecidableEq σ.BaseSort]
      {M : Structure.{u, v, w, x} σ} {substitution : TermSubstitution σ}
      {sourceEnv targetEnv : Env M} (hAdmissible : substitution.Admissible) (hEnv : TermSubstitution.EnvMatches substitution sourceEnv targetEnv) :
      ∀ terms : List (Term σ), (Term.applySubstitutionList substitution terms).map (Term.eval sourceEnv) =
...
mutual
  ...
  theorem Term.eval_renameFreeVars_eq_of_envMatches
      {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ}
      {offset : Nat} {sourceEnv targetEnv : Env M} (hEnv : FreeVarRenaming.EnvMatches offset sourceEnv targetEnv) :
      ∀ term : Term σ,
    ...
    | .lam domain codomain body => by
        ...
        congr 1
        funext value
        exact Term.eval_renameFreeVars_eq_of_envMatches (FreeVarRenaming.EnvMatches.push hEnv value) body
  /-- 自由变量平移的项列表语义。 -/
  theorem Term.evalList_renameFreeVars_eq_of_envMatches
      {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ}
      {offset : Nat} {sourceEnv targetEnv : Env M} (hEnv : FreeVarRenaming.EnvMatches offset sourceEnv targetEnv) :
      ∀ terms : List (Term σ), (Term.renameFreeVarsList offset terms).map (Term.eval sourceEnv) =
          terms.map (Term.eval targetEnv)
    | [] => by
        rfl
    | term :: rest => by
...
mutual
  ...
  theorem Term.eval_shiftAbove {σ : Signature.{u, v, w}}
      {M : Structure.{u, v, w, x} σ} (env : Env M) (amount cutoff : Nat) (term : Term σ) :
  ...
  theorem Term.evalList_shiftAbove {σ : Signature.{u, v, w}}
      {M : Structure.{u, v, w, x} σ} (env : Env M) (amount cutoff : Nat) (terms : List (Term σ)) :
...
mutual
  ...
  theorem Term.eval_instantiateAt {σ : Signature.{u, v, w}}
      {M : Structure.{u, v, w, x} σ} (env : Env M) (depth : Nat) (replacement term : Term σ) :
      Term.eval env (Term.instantiateAt depth replacement term) =
        Term.eval (env.insertAt depth (Term.eval (env.drop depth) replacement)) term := by
    cases term with
    | var value =>
        cases value with
        | bvar sort index =>
            ...
            · by_cases hEq : index = depth
              · subst index
                calc
                  Term.eval env (Term.instantiateAt depth replacement (.var (.bvar sort depth))) =
                  ...
                  _ = Term.eval (env.drop depth) replacement := by
                    ...
                    · intro target id
                      rfl
                  _ = Term.eval (env.insertAt depth (Term.eval (env.drop depth) replacement)) (.var (.bvar sort depth)) := by
    ...
    | lam domain codomain body =>
        ...
        apply Term.eval_eq_of_env_eq
        · intro index
          rw [Term.eval_eq_of_env_eq ((env.push value).drop (depth + 1)) (env.drop depth) (Env.drop_push_bound depth env value)
  ...
  theorem Term.evalList_instantiateAt {σ : Signature.{u, v, w}}
      {M : Structure.{u, v, w, x} σ} (env : Env M) (depth : Nat) (replacement : Term σ) (terms : List (Term σ)) :
      (Term.instantiateListAt depth replacement terms).map (Term.eval env) =
        terms.map (Term.eval (env.insertAt depth (Term.eval (env.drop depth) replacement))) := by
    cases terms with
    | nil =>
        rfl
    | cons head tail =>
...
theorem eval_instantiate {σ : Signature.{u, v, w}}
    {M : Structure.{u, v, w, x} σ} (env : Env M) (replacement body : Term σ) :
...
mutual
  ...
  theorem Term.eval_sort_of_wellSorted {σ : Signature.{u, v, w}}
      ...
      {term : Term σ} {sort : SimpleType σ.BaseSort} (hEnv : Env.WellSorted context env) (hTerm : TermWellSorted context term sort) :
    ...
    | apply hFunction hArgument =>
        simpa [Term.eval] using
          M.applySort _ _ _ _ (Term.eval_sort_of_wellSorted hEnv hFunction) (Term.eval_sort_of_wellSorted hEnv hArgument)
    | lam hBody =>
        simp only [Term.eval]
        apply M.lambdaSort
        intro value hValue
  ...
  theorem args_satisfy_of_wellSorted {σ : Signature.{u, v, w}}
      {M : Structure.{u, v, w, x} σ} {context : Context σ} {env : Env M}
      {terms : List (Term σ)} {sorts : List (SimpleType σ.BaseSort)} (hEnv : Env.WellSorted context env) (hTerms : ArgsWellSorted context terms sorts) :
...
theorem eval_sort_of_inferSortWith {σ : Signature.{u, v, w}}
    ...
    {sort : SimpleType σ.BaseSort} (hEnv : Env.WellSorted context env) (hCheck : term.inferSortWith context = some sort) :
...
theorem semanticEnv_wellSorted {σ : Signature.{u, v, w}} [DecidableEq σ.BaseSort]
    {M : Structure.{u, v, w, x} σ} {substitution : TermSubstitution σ}
    {sourceEnv : Env M} (hAdmissible : substitution.Admissible) (hSource : sourceEnv.WellSorted []) : (semanticEnv substitution sourceEnv).WellSorted [] := by
  constructor
  · intro index sort hLookup
    simp [Context.lookup?] at hLookup
  · intro sort id
    ...
    | some replacement =>
        ...
        simpa [semanticEnv, hLookup] using
          Term.eval_sort_of_inferSortWith hSource (by simpa [Term.inferSort?] using hSort)
end TermSubstitution
namespace ExtensionalContract
/-- β 规则的对象层语义。 -/
theorem eval_beta {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ} (contract : ExtensionalContract M) (env : Env M)
    (domain codomain : SimpleType σ.BaseSort) (body argument : Term σ) (hBody :
      ∀ value, M.sortInterp domain value →
        M.sortInterp codomain (Term.eval (env.push value) body)) (hArgument : M.sortInterp domain (Term.eval env argument)) :
    Term.eval env (.apply (.lam domain codomain body) argument) =
      Term.eval (env.push (Term.eval env argument)) body := by
  simpa [Term.eval] using contract.beta domain codomain (fun value => Term.eval (env.push value) body) (Term.eval env argument)
    hBody hArgument
/-- 规范 η 展开形在外延模型中与原函数项相等。 -/
theorem eval_eta {σ : Signature.{u, v, w}} {M : Structure.{u, v, w, x} σ} (contract : ExtensionalContract M) (env : Env M)
    (domain codomain : SimpleType σ.BaseSort) (function : Term σ) (hFunction :
      M.sortInterp (.arrow domain codomain) (Term.eval env function)) :
    Term.eval env (.lam domain codomain (.apply (Term.shiftAbove 1 0 function) (.var (.bvar domain 0)))) =
      Term.eval env function := by
  simp only [Term.eval]
  have hPointwise : (fun value =>
          M.applyInterp (Term.eval (env.push value) (Term.shiftAbove 1 0 function)) ((env.push value).boundVal 0)) =
...
theorem eval_distinguishes {σ : Signature.{u, v, w}} [DecidableEq σ.BaseSort]
    {M : Structure.{u, v, w, x} σ} (contract : ExtensionalWitnessContract M) (env : Env M) (hEnv : env.WellSorted [])
    (symbol : σ.FuncSymbol) (domain codomain : SimpleType σ.BaseSort) (left right : Term σ) (hWitness : σ.isFunctionExtensionalityWitness symbol = true)
    (hDomain :
      σ.funcDomain symbol = [.arrow domain codomain, .arrow domain codomain]) (hCodomain : σ.funcCodomain symbol = domain)
    (hLeft : left.inferSort? = some (.arrow domain codomain)) (hRight : right.inferSort? = some (.arrow domain codomain))
  ...
  exact contract.distinguishes symbol domain codomain _ _
    hWitness hDomain hCodomain (Term.eval_sort_of_inferSortWith hEnv (by simpa [Term.inferSort?] using hLeft))
