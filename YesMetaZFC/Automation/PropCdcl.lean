...
private structure CdclMachine (σ : Type) where
  ...
  assignmentChanged : Data.MutByteArray σ
  assignmentChanges : Data.MutArray σ Nat
...
@[inline]
private def CdclMachine.noteAssignmentChange {σ} (machine : CdclMachine σ) (var : Nat) :
    ST σ Unit := do
  if (← machine.assignmentChanged.get! var) == 0 then
    machine.assignmentChanged.set! var 1
    machine.assignmentChanges.push var
@[inline]
private def CdclMachine.setAssignment {σ} (machine : CdclMachine σ) (var : Nat) (value : Bool) : ST σ Unit := do
  machine.assignment.set! var (encodeAssignment value)
  machine.noteAssignmentChange var
@[inline]
...
private def CdclMachine.installWatches {σ} (machine : CdclMachine σ) (id : Data.ClauseId) (clause : PackedClause) (first second : Nat) : ST σ Unit := do
  ...
  if second != first && second < clause.size then
    ...
    if slot < 2 * machine.numVars then
      machine.watchTable.pushBucket slot id.raw
private def CdclMachine.installDefaultWatches {σ} (machine : CdclMachine σ) (id : Data.ClauseId) (clause : PackedClause) : ST σ Unit :=
  machine.installWatches id clause 0 (if clause.size > 1 then 1 else 0)
private def CdclMachine.currentWatchPositions {σ} (machine : CdclMachine σ) (clause : PackedClause) : ST σ (Nat × Nat) := do
...
private def CdclMachine.installCurrentWatches {σ} (machine : CdclMachine σ) (id : Data.ClauseId) (clause : PackedClause) : ST σ Unit := do
...
private def CdclMachine.init {σ} (numVars : Nat) (clauses : Array Clause) :
  ...
  let assignment ← Data.MutByteArray.mk (σ := σ) (Data.filledByteArray numVars assignmentUnassigned)
  let assignmentChanged ←
    Data.MutByteArray.mk (σ := σ) (Data.filledByteArray numVars 0)
  let assignmentChanges ← Data.MutArray.mk (σ := σ) (#[] : Array Nat)
...
private def CdclMachine.assign {σ} (machine : CdclMachine σ) (lit : Data.PackedLit) (reason : Data.ClauseId) : ST σ Bool := do
  ...
  return true
private def CdclMachine.decide {σ} (machine : CdclMachine σ) (var : Nat) (value : Bool) : ST σ Unit := do
...
private def chooseBranch? {σ} (machine : CdclMachine σ) :
  ...
  chooseBranchFromHeap? machine
private partial def findNewWatcher {σ} (machine : CdclMachine σ) (header : Data.ClauseHeader) (skipPos index : Nat) : ST σ (Option Nat) := do
...
private def watchActionForClause {σ} (machine : CdclMachine σ) (id : Data.ClauseId) (header : Data.ClauseHeader) (assignedLit : Data.PackedLit) :
    ST σ WatchAction := do
  ...
  | none =>
      ...
      return .conflict id
private partial def inspectClauseAux {σ} (machine : CdclMachine σ) (clause : PackedClause) (index unassigned : Nat) (unitLit : Data.PackedLit) :
...
-/
private def processWatchedLit {σ} (machine : CdclMachine σ) (assignedLit : Data.PackedLit) : ST σ (Option Data.ClauseId) := do
...
private def clauseLevelCount {σ} (machine : CdclMachine σ) (clause : PackedClause) (level : Nat) : ST σ Nat := do
  let mut count := 0
  for lit in clause do
    if (← machine.levels.getD (Data.PackedLit.var lit) 0) == level then
      count := count + 1
  return count
private def CdclMachine.clauseMaxLevel {σ} (machine : CdclMachine σ) (id : Data.ClauseId) : ST σ Nat := do
  ...
  return level
private def latestClauseLitAtLevel? {σ} (machine : CdclMachine σ) (clause : PackedClause) (level : Nat) : ST σ (Option Data.PackedLit) := do
  ...
  for lit in clause do
    ...
    if (← machine.levels.getD var 0) == level then
      ...
      if latestPos < position then
        ...
        latestPos := position
  return latest?
private partial def analyzeConflictLoop {σ} (machine : CdclMachine σ) (arena : Data.ClauseArena) (analysisLevel : Nat)
...
private partial def analyzeFinalConflictLoop {σ} (machine : CdclMachine σ) (arena : Data.ClauseArena) (learned : PackedClause) : ST σ PackedClause := do
...
private def backjumpLevel {σ} (machine : CdclMachine σ) (analysisLevel : Nat) (learned : PackedClause) : ST σ Nat := do
  let mut level := 0
  for lit in learned do
    ...
    if litLevel != analysisLevel && level < litLevel then
      level := litLevel
  return level
private def CdclMachine.backtrack {σ} (machine : CdclMachine σ) (targetLevel : Nat) : ST σ Unit := do
...
private structure ConflictAnalysis where
  ...
  tautological : Bool
private def analyzeConflictAtLevel {σ} (machine : CdclMachine σ) (conflictId : Data.ClauseId) (analysisLevel : Nat) :
...
private def analyzeFinalConflict {σ} (machine : CdclMachine σ) (conflictId : Data.ClauseId) : ST σ ConflictAnalysis := do
  machine.conflictSteps.truncate 0
  let arena ← machine.arena.freeze
...
private def learnedInvariantError {σ} (machine : CdclMachine σ) (message : String) (analysis : ConflictAnalysis) (learned : PackedClause) : ST σ Result := do
...
private def assertingLiteralAfterBackjump? {σ} (machine : CdclMachine σ) (analysis : ConflictAnalysis) : ST σ (Except String (Option Data.PackedLit)) := do
...
private def addLearnedClause {σ} (machine : CdclMachine σ) (learned : PackedClause) (assertLit? : Option Data.PackedLit) :
...
后统一重建。
-/
private def CdclMachine.addExternalClause {σ} (machine : CdclMachine σ) (clause : Clause) : ST σ ExternalClauseAdd := do
  let packed := packClause clause
...
private def bumpLearnedActivity {σ} (machine : CdclMachine σ) (learned : PackedClause) : ST σ Unit := do
  for lit in learned do
...
private def commitLearned {σ} (machine : CdclMachine σ) (analysis : ConflictAnalysis)
  ...
  | .existing _ =>
      pure ()
  return interned.id
private def finishUnsat {σ} (machine : CdclMachine σ) (analysis : ConflictAnalysis) (learned : PackedClause) : ST σ Result := do
...
-/
private def resolveConflict? {σ} (machine : CdclMachine σ) (conflictId : Data.ClauseId) (source : ConflictSource) : ST σ (Option Result) := do
  let analysisLevel ←
    match source with
    | .propagated =>
  ...
  | .ok assertLit? =>
      ...
      return none
private inductive CdclLoopResult where
  | model
  | terminal (result : Result)
  deriving Nonempty
private partial def cdclLoopUntilModel {σ} (machine : CdclMachine σ) (fuel : Nat) :
    ST σ CdclLoopResult := do
  match fuel with
  | 0 =>
      return .terminal {
  ...
  | fuel + 1 =>
      ...
      | some conflictId =>
          ...
          | some result => return .terminal result
          | none => cdclLoopUntilModel machine fuel
      | none =>
          match ← chooseBranch? machine with
          | none => return .model
          | some (var, value) =>
              machine.decide var value
              cdclLoopUntilModel machine fuel
private def cdclLoop {σ} (machine : CdclMachine σ) (fuel : Nat) : ST σ Result := do
  match ← cdclLoopUntilModel machine fuel with
  | .model =>
      return {
...
def solveInitial (config : Config) (numVars : Nat) (initialClauses : Array InitialClause) : Result :=
  solve config numVars (initialClauseDatabase initialClauses)
def unsatProof? (config : Config) (numVars : Nat)
    (initialClauses : Array InitialClause) : Option CdclProof :=
  let result := solveInitial config numVars initialClauses
  match result.outcome with
  | .unsat => some result.proof
  | _ => none
def checkedUnsat? (config : Config) (numVars : Nat) (initialClauses : Array InitialClause) : Option CheckedUnsatCertificate :=
  do
    let proof ← unsatProof? config numVars initialClauses
    CheckedUnsatCertificate.mk? initialClauses proof
...
structure Config where
  ...
  deriving Repr, BEq, DecidableEq, Lean.ToExpr
/--
两次 theory 调用之间发生赋值变动的 selector。
同一变量在一个批次内只出现一次，`value` 是当前 CDCL assignment 中的最终值。
-/
structure AssignmentChange where
  var : Nat
  value : Option Bool
  deriving Repr, Inhabited, BEq, DecidableEq, Lean.ToExpr
structure AssignmentDelta where
  width : Nat
  changes : Array AssignmentChange
  deriving Repr, Inhabited, BEq, DecidableEq, Lean.ToExpr
namespace AssignmentDelta
def apply (delta : AssignmentDelta) (assignment : Array (Option Bool)) :
    Array (Option Bool) := Id.run do
  let mut assignment := Data.ensureArraySize assignment delta.width none
  for change in delta.changes do
    assignment := assignment.set! change.var change.value
  return assignment
def changedVars (delta : AssignmentDelta) : Array Nat :=
  delta.changes.map (·.var)
end AssignmentDelta
private def CdclMachine.takeAssignmentDelta {σ} (machine : CdclMachine σ) :
    ST σ AssignmentDelta := do
  let vars ← machine.assignmentChanges.freeze
  let mut changes := Array.mkEmpty vars.size
  for var in vars do
    changes := changes.push {
      var := var
      value := decodeAssignment (← machine.assignmentCodeAt var)
    }
    machine.assignmentChanged.set! var 0
  machine.assignmentChanges.truncate 0
  return {
    width := machine.numVars
    changes := changes
  }
/--
theory solver 对持久 assignment 应用一个增量批次后的回应。
...
private def finish {τ ε : Type} (result : PropCdcl.Result) (state : τ) (theoryClauses : Array Clause) (theoryEvidence : Array ε)
...
private def finishTheoryUnknown {τ ε : Type} (machine : CdclMachine σ) (state : τ) (theoryClauses : Array Clause) (theoryEvidence : Array ε)
...
private def loop {τ ε : Type} (config : Config) (machine : CdclMachine σ) (remainingRounds : Nat) (theoryRounds : Nat)
    (state : τ) (theoryClauses : Array Clause) (theoryEvidence : Array ε)
    (theory : τ → AssignmentDelta → TheoryResponse τ ε) :
  ...
  | remainingRounds + 1 =>
      match ← cdclLoopUntilModel machine config.cdcl.maxSteps with
      | .model =>
          let delta ← CdclMachine.takeAssignmentDelta machine
          match theory state delta with
          | .model state =>
              return {
                outcome := .model (← machine.freezeAssignment)
                state := state
                theoryClauses := theoryClauses
                theoryEvidence := theoryEvidence
                stats := ← machine.searchStats
          ...
          | .conflict state rawClause evidence =>
              ...
              | some clause =>
                  ...
                  | .conflict interned =>
                      ...
                      | some result =>
                          return finish result state nextClauses nextEvidence (theoryRounds + 1)
                      | none =>
                          loop config machine remainingRounds (theoryRounds + 1) state
                            nextClauses nextEvidence theory
      | .terminal result =>
...
-/
def run {τ ε : Type} (config : Config) (numVars : Nat) (initialClauses : Array Clause) (initialState : τ)
    (theory : τ → AssignmentDelta → TheoryResponse τ ε) :
    RunResult τ ε :=
