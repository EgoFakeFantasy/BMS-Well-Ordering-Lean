...
线性 core checker、selector/witness registry、capability gate 与 problem 对齐证明。
-/
...
instance : ToExpr Term where
  ...
  toExpr := termExpr
private def termBindingExpr (binding : Logic.HigherOrder.TermBinding SearchSignature) : Expr :=
...
private def betaEtaPayloadExpr (payload : BetaEtaPayload) : Expr :=
  mkAppN (higherOrderConst ``HODAGCertificate.BetaEta.Payload.mk)
    #[signatureExpr, toExpr payload.sort, toExpr payload.trace]
instance : ToExpr BetaEtaPayload where
  toTypeExpr := appliedType ``HODAGCertificate.BetaEta.Payload
  toExpr := betaEtaPayloadExpr
...
private def substitutionEvidenceExpr (evidence : SubstitutionEvidence) : Expr :=
...
private def standardizeApartEvidenceExpr (evidence : StandardizeApartEvidence) : Expr :=
...
private def resolutionEvidenceExpr (evidence : ResolutionEvidence) : Expr :=
...
private def factoringEvidenceExpr (evidence : FactoringEvidence) : Expr :=
...
private def equalityResolutionEvidenceExpr (evidence : EqualityResolutionEvidence) : Expr :=
...
private def booleanExtensionalityEvidenceExpr (evidence : BooleanExtensionalityEvidence) : Expr :=
...
private def argumentCongruenceEvidenceExpr (evidence : ArgumentCongruenceEvidence) : Expr :=
...
private def functionExtensionalityEvidenceExpr (evidence : FunctionExtensionalityEvidence) : Expr :=
...
private def propParentClauseLinkExpr (link : PropParentClauseLink) : Expr :=
...
private def propGuardActivationLinkExpr (link : PropGuardActivationLink) : Expr :=
...
private def propLearnedClauseLinkExpr (link : PropLearnedClauseLink) : Expr :=
...
private def propAvatarSkeletonLinkExpr (link : PropAvatarSkeletonLink) : Expr :=
...
private def propInitialJustificationExpr :
  ...
  | .parentClause link =>
      mkAppN (higherOrderConst ``HODAGCertificate.PropInitialJustification.parentClause)
        #[signatureExpr, toExpr link]
  | .guardActivationClause link =>
      mkAppN (higherOrderConst
...
private def propositionalClosurePayloadExpr (payload : PropositionalClosurePayload) : Expr :=
...
private def avatarSplitPayloadExpr (payload : HODAGCertificate.AvatarSplitPayload SearchSignature) : Expr :=
...
private def avatarComponentPayloadExpr (payload : HODAGCertificate.AvatarComponentPayload SearchSignature) : Expr :=
...
private def theoryConflictPayloadExpr (payload : TheoryConflictPayload) : Expr :=
...
private def propositionalLearnedClausePayloadExpr (payload : PropositionalLearnedClausePayload) : Expr :=
...
private def payloadExpr : Payload → Expr
  ...
  | .avatarComponent payload =>
      mkAppN (higherOrderConst ``HODAGCertificate.Payload.avatarComponent)
        #[signatureExpr, toExpr payload]
  | .betaEta payload =>
      mkAppN (higherOrderConst ``HODAGCertificate.Payload.betaEta)
        #[signatureExpr, toExpr payload]
  ...
  | .rewrite kind evidence =>
      mkAppN (higherOrderConst ``HODAGCertificate.Payload.rewrite)
  ...
  | .propositionalLearnedClause payload =>
      mkAppN (higherOrderConst ``HODAGCertificate.Payload.propositionalLearnedClause)
        #[signatureExpr, toExpr payload]
...
节点完整来自材料化结果；source 与 core problem 的一致性由随后执行的线性 core
checker 复核。
-/
