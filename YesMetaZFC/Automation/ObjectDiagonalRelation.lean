...
两层最小输出依次确定数码项和单变量自由代入结果。唯一性覆盖任意对象见证，
只使用码域切分和标准候选输出的正负推导。
-/
...
abbrev minimumSubstitution (D : Delta0CodeDomain) := ObjectMinimumRelation.template D substitution
...
theorem minimum_substitution_positive {T : SetTheory} (S : Support T) {free : SetContext}
    {Γ : Context signature free} (body : UnaryFormula) (number : Nat) :
    Derives T Γ (minimumSubstitution S.core.code_domain (numₘ(code body))
      (numₘ(ObjectNumeralSyntax.value number)) (numₘ(value body number))) := by
  apply ObjectMinimumRelation.positive S.core substitution _ _ _ (domain_open S _)
    (ObjectRelationBinder.closed_three substitution _ _ _ (substitution_positive S body number))
  intro index hIndex
  exact ObjectRelationBinder.closed_three_negative substitution _ _ _
...
theorem minimum_substitution_unique {T : SetTheory} (S : Support T) {free : SetContext}
    {Γ : Context signature free} (body : UnaryFormula) (number : Nat) (point : SetOpenTerm free)
    (h : Derives T Γ (minimumSubstitution S.core.code_domain (numₘ(code body))
      (numₘ(ObjectNumeralSyntax.value number)) point)) :
    Derives T Γ (point ≐ₘ numₘ(value body number)) := by
  apply ObjectMinimumRelation.unique S.core substitution _ _ point _ h
    (ObjectRelationBinder.closed_three substitution _ _ _ (substitution_positive S body number))
  intro index _ hNe
  exact ObjectRelationBinder.closed_three_negative substitution _ _ _
    (substitution_negative S body number index (Ne.symm hNe))

def relationBody (D : Delta0CodeDomain) {bound free : SetContext}
  ...
  minimumNumeral D (input.weakenBound SetSort.set) (numₘ(0)) (.bvar .here) ∧ₘ
    minimumSubstitution D (input.weakenBound SetSort.set) (.bvar .here) (output.weakenBound SetSort.set)

def condition (D : Delta0CodeDomain) {bound free : SetContext}
    (input output : SetTerm bound free) : SetFormula bound free :=
  .existsE SetSort.set (relationBody D input output)
...
@[simp] theorem condition_substituteMapped (D : Delta0CodeDomain) {sb sf tb tf : SetContext}
    ...
    (condition D input output).substituteMapped bs fs =
      condition D (input.substituteMapped bs fs) (output.substituteMapped bs fs) := by
...
def relation (D : Delta0CodeDomain) : FormulaTemplate.Binary where
  body := condition D (.fvar .here) (.fvar (.there .here))

@[simp] theorem relation_apply (D : Delta0CodeDomain) {bound free : SetContext}
    (input output : SetTerm bound free) : relation D input output = condition D input output := by
...
@[simp] theorem relationBody_at (D : Delta0CodeDomain) {free : SetContext}
    ...
    (relationBody D input output).instantiateTop point =
      (minimumNumeral D input (numₘ(0)) point ∧ₘ minimumSubstitution D input point output) := by
...
@[simp] theorem relationBody_open (D : Delta0CodeDomain) {free : SetContext}
    ...
    Formula.openBoundTop (σ := signature) SetSort.set (relationBody D input output) =
      (minimumNumeral D (input.weakenFree SetSort.set) (numₘ(0)) (.fvar .here) ∧ₘ
        minimumSubstitution D (input.weakenFree SetSort.set) (.fvar .here) (output.weakenFree SetSort.set)) := by
...
theorem positive {T : SetTheory} (S : Support T) {free : SetContext}
    {Γ : Context signature free} (body : UnaryFormula) :
    Derives T Γ (relation S.core.code_domain (numₘ(code body)) (numₘ(value body (code body)))) := by
...
theorem unique {T : SetTheory} (S : Support T) {free : SetContext}
    {Γ : Context signature free} (body : UnaryFormula) (point : SetOpenTerm free)
    (h : Derives T Γ (relation S.core.code_domain (numₘ(code body)) point)) :
  ...
  let opened : SetOpenFormula (SetSort.set :: free) :=
    minimumNumeral S.core.code_domain (numₘ(code body)) (numₘ(0)) (.fvar .here) ∧ₘ
      minimumSubstitution S.core.code_domain (numₘ(code body)) (.fvar .here) (point.weakenFree SetSort.set)
  ...
  have hSub' := FirstOrder.Derives.eq_subst
    (body := minimumSubstitution S.core.code_domain (numₘ(code body)) (.bvar .here)
