...
private def internSort (type : Expr) : ReifyM SortId := do
  ...
  return id
private def internSymbol (head : Expr) (inputSorts : Array SortId) (outputSort? : Option SortId) : ReifyM SymbolId := do
  ...
  return id
private def internOpaqueTerm (expression type : Expr) (sort : SortId) : ReifyM OpaqueTermId := do
  ...
  return id
private def internOpaqueFormula (expression : Expr) : ReifyM OpaqueFormulaId := do
  let expression ← closeCurrentBinders expression
  let state ← get
...
private def structuralHead? (expression : Expr) : ReifyM (Option Expr) := do
  ...
  | .const .. =>
      if ← capturesCurrentBinder head then
        ...
        return some head
  | .fvar fvarId =>
      if (← boundVariable? fvarId).isSome || (← capturesCurrentBinder head) then
        return none
      else
        return some head
  | _ =>
      return none
private def withObjectBinder {α : Type} (name : Name) (binderInfo : BinderInfo) (domain : Expr) (sort : SortId) (action : Expr → ReifyM α) : ReifyM α := do
  ...
  private partial def reifyTerm (expression : Expr) : ReifyM Term := do
    ...
    | .fvar fvarId =>
        ...
        let some head ← structuralHead? expression
          | return .opaque sort (← internOpaqueTerm expression type sort)
...
private def boundSort? (bound : Array SortId) (index : Nat) : Option SortId :=
...
namespace Term
mutual
  partial def check (term : Term) (snapshot : Snapshot) (bound : Array SortId := #[]) : Bool :=
    match term with
    | .bvar sort index =>
    ...
    | .opaque sort entry =>
        sortExists snapshot sort &&
          ...
          | none => false
  private partial def checkArguments (snapshot : Snapshot) (bound : Array SortId) (arguments : Array Term) (expected : Array SortId) : Bool :=
    arguments.size == expected.size && (arguments.zip expected).all fun entry =>
        entry.1.sort == entry.2 && entry.1.check snapshot bound
end
end Term
namespace Formula
private def checkAtomArguments (snapshot : Snapshot) (bound : Array SortId) (arguments : Array Term) (expected : Array SortId) : Bool :=
  arguments.size == expected.size && (arguments.zip expected).all fun entry =>
      entry.1.sort == entry.2 && entry.1.check snapshot bound
partial def check (formula : Formula) (snapshot : Snapshot) (bound : Array SortId := #[]) : Bool :=
  match formula with
  | .falsum | .truth =>
      true
...
private def sortExpression (snapshot : Snapshot) (sort : SortId) : MetaM Expr := do
  let some entry := snapshot.sorts[sort]?
    | throwError "host reification lost sort #{sort}"
  return entry.expression
mutual
  private partial def rebuildTerm (snapshot : Snapshot) : Term → MetaM Expr
    | .bvar _ index =>
        return .bvar index
    ...
    | .opaque _ opaqueId => do
        ...
        return entry.expression
  private partial def rebuildFormula (snapshot : Snapshot) : Formula → MetaM Expr
    | .falsum =>
    ...
    | .conj left right =>
        return mkApp2 (mkConst ``And) (← rebuildFormula snapshot left) (← rebuildFormula snapshot right)
    | .disj left right =>
        return mkApp2 (mkConst ``Or) (← rebuildFormula snapshot left) (← rebuildFormula snapshot right)
    | .imp left right => do
    ...
    | .iff left right =>
        return mkApp2 (mkConst ``Iff) (← rebuildFormula snapshot left) (← rebuildFormula snapshot right)
    | .forallE sort binderInfo body => do
        let domain ← sortExpression snapshot sort
        return .forallE `object domain (← rebuildFormula snapshot body) binderInfo
    | .existsE sort body => do
        let domain ← sortExpression snapshot sort
        let predicate := Expr.lam `witness domain (← rebuildFormula snapshot body) .default
