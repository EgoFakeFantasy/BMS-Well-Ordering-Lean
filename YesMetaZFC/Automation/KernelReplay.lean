...
import YesMetaZFC.Automation.HigherOrderNativeReplay
/-!
...
-/
private def preprocessingPayloadExprs (sourceExpr normalizedExpr settingsExpr : Expr) (normalizationTrace : CoreSyntax.NormalForm.Trace)
...
-/
private def sealedBoolTrueProof (expression proof : Expr) : MetaM Expr := do
  let proofType ← mkEq expression (mkConst ``Bool.true)
...
private def reflectedEqualityBoolProof (label : String) (reflectionTheorem : Name) (left right : Expr) : MetaM Expr := do
...
private def nativeBoolTrueProofFor
...
private def boolAndChain (expressions : Array Expr) : MetaM Expr := do
...
private partial def splitBoolAndTrueProofs
...
/--
把一条 learned record 的 resolution slab 分成固定宽连续区间。

宿主只计算区间边界字句；每个区间真值仍由 Lean 中原始 checker 复算，再通过加法定理
拼回完整记录。
-/
private def compactResolutionRangesProof
    (database : Array PropResolution.Clause)
  ...
  let current0 ←
    ...
    | some index =>
        ...
        | none =>
            throwError
              "residual CDCL record starts outside the current database"
    | none =>
        throwError "residual CDCL record has an invalid start clause id"
  let mut current := current0
  let mut currentExpr := startExpr
  let mut checked? : Option Expr := none
  while offset < record.stepsLength do
    let length := min chunkSize (record.stepsLength - offset)
    let mut next := current
    let mut stepIndexWork := 0
    let mut reasonIndexWork := 0
    let mut clauseWork := 0
    let mut normalizationWork := 0
    for stepOffset in [:length] do
      let stepIndex := record.stepsStart + offset + stepOffset
      let some step := proof.journal.steps[stepIndex]?
        | throwError "residual CDCL range profile exceeded step storage"
      let some reasonIndex := (Data.Id.ofNat step.reason : Data.ClauseId).index?
        | throwError "residual CDCL range profile found an invalid reason id"
      let some reason := database[reasonIndex]?
        | throwError "residual CDCL range profile found an invalid reason index"
      stepIndexWork := stepIndexWork + stepIndex
      reasonIndexWork := reasonIndexWork + reasonIndex
      clauseWork := clauseWork + current.size + reason.size
      let some result :=
          PropResolution.compactResolutionStep?
            database proof.journal stepIndex next
        | throwError
            "residual CDCL step #{stepIndex} failed during range construction"
      next := result
      normalizationWork := normalizationWork + result.size * result.size
    let nextOffset := offset + length
    let nextExpr ←
      if nextOffset == record.stepsLength then
        pure targetExpr
      else
        cacheReplayData `_replayResidualResolutionClause (toExpr next)
    let rangeCheck ←
      mkAppM ``PropResolution.compactResolutionStepsValidAgainst
        #[databaseExpr, journalExpr, toExpr length,
          toExpr (record.stepsStart + offset), currentExpr, nextExpr]
    trace[YesMetaZFC.proveAuto.kernelReplay]
      "residual resolution profile [{record.stepsStart + offset}, \
        {record.stepsStart + nextOffset}); \
        stepIndexWork={stepIndexWork}; reasonIndexWork={reasonIndexWork}; \
        clauseWork={clauseWork}; normalizationWork={normalizationWork}"
    let hRange ←
      sealedBoolTrueProof rangeCheck (← mkEqRefl (mkConst ``Bool.true))
    trace[YesMetaZFC.proveAuto.kernelReplay]
      "sealed residual resolution range [{record.stepsStart + offset}, \
        {record.stepsStart + nextOffset})"
    checked? ←
      match checked? with
      | none =>
      ...
      | some checked =>
          some <$> mkAppM
            ``PropResolution.compactResolutionStepsValidAgainst_add
            #[checked, hRange]
    current := next
  ...
  let wholeCheck ←
    mkAppM ``PropResolution.compactResolutionStepsValidAgainst
      #[databaseExpr, journalExpr, toExpr record.stepsLength,
        toExpr record.stepsStart, startExpr, targetExpr]
  match checked? with
  | some checked =>
      cacheBoolTrueProof wholeCheck checked
  | none =>
      sealedBoolTrueProof wholeCheck (← mkEqRefl (mkConst ``Bool.true))

private structure ResolutionTraceStep where
  offset : Nat
  current : Expr
  next : Expr
  checked : Expr

private def compactResolutionTraceProof
    (database : Array PropResolution.Clause)
    (proof : PropResolution.CdclProof)
    (record : PropResolution.LearnRecord)
    (databaseExpr journalExpr startExpr targetExpr : Expr) :
  ...
  let currentClause0 ←
    ...
    | some index =>
        ...
        | some clause => pure clause
        | none =>
            throwError "residual CDCL trace starts outside the current database"
    | none =>
        throwError "residual CDCL trace has an invalid start clause id"
  let mut currentClause := currentClause0
  let mut stepProofs : Array ResolutionTraceStep := #[]
  while offset < record.stepsLength do
    if offset % 8 == 0 then
      trace[YesMetaZFC.proveAuto.kernelReplay]
        "start residual resolution trace step {offset}/{record.stepsLength}"
    let stepIndex := record.stepsStart + offset
    let some step := proof.journal.steps[stepIndex]?
      | throwError "residual CDCL trace exceeded step storage"
    let some reasonIndex :=
        (Data.Id.ofNat step.reason : Data.ClauseId).index?
      | throwError "residual CDCL trace found an invalid reason id"
    let some reason := database[reasonIndex]?
      | throwError "residual CDCL trace found an invalid reason index"
    let some plan :=
        PropResolution.resolutionPlan? currentClause reason step.pivot
      | throwError "residual CDCL trace found an invalid resolution plan"
    let nextClause := plan.result
    ...
    let nextExpr ←
      if nextOffset == record.stepsLength then
        ...
        cacheReplayData `_replayResidualResolutionClause
          (toExpr nextClause)
    let stepExpr ←
      cacheReplayData `_replayResidualResolutionStep (toExpr step)
    let stepLookup ←
      arrayGetElem? journalStepsExpr (toExpr stepIndex)
    let expectedStep ← mkAppM ``Option.some #[stepExpr]
    let hStepData ←
      equalityProof "residual CDCL source step"
        stepLookup expectedStep
    let reasonExpr ←
      cacheReplayData `_replayResidualResolutionReason (toExpr reason)
    let reasonLookup ←
      arrayGetElem? databaseExpr (toExpr reasonIndex)
    let expectedReason ← mkAppM ``Option.some #[reasonExpr]
    let hReasonData ←
      equalityProof "residual CDCL source reason"
        reasonLookup expectedReason
    let stepIdExpr ← clauseIdExpr step.reason
    let stepReasonIndexExpr ←
      mkAppM ``Data.Id.index? #[stepIdExpr]
    let expectedReasonIndex ←
      mkAppM ``Option.some #[toExpr reasonIndex]
    let hReasonId ←
      equalityProof "residual CDCL reason id"
        stepReasonIndexExpr expectedReasonIndex
    let planExpr ←
      cacheReplayData `_replayResidualResolutionPlan (toExpr plan)
    let pivotExpr ←
      mkAppM ``PropResolution.CompactResolutionStep.pivot #[stepExpr]
    let computedPlan ←
      mkAppM ``PropResolution.resolutionPlan?
        #[currentExpr, reasonExpr, pivotExpr]
    let expectedPlan ← mkAppM ``Option.some #[planExpr]
    let hPlan ←
      equalityProof "residual CDCL resolution plan"
        computedPlan expectedPlan
    let planResultExpr ←
      mkAppM ``PropResolution.ResolutionPlan.result #[planExpr]
    let hNext ←
      equalityProof "residual CDCL resolution result"
        planResultExpr nextExpr
    let hStep ←
      mkAppM
        ``PropResolution.compactResolutionStep_eq_some_of_components
        #[hStepData, hReasonId, hReasonData, hPlan, hNext]
    if offset % 8 == 0 then
      trace[YesMetaZFC.proveAuto.kernelReplay]
        "sealed residual resolution trace step {offset}/{record.stepsLength}"
    stepProofs := stepProofs.push {
      offset := offset
      current := currentExpr
      next := nextExpr
      checked := hStep
  ...
  let mut traceProof ←
    mkAppOptM
      ``PropResolution.CheckedCompactResolutionTrace.done
      #[some databaseExpr, some journalExpr, some targetExpr,
        some (toExpr (record.stepsStart + record.stepsLength)),
        some targetExpr, some hTarget]
  for step in stepProofs.reverse do
    traceProof ←
      mkAppOptM
        ``PropResolution.CheckedCompactResolutionTrace.step
        #[some databaseExpr, some journalExpr, some targetExpr,
          some (toExpr (record.stepsLength - step.offset - 1)),
          some (toExpr (record.stepsStart + step.offset)),
          some step.current, some step.next, some step.checked,
          some traceProof]
  mkAppOptM
    ``PropResolution.CheckedCompactResolutionTrace.check_eq_true
    #[some databaseExpr, some journalExpr, some targetExpr,
      some (toExpr record.stepsLength), some (toExpr record.stepsStart),
      some startExpr, some traceProof]
...
private structure DagContractProofs where
  arenaNodes : Expr
  rootExists : Expr
  rootClosed : Expr
  denseIds : Expr
...
private def sourceReplayExprs (kind : SourceReplayKind)
    ...
    (sourceProblem problem admissible settingsExpr : Expr)
      ...
      let nativeCheck ←
        ...
        | .firstOrder =>
            mkAppM ``SourcePreprocessing.FirstOrderReplay.check
              #[toExpr sourceProblemValue, rawPayloadExprs.payload]
        | .fool =>
    ...
    | SourceReplayTickets.avatar _ registryExpr hRegistryCheck =>
      ...
      trace[YesMetaZFC.proveAuto.kernelReplay]
        ...
        #[some replaySearchInput, some checkedArtifact,
          some hAvatar, some registryExpr, some hRegistry, some admissible]
...
def firstOrderReplayExprs
  ...
  sourceReplayExprs .firstOrder
    sourceProblemValue sourceProblem problem admissible settingsExpr
