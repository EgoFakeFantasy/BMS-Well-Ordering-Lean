...
universe x
mutual
  theorem normalizeTermWith_firstOrderIdentity : ∀ fuel term, normalizeTermWith Config.firstOrderIdentity fuel term = term
    | 0, _ => rfl
    | _ + 1, .bvar .. => rfl
    | _ + 1, .fvar .. => rfl
    ...
    | fuel + 1, .apply fn arg => by
        simp only [normalizeTermWith]
        rw [normalizeTermWith_firstOrderIdentity fuel fn, normalizeTermWith_firstOrderIdentity fuel arg]
        rfl
    | _ + 1, .bool .. => rfl
    | fuel + 1, .notE body => by
        simp only [normalizeTermWith]
    ...
    | fuel + 1, .ite sort condition thenTerm elseTerm => by
        simp only [normalizeTermWith]
        rw [normalizeFormulaWith_firstOrderIdentity fuel condition, normalizeTermWith_firstOrderIdentity fuel thenTerm, normalizeTermWith_firstOrderIdentity fuel elseTerm]
        rfl
  theorem normalizeFormulaWith_firstOrderIdentity : ∀ fuel formula, normalizeFormulaWith Config.firstOrderIdentity fuel formula = formula
    ...
    | fuel + 1, .existsE sort body => by
        ...
        rw [normalizeFormulaWith_firstOrderIdentity fuel body]
        rfl
  theorem normalizeTermListWith_firstOrderIdentity : ∀ fuel terms, normalizeTermListWith Config.firstOrderIdentity fuel terms = terms
    | 0, _ => rfl
    | _ + 1, [] => rfl
    | fuel + 1, term :: rest => by
        simp only [normalizeTermListWith]
        rw [normalizeTermWith_firstOrderIdentity fuel term, normalizeTermListWith_firstOrderIdentity fuel rest]
end
...
private def formulaRecSize : Formula → Nat
  ...
  | Formula.iffE left right => formulaRecSize left + formulaRecSize right + 1
  | Formula.forallE _ body
  | Formula.existsE _ body => formulaRecSize body + 1
private theorem formulaRecSize_neg (body : Formula) : formulaRecSize body < formulaRecSize (.neg body) := by simp [formulaRecSize]
private theorem formulaRecSize_binary_left (left right : Formula) (constructor : Formula → Formula → Formula) (hConstructor : formulaRecSize (constructor left right) = formulaRecSize left + formulaRecSize right + 1) : formulaRecSize left < formulaRecSize (constructor left right) := by
  rw [hConstructor]
  omega
private theorem formulaRecSize_binary_right (left right : Formula) (constructor : Formula → Formula → Formula) (hConstructor : formulaRecSize (constructor left right) = formulaRecSize left + formulaRecSize right + 1) : formulaRecSize right < formulaRecSize (constructor left right) := by
  rw [hConstructor]
  omega
private theorem formulaRecSize_quantifier (sort : CoreSort) (body : Formula) (constructor : CoreSort → Formula → Formula) (hConstructor : formulaRecSize (constructor sort body) = formulaRecSize body + 1) : formulaRecSize body < formulaRecSize (constructor sort body) := by
  rw [hConstructor]
  omega
theorem Nnf.satisfies_toNnfWith {M : Model} (env : Env M) (polarity : Polarity) (formula : Formula) : Nnf.Satisfies env (toNnfWith polarity formula) ↔ polaritySatisfiesFormula env polarity formula := by
  ...
  | atom predicate args =>
      cases polarity <;>
        simp [toNnfWith, polaritySatisfiesFormula, Polarity.literal, Nnf.Satisfies, Literal.Satisfies, Atom.Satisfies, Formula.Satisfies, Formula.eval] <;> rfl
  ...
  | disj left right =>
      ...
      have hRightNegative := Nnf.satisfies_toNnfWith env Polarity.negative right
      cases polarity <;>
        simp [toNnfWith, polaritySatisfiesFormula, Nnf.Satisfies, Formula.Satisfies, Formula.eval, hLeftPositive, hLeftNegative, hRightPositive, hRightNegative]
  | iffE left right =>
      have hLeftPositive := Nnf.satisfies_toNnfWith env Polarity.positive left
      have hLeftNegative := Nnf.satisfies_toNnfWith env Polarity.negative left
      have hRightPositive := Nnf.satisfies_toNnfWith env Polarity.positive right
