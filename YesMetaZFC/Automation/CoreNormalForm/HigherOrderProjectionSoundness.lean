...
-/
namespace YesMetaZFC.Automation.HOSearchMaterialization.CoreProjectionSoundness
open CoreSyntax
...
mutual
  def coreTerm : CoreSyntax.Term → Term
    ...
    | .app symbol arguments =>
        .app (FirstOrderProjection.functionSymbol symbol) (coreTermList arguments)
    | .apply function argument => .apply (coreTerm function) (coreTerm argument)
    | .lam domain codomain body => .lam (simpleType domain) (simpleType codomain) (coreTerm body)
...
def coreAtom : CoreSyntax.NormalForm.Atom → Atom
  | .predicate predicate arguments => .rel (.predicate predicate) (coreTermList arguments)
  | .equal sort left right => .equal (simpleType sort) (coreTerm left) (coreTerm right)
  | .boolTerm input => .rel .boolHolds [coreTerm input]
...
@[simp]
theorem coreSort_simpleType (sort : CoreSort) : coreSort (simpleType sort) = sort := by
  induction sort with
  ...
  relInterp := fun symbol arguments =>
    ...
    | .definition id arity =>
        M.predicateInterp
          ...
          arguments
    | .predicate predicate => M.predicateInterp predicate arguments
...
def extensionalContract (M : Semantics.Model) (contract : Semantics.FoolLambdaContract M) (functionSort : ∀ symbol arguments, M.sortInterp symbol.outputSort (M.functionInterp symbol arguments)) : Logic.HigherOrder.ExtensionalContract (searchStructure M contract functionSort) where
  ...
  functionExtensionality := by
    ...
    exact contract.function_extensionality (coreSort domain) (coreSort codomain) left right hLeft hRight
def coreEnv {M : Semantics.Model} {contract : Semantics.FoolLambdaContract M} {functionSort : ∀ symbol arguments, M.sortInterp symbol.outputSort (M.functionInterp symbol arguments)} (env : Logic.HigherOrder.Env (searchStructure M contract functionSort)) : Semantics.Env M where
  boundVal := env.boundVal
  freeVal := fun sort id => env.freeVal (simpleType sort) id
def alignedEnv {M : Semantics.Model} {contract : Semantics.FoolLambdaContract M} {functionSort : ∀ symbol arguments, M.sortInterp symbol.outputSort (M.functionInterp symbol arguments)} (base : Semantics.Env M) (env : Logic.HigherOrder.Env (searchStructure M contract functionSort)) : Logic.HigherOrder.Env (searchStructure M contract functionSort) where
  boundVal := base.boundVal
  freeVal := env.freeVal
mutual
  theorem coreTerm_eval {M : Semantics.Model} {contract : Semantics.FoolLambdaContract M} {functionSort : ∀ symbol arguments, M.sortInterp symbol.outputSort (M.functionInterp symbol arguments)} (env : Logic.HigherOrder.Env (searchStructure M contract functionSort)) : ∀ term, Native.term term = true → Logic.HigherOrder.Term.eval env (coreTerm term) = Semantics.Term.eval (coreEnv env) term
    ...
    | .app symbol arguments, hNative => by
        ...
        simp only [coreTerm, Logic.HigherOrder.Term.eval, Semantics.Term.eval, searchStructure]
        rw [SearchMaterialization.CoreProjectionSoundness.functionSymbol_toCore]
        exact congrArg (M.functionInterp symbol) (coreTermList_eval env arguments hNative.2)
    | .apply function argument, hNative => by
        simp only [Native.term, Bool.and_eq_true_iff] at hNative
        simp only [coreTerm, Logic.HigherOrder.Term.eval, Semantics.Term.eval, searchStructure]
        calc
          M.applyInterp (Logic.HigherOrder.Term.eval env (coreTerm function)) (Logic.HigherOrder.Term.eval env (coreTerm argument)) =
            M.applyInterp (Semantics.Term.eval (coreEnv env) function) (Logic.HigherOrder.Term.eval env (coreTerm argument)) :=
            congrArg (fun value => M.applyInterp value (Logic.HigherOrder.Term.eval env (coreTerm argument))) (coreTerm_eval env function hNative.1)
          _ =
            M.applyInterp (Semantics.Term.eval (coreEnv env) function) (Semantics.Term.eval (coreEnv env) argument) :=
            congrArg (M.applyInterp (Semantics.Term.eval (coreEnv env) function)) (coreTerm_eval env argument hNative.2)
  ...
  theorem coreTermList_eval {M : Semantics.Model} {contract : Semantics.FoolLambdaContract M} {functionSort : ∀ symbol arguments, M.sortInterp symbol.outputSort (M.functionInterp symbol arguments)} (env : Logic.HigherOrder.Env (searchStructure M contract functionSort)) : ∀ terms, Native.termList terms = true → (coreTermList terms).map (Logic.HigherOrder.Term.eval env) = terms.map (Semantics.Term.eval (coreEnv env))
    | [], _ => rfl
        ...
        calc
          Logic.HigherOrder.Term.eval env (coreTerm head) ::
            ...
            Semantics.Term.eval (coreEnv env) head ::
              List.map (Logic.HigherOrder.Term.eval env) (coreTermList tail) :=
            congrArg (fun value => value :: List.map (Logic.HigherOrder.Term.eval env) (coreTermList tail)) (coreTerm_eval env head hNative.1)
          _ =
            Semantics.Term.eval (coreEnv env) head ::
              List.map (Semantics.Term.eval (coreEnv env)) tail :=
            congrArg (List.cons (Semantics.Term.eval (coreEnv env) head)) (coreTermList_eval env tail hNative.2)
end
theorem coreAtom_satisfies {M : Semantics.Model} {contract : Semantics.FoolLambdaContract M} {functionSort : ∀ symbol arguments, M.sortInterp symbol.outputSort (M.functionInterp symbol arguments)} (env : Logic.HigherOrder.Env (searchStructure M contract functionSort)) (atom : CoreSyntax.NormalForm.Atom) (hNative : Native.atom atom = true) : HODAGCertificate.Atom.Satisfies env (coreAtom atom) ↔ Semantics.Atom.Satisfies (coreEnv env) atom := by
  cases atom with
  | predicate predicate arguments =>
      simp only [Native.atom] at hNative
      simp only [coreAtom, HODAGCertificate.Atom.Satisfies, Semantics.Atom.Satisfies, searchStructure]
      exact iff_of_eq <| congrArg (M.predicateInterp predicate) (coreTermList_eval env arguments hNative)
  | equal sort left right =>
      simp only [Native.atom, Bool.and_eq_true_iff] at hNative
      simp only [coreAtom, HODAGCertificate.Atom.Satisfies, Semantics.Atom.Satisfies]
      constructor
      · intro hEqual
        rw [coreTerm_eval env left hNative.1, coreTerm_eval env right hNative.2] at hEqual
        exact hEqual
      · intro hEqual
        rw [coreTerm_eval env left hNative.1, coreTerm_eval env right hNative.2]
        exact hEqual
  | boolTerm input =>
      simp only [Native.atom] at hNative
      simp only [coreAtom, HODAGCertificate.Atom.Satisfies, Semantics.Atom.Satisfies, searchStructure, List.map_cons, List.map_nil]
      exact iff_of_eq <| congrArg M.boolHolds (coreTerm_eval env input hNative)
theorem coreLiteral_satisfies {M : Semantics.Model} {contract : Semantics.FoolLambdaContract M} {functionSort : ∀ symbol arguments, M.sortInterp symbol.outputSort (M.functionInterp symbol arguments)} (env : Logic.HigherOrder.Env (searchStructure M contract functionSort)) (literal : CoreSyntax.NormalForm.Literal) (hNative : Native.literal literal = true) : HODAGCertificate.Literal.Satisfies env (coreLiteral literal) ↔ Semantics.Literal.Satisfies (coreEnv env) literal := by
  cases literal with
  | mk positive atom =>
      ...
      | true => exact hAtom
theorem coreClause_satisfies {M : Semantics.Model} {contract : Semantics.FoolLambdaContract M} {functionSort : ∀ symbol arguments, M.sortInterp symbol.outputSort (M.functionInterp symbol arguments)} (env : Logic.HigherOrder.Env (searchStructure M contract functionSort)) (clause : CoreSyntax.NormalForm.Clause) (hNative : Native.clause clause = true) (hClause : Semantics.Clause.Satisfies (coreEnv env) clause) : HODAGCertificate.Clause.Satisfies env (coreClause clause) := by
  ...
  · apply Array.mem_def.mpr
    simpa [coreClause, Array.toList_map] using (List.mem_map.mpr ⟨literal, hMem, rfl⟩)
  · exact (coreLiteral_satisfies env literal hLiteralNative).mpr hLiteral
theorem coreProblem_valid {M : Semantics.Model} (contract : Semantics.FoolLambdaContract M) (functionSort : ∀ symbol arguments, M.sortInterp symbol.outputSort (M.functionInterp symbol arguments)) (base : Semantics.Env M)
    (clauses : CoreSyntax.NormalForm.ClauseSet) (hNative : Native.clauseSet clauses = true) (hChecks : (coreProblem clauses).initialClauses.all HODAGCertificate.Clause.check = true) (hClauses : ∀ env,
    Semantics.Env.RespectsFree env → Semantics.LocalSkolemChoice.SameBoundStack env base → Semantics.ClauseSet.Satisfies env clauses) : (coreProblem clauses).Valid (searchStructure M contract functionSort) := by
  ...
  | none => simp [hCoreGet] at hTargetClause
  | some coreClauseValue =>
      ...
      have hCoreNative : Native.clause coreClauseValue = true := by simpa [hGet] using hAllNative index hIndex
      have hAlignedSat :
      ...
      have hClauseCheck :
        ...
        have hMappedIndex : index < (coreProblem clauses).initialClauses.size := by simpa [coreProblem] using hIndex
        simpa [coreProblem, hGet] using hAllChecks index hMappedIndex
      have hWellFormed :=
        HODAGCertificate.Clause.checkWith_sound (context := []) (clause := coreClause coreClauseValue) hClauseCheck
      exact (HODAGCertificate.Clause.satisfies_iff_of_wellFormed_env (env₁ := sourceEnv) (env₂ := targetEnv) hWellFormed (by
            intro boundIndex sort hLookup
            cases boundIndex <;> simp [Logic.HigherOrder.Context.lookup?] at hLookup) (by
            intro sort id
            rfl)).mp hAlignedSat
