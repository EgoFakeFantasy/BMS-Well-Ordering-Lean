...
namespace Term
theorem eval_mem_of_inferSortWith {M : Model} (contract : FoolLambdaContract M) (bound : List CoreSort) (env : Env M) (hBound : Env.RespectsBound bound env) (hFree : Env.RespectsFree env) : ∀ (term : Term) (sort : CoreSort), Term.inferSortWith bound term = some sort → M.sortInterp sort (Term.eval env term)
  ...
  | Term.lam domain codomain body, sort, hSort => by
      ...
      | some bodySort =>
          ...
          intro value hValue
          exact eval_mem_of_inferSortWith contract (domain :: bound) (env.push value) (Env.respectsBound_push hBound hValue) (Env.respectsFree_push hFree value)
      ...
      next =>
        ...
        | some thenSort =>
            ...
            | some elseSort =>
                ...
                subst sort
                simpa only [Term.eval] using
                  contract.ite_sort targetSort _ _ _ (eval_mem_of_inferSortWith contract bound env hBound hFree thenTerm targetSort hThenSort)
                    (eval_mem_of_inferSortWith contract bound env hBound hFree elseTerm targetSort hElseSort)
end Term
namespace Term
    ...
    | forallE sort body =>
        ...
        | some value =>
            ...
            have hBodySem (value : M.Carrier) (hValue : M.sortInterp sort value) :=
              Formula.rewriteOnceFormula?_sound contract config (sort :: bound) (env.push value) (Env.respectsBound_push hBound hValue)
  ...
  theorem Term.rewriteOnceTerm?_sound {M : Model} (contract : FoolLambdaContract M) (config : Config) (bound : List CoreSort) (env : Env M) (hBound : Env.RespectsBound bound env) (hFree : Env.RespectsFree env) {source
    ...
    | lam domain codomain body =>
        ...
        | some value =>
            ...
            intro value hValue
            exact
              Term.rewriteOnceTerm?_sound contract config (domain :: bound) (env.push value) (Env.respectsBound_push hBound hValue)
                (Env.respectsFree_push hFree value) hSourceBody hTargetBody hRewrite
        | none =>
            simp [hRewrite] at hStep
            exact rewriteRootTerm?_sound_of_typed contract config bound env
