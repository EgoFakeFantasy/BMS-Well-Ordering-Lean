import YesMetaZFC.Automation.RelationalTranslation
...
/-- 异质值列读取成自由变量赋值。 -/
def valuesAssignment {M : Structure.{0, 0, 0, x} τ} {sorts : SortContext τ}
    (values : Values M.Carrier sorts) : Assignment M sorts :=
  fun entry => match values, entry with
    | .cons head _, .here => head
    | .cons _ tail, .there previous => valuesAssignment tail previous

def templateEnv {M : Structure.{0, 0, 0, x} τ} {sorts : SortContext τ}
    (values : Values M.Carrier sorts) : Env M [] sorts where
