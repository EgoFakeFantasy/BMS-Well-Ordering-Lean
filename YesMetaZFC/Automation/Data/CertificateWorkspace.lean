...
namespace Data
structure ResidualCdclWorkspace (Atom Parent InitialFeature AvatarFeature LearnedFeature GuardFeature : Type) where
  atoms : Array Atom := #[]
  initialIndex : Nat := 0
...
def setAtoms (workspace : ResidualCdclWorkspace Atom Parent Initial Avatar Learned Guard) (atoms : Array Atom) :
    ResidualCdclWorkspace Atom Parent Initial Avatar Learned Guard :=
  { workspace with atoms := atoms }
private def addParent (workspace : ResidualCdclWorkspace Atom Parent Initial Avatar Learned Guard) (parentId : Nat) (parent : Parent) :
    ResidualCdclWorkspace Atom Parent Initial Avatar Learned Guard :=
...
def pushInitial (workspace : ResidualCdclWorkspace Atom Parent Initial Avatar Learned Guard)
...
def pushAvatar (workspace : ResidualCdclWorkspace Atom Parent Initial Avatar Learned Guard) (parentId : Nat) (parent : Parent) (feature : Avatar) :
    ResidualCdclWorkspace Atom Parent Initial Avatar Learned Guard :=
  let workspace := workspace.addParent parentId parent
  {
    workspace with
...
def pushLearned (workspace : ResidualCdclWorkspace Atom Parent Initial Avatar Learned Guard) (parentId : Nat) (parent : Parent) (feature : Learned) :
    ResidualCdclWorkspace Atom Parent Initial Avatar Learned Guard :=
  let workspace := workspace.addParent parentId parent
  {
    workspace with
...
def pushGuard (workspace : ResidualCdclWorkspace Atom Parent Initial Avatar Learned Guard)
...
-/
structure CertificateMaterializationWorkspace (Certificate Dag ArenaMap Node : Type) where
...
def setDag (workspace : CertificateMaterializationWorkspace Certificate Dag ArenaMap Node) (dag : Dag) :
    CertificateMaterializationWorkspace Certificate Dag ArenaMap Node :=
  { workspace with dag := dag }
def pushTheoryConflict (workspace : CertificateMaterializationWorkspace Certificate Dag ArenaMap Node) (node : Node) :
    CertificateMaterializationWorkspace Certificate Dag ArenaMap Node :=
  { workspace with theoryConflictNodes := workspace.theoryConflictNodes.push node }
def pushLearnedClause (workspace : CertificateMaterializationWorkspace Certificate Dag ArenaMap Node) (node : Node) :
    CertificateMaterializationWorkspace Certificate Dag ArenaMap Node :=
  { workspace with learnedClauseNodes := workspace.learnedClauseNodes.push node }
def sources (workspace : CertificateMaterializationWorkspace Certificate Dag ArenaMap Node) :
    Array Node :=
