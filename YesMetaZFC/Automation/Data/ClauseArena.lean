...
namespace PackedLit
@[inline]
def pack (var : Nat) (positive : Bool) : PackedLit := (var <<< 1) ||| if positive then 1 else 0
@[inline]
...
def watches (header : ClauseHeader) : Nat × Nat := (header.watch0, header.watch1)
...
def hashPackedClause (clause : Array PackedLit) : UInt64 :=
  clause.foldl (fun hash lit => mixHash hash (Hashable.hash lit)) (1469598103934665603 : UInt64)
...
def packedClause (arena : ClauseArena) (id : ClauseId) : Array PackedLit :=
  ...
  | none => #[]
def clauseEqPacked (arena : ClauseArena) (id : ClauseId) (clause : Array PackedLit) : Bool :=
  match arena.header? id with
  | none => false
  | some header =>
...
private def registerClauseId {σ : Type} (builder : Builder σ) (hash : UInt64) (id : ClauseId) : ST σ Unit :=
...
def empty {σ : Type} : ST σ (Builder σ) := do
  ...
  let buckets ← ST.mkRef (σ := σ) (Std.HashMap.emptyWithCapacity : Std.HashMap UInt64 (Array Nat))
...
def ofArena {σ : Type} (arena : ClauseArena) : ST σ (Builder σ) := do
  let builder : Builder σ := {
    ...
    buckets := ← ST.mkRef (σ := σ) (Std.HashMap.emptyWithCapacity : Std.HashMap UInt64 (Array Nat))
  }
...
@[inline]
def setHeader {σ : Type} (builder : Builder σ) (id : ClauseId) (header : ClauseHeader) : ST σ Unit := do
  let _ ← builder.headers.set? id header
  pure ()
@[inline]
def modifyHeader {σ : Type} (builder : Builder σ) (id : ClauseId) (f : ClauseHeader → ClauseHeader) : ST σ Unit := do
  match ← builder.header? id with
  | some header => builder.setHeader id (f header)
  | none => pure ()
@[inline]
def setWatches {σ : Type} (builder : Builder σ) (id : ClauseId) (first second : Nat) : ST σ Unit :=
...
def clauseEqPacked {σ : Type} (builder : Builder σ) (id : ClauseId) (clause : @& Array PackedLit) : ST σ Bool := do
  ...
  for position in [:header.length] do
    if (← builder.literals.get! (header.start + position)) != clause[position]! then
      return false
  return true
def lookupClause? {σ : Type} (builder : Builder σ) (clause : @& Array PackedLit) : ST σ (Option ClauseId) := do
  ...
  for rawId in candidates do
    ...
    if ← builder.clauseEqPacked id clause then
      return some id
  return none
def pushClause {σ : Type} (builder : Builder σ) (clause : @& Array PackedLit) (flags : UInt8 := 0) : ST σ ClauseId := do
...
-/
def internClause {σ : Type} (builder : Builder σ) (clause : @& Array PackedLit) (flags : UInt8 := 0) : ST σ InternResult := do
