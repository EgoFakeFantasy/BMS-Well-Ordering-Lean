...
structure Package where
  rules : Array Rule := #[]
  stats : Stats := {}
private partial def peelTelescopeLoop (expression : Expr) (types : Array Expr) (infos : Array BinderInfo) : Telescope :=
...
private def compileTheorem (declaration : Name) (phase : Phase) (type : Expr) : Rule :=
...
private def compileDefinition (declaration : Name) (phase : Phase) (type : Expr) (value? : Option Expr) (equations : Array Name) : Rule :=
  {
    declaration
    phase
    kind := .definition
...
private def pushRuleUnique (rules : Array Rule) (rule : Rule) : Array Rule :=
  if rules.any fun existing =>
    ...
    rules.push rule
private def appendPhase (rules : Array Rule) (phase : Phase) (registered : Array RegisteredRule) : MetaM (Array Rule) := do
