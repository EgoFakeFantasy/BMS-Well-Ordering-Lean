...
private def usedDeclarations (stats : Simp.Stats) : Array Name :=
  stats.usedTheorems.toArray.foldl (fun declarations origin =>
      pushNameUnique declarations origin.key) #[]
private def ruleCoversDeclaration (rule : RuleCompiler.Rule) (declaration : Name) : Bool :=
  rule.simpDeclarations.contains declaration
private def usedRuleNames (rules : Array RuleCompiler.Rule) (candidateIds : Array RestrictedSuperposition.RuleId)
  ...
  return result
private def candidateRuleNames (rules : Array RuleCompiler.Rule) (candidateIds : Array RestrictedSuperposition.RuleId) : Array Name := Id.run do
  let mut result := #[]
  for ruleId in candidateIds do
    if let some rule := rules[ruleId]? then
      result := pushNameUnique result rule.declaration
  return result
private def candidateSimpTheorems (prepared : Prepared) (candidateIds : Array RestrictedSuperposition.RuleId) :
  ...
  return result
private def prepareRule (rule : RuleCompiler.Rule) : MetaM SimpTheorems := do
...
private def simpContext (theorems : SimpTheoremsArray) (maxSteps : Nat) : MetaM Simp.Context := do
...
private def attemptPhase (prepared : Prepared) (phase : Phase) (candidateIds : Array RestrictedSuperposition.RuleId)
    (source : Expr) (maxSteps : Nat) : MetaM Attempt := do
  let saved ← saveState
  try
    let context ← simpContext (candidateSimpTheorems prepared candidateIds) maxSteps
    ...
    saved.restore
    return .unresolved 0
private def attemptConservative (source : Expr) (maxSteps : Nat) : MetaM Attempt := do
  try
    let result ←
    ...
    return .unresolved 0
private def pathToNode (nodes : Array Node) (target : NodeId) : Array Step := Id.run do
...
private def composePath (source : Expr) (path : Array Step) : MetaM Expr := do
...
def Result.validate (result : Result) : MetaM Unit := do
  ...
  unless !result.stats.terminatedAtTrue || (result.stats.fixedPoint && result.normal.isTrue) do
...
def Result.toCertificateNode (result : Result) : Certificate.Node :=
  {
    id := 0
...
private def updateQueryStats (stats : Stats) (query : RestrictedSuperposition.QueryResult) : Stats :=
  {
    stats with
...
-/
def closeWithPrepared (source : Expr) (prepared : Prepared) (config : Config := {}) : MetaM Result := do
  let profile ←
...
def closeWithPackage (source : Expr) (package : RuleCompiler.Package) (config : Config := {}) : MetaM Result := do
