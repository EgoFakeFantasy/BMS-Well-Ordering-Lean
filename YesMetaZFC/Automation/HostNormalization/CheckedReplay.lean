...
namespace Step
private def findRule? (package : RuleCompiler.Package) (phase : Phase) (declaration : Name) :
    Option RuleCompiler.Rule :=
  package.rules.find? fun rule =>
    rule.declaration == declaration &&
      rule.phase == phase
private def candidateRuleCheck (package : RuleCompiler.Package) (step : Step) : Bool :=
...
private def usedRuleCheck (package : RuleCompiler.Package) (step : Step) : Bool :=
  match step.kind with
  | .conservativeReduction =>
      step.usedRules.isEmpty
  | .phase normalizationPhase =>
      step.usedRules.all fun declaration =>
        step.candidateRules.contains declaration && (findRule? package normalizationPhase declaration).isSome
private def usedDeclarationCheck (package : RuleCompiler.Package) (step : Step) : Bool :=
  match step.kind with
  | .conservativeReduction =>
...
def ofClosure (_package : RuleCompiler.Package) (step : EqualityClosure.Step) : Except String Step :=
  .ok {
...
def checkRule (package : RuleCompiler.Package) (step : Step) : Bool :=
  candidateRuleCheck package step &&
    usedRuleCheck package step &&
      usedDeclarationCheck package step && (match step.kind with
        | .conservativeReduction =>
...
private def reductionCheck (payload : Payload) : Bool :=
  payload.steps.foldl (fun total step =>
...
private def coverageCheck (package : RuleCompiler.Package) (payload : Payload) : Bool :=
      ...
      pathCheck payload &&
        reductionCheck payload &&
          coverageCheck package payload &&
            payload.nodes > 0 && (payload.cycles > 0 || (payload.cycles == 0 &&
                  payload.exhausted &&
                    payload.attempts == 0)) &&
                    ...
                    payload.successfulEdges <= payload.attempts &&
                      payload.backtracks <= payload.attempts &&
                        payload.candidateRules >=
                          payload.steps.foldl (fun total step => total + step.candidateRules.size) 0 &&
                        payload.rewrites >=
                          payload.steps.foldl (fun total step => total + step.rewrites) 0 && (payload.fixedPoint || payload.exhausted) &&
                          !(payload.fixedPoint && payload.exhausted) && (!payload.terminatedAtTrue || (payload.fixedPoint && payload.normal.isTrue))
def ofResult (package : RuleCompiler.Package) (result : EqualityClosure.Result) : Except String Payload := do
...
def toCertificateNode
    {package : RuleCompiler.Package} (artifact : Artifact package) (id : Certificate.NodeId := 0) (dependencies : Array Certificate.NodeId := #[]) :
    Certificate.Node :=
...
def replayPruned {package : RuleCompiler.Package} (result : CloseResult package) : Bool :=
  result.artifact?.isNone
end CloseResult
private def replayEdge (step : EqualityClosure.Step) : MetaM Expr := do
  ...
  return proof
private def composeReplay (source : Expr) (steps : Array EqualityClosure.Step) : MetaM Expr := do
  let mut proof ← mkEqRefl source
  for step in steps do
    proof ← mkEqTrans proof (← replayEdge step)
  return proof
def validate
    {package : RuleCompiler.Package} (artifact : Artifact package) : MetaM Unit := do
  let payload := artifact.checked.payload
...
private def replayValidatedWithPackage (package : RuleCompiler.Package) (result : EqualityClosure.Result) :
...
def replayWithPackage (source : Expr) (package : RuleCompiler.Package) (result : EqualityClosure.Result) :
    MetaM (Artifact package) := do
...
def closeWithPackage (source : Expr) (package : RuleCompiler.Package) (config : EqualityClosure.Config := {}) :
...
def closeWithPrepared (source : Expr) (prepared : EqualityClosure.Prepared) (config : EqualityClosure.Config := {}) :
...
def close (source : Expr) (config : EqualityClosure.Config := {}) :
    MetaM (Sigma fun package : RuleCompiler.Package =>
