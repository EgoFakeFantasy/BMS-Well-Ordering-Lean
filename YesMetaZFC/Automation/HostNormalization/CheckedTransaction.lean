...
deriving Repr
inductive HypothesisSelection where
  | allSafe
  | resourcesOnly
  | none
...
structure Config where
  ...
  hypothesisSelection : HypothesisSelection := .allSafe
deriving Repr, Inhabited
def configFromOptions (options : Options) : Config :=
  {
    closure := {
      ...
      maxCycles :=
        Nat.max 1 <| options.get `prove_auto.normalizer.maxCycles 32
      maxSteps :=
...
private def pushFVarUnique (resources : Array FVarId) (resource : FVarId) : Array FVarId :=
  if resources.contains resource then resources else resources.push resource
private def closureUsedDeclarations (result : EqualityClosure.Result) : Array Name :=
  result.path.foldl (fun declarations step =>
      step.usedDeclarations.foldl pushNameUnique declarations) #[]
private def ClosureJournal.ofCloseResult
    {package : RuleCompiler.Package} (closeResult : CheckedReplay.CloseResult package) (elapsedNs : Nat) : ClosureJournal :=
...
private def Journal.recordClosure (journal : Journal) (closure : ClosureJournal) : Journal :=
  {
    journal with
...
private def localDeclarations (goal : MVarId) :
  ...
  return declarations
private def declarationDependsOn (declaration : LocalDecl) (fvarId : FVarId) : Bool :=
...
显式相继式规则，不在本事务中猜测迁移。
-/
private def safePropositionHypotheses (goal : MVarId) (target : Expr)
    (resources : Array FVarId) (selection : HypothesisSelection) :
  ...
  for index in [0 : declarations.size] do
    ...
    if declaration.isLet || declaration.isImplementationDetail ||
      ...
      continue
    if selection == .resourcesOnly &&
        !resources.contains declaration.fvarId then
      continue
...
private def mappedFVar? (substitution : FVarSubst) (resource : FVarId) :
...
private def remapResources (resources : Array FVarId) (source : FVarId) (replacement? : Option FVarId) (substitution : FVarSubst) :
...
private def validateGoalResources (goal : MVarId) (resources : Array FVarId) : MetaM Unit :=
  goal.withContext do
    ...
    for resource in resources do
      ...
      if proposition.hasMVar || !(← isProp proposition) then
        throwError
          ...
          `{resource.name}`"
private def ensureStableClosure (result : EqualityClosure.Result) : MetaM Unit := do
...
private def normalizeHypothesis (prepared : Prepared) (config : Config) (goal : MVarId) (resources : Array FVarId) (source : FVarId) :
    TacticM (MVarId × Array FVarId × HypothesisJournal) :=
  goal.withContext do
...
private def normalizeTarget (prepared : Prepared) (config : Config) (goal : MVarId) (resources : Array FVarId) :
    TacticM (Option MVarId × Bool × ClosureJournal) :=
  goal.withContext do
...
-/
def run (goal : MVarId) (resources : Array FVarId) (prepared : Prepared) (config : Config := {}) :
    TacticM Outcome := do
  ...
  try
    ...
    let (hypotheses, skipped) ←
      safePropositionHypotheses
        goal target resources config.hypothesisSelection
    journal := {
      journal with
    ...
    journal := {
      journal with
      hypothesisElapsedNs := (← IO.monoNanosNow) - hypothesisStarted
    }
