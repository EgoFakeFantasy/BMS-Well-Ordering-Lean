...
structure RegisteredRule where
  ...
  phase : Phase
deriving Inhabited
private def pushRegisteredRuleUnique (rules : Array RegisteredRule) (rule : RegisteredRule) :
...
private initialize normalizationRuleExtension :
    PersistentEnvExtension RegisteredRule RegisteredRule (Array RegisteredRule) ←
  registerPersistentEnvExtension {
    name := `YesMetaZFC.Automation.HostNormalization.normalizationRuleExtension
    mkInitial := pure #[]
    addImportedFn := fun imported =>
      pure <| imported.foldl (fun rules entries =>
...
private def addRegisteredRule (declaration : Name) (phase : Phase) : AttrM Unit := do
  let action : MetaM Unit := do
    ...
    modifyEnv fun environment =>
      normalizationRuleExtension.addEntry (asyncDecl := declaration) environment { declaration, phase }
  discard <| action.run {} {}
/--
