...
private def Index.setPhaseIndex (index : Index) (phase : Phase) (phaseIndex : PhaseIndex) : Index :=
  match phase with
...
private def Index.setStats (index : Index) (stats : BuildStats) : Index :=
  { index with stats }
private def instantiatedTheoremPattern? (rule : RuleCompiler.Rule) : MetaM (Option Expr) := do
  ...
  let left? :=
    if body.isAppOfArity ``Eq 3 then
      ...
      none
  return left?
private def insertRoot (roots : NameMap (Array RuleId)) (declaration : Name) (ruleId : RuleId) :
...
private def rootDeclaration? (expression : Expr) : Option Name :=
  ...
  | _ => none
private def insertTheorem (phaseIndex : PhaseIndex) (ruleId : RuleId) (rule : RuleCompiler.Rule) :
  ...
  try
    ...
    saved.restore
    return ({ phaseIndex with fallback := phaseIndex.fallback.push ruleId }, false)
private def insertRule (index : Index) (ruleId : RuleId) (rule : RuleCompiler.Rule) :
    MetaM Index := do
...
private structure QueryState where
  ...
  ruleIds : Array RuleId := #[]
  stats : QueryStats := {}
private def QueryState.insert (state : QueryState) (ruleId : RuleId) (record : QueryStats → QueryStats) : QueryState :=
  if h : ruleId < state.seen.size then
    ...
    state
private def QueryState.insertIndexed (state : QueryState) (ruleId : RuleId) : QueryState :=
  state.insert ruleId fun stats => {
    stats with indexedCandidates := stats.indexedCandidates + 1
  }
private def QueryState.insertRoot (state : QueryState) (ruleId : RuleId) : QueryState :=
  state.insert ruleId fun stats => {
    stats with rootCandidates := stats.rootCandidates + 1
  }
private def QueryState.insertFallback (state : QueryState) (ruleId : RuleId) : QueryState :=
  state.insert ruleId fun stats => {
    stats with fallbackCandidates := stats.fallbackCandidates + 1
  }
private partial def queryExpression (phaseIndex : PhaseIndex) (expression : Expr) (state : QueryState) :
    MetaM QueryState := do
...
-/
def Index.query (index : Index) (phase : Phase) (expression : Expr) : MetaM QueryResult := do
  let phaseIndex := index.phaseIndex phase
