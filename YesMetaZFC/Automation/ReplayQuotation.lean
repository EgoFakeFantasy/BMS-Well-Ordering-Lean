...
连续 Arena/线性节点检查、capability gate 与 problem 对齐的内核证明。
-/
...
instance : ToExpr Clause where
  ...
  toExpr := clauseExpr
private def registryTypeExpr : Expr :=
  mkApp (mkConst ``DAGCertificate.AvatarSelectorComponent.Registry) signatureExpr
private def registryExpr
    (registry : DAGCertificate.AvatarSelectorComponent.Registry SearchSignature) : Expr :=
  mkAppN (mkConst ``DAGCertificate.AvatarSelectorComponent.Registry.mk)
    #[signatureExpr, toExpr registry.slots]
instance : ToExpr (DAGCertificate.AvatarSelectorComponent.Registry SearchSignature) where
  toTypeExpr := registryTypeExpr
  toExpr := registryExpr
...
private def substitutionExpr :
  ...
  | [] =>
      let entryType :=
        mkApp2 (binaryZeroConst ``Prod) (mkConst ``SearchSort) (mkApp2 (binaryZeroConst ``Prod) (mkConst ``Nat) termTypeExpr)
...
private def standardizeApartSideExpr (evidence : DAGCertificate.StandardizeApartSideEvidence SearchSignature) : Expr :=
  mkAppN (mkConst ``DAGCertificate.StandardizeApartSideEvidence.mk)
    #[signatureExpr, toExpr evidence.original, toExpr evidence.offset, toExpr evidence.renamed]
...
private def standardizeApartExpr (evidence : DAGCertificate.StandardizeApartEvidence SearchSignature) : Expr :=
...
private def resolutionEvidenceExpr (evidence : DAGCertificate.ResolutionEvidence SearchSignature) : Expr :=
  mkAppN (mkConst ``DAGCertificate.ResolutionEvidence.mk)
    #[signatureExpr, toExpr evidence.left, toExpr evidence.right,
      toExpr evidence.pivot, toExpr evidence.leftPolarity,
      substitutionExpr evidence.substitution, toExpr evidence.standardizeApart?]
...
private def factoringEvidenceExpr (evidence : DAGCertificate.FactoringEvidence SearchSignature) : Expr :=
...
private def equalityResolutionEvidenceExpr (evidence : DAGCertificate.EqualityResolutionEvidence SearchSignature) : Expr :=
...
private def rewriteEvidenceExpr (evidence : DAGCertificate.RewriteEvidence SearchSignature) : Expr :=
...
private def localRuleEvidenceExpr :
  ...
  | .rewrite kind evidence =>
      mkAppN (mkConst ``DAGCertificate.LocalRuleEvidence.rewrite)
        #[signatureExpr, toExpr kind, toExpr evidence]
...
private def localRulePayloadExpr (payload : DAGCertificate.LocalRulePayload SearchSignature) : Expr :=
...
private def propLiteralLinkExpr (link : DAGCertificate.PropLiteralLink SearchSignature) : Expr :=
...
private def propParentClauseLinkExpr (link : DAGCertificate.PropParentClauseLink SearchSignature) : Expr :=
...
private def propGuardActivationLinkExpr (link : DAGCertificate.PropGuardActivationLink SearchSignature) : Expr :=
...
private def propLearnedClauseLinkExpr (link : DAGCertificate.PropLearnedClauseLink) : Expr :=
...
private def propAvatarSkeletonLinkExpr (link : DAGCertificate.PropAvatarSkeletonLink) : Expr :=
...
private def propositionalClosurePayloadExpr (payload : DAGCertificate.PropositionalClosurePayload SearchSignature) : Expr :=
...
private def avatarSplitPayloadExpr (payload : DAGCertificate.AvatarSplitPayload SearchSignature) : Expr :=
...
private def avatarComponentPayloadExpr (payload : DAGCertificate.AvatarComponentPayload SearchSignature) : Expr :=
...
private def theoryConflictPayloadExpr (payload : DAGCertificate.TheoryConflictPayload SearchSignature) : Expr :=
...
private def propositionalLearnedClausePayloadExpr (payload : DAGCertificate.PropositionalLearnedClausePayload) : Expr :=
...
节点仍完整来自材料化结果；source 对 clause problem 的一致性由随后执行的连续 Arena
检查复核。
-/
