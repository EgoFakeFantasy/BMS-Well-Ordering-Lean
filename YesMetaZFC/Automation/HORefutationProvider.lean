...
def arenaMappingCheck (dag : HOSearchMaterialization.DAG) (clauses : Array SearchClause) (nodeIds : Array Nat) : Bool :=
  clauses.size == nodeIds.size && (clauses.mapIdx fun clauseId clause =>
...
theorem problemEq_sound {left right : HOSearchMaterialization.Problem} (hEq : problemEq left right = true) : left = right := by
...
theorem initialChecks_of_problem_eq
    {left right : HOSearchMaterialization.Problem} (hEq : left = right) (hChecks :
...
structure CheckedReplayArtifact (clauses : CoreClauseSet) where
  ...
  initialChecks : (coreProblem clauses).initialClauses.all
...
def replay {clauses : CoreClauseSet} (artifact : CheckedArtifact clauses) : CheckedReplayArtifact clauses := {
...
def selectorValuation {clauses : CoreClauseSet} (artifact : CheckedArtifact clauses)
    {M :
      Logic.HigherOrder.Structure
        HOSearchMaterialization.SearchSignature} (base : Logic.HigherOrder.Env M) : PropResolution.Valuation :=
  artifact.checked.selectorValuation base
def clauseNodeId? {clauses : CoreClauseSet} (artifact : CheckedArtifact clauses) (clauseId : Nat) : Option Nat :=
  artifact.clauseNodeIds[clauseId]?
end CheckedArtifact
namespace CheckedReplayArtifact
private theorem problemValidAfterExpansion {clauses : CoreClauseSet} (artifact : CheckedReplayArtifact clauses) (M : CoreSyntax.NormalForm.Semantics.Model)
    (contract : CoreSyntax.NormalForm.Semantics.FoolLambdaContract M) (functionSort :
      ∀ symbol arguments,
        M.sortInterp symbol.outputSort (M.functionInterp symbol arguments)) (base : CoreSyntax.NormalForm.Semantics.Env M) (hClauses :
    ...
    artifact.checked.checked.dag.problem.Valid (HOExtensionalWitnessSoundness.Structure.overrideExtensionalWitnesses
        target targetContract) := by
  dsimp
  have hCoreValid : (coreProblem clauses).Valid (searchStructure M contract functionSort) :=
    coreProblem_valid contract functionSort base clauses artifact.native
      artifact.initialChecks hClauses
  have hProblemValid :
      artifact.checked.checked.dag.problem.Valid (searchStructure M contract functionSort) := by
    rw [artifact.problem_eq]
    exact hCoreValid
  exact
    HOExtensionalWitnessSoundness.checkedRegistry_problemValidAfterExpansion
      artifact.witnessRegistry (extensionalContract M contract functionSort) hProblemValid
/--
core 模型满足全部原生高阶初始字句时，HO-AVATAR residual 空根推出矛盾。
-/
theorem refutesCoreModel {clauses : CoreClauseSet} (artifact : CheckedReplayArtifact clauses) (M : CoreSyntax.NormalForm.Semantics.Model)
    (contract : CoreSyntax.NormalForm.Semantics.FoolLambdaContract M) (functionSort :
      ∀ symbol arguments,
        M.sortInterp symbol.outputSort (M.functionInterp symbol arguments)) (base : CoreSyntax.NormalForm.Semantics.Env M) (hClauses :
      ∀ env, CoreSyntax.NormalForm.Semantics.Env.RespectsFree env →
  ...
  exact artifact.checked.rootEmptyContradiction (HOExtensionalWitnessSoundness.extensionalContractPreserved
      targetContract) (HOExtensionalWitnessSoundness.extensionalWitnessContract
      targetContract) (Logic.HigherOrder.Env.canonical expanded) (Logic.HigherOrder.Env.canonical_wellSorted expanded)
    hExpandedProblem artifact.avatarSoundnessSupported
end CheckedReplayArtifact
namespace CheckedArtifact
theorem refutesCoreModel {clauses : CoreClauseSet} (artifact : CheckedArtifact clauses) (M : CoreSyntax.NormalForm.Semantics.Model)
    (contract : CoreSyntax.NormalForm.Semantics.FoolLambdaContract M) (functionSort :
      ∀ symbol arguments,
        M.sortInterp symbol.outputSort (M.functionInterp symbol arguments)) (base : CoreSyntax.NormalForm.Semantics.Env M) (hClauses :
      ∀ env, CoreSyntax.NormalForm.Semantics.Env.RespectsFree env →
...
private def requireUnsat (result : HOAvatar.RunResult) :
  ...
  | .invariantViolation message _ =>
      throw (diagnostic .backendCheck ("native HO-AVATAR CDCL invariant violation: " ++ message))
  | .theoryUnknown message =>
      throw (diagnostic .saturation ("native HO-AVATAR theory loop stopped: " ++ message))
/--
原生 HO core 字句进入真正的 AVATAR saturation/CDCL 双核，并材料化
`CheckedAvatarDAG` residual 空根。
-/
def run (clauses : CoreClauseSet) (hNative : Native.clauseSet clauses = true) (config : HOAvatar.Config := {}) :
    Except Diagnostic (CheckedArtifact clauses) := do
  let projectionState :=
    CoreSyntax.NormalForm.FirstOrderProjection.initialState (CoreSyntax.Formula.conjunctionList [])
  let (searchClauses, _) ←
    match
