...
def checkAgainst (seed : ArenaSeed) (dag : DAG) : Bool :=
  seed.check && (seed.clauses.mapIdx fun clauseId clause =>
...
private partial def subterms : SearchTerm → List SearchTerm
  ...
  | term@(.lam _ _ body) =>
      term :: subterms body
private def pushTerm? (config : Config) (terms : Array SearchTerm) (term : SearchTerm) : Array SearchTerm :=
...
private partial def termMaxFunctionIdSucc : SearchTerm → Nat
  ...
  | .app symbol arguments =>
      arguments.foldl (fun result argument => Nat.max result (termMaxFunctionIdSucc argument)) (symbol.id + 1)
...
private def clauseMaxFunctionIdSucc (clause : SearchClause) : Nat :=
  clause.foldl (fun result literal =>
      Nat.max result (Nat.max (termMaxFunctionIdSucc literal.left) (termMaxFunctionIdSucc literal.right)))
    0
...
private def booleanExtensionalityResource? (dag : DAG) (parentId literalIndex : Nat) : Option Resource := do
  let parentNode ← dag.node? parentId
  let parentClause ← parentNode.conclusion? dag.problem
...
-/
private def argumentCongruenceResource? (dag : DAG) (parentId literalIndex : Nat) : Option Resource := do
  let parentNode ← dag.node? parentId
  let parentClause ← parentNode.conclusion? dag.problem
  let parentLiteral ← parentClause.literals[literalIndex]?
  let parent ← parentRef? dag parentId
  ...
  | _, _ =>
      none
private def hasFunctionExtensionality (dag : DAG) (parentId : Nat) (needle : Literal) : Bool :=
  dag.nodes.any fun node =>
    ...
    | _ =>
        false
private def functionExtensionalityResource? (dag : DAG) (parentId literalIndex witnessId : Nat) : Option Resource := do
  let parentNode ← dag.node? parentId
  let parentClause ← parentNode.conclusion? dag.problem
  let parentLiteral ← parentClause.literals[literalIndex]?
  let parent ← parentRef? dag parentId
...
-/
private def extensionalParentEligible (discipline : ExtensionalParentDiscipline) (node : Node) : Bool :=
...
private def expectedGuardedConclusion? (dag : DAG) (resource : Resource) :
  ...
  | .local witness =>
      ...
      some (guards, result)
private def containsGuardedConclusion (dag : DAG) (guards : GuardSet) (conclusion : Clause) : Bool :=
  dag.nodes.any fun node =>
    Guards.eq node.guards guards &&
      match node.conclusion? dag.problem with
...
private def pushResource? (dag : DAG) (resource : Resource) :
  ...
  if containsGuardedConclusion dag guards conclusion then
    ...
    some (next, true)
private def State.tryResource (config : Config) (state : State) (resource : Resource) : State × Bool :=
...
private def runRound (discipline : ExtensionalParentDiscipline) (config : Config) (input : State) : State × Nat := Id.run do
  ...
  for candidate in terms do
    if config.beta && state.stats.generated < config.maxGenerated && (checkedBetaPayload? candidate).isSome then
      state := (state.tryResource config (.beta candidate)).1
    if config.eta && state.stats.generated < config.maxGenerated && (checkedEtaPayload? candidate).isSome then
...
residual CDCL 闭合节点。
-/
private def runWithDiscipline (discipline : ExtensionalParentDiscipline) (dag : DAG) (config : Config) : Result :=
  let initial : State := {
    dag := dag
...
def runGuardedSources? (inputs : Array GuardedSourceInput) (root : Nat := 0) (config : Config := {}) : Option Result := do
  if inputs.isEmpty || root ≥ inputs.size then
    none
  else
    let dag ← guardedSourceDAG? inputs root
    some (run dag config)
def runSources? (inputs : Array SearchClause) (root : Nat := 0) (config : Config := {}) : Option Result :=
  runGuardedSources? (inputs.map fun clause => ({ clause := clause } : GuardedSourceInput))
    root config
end HOSearch
end Automation
end YesMetaZFC
